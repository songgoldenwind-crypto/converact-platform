...
import {
  IVEKIT_IM_REQUIRED_ACCEPTANCE_CHECKS,
  createIveKitImAcceptanceTemplate,
  renderIveKitImAcceptanceRunbook,
  runIveKitImAcceptance,
  runIveKitImAcceptanceFromEnv,
  writeIveKitImAcceptanceTemplate
} from '../scripts/ivekit-im-client-acceptance.js';

test('IM acceptance template is intentionally incomplete and covers every real check', () => {
  const template = createIveKitImAcceptanceTemplate();
...
test('IM acceptance validates complete secret-safe two-browser evidence', () => {
  const dir = mkdtempSync(join(tmpdir(), 'ivekit-im-acceptance-pass-'));
  const reportFile = join(dir, 'report.json');
  try {
    const report = completeReport(dir);
    writeFileSync(reportFile, `${JSON.stringify(report, null, 2)}\n`);
    const result = runIveKitImAcceptance({ reportFile });
    assert.equal(result.ok, true);
    assert.equal(result.status, 'ready_for_review');
    assert.equal(result.summary.passed, IVEKIT_IM_REQUIRED_ACCEPTANCE_CHECKS.length);
...
test('IM acceptance rejects placeholders secrets missing evidence and identical identities', () => {
  const dir = mkdtempSync(join(tmpdir(), 'ivekit-im-acceptance-fail-'));
  const reportFile = join(dir, 'report.json');
  try {
...
test('IM acceptance rejects stale artifacts and report output path collisions', () => {
  const dir = mkdtempSync(join(tmpdir(), 'ivekit-im-acceptance-window-'));
  const reportFile = join(dir, 'report.json');
  try {
    const report = completeReport(dir) as any;
    ...
    assert.throws(() => runIveKitImAcceptance({ reportFile, outputFile: reportFile }), /must differ/);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test('IM acceptance binds each unique JSON observation to its check run and environment', () => {
  const dir = mkdtempSync(join(tmpdir(), 'ivekit-im-acceptance-binding-'));
  const reportFile = join(dir, 'report.json');
  try {
    const report = completeReport(dir) as any;
...
test('IM acceptance rejects symlink evidence and common Basic or Cookie secrets', () => {
  const dir = mkdtempSync(join(tmpdir(), 'ivekit-im-acceptance-secret-'));
  const outside = mkdtempSync(join(tmpdir(), 'ivekit-im-acceptance-outside-'));
  const reportFile = join(dir, 'report.json');
  try {
    const report = completeReport(dir) as any;
...
test('IM acceptance writes a template and a runbook with the two-browser procedure', () => {
  const dir = mkdtempSync(join(tmpdir(), 'ivekit-im-acceptance-template-'));
  const templateFile = join(dir, 'template.json');
  try {
    writeIveKitImAcceptanceTemplate(templateFile);
    const template = JSON.parse(readFileSync(templateFile, 'utf8'));
    assert.equal(Object.keys(template.checks).length, IVEKIT_IM_REQUIRED_ACCEPTANCE_CHECKS.length);
    const runbook = renderIveKitImAcceptanceRunbook();
...
function completeReport(dir: string): Record<string, any> {
  const runId = 'ivekit-im-real-run-1';
  ...
  return {
    ...
    endpoints: { ivekit: 'https://ivekit.example.com', tinode: 'wss://tinode.example.com/v0/channels' },
    checks
  };
}
