...
test('LiveKit Egress build and Kubernetes pools enforce the source-level policy boundary', () => {
  ...
  const publicBuild = readFileSync('infra/converact/livekit-egress/build-converact.sh', 'utf8');
  ...
  for (const input of [
    ...
    assert.match(publicBuild, new RegExp(`CONVERACT_FABRIC_LIVEKIT_EGRESS_${input}_IMAGE`));
  }
  ...
  assert.match(deployment, /regexMatch "\(\^\|\/\)converact-livekit-egress\$"/);
  assert.match(deployment, /must identify the Converact Fabric image ending with converact-livekit-egress/);
  ...
  assert.match(deployment, /preStop:[\s\S]*touch \/var\/run\/converact-egress\/draining/);
...
test('Stage 2 Helm gate rejects upstream aliases and arbitrary Egress repositories', () => {
  const gate = readFileSync('scripts/verify-converact-stage2-deployment.sh', 'utf8');
  ...
  assert.match(gate, /untrusted\.example\.invalid\/converact\/livekit-egress/);
  assert.match(gate, /registry\.example\.invalid\/converact\/livekit-egress/);
