...
import {
  dispatchConveractFabricVoiceCallCommand,
  converactFabricVoiceWorkerConfig,
  startConveractFabricVoiceProviderEventWorker,
  type ConveractFabricVoiceWorkerConfig
...
test('Voice workers are disabled without requiring provider or address secrets', () => {
  assert.deepEqual(converactFabricVoiceWorkerConfig({}), {
    enabled: false,
    ...
    tenant_limit: 100
  } satisfies ConveractFabricVoiceWorkerConfig);
});
...
test('Voice runtime dispatches LiveKit bridge commands to the specialized executor', async () => {
  ...
  const bridgeResult = await dispatchConveractFabricVoiceCallCommand(
    { kind: 'livekit_bridge_create' } as never,
    provider,
    bridge
  );
  const providerResult = await dispatchConveractFabricVoiceCallCommand(
...
test('Voice worker config validates keys, bounds, retry delays, and provider lease budget', () => {
  const valid = converactFabricVoiceWorkerConfig({
    ...
    CONVERACT_FABRIC_VOICE_PROVIDER_TIMEOUT_MS: '20000',
    CONVERACT_FABRIC_VOICE_COMMAND_LEASE_MS: '25001',
    CONVERACT_FABRIC_VOICE_EVENT_LEASE_MS: '25001',
    CONVERACT_FABRIC_VOICE_COMMAND_RETRY_DELAYS_MS: '2000,10000,60000'
  });
  ...
  assert.throws(() => converactFabricVoiceWorkerConfig({
    CONVERACT_FABRIC_VOICE_WORKERS_ENABLED: 'yes'
  }), /CONVERACT_FABRIC_VOICE_WORKERS_ENABLED/);
  assert.throws(() => converactFabricVoiceWorkerConfig({
    CONVERACT_FABRIC_VOICE_WORKERS_ENABLED: '1',
    CONVERACT_FABRIC_VOICE_ADDRESS_KEY: 'not-a-key',
    CONVERACT_FABRIC_VOICE_ADDRESS_HMAC_KEY: HMAC_KEY
  }), /CONVERACT_FABRIC_VOICE_ADDRESS_KEY/);
  assert.throws(() => converactFabricVoiceWorkerConfig({
    CONVERACT_FABRIC_VOICE_WORKERS_ENABLED: '1',
    CONVERACT_FABRIC_VOICE_ADDRESS_KEY: ADDRESS_KEY,
    CONVERACT_FABRIC_VOICE_ADDRESS_HMAC_KEY: HMAC_KEY,
    CONVERACT_FABRIC_VOICE_COMMAND_RETRY_DELAYS_MS: '1000,-1'
  }), /CONVERACT_FABRIC_VOICE_COMMAND_RETRY_DELAYS_MS/);
  assert.throws(() => converactFabricVoiceWorkerConfig({
    CONVERACT_FABRIC_VOICE_WORKERS_ENABLED: '1',
    CONVERACT_FABRIC_VOICE_ADDRESS_KEY: ADDRESS_KEY,
    CONVERACT_FABRIC_VOICE_ADDRESS_HMAC_KEY: HMAC_KEY,
    CONVERACT_FABRIC_VOICE_PROVIDER_TIMEOUT_MS: '30000',
    CONVERACT_FABRIC_VOICE_COMMAND_LEASE_MS: '30000'
  }), /COMMAND_LEASE_MS.*provider timeout/i);
});

test('Converact Fabric application starts Voice workers only when enabled and stops them in reverse order', async () => {
  const events: string[] = [];
  ...
  const disabled = startConveractFabricApplication({
  ...
  events.length = 0;
  const enabled = startConveractFabricApplication({
    pg: new MemoryPg(),
    env: {
      CONVERACT_FABRIC_VOICE_WORKERS_ENABLED: '1',
      CONVERACT_FABRIC_VOICE_ADDRESS_KEY: ADDRESS_KEY,
...
test('Converact Fabric application waits for every Voice stop and aggregates failures', async () => {
...
test('production Voice scheduler stop waits for an active tenant-discovery batch', async () => {
  ...
  const worker = startConveractFabricVoiceProviderEventWorker({
    pg,
    env: {
      CONVERACT_FABRIC_VOICE_WORKERS_ENABLED: '1',
      CONVERACT_FABRIC_VOICE_ADDRESS_KEY: ADDRESS_KEY,
