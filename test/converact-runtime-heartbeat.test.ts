...
test('runtime heartbeat publishes bounded components and terminal shutdown state', async () => {
  ...
  let tick = 0;
  const handle = startConveractFabricRuntimeHeartbeat({
    pg, instance_id: 'converact-a', components: ['api', 'retention_worker', 'api'],
...
test('runtime component inventory reflects only enabled worker groups', () => {
  assert.deepEqual(converactFabricRuntimeComponents({
