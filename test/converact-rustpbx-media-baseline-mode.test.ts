...
const images = {
  RUSTPBX_IMAGE: 'converact/rustpbx:0.4.11-ivekit.19-6c49ee76',
  KAMAILIO_IMAGE: 'converact/kamailio:6.0.7-capacity-ced1eeb0',
...
test('RustPBX media baseline can force an anchored RTP path', () => {
  const output = mkdtempSync(join(tmpdir(), 'converact-rustpbx-media-mode-'));
  try {
...
test('RustPBX media baseline rejects unsafe UDP socket buffers', () => {
  const output = mkdtempSync(join(tmpdir(), 'converact-rustpbx-media-buffer-'));
  try {
    const result = spawnSync('python3', [prepare, output], {
      encoding: 'utf8',
      env: {
...
test('RustPBX media baseline rejects unsafe RTP port ranges', () => {
  for (const range of [
    ...
    const output = mkdtempSync(join(tmpdir(), 'converact-rustpbx-media-range-'));
    try {
      const result = spawnSync('python3', [prepare, output], {
        encoding: 'utf8',
        env: {
