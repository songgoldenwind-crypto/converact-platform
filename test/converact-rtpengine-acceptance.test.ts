...
} from '../scripts/converact-rtpengine-acceptance.js';
...
describe('Converact Fabric RTPengine media generator', () => {
  it('ships the signalling templates referenced by media-control profiles', () => {
    ...
    assert.match(configuration, /^templates=converact-signalling-templates$/m);
    assert.match(configuration, /^\[converact-signalling-templates\]$/m);
  ...
  it('round-trips AES_CM_128_HMAC_SHA1_80 SRTP without exposing plaintext', () => {
    ...
    const plaintext = buildPcmuRtpPacket({
      ...
      ssrc: 0xdecafbad,
      payload: Buffer.from('converact-plaintext-payload'.padEnd(160, '.'), 'ascii')
    });

    const protectedPacket = protectSrtpPacket(context, plaintext);

    assert.equal(
      protectedPacket.includes(Buffer.from('converact-plaintext-payload', 'ascii')),
      false
    );
...
describe('Converact Fabric RTPengine real acceptance evidence', () => {
  const identity = {
  ...
  it('acceptance CLI confines lifecycle actions to its container prefix', () => {
    const env = {
      CONVERACT_FABRIC_MEDIA_CONTROL_ENDPOINT: 'http://127.0.0.1:33211',
      CONVERACT_FABRIC_MEDIA_CONTROL_TOKEN: 'task9-media-control-token-123456789',
    ...
    const authoritative = loadRtpengineAcceptanceCliConfig({
      ...
      CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_TENANT_ID: 'goal3-tenant',
      CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_REGION_ID: 'region-a',
      CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_ZONE_ID: 'zone-a',
      CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_CELL_ID: 'converact-cell-a',
      CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_PROFILE_ID: 'voice-ordinary-v1',
    ...
    assert.throws(
      () => loadRtpengineAcceptanceCliConfig({
        ...env,
        CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_CONTAINER: 'led-platform-api-1'
      }),
  ...
  it('runs offer, answer, real bidirectional UDP, RTCP, and delete', async () => {
    ...
    try {
      ...
      assert.ok(commands.every((command) =>
        ...
        command.cell_id === 'converact-cell-a' &&
