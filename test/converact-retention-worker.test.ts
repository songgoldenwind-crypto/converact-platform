...
import {
  ...
  type ConveractFabricRetentionRepository
...
test('retention worker claims due tenant policies and records deletion and holds', async () => {
  ...
  let unclaimed = true;
  const repository: ConveractFabricRetentionRepository = {
  ...
  const summary = await new ConveractFabricRetentionWorker({
  ...
  assert.equal(converactFabricRetentionMetricDefinitions.some((item) => item.labels.includes('tenant' as never)), false);
});
