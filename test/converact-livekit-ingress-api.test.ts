...
test('Converact Fabric LiveKit Ingress API is tenant fenced, idempotent and complete', async () => {
...
test('Converact Fabric LiveKit Ingress SDK sends complete lifecycle requests', async () => {
  ...
  const sdk = createConveractFabricHttpSdk({
    baseUrl: 'https://converact.example.com', tenantId: 'tenant-sdk', apiKey: 'key', fetch
  });
