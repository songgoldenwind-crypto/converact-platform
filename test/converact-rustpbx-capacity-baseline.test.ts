...
test('RustPBX baseline preparation creates private runtime secrets without leaking them', () => {
  const output = mkdtempSync(join(tmpdir(), 'converact-rustpbx-baseline-'));
  const result = spawnSync('python3', [join(root, 'prepare.py'), output], {
    cwd: process.cwd(),
    encoding: 'utf8',
    env: {
      ...process.env,
      RUSTPBX_IMAGE: 'converact/rustpbx:0.4.11-ivekit.27-6c49ee76',
  ...
  assert.match(env, /^COMPOSE_PROJECT_NAME=converact-rustpbx-baseline$/m);
  assert.match(env, /^RUSTPBX_IMAGE=converact\/rustpbx:0\.4\.11-ivekit\.27-6c49ee76$/m);
...
test('RustPBX direct baseline trusts the SIPp source instead of the absent Kamailio hop', () => {
  ...
  assert.match(config, /^sip_max_active_transactions = 1$/m);
});
