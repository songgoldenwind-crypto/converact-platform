...
test('RustPBX management client maps bounded authenticated endpoints', async () => {
  ...
  try {
    ...
    const trunkInput = {
      resource_id: 'trunk-a',
      desired_state: {
        provider_name: 'converact-trunk-a', name: 'Carrier A', direction: 'both', transport: 'tls',
...
function validTrunkInput() {
  return {
    resource_id: 'trunk-a',
    desired_state: {
      provider_name: 'converact-trunk-a', name: 'Carrier A', direction: 'both', transport: 'udp',
