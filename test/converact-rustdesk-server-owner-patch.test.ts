...
test('RustDesk Server 1.1.16 image workflow binds exact source and shared OCI gate', () => {
  const workflow = readFileSync(
    '.github/workflows/converact-rustdesk-server-image.yml',
    'utf8'
  );
  ...
  assert.match(workflow, /image: ghcr\.io\/songgoldenwind-crypto\/converact-rustdesk-server/);
...
test('RustDesk Server hook is a no-op when Converact Fabric ownership is not configured', () => {
  const hook = readFileSync('infra/converact/rustdesk-server/server-hook.rs', 'utf8');
