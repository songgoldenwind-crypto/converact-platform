...
const R4_COMPATIBILITY_COMMIT =
  "532350f34c9191092f62d9a1e89287de06f6c083";
...
function assertArtifactBinding(
  ...
  const currentDigest = sha256(expectedPath);
  if (binding.sha256 === currentDigest) return;
  const frozen = execFileSync("git", [
    "show",
    `${R4_COMPATIBILITY_COMMIT}:${expectedPath}`,
  ]);
  assert.equal(
    binding.sha256,
    createHash("sha256").update(frozen).digest("hex"),
    `${expectedPath} no longer matches its frozen compatibility source`,
  );
}
...
test("R4 traceability schema validates a closed row-level contract", () => {
  ...
  assert.equal(
    schema.$id,
    "https://opc.local/schemas/unified-voice-foundation-r4-traceability.schema.json",
  );
  const validate = validator();
  const value = json(traceContractPath);
  assert.equal(
