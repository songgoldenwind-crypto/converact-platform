...
import {
  ...
  buildConveractFabricDeliveryBundle,
  listDeliveryFiles,
  loadControlledAcceptancePackage,
  validateConveractFabricDeliveryBundle
} from '../scripts/converact-delivery-bundle.js';
import { VOICE_REQUIRED_ACCEPTANCE_CHECKS } from '../scripts/converact-voice-acceptance.js';
...
test('capacity runtime handoff typechecks without borrowing repository source', () => {
  const root = mkdtempSync(join(tmpdir(), 'converact-capacity-runtime-'));
  try {
    ...
    const smoke = spawnSync(
      ...
      [
        ...
        [
          'await Promise.all([',
          'import("./scripts/converact-capacity-dispatcher.ts"),',
          'import("./scripts/converact-capacity-controller.ts"),',
          'import("./scripts/converact-capacity-finalizer.ts"),',
          'import("./scripts/converact-capacity-worker.ts"),',
          'import("./scripts/converact-cell-admission.ts"),',
          'import("./scripts/converact-cell-capacity-projector.ts"),',
          'import("./scripts/converact-component-node-admission.ts"),',
...
test('Converact Fabric delivery bundle contains only curated handoff artifacts with verified hashes', () => {
  const root = mkdtempSync(join(tmpdir(), 'converact-delivery-'));
  const outputDir = join(root, 'bundle');
  const sdkTarball = join(root, 'converact-sdk-0.1.0.tgz');
  ...
  writeFileSync(join(clientDist, 'index.html'), '<!doctype html><title>Converact Fabric</title>');
  writeFileSync(join(clientDist, 'assets', 'index.js'), 'console.log("Converact Fabric")');

  try {
    const result = buildConveractFabricDeliveryBundle({
      repoRoot,
      outputDir,
      sdkTarball,
      clientDist,
    ...
    assert.equal(result.manifest.contents.voice_helm, 'deploy/kubernetes/converact/');
    ...
    assert.equal(result.manifest.contents.operations, 'docs/converact-fabric-v3-intelligence-operations.md');
    ...
    assert.deepEqual(validateConveractFabricDeliveryBundle(outputDir), result.manifest);
    assert.equal(
      files.some((file) => [
    ...
    assert.equal(files.includes('acceptance/tools/converact-controlled-voice-provider.ts'), true);
    assert.equal(files.includes('acceptance/tools/converact-voice-acceptance.ts'), true);
    ...
    const compiledSippAcceptance = readFileSync(
      join(outputDir, 'acceptance', 'rustpbx', 'scripts', 'converact-rustpbx-sipp-acceptance.js'),
      'utf8'
    );
    ...
    const compiledRwiAcceptance = readFileSync(
      join(outputDir, 'acceptance', 'rustpbx', 'scripts', 'converact-rustpbx-rwi-acceptance.js'),
      'utf8'
    );
    ...
    assert.equal(files.includes('docs/converact-fabric-voice-foundation-v1-design.md'), true);
    assert.equal(files.includes('deploy/application/docker-compose.voice.yml'), true);
    assert.equal(files.includes('deploy/kubernetes/converact/Chart.yaml'), true);
    assert.equal(files.includes('deploy/kubernetes/converact/values.yaml'), true);
    assert.equal(files.includes('deploy/kubernetes/converact/templates/rustpbx-deployment.yaml'), true);
    assert.equal(files.includes('deploy/kubernetes/converact/templates/kamailio-config.yaml'), true);
    assert.equal(files.includes('deploy/kubernetes/converact/templates/kamailio-deployment.yaml'), true);
    assert.equal(files.includes('deploy/kubernetes/converact/templates/kamailio-network-policy.yaml'), true);
    assert.equal(files.includes('deploy/kubernetes/converact/templates/migrate-job.yaml'), true);
    assert.equal(files.includes('deploy/kubernetes/converact/templates/deployment.yaml'), true);
    assert.equal(files.includes('deploy/kubernetes/converact/templates/service-monitor.yaml'), true);
    assert.equal(files.includes('deploy/kubernetes/converact/templates/prometheus-rule.yaml'), true);
    assert.equal(files.includes('deploy/kubernetes/converact/templates/grafana-dashboard.yaml'), true);
    assert.equal(files.includes('deploy/kubernetes/converact/files/prometheus-rules.yaml'), true);
    assert.equal(files.includes('deploy/kubernetes/converact/files/grafana-dashboard.json'), true);
    ...
    assert.equal(files.includes('docs/converact-component-governance.md'), true);
    assert.equal(
    ...
    for (const profile of ['core', 'ai', 'observability', 'benchmark']) {
      assert.equal(
        files.includes(`deploy/kubernetes/converact/profiles/${profile}.values.yaml`),
        true
      );
    }
    assert.equal(
      files.includes('deploy/kubernetes/converact/templates/sip-exporter.yaml'),
      true
    );
    ...
    assert.equal(files.includes('docs/converact-monitoring-runbook.md'), true);
    assert.equal(files.includes('docs/converact-fabric-voice-cdr-durability-runbook.md'), true);
    assert.equal(
    ...
    assert.equal(
      files.includes('capacity-runtime/scripts/converact-capacity-network-impairment.ts'),
      true
    );
    assert.equal(
    ...
    assert.equal(
      files.includes('capacity-runtime/scripts/converact-capacity-controller.ts'),
      true
    );
    assert.equal(
      files.includes('capacity-runtime/scripts/converact-capacity-scaling-finalizer.ts'),
      true
    );
    assert.equal(
      files.includes('capacity-runtime/scripts/converact-capacity-platform-finalizer.ts'),
      true
    );
    assert.equal(
    ...
    assert.equal(
      files.includes('capacity-runtime/scripts/converact-cell-admission.ts'),
      true
    );
    assert.equal(
      files.includes('capacity-runtime/scripts/converact-rustdesk-owner-binding.ts'),
      true
    );
    assert.equal(
      files.includes(
    ...
    assert.equal(
      files.includes('deploy/rustdesk-server-fork/converact-rustdesk-owner-binding.ts'),
      true
    );
    assert.equal(
    ...
    assert.equal(migrationManifest.migrations.length, 107);
    ...
    assert.equal(migrationManifest.migrations.some((entry) => entry.file === '123_converact_audit_runtime_roles.sql'), true);
    ...
    assert.equal(migrationManifest.migrations.some((entry) => entry.file === '103_ivekit_voice_cdr_convergence.sql'), true);
    ...
    assert.deepEqual(imageMetadata, {
      ...
      reference: `converact-fabric:${'a'.repeat(12)}`,
      digest: '',
    ...
    assert.doesNotMatch(applicationCompose, /(?:ivekit-(?:opc|service)|converact-fabric):local/);
    assert.match(applicationCompose, /CONVERACT_FABRIC_IMAGE:\?CONVERACT_FABRIC_IMAGE is required/);
    assert.match(
      applicationCompose,
      /CONVERACT_POSTGRES_IMAGE:\?CONVERACT_POSTGRES_IMAGE immutable digest reference is required/
    );
    assert.match(applicationCompose, /CLAMAV_IMAGE:\?CLAMAV_IMAGE immutable digest reference is required/);
    assert.match(applicationCompose, /^  converact:/m);
    ...
    assert.equal(files.includes('docs/converact-fabric-v3-intelligence-operations.md'), true);
    assert.equal(files.includes('docs/converact-fabric-v3-completion-audit.md'), true);
    assert.equal(files.includes('docs/converact-fabric-v5-shared-foundation-design.md'), true);
    assert.equal(files.includes('docs/converact-fabric-v5-stage1-content-intelligence-plan.md'), true);
    assert.equal(files.includes('docs/converact-fabric-v5-stage1-provider-resilience-plan.md'), true);
    assert.equal(files.includes('docs/converact-fabric-v5-stage2-im-livekit-file-plan.md'), true);
    assert.equal(files.includes('docs/converact-fabric-v6-production-closure-design.md'), true);
    assert.equal(files.includes('docs/converact-fabric-v6-production-closure-plan.md'), true);
    assert.equal(files.includes('docs/converact-fabric-v6-real-environment-acceptance.md'), true);
    assert.equal(files.includes('acceptance/tools/converact-controlled-provider.ts'), true);
    assert.equal(files.includes('acceptance/tools/converact-v6-real-acceptance.ts'), true);
    assert.equal(files.includes('deploy/kubernetes/converact/templates/tinode-deployment.yaml'), true);
    assert.equal(files.includes('edge/windows/Publish-ConveractFabricRustDeskEvidence.ps1'), true);
...
test('delivery validation rejects tampered Voice acceptance assets', () => {
  const root = mkdtempSync(join(tmpdir(), 'converact-delivery-voice-acceptance-'));
  const outputDir = join(root, 'bundle');
...
test('delivery validation rejects stale, duplicate, or placeholder acceptance metadata', () => {
  const root = mkdtempSync(join(tmpdir(), 'converact-delivery-acceptance-'));
  const outputDir = join(root, 'bundle');
  const sdkTarball = join(root, 'sdk.tgz');
  const clientDist = join(root, 'client-dist');
  writeFileSync(sdkTarball, 'sdk');
  ...
  try {
    ...
    assert.throws(() => validateConveractFabricDeliveryBundle(outputDir), /acceptance source commit/i);

    writeFileSync(path, `${JSON.stringify({
      ...original,
      known_not_run: [...original.known_not_run, original.known_not_run[0]]
    }, null, 2)}\n`);
    assert.throws(() => validateConveractFabricDeliveryBundle(outputDir), /duplicate known_not_run/i);
    ...
    assert.throws(() => validateConveractFabricDeliveryBundle(outputDir), /placeholder known_not_run/i);
    ...
    assert.throws(() => validateConveractFabricDeliveryBundle(outputDir), /controlled acceptance contract/i);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});

test('Converact Fabric delivery validation rejects extra files and secret material', () => {
  const root = mkdtempSync(join(tmpdir(), 'converact-delivery-secret-'));
  const outputDir = join(root, 'bundle');
  const sdkTarball = join(root, 'sdk.tgz');
  const clientDist = join(root, 'client-dist');
  writeFileSync(sdkTarball, 'sdk');
...
test('delivery CLI source binding rejects invalid commits and dirty worktrees', () => {
  assert.doesNotThrow(() => assertConveractFabricDeliverySourceState('a'.repeat(40), ''));
  assert.throws(
    () => assertConveractFabricDeliverySourceState('not-a-full-commit', ''),
    /full 40-character Git commit/i
  );
  assert.throws(
    () => assertConveractFabricDeliverySourceState('a'.repeat(40), ' M docs/a.md\n?? secret.txt\n'),
    /worktree is dirty/i
  );
});

test('controlled acceptance package binds passed checks to source-scoped evidence', () => {
  const root = mkdtempSync(join(tmpdir(), 'converact-controlled-acceptance-'));
  ...
  try {
    ...
    mkdirSync(clientDist);
    writeFileSync(join(clientDist, 'index.html'), '<!doctype html>');
    const built = buildConveractFabricDeliveryBundle({
      repoRoot,
      outputDir,
      sdkTarball,
      clientDist,
    ...
    assert.doesNotThrow(() => validateConveractFabricDeliveryBundle(outputDir));
...
test('V3 handoff documents state implemented, configurable, and not-run boundaries', () => {
  const roadmap = readFileSync('docs/converact-client-delivery-v1-roadmap.md', 'utf8');
  const design = readFileSync('docs/converact-fabric-video-im-capability-design.md', 'utf8');
  const audit = readFileSync('docs/converact-fabric-v3-completion-audit.md', 'utf8');
  const voiceDesign = readFileSync('docs/converact-fabric-voice-foundation-v1-design.md', 'utf8');
...
test('delivery generation refuses to erase an unowned existing directory', () => {
  const root = mkdtempSync(join(tmpdir(), 'converact-delivery-unowned-'));
  ...
  try {
    assert.throws(
      ...
      /refusing to replace an existing directory without the Converact Fabric ownership marker/
    );
