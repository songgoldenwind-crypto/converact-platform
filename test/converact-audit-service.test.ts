...
import {
  ...
  type ConveractFabricAuditAppendInput,
  type ConveractFabricAuditEvent,
  type ConveractFabricAuditRepository
} from '../src/agent-runtime/converact/operations/audit/index.js';

class MemoryAuditRepository implements ConveractFabricAuditRepository {
  events: ConveractFabricAuditEvent[] = [];
  async append(input: ConveractFabricAuditAppendInput) {
    ...
    const event = { ...input, id: `audit-${this.events.length + 1}`, previous_hash: previous,
      event_hash: String(this.events.length + 1).padStart(64, 'a') } as ConveractFabricAuditEvent;
