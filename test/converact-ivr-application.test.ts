...
import {
  ...
  type ConveractFabricIvrWorkerConfig
...
test('IVR workers are disabled by default with bounded production defaults', () => {
  assert.deepEqual(converactFabricIvrWorkerConfig({}), {
    enabled: false,
    ...
    tenant_limit: 100
  } satisfies ConveractFabricIvrWorkerConfig);
});

test('IVR worker config rejects ambiguous flags, invalid bounds, and retry budgets', () => {
  assert.equal(converactFabricIvrWorkerConfig({ CONVERACT_FABRIC_IVR_WORKERS_ENABLED: '1' }).enabled, true);
  assert.throws(() => converactFabricIvrWorkerConfig({
...
test('enabled IVR worker fails startup when no executor is injected', () => {
  assert.throws(() => startConveractFabricIvrPendingActionWorker({
    pg: new MemoryPg(),
    env: { CONVERACT_FABRIC_IVR_WORKERS_ENABLED: '1' }
  }), /IVR pending-action executor/i);
});

test('Converact Fabric application validates both IVR adapters before starting any worker', () => {
  let starts = 0;
  const worker = () => { starts += 1; return { async stop() {} }; };
  assert.throws(() => startConveractFabricApplication({
    pg: new MemoryPg(),
    env: { CONVERACT_FABRIC_IVR_WORKERS_ENABLED: '1' },
...
test('Converact Fabric application starts IVR workers only when enabled and stops in reverse order', async () => {
  const events: string[] = [];
  ...
  const disabled = startConveractFabricApplication({ pg: new MemoryPg(), env: {}, adapters });
  await disabled.stop();
...
test('production IVR scheduler stop waits for active tenant discovery', async () => {
  ...
  const worker = startConveractFabricIvrPendingActionWorker({
    pg,
    env: { CONVERACT_FABRIC_IVR_WORKERS_ENABLED: '1' },
