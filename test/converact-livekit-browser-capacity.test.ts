...
import type { LiveKitBrowserRoomMeasurement } from '../scripts/converact-livekit-browser-capacity.js';

const RUNNER_PATH = new URL('../scripts/converact-livekit-browser-capacity.ts', import.meta.url);
...
test('LiveKit browser capacity collector is a packaged executable', () => {
  ...
  assert.equal(
    packageJson.scripts['converact:capacity:livekit-browser'],
    'tsx scripts/converact-livekit-browser-capacity.ts'
  );
  assert.equal(
    packageJson.scripts['converact:capacity:livekit-browser-evidence'],
    'tsx scripts/converact-livekit-browser-evidence.ts'
  );
});

test('LiveKit browser capacity collector validates exact token coverage without leaking tokens', async () => {
  const module = await import('../scripts/converact-livekit-browser-capacity.js');
...
test('LiveKit evaluator fails closed on missing or inconsistent latency distributions', async () => {
  const collector = await import('../scripts/converact-livekit-browser-capacity.js');
...
test('LiveKit browser evidence CLI writes private non-overwriting evaluated evidence', async () => {
  const collector = await import('../scripts/converact-livekit-browser-capacity.js');
  const evidenceCli = await import('../scripts/converact-livekit-browser-evidence.js');
  const directory = mkdtempSync(join(tmpdir(), 'converact-livekit-browser-evidence-'));
...
test('LiveKit browser token input and result evidence use private non-overwriting files', async () => {
  ...
  const directory = mkdtempSync(join(tmpdir(), 'converact-livekit-browser-capacity-'));
...
test('LiveKit browser default runtime exposes a real Playwright measurement boundary', async () => {
  ...
  assert.equal(
    serializedPageFunction.match(/converact-capacity-av/g)?.length,
    2,
...
function liveKitBrowserInput(): LiveKitCapacityProcessInput {
  return {
    ...
    result_path: '/var/lib/converact/run-livekit-browser-001/evidence.json'
  };
}
