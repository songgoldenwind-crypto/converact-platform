...
} from '../scripts/converact-component-node-admission.js';
...
test('capacity image and deployment templates ship the component node agent', () => {
  ...
  assert.match(dockerfile, /converact-component-node-admission\.ts/);
  assert.match(
    dockerfile,
  ...
  assert.equal(
    packageJson.scripts['converact:capacity:component-node'],
    'node --import tsx scripts/converact-component-node-admission.ts'
  );
});
