...
test('Helm keeps V3 provider tokens in Secret and renders all worker controls', () => {
  ...
  const secretKeys = [
    'ivekit-ocr-provider-token',
    'ivekit-asr-provider-token',
    'ivekit-quality-provider-token',
    'ivekit-translation-provider-token'
  ];
...
test('V3 operations and LED handoff docs preserve deployment and validation boundaries', () => {
  const operations = readFileSync('docs/ivekit-v3-intelligence-operations.md', 'utf8');
  const openapi = readFileSync('docs/ivekit-openapi.md', 'utf8');
  const led = readFileSync('docs/ivekit-led-integration-guide.md', 'utf8');
  ...
  assert.match(operations, /ivekit:controlled-provider/);
  assert.match(operations, /ivekit:intelligence-preflight/);
