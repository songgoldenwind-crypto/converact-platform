...
import { createConveractFabricModule } from '../src/agent-runtime/converact/index.js';
...
test('createConveractFabricModule exposes the reusable first-version facades', () => {
  const db = createDatabase(':memory:');
  const pg = new MemoryPg();
  const converactFabric = createConveractFabricModule({ db, pg });

  assert.equal(typeof converactFabric.sessions.open, 'function');
  assert.equal(typeof converactFabric.sessions.getByBusinessRef, 'function');
  assert.equal(typeof converactFabric.media.createRoom, 'function');
  assert.equal(typeof converactFabric.media.issueJoinPlan, 'function');
  assert.equal(typeof converactFabric.collaboration.postMessage, 'function');
  assert.equal(typeof converactFabric.collaboration.addTranslation, 'function');
  assert.equal(typeof converactFabric.remote.create, 'function');
  assert.equal(typeof converactFabric.remote.createWebAssistJoin, 'function');
  assert.equal(typeof converactFabric.remote.verifyWebAssistJoin, 'function');
  assert.equal(typeof converactFabric.remote.recordAssistEvent, 'function');
  assert.equal(typeof converactFabric.evidence.record, 'function');
  assert.equal(typeof converactFabric.remote.endExternalTool, 'function');
  assert.equal(typeof converactFabric.remote.end, 'function');
  assert.equal(typeof converactFabric.rustdesk.registerDevice, 'function');
  assert.equal(typeof converactFabric.rustdesk.listDevicesByBusinessRef, 'function');
  assert.equal(typeof converactFabric.rustdesk.getClientConfig, 'function');
  assert.equal(typeof converactFabric.rustdesk.getGatewayLaunchPlan, 'function');
  assert.equal(typeof converactFabric.rustdesk.startGatewaySession, 'function');
  assert.equal(typeof converactFabric.rustdesk.recordGatewayEvent, 'function');
  assert.equal(typeof converactFabric.rustdesk.listGatewayAuditEvents, 'function');
  assert.equal(typeof converactFabric.rustdesk.listGatewaySessions, 'function');
  assert.equal(typeof converactFabric.rustdesk.endGatewaySession, 'function');
  assert.equal(typeof converactFabric.rustdesk.heartbeatDevice, 'function');
  assert.equal(typeof converactFabric.rustdesk.deactivateDevice, 'function');
...
test('sessions.open creates a LED service order video and Web Assist bundle', async () => {
  ...
  const bundle = await converactFabric.sessions.open({
    tenant_id: tenantId,
    business_ref: {
      tenant_id: tenantId,
      type: 'service_order',
  ...
  const sessions = await converactFabric.sessions.getByBusinessRef({
    tenant_id: tenantId,
...
test('sessions.close facade applies participant revocation and closed-session write guards', async () => {
  ...
  const converactFabric = createConveractFabricModule({ db, pg, chatGateway: gateway });
  const bundle = await converactFabric.sessions.open({
    tenant_id: tenant.id,
    business_ref: {
      tenant_id: tenant.id,
  ...
  await converactFabric.sessions.close({
    tenant_id: tenant.id,
  ...
  await assert.rejects(
    () => converactFabric.collaboration.postMessage({
      tenant_id: tenant.id,
      session_id: bundle.collaboration_session_id,
...
test('remote.createWebAssistJoin requires active consent and returns a short lived join path', async () => {
  ...
  await assert.rejects(
    () =>
      converactFabric.remote.createWebAssistJoin({
        tenant_id: tenantId,
  ...
  await converactFabric.remote.grantConsent({
    tenant_id: tenantId,
    remote_session_id: bundle.remote_session_id!,
  ...
  const join = await converactFabric.remote.createWebAssistJoin({
    tenant_id: tenantId,
    remote_session_id: bundle.remote_session_id!,
    actor_identity: 'engineer_42',
    role: 'engineer',
  ...
  const audit = await converactFabric.remote.listAuditEvents({
    tenant_id: tenantId,
    remote_session_id: bundle.remote_session_id!
  });
...
test('remote.verifyWebAssistJoin accepts issued tokens and rejects tampered or expired tokens', async () => {
  ...
  const tenantId = tenant.id;
  const converactFabric = createConveractFabricModule({
    db,
    pg,
    media: {
      livekit: {
  ...
  const verified = await converactFabric.remote.verifyWebAssistJoin({
    tenant_id: tenantId,
    remote_session_id: bundle.remote_session_id!,
    token
  });
  ...
  await assert.rejects(
    () =>
      converactFabric.remote.verifyWebAssistJoin({
        tenant_id: tenantId,
        remote_session_id: bundle.remote_session_id!,
...
test('remote.recordAssistEvent stores Web Assist events and stops after consent revocation', async () => {
  ...
  const pointer = await converactFabric.remote.recordAssistEvent({
    tenant_id: tenantId,
    remote_session_id: bundle.remote_session_id!,
    actor_identity: 'engineer_42',
  ...
  await converactFabric.remote.revokeConsent({
    tenant_id: tenantId,
    remote_session_id: bundle.remote_session_id!,
    actor_identity: 'buyer_1003',
    scopes: ['view_screen']
  ...
  await assert.rejects(
    () =>
      converactFabric.remote.recordAssistEvent({
        tenant_id: tenantId,
        remote_session_id: bundle.remote_session_id!,
        actor_identity: 'engineer_42',
...
test('Converact Fabric facade types expose attachment messages and inline control events', async () => {
  const db = createDatabase(':memory:');
  const pg = new MemoryPg();
  ...
  const message = await converactFabric.collaboration.postMessage({
    tenant_id: tenantId,
    session_id: bundle.collaboration_session_id,
    ...
    attachments: [
      {
        kind: 'image',
        storage_url: 's3://converact-chat/order_facade_contract_1007/photo.png',
        filename: 'photo.png',
  ...
  const action = await converactFabric.remote.recordAssistEvent({
    tenant_id: tenantId,
    remote_session_id: bundle.remote_session_id!,
    actor_identity: 'engineer_42',
  ...
  const result = await converactFabric.remote.recordAssistEvent({
    tenant_id: tenantId,
    remote_session_id: bundle.remote_session_id!,
    actor_identity: 'buyer_1007',
...
test('remote facade can explicitly end external tools and remote sessions', async () => {
  ...
  const firstTool = await converactFabric.remote.startExternalTool({
    tenant_id: tenantId,
    remote_session_id: bundle.remote_session_id!,
    actor_identity: 'engineer_42',
    provider: 'anydesk',
  ...
  const endedTool = await converactFabric.remote.endExternalTool({
    tenant_id: tenantId,
    remote_session_id: bundle.remote_session_id!,
  ...
  const secondTool = await converactFabric.remote.startExternalTool({
    tenant_id: tenantId,
    remote_session_id: bundle.remote_session_id!,
    actor_identity: 'engineer_42',
    provider: 'anydesk',
  ...
  const endedRemote = await converactFabric.remote.end({
    tenant_id: tenantId,
    remote_session_id: bundle.remote_session_id!,
    actor_identity: 'engineer_42'
  });
...
test('remote facade ends RustDesk upstream gateway sessions when ending a tool', async () => {
  ...
  const tool = await collaboration.remote.startGatewayToolSession({
    ...
    gateway: {
      ...
      launch_url: 'https://converact.example.com/remote/rustdesk/launch?session_id=rdgw-facade-end-1',
  ...
  const ended = await converactFabric.remote.endExternalTool({
    tenant_id: tenantId,
    remote_session_id: bundle.remote_session_id!,
...
test('remote facade revoke closes a local RustDesk gateway and queues physical disconnect', async () => {
  ...
  const device = await converactFabric.rustdesk.registerDevice({
    tenant_id: tenantId,
    business_ref: bundle.business_ref,
  ...
  const gateway = await new RustDeskGatewaySessionStore(pg).createSession({
    ...
    permissions: ['view_screen', 'control_mouse_keyboard'],
    actor_identity: 'engineer_42',
    launch_url: 'https://converact.example.com/remote/rustdesk/launch?session_id=local-revoke',
    metadata: {
  ...
  const revoked = await converactFabric.remote.revokeConsent({
    tenant_id: tenantId,
    remote_session_id: bundle.remote_session_id!,
    actor_identity: 'buyer_1012',
    scopes: ['view_screen', 'control_mouse_keyboard']
...
test('remote facade ends active RustDesk upstream gateway sessions when ending a remote session', async () => {
  ...
  await collaboration.remote.startGatewayToolSession({
    ...
    gateway: {
      ...
      launch_url: 'https://converact.example.com/remote/rustdesk/launch?session_id=rdgw-facade-remote-end-1',
  ...
  const ended = await converactFabric.remote.end({
    tenant_id: tenantId,
    remote_session_id: bundle.remote_session_id!,
    actor_identity: 'engineer_42'
  });
...
test('rustdesk facade requires a fresh online heartbeat before starting registered device sessions when enabled', async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => converactFabric.rustdesk.startGatewaySession({
        tenant_id: tenantId,
        remote_session_id: bundle.remote_session_id!,
        actor_identity: 'engineer_42',
        device_id: device.id,
    ...
    await converactFabric.rustdesk.heartbeatDevice({
      tenant_id: tenantId,
      device_id: device.id,
    ...
    await converactFabric.rustdesk.heartbeatDevice({
      ...
      metadata: {
        ...
        edge_instance_id: 'edge-converact-module'
      }
    });

    const tool = await converactFabric.rustdesk.startGatewaySession({
      tenant_id: tenantId,
      remote_session_id: bundle.remote_session_id!,
      actor_identity: 'engineer_42',
      device_id: device.id,
...
test('rustdesk facade registers devices and starts gateway sessions by registered device', async () => {
  ...
  process.env.CONVERACT_RUSTDESK_ID_SERVER = 'id.converact.example';
  process.env.CONVERACT_RUSTDESK_RELAY_SERVER = 'relay.converact.example';
  ...
  try {
    ...
    const devices = await converactFabric.rustdesk.listDevicesByBusinessRef({
      tenant_id: tenantId,
      business_ref: businessRef
    });
    ...
    const heartbeat = await converactFabric.rustdesk.heartbeatDevice({
      tenant_id: tenantId,
      device_id: device.id,
      actor_identity: 'rustdesk-edge-agent',
      runtime_status: 'online',
    ...
    const clientConfig = await converactFabric.rustdesk.getClientConfig();
    assert.equal(clientConfig.public_key, RUSTDESK_PUBLIC_KEY);
    assert.equal(clientConfig.id_server, 'id.converact.example');
    assert.equal(clientConfig.manual_fields.relay_server, 'relay.converact.example');
    ...
    const gatewaySessions = await converactFabric.rustdesk.listGatewaySessions({
      tenant_id: tenantId,
      status: 'active'
    });
    ...
    const launchPlan = await converactFabric.rustdesk.getGatewayLaunchPlan({
      tenant_id: tenantId,
      external_id: tool.external_id
    });
    ...
    assert.equal(launchPlan.runtime.id_server, 'id.converact.example');
    assert.equal(launchPlan.runtime.relay_server, 'relay.converact.example');
    assert.deepEqual(launchPlan.client_config.manual_fields, {
      id_server: 'id.converact.example',
      relay_server: 'relay.converact.example',
    ...
    await assert.rejects(
      () => converactFabric.rustdesk.getGatewayLaunchPlan({
    ...
    await assert.rejects(
      () => converactFabric.rustdesk.recordGatewayEvent({
        tenant_id: tenantId,
        external_id: tool.external_id,
    ...
    const recordedEvent = await converactFabric.rustdesk.recordGatewayEvent({
      tenant_id: tenantId,
      external_id: tool.external_id,
      event_type: 'remote.rustdesk.recording.started',
      actor_identity: 'engineer_42',
    ...
    const retriedRecordedEvent = await converactFabric.rustdesk.recordGatewayEvent({
      tenant_id: tenantId,
      external_id: tool.external_id,
      event_type: 'remote.rustdesk.recording.started',
      actor_identity: 'engineer_42',
    ...
    const timelineAfterEvent = await converactFabric.remote.listAuditEvents({
      tenant_id: tenantId,
      remote_session_id: bundle.remote_session_id!
    });
    assert.equal(
    ...
    const gatewayAudit = await converactFabric.rustdesk.listGatewayAuditEvents({
      tenant_id: tenantId,
      external_id: tool.external_id
    });
    ...
    await assert.rejects(
      () => converactFabric.rustdesk.listGatewayAuditEvents({
        tenant_id: tenantId,
        external_id: tool.external_id,
        since: 'not-a-date'
      }),
    ...
    await assert.rejects(
      () => converactFabric.rustdesk.endGatewaySession({
        tenant_id: 'tenant_other_rustdesk_facade',
        external_id: tool.external_id,
        actor_identity: 'engineer_42'
      }),
    ...
    const endedGatewaySession = await converactFabric.rustdesk.endGatewaySession({
      tenant_id: tenantId,
      external_id: tool.external_id,
      actor_identity: 'engineer_42'
    });
    ...
    const launchAfterEnd = await converactFabric.rustdesk.getGatewayLaunchPlan({
      tenant_id: tenantId,
      external_id: tool.external_id
    });
    assert.equal(launchAfterEnd.actions.can_launch, false);
    const timelineAfterEnd = await converactFabric.remote.listAuditEvents({
      tenant_id: tenantId,
      remote_session_id: bundle.remote_session_id!
    });
    assert.equal(
    ...
    const deactivated = await converactFabric.rustdesk.deactivateDevice({
      tenant_id: tenantId,
      device_id: device.id
    });
...
test('evidence facade records and lists reusable media evidence', async () => {
  ...
  const evidence = await converactFabric.evidence.record({
    tenant_id: tenantId,
    business_ref: businessRef,
    session_id: 'remote_session_1004',
    kind: 'screen_recording',
    storage_url: 's3://converact-evidence/order_evidence_1004.webm',
  ...
  const byBusiness = await converactFabric.evidence.listByBusinessRef({
    tenant_id: tenantId,
    business_ref: businessRef
  });
  ...
  const bySession = await converactFabric.evidence.listBySession({
    tenant_id: tenantId,
    session_id: 'remote_session_1004'
  });
...
class RecordingGatewayClient implements RemoteGatewayClient {
  ...
  async createSession(input: Parameters<RemoteGatewayClient['createSession']>[0]): Promise<Awaited<ReturnType<RemoteGatewayClient['createSession']>>> {
    ...
    return {
      ...
      launch_url: `https://converact.example.com/remote/rustdesk/launch?session_id=rdgw-recording-${this.createSequence}`,
      target: input.target,
