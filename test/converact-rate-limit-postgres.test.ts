...
import { PostgresConveractFabricRateLimitStore } from '../src/agent-runtime/converact/operations/rate-limit/index.js';
...
test('Postgres rate limit store atomically increments fixed-window buckets', async () => {
  const pg = new RecordingPg((sql, params) => /INSERT INTO ivekit_rate_limit_buckets/i.test(sql)
    ...
    : []);
  const result = await new PostgresConveractFabricRateLimitStore(pg).reserve(reservation());
