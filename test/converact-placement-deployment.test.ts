...
test('standalone Compose and source policy ship the placement projector and migration', () => {
  ...
  assert.match(compose, /placement_snapshot:\/run\/converact-placement/);
...
test('Cell admission examples declare component capabilities instead of interchangeable nodes', () => {
  ...
  assert.match(
    env,
    /"node_id":"converact-rustdesk-cell-0"[\s\S]*"interaction_kinds":\["rustdesk_remote"\]/
  );
  assert.match(
    env,
    /"converact-rustdesk-cell-0":\{"id_server":"rustdesk-0\.example\.com"[\s\S]*"owner_binding_endpoint":"http:\/\/converact-rustdesk-cell-0\.converact-rustdesk-cell:3211"/
  );
...
test('standalone Cell deployment admits Tinode through its native owner guard', () => {
  ...
  assert.match(admission, /CONVERACT_FABRIC_CELL_INTERACTION_KINDS: \$\{CONVERACT_FABRIC_CELL_INTERACTION_KINDS/);
  assert.match(admission, /CONVERACT_FABRIC_CELL_DIMENSIONS_JSON: \$\{CONVERACT_FABRIC_CELL_DIMENSIONS_JSON/);
  assert.match(admission, /CONVERACT_FABRIC_CELL_NODES_JSON: \$\{CONVERACT_FABRIC_CELL_NODES_JSON/);
  assert.match(capacity, /CONVERACT_FABRIC_CELL_PROBES_JSON: \$\{CONVERACT_FABRIC_CELL_CAPACITY_PROBES_JSON/);
  assert.match(capacity, /tinode-component-node:[\s\S]*condition: service_healthy/);

  const kinds = envValue(env, 'CONVERACT_FABRIC_CELL_INTERACTION_KINDS').split(',');
  const nodes = JSON.parse(envValue(env, 'CONVERACT_FABRIC_CELL_NODES_JSON')) as Array<{
    node_id: string;
    interaction_kinds: string[];
  }>;
  const probes = JSON.parse(
    envValue(env, 'CONVERACT_FABRIC_CELL_CAPACITY_PROBES_JSON')
  ) as Array<{
    component: string;
    metrics_url: string;
