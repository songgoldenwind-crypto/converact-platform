...
test('LED webhook receiver example uses a durable external inbox and retry-safe status codes', () => {
  const source = readFileSync('sdk/converact/examples/webhook-receiver.ts', 'utf8');
  assert.match(source, /verifyConveractFabricWebhook/);
  assert.match(source, /ConveractFabricWebhookReplayStore/);
