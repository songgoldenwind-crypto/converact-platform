...
const REQUIRED_FILES = [
  ...
  `${ROOT}/README.md`,
  'scripts/converact-env-compat.sh'
];
...
test('RTPengine build inputs are complete and immutable', () => {
  ...
  assert.equal(
    ...
    'converact/rtpengine-toolchain:trixie-20260725-amd64'
  );
  ...
  assert.equal(
    ...
    'converact/rtpengine-toolchain:trixie-20260725-arm64'
  );
...
test('toolchain uses a pinned Debian snapshot and contains build plus runtime dependencies', () => {
  ...
  assert.match(source, /\/opt\/converact-runtime-rootfs/);
  assert.match(source, /\/var\/lib\/dpkg\/status/);
  assert.match(source, /converact-runtime-dpkg-status/);
...
test('runtime Dockerfile creates separate offline artifacts with immutable labels', () => {
  ...
  assert.match(source, /^ARG CONVERACT_FABRIC_RTPENGINE_TOOLCHAIN_IMAGE$/m);
  assert.match(
    source,
    /^FROM \$\{CONVERACT_FABRIC_RTPENGINE_TOOLCHAIN_IMAGE\} AS source-base$/m
  );
  ...
  assert.match(kernelRuntimeIdentity, /CONVERACT_FABRIC_RTPENGINE_KERNEL_SRCVERSION/);
...
test('build script refuses cross compilation and performs final builds offline', () => {
  ...
  assert.match(source, /CONVERACT_FABRIC_RTPENGINE_TOOLCHAIN_IMAGE/);
  ...
  assert.match(source, /CONVERACT_FABRIC_RTPENGINE_KERNEL_HEADERS_DIR/);
...
test('build script exposes the supply-chain evidence gate', () => {
  ...
  assert.match(source, /converact-rtpengine-supply-chain\.ts/);
  assert.match(source, /node --import tsx/);
  assert.match(source, /CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_EVIDENCE_OUTPUT/);
});
...
test('entrypoint resolves kernel identity without writing the root filesystem', () => {
  ...
  assert.match(source, /CONVERACT_FABRIC_RTPENGINE_RUNTIME_MODE/);
  assert.match(
    source,
  ...
  assert.doesNotMatch(source, /CONVERACT_FABRIC_RTPENGINE_KERNEL_SRCVERSION(?:_PATH)?/);
  assert.match(source, /IVEKIT_RTPENGINE_USERSPACE_FALLBACK=true/);
  assert.match(source, /\/run\/converact-rtpengine/);
  ...
  const forged = spawnSync('sh', [`${ROOT}/entrypoint.sh`], {
    ...
    env: {
      ...process.env,
      CONVERACT_FABRIC_RTPENGINE_RUNTIME_MODE: 'kernel',
      CONVERACT_FABRIC_RTPENGINE_KERNEL_SRCVERSION: 'FORGED'
    }
  });
  assert.equal(forged.status, 78);
  assert.match(forged.stderr, /identity does not match image metadata/);

  const root = mkdtempSync(join(tmpdir(), 'converact-rtpengine-entrypoint-'));
  ...
  spawnSync('sh', [`${ROOT}/entrypoint.sh`], {
    ...
    env: {
      ...process.env,
      CONVERACT_FABRIC_RTPENGINE_RUNTIME_MODE: 'auto',
      CONVERACT_FABRIC_RTPENGINE_RUNTIME_DIR: runtime,
      CONVERACT_FABRIC_RTPENGINE_CONFIG_TEMPLATE: template,
      CONVERACT_FABRIC_RTPENGINE_CONFIG_PATH: config,
      CONVERACT_FABRIC_RTPENGINE_INTERFACE: 'public/127.0.0.1!203.0.113.10'
    }
  });
  assert.match(
  ...
  assert.match(
    ...
    /converact_rtpengine_userspace_fallback\{[^}]+\} 1/
  );
  assert.match(
...
test('userspace runtime contract excludes build and package-manager tools', () => {
  ...
  assert.match(entrypoint, /CONVERACT_FABRIC_RTPENGINE_CONFIG_TEMPLATE/);
  assert.match(entrypoint, /CONVERACT_FABRIC_RTPENGINE_CONFIG_PATH/);
  assert.match(entrypoint, /CONVERACT_FABRIC_RTPENGINE_LISTEN_TCP_NG/);
  assert.match(
...
test('Goal 2 command executes the Task 4 build contract', () => {
  ...
  assert.match(
    pkg.scripts['test:converact:voice-media-goal2'],
    /test\/converact-rtpengine-build-contract\.test\.ts/
  );
});
