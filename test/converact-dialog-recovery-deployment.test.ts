...
test('Compose co-locates a durable production dialog-shadow agent with each RustPBX owner', () => {
  for (const compose of [sourceCompose, standaloneCompose]) {
    ...
    assert.match(compose, /CONVERACT_FABRIC_DIALOG_SHADOW_PRODUCTION: "true"/);
    assert.match(compose, /CONVERACT_FABRIC_DIALOG_SHADOW_NODE_ID: \$\{RUSTPBX_OWNER_NODE_ID:/);
    assert.match(compose, /CONVERACT_FABRIC_DIALOG_SHADOW_JOURNAL_PATH: \/app\/dialog-shadow\/dialog-shadow\.wal/);
    assert.match(compose, /CONVERACT_FABRIC_DIALOG_SHADOW_SERVICE_TOKEN_FILE: \/run\/secrets\/dialog-shadow-token/);
    assert.match(compose, /CONVERACT_FABRIC_DIALOG_SHADOW_TLS_KEY_FILE: \/run\/secrets\/dialog-shadow-server-key/);
    assert.match(compose, /CONVERACT_FABRIC_DIALOG_RECOVERY_DATABASE_URL_FILE: \/run\/secrets\/dialog-recovery-database-url/);
    assert.match(compose, /CONVERACT_FABRIC_DIALOG_TERMINAL_REPAIR_INTERVAL_MS: \$\{CONVERACT_FABRIC_DIALOG_TERMINAL_REPAIR_INTERVAL_MS:-1000\}/);
    assert.match(compose, /CONVERACT_FABRIC_DIALOG_TERMINAL_REPAIR_LEASE_TTL_MS: \$\{CONVERACT_FABRIC_DIALOG_TERMINAL_REPAIR_LEASE_TTL_MS:-10000\}/);
    assert.match(compose, /CONVERACT_FABRIC_DIALOG_TERMINAL_REPAIR_TENANT_BATCH_SIZE: \$\{CONVERACT_FABRIC_DIALOG_TERMINAL_REPAIR_TENANT_BATCH_SIZE:-32\}/);
    assert.match(compose, /CONVERACT_FABRIC_DIALOG_SHADOW_NATS_SERVER_FAULT_DOMAINS_FILE: \/run\/secrets\/dialog-shadow-nats-fault-domains/);
    ...
    assert.doesNotMatch(
      compose,
      /CONVERACT_FABRIC_DIALOG_SHADOW_SERVICE_TOKEN:\s+\$\{/
    );
    assert.doesNotMatch(
      compose,
      /CONVERACT_FABRIC_DIALOG_RECOVERY_DATABASE_URL:\s+\$\{/
    );
  }
  ...
  assert.match(
    standaloneCompose,
    /CONVERACT_FABRIC_DIALOG_SHADOW_NODE_ID: \$\{RUSTPBX_OWNER_NODE_ID_B:/
  );
  assert.match(
    standaloneCompose,
...
test('terminal shadow repair tuning is explicit in every supported environment surface', () => {
  for (const example of envExamples) {
    assert.match(example, /^CONVERACT_FABRIC_DIALOG_TERMINAL_REPAIR_INTERVAL_MS=1000$/m);
    assert.match(example, /^CONVERACT_FABRIC_DIALOG_TERMINAL_REPAIR_LEASE_TTL_MS=10000$/m);
    assert.match(example, /^CONVERACT_FABRIC_DIALOG_TERMINAL_REPAIR_TENANT_BATCH_SIZE=32$/m);
  }
});
...
test('Helm makes dialog recovery a fail-closed node-local sidecar contract', () => {
  ...
  assert.match(helmValues, /serviceTokenKey: converact-dialog-shadow-token/);
  assert.match(helmValues, /recoveryDatabaseUrlKey: converact-dialog-recovery-database-url/);
  ...
  assert.match(helmRustPbx, /CONVERACT_FABRIC_DIALOG_SHADOW_PRODUCTION/);
  assert.match(helmRustPbx, /CONVERACT_FABRIC_DIALOG_SHADOW_SPIFFE_TRUST_DOMAIN/);
  assert.match(helmRustPbx, /CONVERACT_FABRIC_DIALOG_RECOVERY_DATABASE_URL_FILE/);
  assert.match(
    helmRustPbx,
    /CONVERACT_FABRIC_DIALOG_TERMINAL_REPAIR_INTERVAL_MS[\s\S]*voice\.dialogShadow\.recovery\.terminalRepairIntervalMs/
  );
  assert.match(
    helmRustPbx,
    /CONVERACT_FABRIC_DIALOG_TERMINAL_REPAIR_LEASE_TTL_MS[\s\S]*voice\.dialogShadow\.recovery\.terminalRepairLeaseTtlMs/
  );
  assert.match(
    helmRustPbx,
    /CONVERACT_FABRIC_DIALOG_TERMINAL_REPAIR_TENANT_BATCH_SIZE[\s\S]*voice\.dialogShadow\.recovery\.terminalRepairTenantBatchSize/
  );
  assert.match(helmRustPbx, /CONVERACT_FABRIC_DIALOG_SHADOW_NATS_PLACEMENT_CLUSTER/);
...
test('legacy Helm entrypoint carries the same fail-closed T1 recovery contract', () => {
  ...
  assert.match(legacyHelmRustPbx, /CONVERACT_FABRIC_DIALOG_RECOVERY_DATABASE_URL_FILE/);
  assert.match(legacyHelmRustPbx, /CONVERACT_FABRIC_DIALOG_TERMINAL_REPAIR_INTERVAL_MS/);
  assert.match(legacyHelmRustPbx, /CONVERACT_FABRIC_DIALOG_SHADOW_NATS_PLACEMENT_CLUSTER/);
