...
test('full platform chart preserves the same Pod-bound gateway contract', () => {
  ...
  assertPodBoundGateway(platformDeployment, '%s-converact-audio-tap.%s.svc');
...
test('single-instance Compose keeps the gateway private to its service network', () => {
  assert.match(
    standaloneCompose,
    /CONVERACT_FABRIC_LIVEKIT_AUDIO_TAP_GATEWAY_URL: \$\{CONVERACT_FABRIC_LIVEKIT_AUDIO_TAP_GATEWAY_URL:-ws:\/\/converact:3010\/api\/ivekit\/realtime-audio-tap\/livekit\}/
  );
  assert.match(
    platformCompose,
    /CONVERACT_FABRIC_LIVEKIT_AUDIO_TAP_GATEWAY_URL: \$\{CONVERACT_FABRIC_LIVEKIT_AUDIO_TAP_GATEWAY_URL:-ws:\/\/converact:3010\/api\/ivekit\/realtime-audio-tap\/livekit\}/
...
function assertRustPbxComposeTap(source: string): void {
  ...
  assert.match(
    source,
    /RUSTPBX_REALTIME_AUDIO_TAP_SOCKET_PATH: \/run\/converact\/realtime-audio-tap\.sock/
  );
  assert.match(rustPbx, /realtime_audio_tap:\/run\/converact/);
}
