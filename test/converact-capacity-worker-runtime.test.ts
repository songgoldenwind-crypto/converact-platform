...
} from '../scripts/converact-capacity-worker.js';
...
test('external worker driver binds immutable binary identity and the fenced command', async () => {
  ...
  const driver = new ExternalJsonCapacityShardDriver({
    spec: {
      ...
      executable: '/opt/converact/bin/tinode-capacity-worker',
  ...
  assert.match(plans[0].result_path, /^[\/]var\/lib\/converact-capacity\/results\/[a-f0-9]{64}\.json$/);
...
test('capacity worker config is explicit, bounded and restart-stable', () => {
  const config = capacityWorkerConfig({
    ...
    CONVERACT_FABRIC_CAPACITY_DRIVER_SPEC_PATH: '/run/converact-capacity/driver-spec.json',
    CONVERACT_FABRIC_CAPACITY_EVIDENCE_PREFIX: 'capacity/cell-10k',
    CONVERACT_FABRIC_CAPACITY_EVIDENCE_S3_BUCKET: 'converact-capacity-evidence',
