...
test('capacity runner renders Kamailio, targets the edge and records RTT plus core metrics', async () => {
  ...
  assert.match(runner, /CONVERACT_FABRIC_CAPACITY_INCLUDE_KAMAILIO/);
  assert.match(runner, /CONVERACT_FABRIC_CAPACITY_RUN_ID/);
  assert.match(runner, /CONVERACT_FABRIC_SIP_TARGET_IP/);
