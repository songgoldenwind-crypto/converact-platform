...
} from '../scripts/converact-rustpbx-media-goal3-finalize.js';
import type {
  RustPbxMediaGoal3AcceptanceEvidence
} from '../scripts/converact-rustpbx-media-goal3-acceptance.js';
...
test('Goal 3 finalizer CLI is exposed through the root package', () => {
  ...
  assert.equal(
    packageJson.scripts['converact:rustpbx-media-goal3:finalize'],
    'node --import tsx scripts/converact-rustpbx-media-goal3-finalize.ts'
  );
});
...
function acceptance(
  ...
  return {
    ...
    environment: {
      ...
      project_name: 'converact-goal3-finalizer',
