...
test('LiveKit native evidence CLI is packaged for repeatable capacity campaigns', () => {
  ...
  assert.equal(
    packageJson.scripts['converact:capacity:livekit-native-evidence'],
    'tsx scripts/converact-livekit-native-evidence.ts'
  );
  assert.equal(
    existsSync(new URL('../scripts/converact-livekit-native-evidence.ts', import.meta.url)),
    true
  );
});

test('LiveKit native evidence CLI parses a fenced file-based evaluation', async () => {
  const module = await import('../scripts/converact-livekit-native-evidence.js');
...
test('LiveKit native evidence CLI writes private non-overwriting evaluated evidence', async () => {
  const module = await import('../scripts/converact-livekit-native-evidence.js');
  const directory = mkdtempSync(join(tmpdir(), 'converact-livekit-native-evidence-'));
