...
} from '../scripts/converact-cell-admission.js';
...
test('Cell admission startup separates ledger cutoff time from recovery lease time', () => {
  const source = readFileSync('scripts/converact-cell-admission.ts', 'utf8');
...
test('capacity deployment templates contain a dedicated admission process', () => {
  ...
  assert.match(compose, /converact-cell-admission\.ts/);
  assert.match(compose, /converact-cell-capacity-projector\.ts/);
