...
test('media-control selects the hybrid router and bounds every transport input', () => {
  const runtime = read('scripts/converact-media-control-agent.ts');
  ...
  for (const name of [
    'CONVERACT_FABRIC_RTPENGINE_NG_ENDPOINT',
    'CONVERACT_FABRIC_RTPENGINE_RUNTIME_MODE',
    ...
    'CONVERACT_FABRIC_MEDIA_CONTROL_WAL_DIRECTORY',
  ...
  assert.match(dockerfile, /\/var\/lib\/converact-media-control/);
...
test('Compose runs an independent bounded RTPengine and persistent media-control WAL', () => {
  ...
  assert.match(rtpengine.image, /\$\{CONVERACT_FABRIC_RTPENGINE_IMAGE:\?/);
  ...
  assert.equal(
    ...
    '${CONVERACT_FABRIC_RTPENGINE_MAX_ACTIVE_CALLS:-100000}'
  );
  assert.equal(
    rtpengine.environment.IVEKIT_RTPENGINE_GUARD_MAX_ENTRIES,
    '${CONVERACT_FABRIC_RTPENGINE_GUARD_MAX_ENTRIES:-1600000}'
  );
  assert.equal(
    rtpengine.environment.IVEKIT_RTPENGINE_REPLAY_SDP_MAX_BYTES,
    '${CONVERACT_FABRIC_RTPENGINE_REPLAY_SDP_MAX_BYTES:-268435456}'
  );
  assert.equal(
  ...
  assert.equal(
    mediaControl.environment.CONVERACT_FABRIC_MEDIA_CONTROL_TRANSPORT,
    'hybrid'
  );
  assert.equal(
    mediaControl.environment.CONVERACT_FABRIC_RTPENGINE_NG_ENDPOINT,
    'tcp://rtpengine:22222'
  );
  assert.equal(
    mediaControl.environment.CONVERACT_FABRIC_MEDIA_CONTROL_WAL_DIRECTORY,
    '/var/lib/converact-media-control'
  );
  assert.ok(
    mediaControl.volumes.includes(
      'media-control-wal:/var/lib/converact-media-control'
    )
  );
  assert.equal(
  ...
  assert.equal(
    mediaControl.environment.CONVERACT_FABRIC_PROCESSING_MEDIA_ENDPOINT,
    'http://127.0.0.1:8093'
  );
  assert.equal(
...
test('Compose preflight rejects mutable images and invalid media ranges', () => {
  const script = 'scripts/converact-rtpengine-deployment-preflight.ts';
  const baseEnvironment = {
    ...process.env,
    CONVERACT_FABRIC_RTPENGINE_RUNTIME_MODE: 'userspace',
    CONVERACT_FABRIC_RTPENGINE_INTERFACE: 'public/203.0.113.10',
    CONVERACT_FABRIC_RTPENGINE_PORT_MIN: '23000',
    CONVERACT_FABRIC_RTPENGINE_PORT_MAX: '32768',
    CONVERACT_FABRIC_PROCESSING_MEDIA_RTP_PORT_START: '40000',
    CONVERACT_FABRIC_PROCESSING_MEDIA_RTP_PORT_END: '59998'
  };
  const mutable = spawnSync(
    ...
    ['--import', 'tsx', script],
    {
      ...
      env: {
        ...baseEnvironment,
        CONVERACT_FABRIC_RTPENGINE_IMAGE: 'example/rtpengine:latest'
      }
    }
  );
  ...
  const reversedRange = spawnSync(
    ...
    ['--import', 'tsx', script],
    {
      ...
      env: {
        ...baseEnvironment,
        CONVERACT_FABRIC_RTPENGINE_IMAGE:
          `example/rtpengine@sha256:${'a'.repeat(64)}`,
        CONVERACT_FABRIC_RTPENGINE_PORT_MIN: '32768',
        CONVERACT_FABRIC_RTPENGINE_PORT_MAX: '23000'
      }
    }
  );
  ...
  const overlappingRange = spawnSync(
    ...
    ['--import', 'tsx', script],
    {
      ...
      env: {
        ...
        CONVERACT_FABRIC_PROCESSING_MEDIA_RTP_PORT_START: '30000',
        CONVERACT_FABRIC_PROCESSING_MEDIA_RTP_PORT_END: '40000'
      }
    }
  );
...
test('Helm chart deploys digest-only userspace media nodes with private NG control', () => {
  ...
  assert.match(daemonset, /CONVERACT_FABRIC_MEDIA_CONTROL_WAL_MAX_BYTES/);
  assert.match(daemonset, /name: voice-media/);
  assert.match(daemonset, /CONVERACT_FABRIC_PROCESSING_MEDIA_ENDPOINT/);
  ...
  assert.match(daemonset, /preStop:[\s\S]*converact-rtpengine-drain/);
...
test('kernel profile fails closed without exact node scope mounts and capabilities', () => {
  ...
  assert.equal(
    kernel.nodeSelector['converact.io/rtpengine-kernel-ready'],
    'true'
  );
