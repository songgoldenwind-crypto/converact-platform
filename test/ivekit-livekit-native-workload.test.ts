...
test('LiveKit native workload CLI is packaged for repeatable campaigns', () => {
  ...
  assert.equal(
    packageJson.scripts['ivekit:capacity:livekit-native-workload'],
    'tsx scripts/ivekit-livekit-native-workload.ts'
  );
});

test('LiveKit native workload CLI writes private non-overwriting manifests', async () => {
  const module = await import('../scripts/ivekit-livekit-native-workload.js');
  const directory = mkdtempSync(join(tmpdir(), 'ivekit-livekit-native-workload-'));
