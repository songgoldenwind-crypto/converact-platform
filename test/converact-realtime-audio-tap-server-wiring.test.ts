...
const source = readFileSync(
  new URL('../src/converact-server.ts', import.meta.url),
  'utf8'
);

test('Converact Fabric server starts realtime audio tap before accepting traffic', () => {
  ...
  const application = source.indexOf('application = startConveractFabricApplication');
...
test('Converact Fabric server exposes grants and injects an authorizer only when available', () => {
  assert.match(
    source,
...
test('Converact Fabric shutdown drains realtime audio tap before application workers', () => {
