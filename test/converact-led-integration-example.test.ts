...
import { runConveractFabricLedExample } from '../scripts/converact-led-integration-example.js';

test('Converact Fabric LED example executes the reusable media and chat sequence without exposing credentials', async () => {
  ...
  const result = await runConveractFabricLedExample({
    baseUrl: 'https://converact.example.com',
    apiKey: 'secret-converact-key',
    tenantId: 'tenant_led',
    userId: 'agent_led',
  ...
  assert.equal(JSON.stringify(result).includes('secret-converact-key'), false);
