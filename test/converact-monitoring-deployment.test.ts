...
test('Converact Fabric Prometheus rules cover shared foundation failure domains with unique alerts', () => {
  ...
  for (const required of [
    'ConveractFabricApiUnavailable',
    'ConveractFabricNotificationQueueStalled',
    'ConveractFabricEventWebhookLag',
    'ConveractFabricEventWebhookFailures',
    'ConveractFabricNotificationDeadLetters',
    'ConveractFabricNotificationProviderUnhealthy',
    'ConveractFabricTinodeDeliveryLag',
    'ConveractFabricIntelligenceRouteExhausted',
    'ConveractFabricMediaPacketLossHigh',
    'ConveractFabricVoiceCommandUncertain',
    'ConveractFabricRealtimeAudioTapFailure',
    'ConveractFabricRealtimeAudioTapDroppingAudio',
    'ConveractFabricRealtimeAudioTapReplayAttempt',
    'ConveractFabricRetentionFailure'
  ]) assert.equal(names.includes(required), true, required);
  assert.equal(rules.every((rule) => Boolean(rule.expr) && Boolean(rule.for)), true);
});

test('Converact Fabric Grafana dashboard is importable and uses only bounded shared metrics', () => {
  ...
  assert.equal(dashboard.uid, 'converact-shared-foundation');
  assert.match(dashboard.title, /Converact Fabric Shared Foundation/);
