...
import {
  ...
  type ConveractFabricRateLimitReservationInput,
  type ConveractFabricRateLimitRepository
...
test('rate limiter HMACs all sensitive dimensions before repository reservation', async () => {
  let captured: ConveractFabricRateLimitReservationInput | null = null;
  const repository: ConveractFabricRateLimitRepository = {
    async reserve(input) {
      captured = input;
      return { allowed: true, retry_after_seconds: 0, denied_scope: null };
    }
  };
  await new ConveractFabricRateLimiter({
    repository,
...
test('rate limiter returns a structured retryable 429 decision', async () => {
  ...
  await assert.rejects(
    () => new ConveractFabricRateLimiter({
      repository,
    ...
    (error: unknown) => error instanceof ConveractFabricRateLimitError
