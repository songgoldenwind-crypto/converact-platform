...
test('Tinode Helm exposes compact and three-node cluster modes with fail-closed validation', () => {
  ...
  assert.match(values, /allowedRepositories:[\s\S]*?converact-tinode-server/);
  ...
  assert.match(helpers, /tinode cluster mode requires the maintained Converact Fabric image repository/);
...
test('Tinode cluster is a stable StatefulSet with separate ring and client services', () => {
  ...
  assert.match(statefulSet, /serviceName: \{\{ include "converact\.tinodeHeadlessFullname" \. \}\}/);
