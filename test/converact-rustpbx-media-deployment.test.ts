...
test('Compose projects fresh RustPBX capacity into Converact placement admission', () => {
  assert.match(voiceCompose, /^  rustpbx-capacity-projector:\n/m);
  assert.match(
    voiceCompose,
    /rustpbx-capacity-projector:[\s\S]*scripts\/converact-cell-capacity-projector\.ts/
  );
  ...
  assert.match(
    voiceCompose,
    /rustpbx-placement-snapshot-projector:[\s\S]*converact-placement-snapshot:\/run\/converact-placement/
  );
  assert.match(
    voiceCompose,
    /CONVERACT_FABRIC_PLACEMENT_ENABLED: \$\{CONVERACT_FABRIC_PLACEMENT_ENABLED:-0\}[\s\S]*CONVERACT_FABRIC_PLACEMENT_SNAPSHOT_FILE: \/run\/converact-placement\/placement\.json/
  );
  assert.match(
    voiceCompose,
    /converact-placement-snapshot:\/run\/converact-placement:ro/
  );
  assert.match(
    voiceCompose,
...
test('RustPBX reaches node-local media-control through mTLS and file secrets only', () => {
  ...
  assert.match(
    rtpengineDaemonSet,
    /CONVERACT_FABRIC_MEDIA_CONTROL_HOST[\s\S]*mediaControl\.host/
  );
  assert.doesNotMatch(
    rtpengineDaemonSet,
    /CONVERACT_FABRIC_MEDIA_CONTROL_HOST[\s\S]{0,100}value: "0\.0\.0\.0"/
  );
  ...
  assert.match(voiceCompose, /CONVERACT_FABRIC_MEDIA_CONTROL_PRODUCTION: "true"/);
  assert.match(voiceCompose, /CONVERACT_FABRIC_MEDIA_CONTROL_REQUIRE_MTLS: "true"/);
  assert.match(voiceCompose, /CONVERACT_FABRIC_MEDIA_CONTROL_HOST: 127\.0\.0\.1/);
  assert.match(
    voiceCompose,
  ...
  assert.match(
    voiceCompose,
    /CONVERACT_FABRIC_MEDIA_CONTROL_TLS_KEY_FILE: \/run\/secrets\/media-control-server-key/
  );
  assert.match(
    voiceCompose,
    /CONVERACT_FABRIC_MEDIA_CONTROL_TLS_CERT_FILE: \/run\/secrets\/media-control-server-cert/
  );
  assert.match(
    voiceCompose,
    /CONVERACT_FABRIC_MEDIA_CONTROL_TLS_CA_FILE: \/run\/secrets\/media-control-ca/
  );
  assert.doesNotMatch(
    voiceCompose,
    /CONVERACT_FABRIC_MEDIA_CONTROL_TOKEN: \$\{CONVERACT_FABRIC_MEDIA_CONTROL_TOKEN/
  );
  for (const secret of [
    'media-control-token',
...
test('deployment wires composite readiness, ordered drain, and fault-domain gates', () => {
  ...
  for (const template of [helmRustPbx, legacyHelmRustPbx]) {
    ...
    assert.match(template, /converact\.io\/t1-shadow-fault-domain/);
  }
...
test('recording, AI tap, and ordinary relay have independent capacity alerts and rollback rules', () => {
  ...
  for (const rules of [canonicalPrometheusRules, legacyHelmRustPbx]) {
    assert.match(rules, /ConveractFabricRustPbxOrdinaryRelayCapacityUnavailable/);
    assert.match(rules, /ConveractFabricRustPbxRecordingProfileExhausted/);
    assert.match(rules, /ConveractFabricRustPbxAiTapProfileExhausted/);
