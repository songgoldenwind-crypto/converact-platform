...
test('component node HTTP runs the local capacity gate only for new reservations', async (t) => {
  ...
  const server = createComponentNodeAdmissionHttpServer({
    ...
    additional_metrics: () => 'converact_local_capacity_gate 1\n'
  });
  const port = await listenOrSkip(t, server);
  if (port === null) return;
  const client = new HttpComponentNodeAdmissionClient({
  ...
  assert.match(metrics, /converact_local_capacity_gate 1/);
});
...
function testCertificates(): {
  ...
  const directory = mkdtempSync(join(tmpdir(), 'converact-component-mtls-'));
  ...
  run(
    ...
    '-subj', '/CN=converact-component-test-ca'
  );
  run(
  ...
  run(
    ...
    '-subj', '/CN=converact-component-client',
