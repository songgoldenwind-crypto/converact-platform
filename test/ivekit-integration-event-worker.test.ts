...
test('integration event projection is versioned and extracts only explicit business refs', () => {
  ...
  assert.equal(projectIveKitIntegrationEvent(event({ payload: { business_ref: { type: '', id: 'SO-1' } } })).business_ref, null);
});
...
test('application owns event webhook lifecycle after notification delivery', async () => {
  ...
  assert.ok(iveKitRuntimeComponents(env).includes('event_webhook_worker'));
});
...
function subscription(overrides: Partial<IveKitEventWebhookSubscription> = {}): IveKitEventWebhookSubscription {
  return {
...
function event(overrides: Partial<IveKitStoredIntegrationEvent> = {}): IveKitStoredIntegrationEvent {
  return {
