...
const API_KEY = 'converact-context-api-key';
const JWT_SECRET = 'converact-context-jwt-secret-with-enough-length';
...
test('Converact Fabric context returns a projected system view without provider secrets', async () => {
...
test('Converact Fabric context scopes chat and remote assistance to active membership', async () => {
  const previous = { apiKey: process.env.CONVERACT_API_KEY, jwtSecret: process.env.CONVERACT_JWT_SECRET };
  process.env.CONVERACT_API_KEY = API_KEY;
  process.env.CONVERACT_JWT_SECRET = JWT_SECRET;
  try {
...
test('Converact Fabric context validates query and rejects mutations', async () => {
  process.env.CONVERACT_API_KEY = API_KEY;
  const pg = new MemoryPg();
...
test('Converact Fabric unified timeline is stable, paged, redacted, and viewer scoped', async () => {
  const previous = { apiKey: process.env.CONVERACT_API_KEY, jwtSecret: process.env.CONVERACT_JWT_SECRET };
  process.env.CONVERACT_API_KEY = API_KEY;
  process.env.CONVERACT_JWT_SECRET = JWT_SECRET;
  try {
