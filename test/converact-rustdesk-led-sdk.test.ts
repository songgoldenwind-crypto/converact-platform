...
test('Converact Fabric RustDesk LED SDK reuses a registered device and starts a launchable session', async () => {
  const calls: string[] = [];
  ...
  assert.equal(result.launch.openUrl, 'https://converact.example.com/remote/rustdesk/launch?session_id=rdgw_1&token=abc&expires_at=2026-07-06T00%3A15%3A00.000Z');
...
test('Converact Fabric RustDesk LED SDK registers a device when no matching business device exists', async () => {
  const calls: string[] = [];
  const registered: RegisterConveractFabricRustDeskDeviceInput[] = [];
  const sdk = createConveractFabricRustDeskLedSdk({
    tenantId: 'tenant_led',
    source: 'led-sdk-test',
...
test('Converact Fabric RustDesk LED SDK exposes audit and end helpers', async () => {
  const calls: string[] = [];
  const sdk = createConveractFabricRustDeskLedSdk({
    tenantId: 'tenant_led',
    client: fakeLedClient(calls)
...
test('Converact Fabric RustDesk LED SDK standardizes typed operation audit helpers', async () => {
  const calls: string[] = [];
  const events: RecordConveractFabricRustDeskGatewayEventInput[] = [];
  const sdk = createConveractFabricRustDeskLedSdk({
    tenantId: 'tenant_led',
  ...
  await sdk.recordScreenRecording('rdgw_1', {
    ...
    recordingId: 'rec_1',
    status: 'stopped',
    storageUrl: 's3://converact-rustdesk/tenant_led/rec_1.mp4',
    durationMs: 1200
  });
  ...
  assert.deepEqual(events.map((event) => event.metadata), [
    ...
    },
    {
      ...
      storage_url: 's3://converact-rustdesk/tenant_led/rec_1.mp4',
      duration_ms: 1200
    },
    {
      clipboard_id: 'clip_1',
...
test('Converact Fabric RustDesk LED SDK preserves the bounded evidence security label', async () => {
  const calls: string[] = [];
  const events: RecordConveractFabricRustDeskGatewayEventInput[] = [];
  const sdk = createConveractFabricRustDeskLedSdk({
    tenantId: 'tenant_led',
...
function fakeLedClient(
  ...
  options: {
    ...
    registered?: RegisterConveractFabricRustDeskDeviceInput[];
    events?: RecordConveractFabricRustDeskGatewayEventInput[];
  } = {}
): ConveractFabricRustDeskHttpClient {
  return {
