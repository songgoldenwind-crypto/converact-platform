...
test('iveKit RustPBX build pins source, toolchain, lockfile, and runtime base', () => {
  ...
  assert.match(buildScript, /PATCHSET="ivekit\.38"/);
  assert.match(
    buildScript,
...
test('RustPBX deployment examples reference the current patchset', () => {
  for (const path of [
    ...
    assert.match(
      ...
      /RUSTPBX_IMAGE=ivekit\/rustpbx:0\.4\.11-ivekit\.38-6c49ee76/,
      path
    );
  }
});
...
test('iveKit publishes native amd64 and arm64 RustPBX images as one manifest', () => {
  ...
  assert.match(imageWorkflow, /VERSION: 0\.4\.11-ivekit\.38-6c49ee76/);
