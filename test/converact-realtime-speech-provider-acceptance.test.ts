...
import {
  runConveractFabricRealtimeSpeechProviderAcceptance
} from '../scripts/converact-realtime-speech-provider-acceptance.js';
...
test('controlled realtime speech acceptance covers transport and routing failure modes', async () => {
  ...
  assert.equal(
    pkg.scripts['converact:realtime-speech-provider-acceptance'],
    'node --import tsx scripts/converact-realtime-speech-provider-acceptance.ts'
  );
});
