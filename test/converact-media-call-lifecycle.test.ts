...
const JWT_SECRET = 'converact-media-call-lifecycle-secret-32-bytes';

test('Converact Fabric media call lifecycle is durable idempotent and tenant scoped', async () => {
  ...
  process.env.CONVERACT_API_KEY = 'converact-media-call-system-key';
  ...
  try {
    ...
    const apiCreated = await mediaRoute(db, pg, 'POST', '/api/ivekit/media/calls', {
      ...
      'X-API-Key': 'converact-media-call-system-key',
...
test('Converact Fabric media call migration defines indexed FORCE RLS lifecycle tables', () => {
