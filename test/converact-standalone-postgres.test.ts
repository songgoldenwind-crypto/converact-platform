...
function standaloneMigrations(): { directory: string; cleanup(): void } {
  const root = mkdtempSync(join(tmpdir(), 'converact-standalone-postgres-'));
...
function converactMigrationsWithoutVoiceFoundation(): { directory: string; cleanup(): void } {
  const root = mkdtempSync(join(tmpdir(), 'converact-v3-shaped-migrations-'));
...
async function seedVoiceIvrTenant(pg: Pool, suffix: string): Promise<void> {
  const tenantId = `converact_rls_${suffix}`;
  const profileId = `converact_voice_profile_${suffix}`;
  const routeId = `converact_voice_route_${suffix}`;
  const callId = `converact_voice_call_${suffix}`;
  const flowId = `converact_ivr_flow_${suffix}`;
  ...
  await pg.query(
    ...
    [`converact_voice_route_version_${suffix}`, tenantId, routeId, hash('route')]
  );
  await pg.query(
  ...
  await pg.query(
    ...
    [`converact_ivr_flow_version_${suffix}`, tenantId, flowId, graph, hash('graph')]
  );
  await pg.query(
    `INSERT INTO ivekit_ivr_sessions
      (id, tenant_id, call_id, flow_id, flow_version, current_node_id)
     VALUES ($1, $2, $3, $4, 1, 'start')`,
    [`converact_ivr_session_${suffix}`, tenantId, callId, flowId]
  );
  await pg.query(
    ...
    [`converact_ivr_step_${suffix}`, tenantId, `converact_ivr_session_${suffix}`, flowId]
  );
}
...
freshTest('standalone PostgreSQL fresh migration is minimal, checksummed, idempotent, and RLS enforced', async () => {
  ...
  try {
    ...
    await applyConveractFabricMigrations(admin, {
      ...
      advisoryLockName: 'converact_test_fresh_migrations'
    });
    ...
    await admin.query(`
      INSERT INTO tenants (id, name) VALUES
        ('converact_rls_a', 'A'),
        ('converact_rls_b', 'B'),
        ('converact_retention_held', 'Retention held'),
        ('converact_retention_ready', 'Retention ready')
    `);
    await admin.query(`
      INSERT INTO collaboration_sessions
        (id, tenant_id, business_ref_type, business_ref_id)
      VALUES ('converact_mutation_recovery_session', 'converact_rls_a', 'test', 'mutation-recovery');
      INSERT INTO collaboration_messages
        (id, tenant_id, session_id, sender_identity, message_type, body,
         provider, provider_topic_id, provider_message_id)
      VALUES
        ('converact_mutation_recovery_message', 'converact_rls_a',
         'converact_mutation_recovery_session', 'sender', 'text', 'before',
      ...
      VALUES
        ('converact_mutation_recovery', 'converact_rls_a',
         'converact_mutation_recovery_session', 'converact_mutation_recovery_message',
      ...
      VALUES
        ('converact_mutation_recovery_outbox', 'converact_rls_a',
         'converact_mutation_recovery_session', 'converact_mutation_recovery_message',
         'converact_mutation_recovery', 1, 'edit', 'grpMutationRecovery', 'after',
    ...
    const recoveredMutationClaim = await new TinodeMessageMutationStore(runtime).claimNext({
      tenant_id: 'converact_rls_a',
      now: new Date('2026-07-16T00:00:00.000Z'),
      lease_ms: 30_000
    });
    assert.equal(recoveredMutationClaim?.id, 'converact_mutation_recovery_outbox');
    ...
    const heldEvent = await admin.query<{ id: string }>(`
      ...
      VALUES ('converact_retention_held', 'retention.held', '2026-07-01T00:00:00.000Z')
      RETURNING id::text
    `);
    await admin.query(`
      INSERT INTO ivekit_tenant_events (tenant_id, event_type, expires_at)
      VALUES ('converact_retention_ready', 'retention.ready', '2026-07-02T00:00:00.000Z')
    `);
    await admin.query(`
      ...
      VALUES
        ('converact_event_hold', 'converact_retention_held', 'tenant_events', 'tenant_event',
         $1, 'legal_case', 'converact-event-hold', 'retention-test')
    `, [heldEvent.rows[0]!.id]);
    ...
    `);
    assert.deepEqual(retentionTenants.rows, [{ tenant_id: 'converact_retention_ready' }]);
    await admin.query(`DELETE FROM ivekit_legal_holds WHERE id = 'converact_event_hold'`);
    await admin.query(`
      DELETE FROM ivekit_tenant_events
      WHERE tenant_id IN ('converact_retention_held', 'converact_retention_ready')
    `);
    await admin.query(`
      DELETE FROM tenants WHERE id IN ('converact_retention_held', 'converact_retention_ready')
    `);
    await admin.query(`
      INSERT INTO collaboration_sessions
        (id, tenant_id, business_ref_type, business_ref_id, title)
      VALUES
        ('converact_authorization_session_a', 'converact_rls_a', 'order', 'AUTH-A', 'Authorization A'),
        ('converact_authorization_session_b', 'converact_rls_b', 'order', 'AUTH-B', 'Authorization B');
      ...
      VALUES
        ('converact_authorization_remote_a', 'converact_rls_a', 'converact_authorization_session_a',
         'order', 'AUTH-A', 'active', 'remote_desktop_gateway', 'rustdesk', 'engineer-a'),
        ('converact_authorization_remote_b', 'converact_rls_b', 'converact_authorization_session_b',
      ...
      VALUES
        ('converact_authorization_device_a', 'converact_rls_a', 'order', 'AUTH-A', 'rustdesk-auth-a', 'Auth A'),
        ('converact_authorization_device_b', 'converact_rls_b', 'order', 'AUTH-B', 'rustdesk-auth-b', 'Auth B');
      INSERT INTO rustdesk_gateway_sessions
        (external_id, tenant_id, target_id, permissions, actor_identity, launch_url)
      VALUES
        ('converact_authorization_gateway_a', 'converact_rls_a', 'rustdesk-auth-a',
         '["view_screen"]', 'engineer-a', 'https://converact.example.test/remote/rustdesk/launch')
    `);
    const authorizationCodes = new RustDeskAuthorizationCodeStore(runtime, {
      secret: 'converact-postgres-authorization-code-secret-at-least-32-bytes'
    });
    const authorizationCode = await authorizationCodes.create({
      tenant_id: 'converact_rls_a',
      remote_session_id: 'converact_authorization_remote_a',
      device_id: 'converact_authorization_device_a',
      scopes: ['view_screen'],
      requested_by: 'customer-a',
      idempotency_key: 'converact-authorization-postgres-a',
      now: '2099-01-01T00:00:00.000Z'
    });
    assert.match(authorizationCode.code || '', /^\d{8}$/);
    assert.equal(await authorizationCodes.get({
      tenant_id: 'converact_rls_b',
    ...
    const consumedAuthorization = await authorizationCodes.consume({
      ...
      authorization_id: authorizationCode.authorization.id,
      verified_by: 'engineer-a',
      external_id: 'converact_authorization_gateway_a',
    ...
    await admin.query(`
      ...
      VALUES
        ('converact_quality_call_a', 'converact_rls_a', 'converact-quality-room-a', 'video', 'active',
         'quality-host-a', 'acceptance', 'QUALITY-A', 'Quality A'),
        ('converact_quality_call_b', 'converact_rls_b', 'converact-quality-room-b', 'video', 'active',
      ...
      VALUES
        ('converact_quality_participant_a', 'converact_rls_a', 'converact_quality_call_a',
         'quality-host-a', 'host', 'joined', 'Quality A', CURRENT_TIMESTAMP),
        ('converact_quality_participant_b', 'converact_rls_b', 'converact_quality_call_b',
    ...
    await admin.query(`
      INSERT INTO collaboration_sessions
        (id, tenant_id, business_ref_type, business_ref_id, title)
      VALUES
        ('converact_visual_session_a', 'converact_rls_a', 'order', 'VISUAL-A', 'Visual A'),
        ('converact_visual_session_b', 'converact_rls_b', 'order', 'VISUAL-B', 'Visual B');
      INSERT INTO collaboration_messages
        (id, tenant_id, session_id, sender_identity, message_type, body)
      VALUES
        ('converact_visual_message_a', 'converact_rls_a', 'converact_visual_session_a', 'agent-a', 'video', ''),
        ('converact_visual_message_b', 'converact_rls_b', 'converact_visual_session_b', 'agent-b', 'video', '');
      ...
      VALUES
        ('converact_visual_attachment_a', 'converact_rls_a', 'converact_visual_session_a',
         'converact_visual_message_a', 'video', 'ivekit://controlled/visual-a',
         'visual-a.mp4', 'video/mp4', 128, '${'a'.repeat(64)}', 'ready'),
        ('converact_visual_attachment_b', 'converact_rls_b', 'converact_visual_session_b',
         'converact_visual_message_b', 'video', 'ivekit://controlled/visual-b',
      ...
      VALUES
        ('converact_visual_job_a', 'converact_rls_a', 'converact_visual_session_a',
         'converact_visual_message_a', 'converact_visual_attachment_a', 'video_frame_ocr', 'succeeded'),
        ('converact_visual_job_b', 'converact_rls_b', 'converact_visual_session_b',
         'converact_visual_message_b', 'converact_visual_attachment_b', 'video_frame_ocr', 'succeeded');
      ...
      VALUES
        ('converact_visual_observation_a', 'converact_rls_a', 'converact_visual_session_a',
         'converact_visual_message_a', 'converact_visual_attachment_a', 'converact_visual_job_a',
         'qr_code', '${'c'.repeat(64)}', 'QR_CODE', 0.99, 2000),
        ('converact_visual_observation_b', 'converact_rls_b', 'converact_visual_session_b',
         'converact_visual_message_b', 'converact_visual_attachment_b', 'converact_visual_job_b',
    ...
    const reservations = await Promise.all([
      new IntelligenceProviderGovernanceStore(runtime).reserve({
        tenant_id: 'converact_rls_a', capability: 'translation',
    ...
    await new IntelligenceProviderGovernanceStore(runtime).complete({
      tenant_id: 'converact_rls_a', lease_id: grantedReservation.lease_id, outcome: 'success'
    });
    const governanceRuntime = await new IntelligenceProviderGovernanceStore(runtime)
      .listRuntime('converact_rls_a');
    ...
    const securePart = {
      tenant_id: 'converact_rls_a', secure_file_id: secureFile.id, part_number: 1,
      size_bytes: 6, sha256: 'a'.repeat(64),
      object_key: `converact_rls_a/${secureFile.id}/parts/1`, etag: 'postgres-etag-1'
    };
    ...
    const completedSecureFile = await secureFiles.completeUpload({
      tenant_id: 'converact_rls_a', secure_file_id: secureFile.id, size_bytes: 6,
      sha256: 'c'.repeat(64), object_key: `converact_rls_a/${secureFile.id}/original`
    });
    assert.equal(completedSecureFile.status, 'scanning');
    await withPgTenant(runtime, 'converact_rls_a', async (tenantPg) => {
      await tenantPg.query(
        ...
        ['converact_secure_message_a', 'converact_rls_a', 'converact_visual_session_a']
      );
      await tenantPg.query(
        `INSERT INTO collaboration_message_attachments
         ...
         VALUES ($1, $2, $3, $4, 'image', 'postgres-contract.png', 'image/png',
                 6, $5, 'pending', $6)`,
        [
          'converact_secure_attachment_a',
          'converact_rls_a',
          'converact_visual_session_a',
          'converact_secure_message_a',
          'c'.repeat(64),
          secureFile.id
        ]
      );
    ...
    const waitingTransition = await deliveryGate.reconcileMessage({
      tenant_id: 'converact_rls_a', message_id: 'converact_secure_message_a'
    });
    assert.equal(waitingTransition?.status, 'blocked_by_file_security');
    assert.equal(waitingTransition?.pending_file_count, 1);
    const waitingMessage = await withPgTenant(runtime, 'converact_rls_a', (tenantPg) => tenantPg.query(
    ...
    await admin.query(
      `INSERT INTO collaboration_chat_bindings
       ...
       VALUES ('converact_tinode_binding_a', 'converact_rls_a', 'converact_visual_session_a',
    ...
    await admin.query(
      `INSERT INTO tinode_inbound_events
       ...
       VALUES ('converact_tinode_event_a', 'converact_rls_a', 'converact_tinode_binding_a',
    ...
    await admin.query(
      `INSERT INTO tinode_inbound_dead_letters
       ...
       VALUES ('converact_tinode_dead_a', 'converact_rls_a', 'converact_tinode_binding_a',
               'converact_tinode_event_a', 'provider_user_unmapped',
               'provider user is not mapped', $1, 0)`,
      ['e'.repeat(64)]
    );
    const tinodeOperations = new TinodeOperationsService({ pg: runtime });
    const tinodeSnapshot = await tinodeOperations.snapshot('converact_rls_a');
    ...
    assert.equal((await tinodeOperations.listDeadLetters({
      tenant_id: 'converact_rls_a'
    }))[0]?.id, 'converact_tinode_dead_a');
    assert.equal((await tinodeOperations.listDeadLetters({
      tenant_id: 'converact_rls_b'
    })).length, 0);
    const manualReplay = await tinodeOperations.replayDeadLetter({
      tenant_id: 'converact_rls_a', dead_letter_id: 'converact_tinode_dead_a',
    ...
    await assert.rejects(
      () => withPgTenant(runtime, 'converact_rls_a', (tenantPg) => tenantPg.query(
    ...
    assert.ok((await secureFiles.discoverScanTenantIds()).includes('converact_rls_a'));
    const scanClaims = await secureFiles.claimScanJobs({
      tenant_id: 'converact_rls_a', worker_id: 'postgres-scan-worker-a',
    ...
    assert.equal((await secureFiles.claimScanJobs({
      tenant_id: 'converact_rls_a', worker_id: 'postgres-scan-worker-b',
      limit: 1, lease_ms: 30_000, max_attempts: 3
    })).length, 0);
    const processingSecureFile = await secureFiles.finishScanJob({
      tenant_id: 'converact_rls_a', secure_file_id: secureFile.id,
    ...
    assert.ok((await derivatives.discoverTenantIds()).includes('converact_rls_a'));
    ...
    const derivativeClaims = await derivatives.claimJobs({
      tenant_id: 'converact_rls_a', worker_id: 'postgres-derivative-worker-a',
      limit: 1, lease_ms: 30_000, max_attempts: 3
    });
    assert.equal(derivativeClaims.length, 1);
    assert.equal((await derivatives.claimJobs({
      tenant_id: 'converact_rls_a', worker_id: 'postgres-derivative-worker-b',
      limit: 1, lease_ms: 30_000, max_attempts: 3
    })).length, 0);
    const readyDerivative = await derivatives.finishJob({
      ...
      object_key: `converact_rls_a/${secureFile.id}/derivatives/image-thumbnail`,
    ...
    await secureFiles.beginUpload({
      tenant_id: 'converact_rls_a', secure_file_id: rustDeskEvidence.id
    });
    await secureFiles.completeUpload({
      tenant_id: 'converact_rls_a', secure_file_id: rustDeskEvidence.id,
      size_bytes: 4, sha256: '8'.repeat(64),
      object_key: `converact_rls_a/${rustDeskEvidence.id}/original`
    });
    ...
    assert.ok((await secureFiles.discoverCleanupTenantIds()).includes('converact_rls_a'));
    const cleanupClaims = await secureFiles.claimCleanupJobs({
      tenant_id: 'converact_rls_a', worker_id: 'postgres-cleanup-worker-a',
      limit: 1, lease_ms: 30_000
    });
    assert.equal(cleanupClaims.length, 1);
    assert.equal((await secureFiles.claimCleanupJobs({
      tenant_id: 'converact_rls_a', worker_id: 'postgres-cleanup-worker-b',
      limit: 1, lease_ms: 30_000
    })).length, 0);
    ...
    assert.equal((await secureFiles.listParts('converact_rls_a', secureFile.id))[0]?.status, 'aborted');
    assert.equal((await derivatives.listJobs('converact_rls_a', secureFile.id))[0]?.status, 'expired');
    await assert.rejects(
      () => secureFiles.getFile('converact_rls_b', secureFile.id),
    ...
    await admin.query(`
      ...
      VALUES
        ('converact_cc_skill_a', 'converact_rls_a', 'Skill A', 'postgres-test', 'postgres-test'),
        ('converact_cc_skill_b', 'converact_rls_b', 'Skill B', 'postgres-test', 'postgres-test')
    `);
    ...
    await withPgTenant(runtime, 'converact_rls_a', async (tenantPg) => {
      ...
      assert.deepEqual(calls.rows, [{ tenant_id: 'converact_rls_a', id: 'converact_voice_call_a' }]);
      const sessions = await tenantPg.query<{ tenant_id: string; id: string }>(
        `SELECT tenant_id, id FROM ivekit_ivr_sessions ORDER BY id`
      );
      assert.deepEqual(sessions.rows, [{ tenant_id: 'converact_rls_a', id: 'converact_ivr_session_a' }]);
      const skills = await tenantPg.query<{ tenant_id: string; id: string }>(
        `SELECT tenant_id, id FROM ivekit_cc_skills ORDER BY id`
      );
      assert.deepEqual(skills.rows, [{ tenant_id: 'converact_rls_a', id: 'converact_cc_skill_a' }]);
      ...
      assert.deepEqual(providerRuntime.rows, [{ tenant_id: 'converact_rls_a' }]);
      ...
      assert.deepEqual(providerLeases.rows, [{ tenant_id: 'converact_rls_a' }]);
      ...
      assert.deepEqual(observations.rows, [{
        id: 'converact_visual_observation_a',
        tenant_id: 'converact_rls_a',
      ...
      await assert.rejects(
        () => tenantPg.query(`
          INSERT INTO ivekit_cc_skills (id, tenant_id, name, created_by, updated_by)
          VALUES ('converact_cc_cross_tenant', 'converact_rls_b', 'Cross tenant', 'postgres-test', 'postgres-test')
        `),
    ...
    await withPgTenant(runtime, 'converact_rls_a', async (tenantPg) => {
      await assert.rejects(
        () => tenantPg.query(`
          ...
          VALUES ('converact_rls_b', 'translation', 'cross-tenant-provider')
        `),
        /row-level security policy/i
      );
    });
    await withPgTenant(runtime, 'converact_rls_a', async (tenantPg) => {
      await assert.rejects(
        () => tenantPg.query(`
          ...
          VALUES
            ('converact_visual_cross_tenant', 'converact_rls_b', 'converact_visual_session_b',
             'converact_visual_message_b', 'converact_visual_attachment_b', 'converact_visual_job_b',
             'qr_code', '${'e'.repeat(64)}')
        `),
        /row-level security policy/i
    ...
    await admin.query(`
      ...
      VALUES
        ('converact_voice_config_a_pending', 'converact_rls_a', 'converact_voice_profile_a',
         'sip_trunk', 'trunk-a', 'apply', 'pending', 'voice-config-a-pending',
         '${'a'.repeat(64)}', 0, 3, NULL, ''),
        ('converact_voice_config_a_expired', 'converact_rls_a', 'converact_voice_profile_a',
         'route', 'route-a', 'apply', 'processing', 'voice-config-a-expired',
         '${'b'.repeat(64)}', 1, 3, '2026-07-12T11:59:00.000Z', 'crashed-worker'),
        ('converact_voice_config_a_race', 'converact_rls_a', 'converact_voice_profile_a',
         'extension', 'extension-a', 'apply', 'pending', 'voice-config-a-race',
         '${'c'.repeat(64)}', 0, 3, NULL, ''),
        ('converact_voice_config_b_pending', 'converact_rls_b', 'converact_voice_profile_b',
    ...
    await withPgTenant(runtime, 'converact_rls_a', async (tenantPg) => {
      ...
      assert.deepEqual(commands.rows, [
        { id: 'converact_voice_config_a_expired', tenant_id: 'converact_rls_a' },
        { id: 'converact_voice_config_a_pending', tenant_id: 'converact_rls_a' },
        { id: 'converact_voice_config_a_race', tenant_id: 'converact_rls_a' }
      ]);
      const foreignClaim = await tenantPg.query(`
        UPDATE ivekit_voice_configuration_commands
        SET worker_id = 'cross-tenant-worker'
        WHERE id = 'converact_voice_config_b_pending'
        RETURNING id
      `);
    ...
    const directProfiles = await runtime.query(
      `SELECT id FROM ivekit_voice_deployment_profiles WHERE id = 'converact_voice_profile_b'`
    );
    ...
    }>(`SELECT * FROM opc_ivekit_voice_profile_context($1)`, ['converact_voice_profile_b']);
    assert.deepEqual(profileContext.rows, [{
      tenant_id: 'converact_rls_b',
      profile_id: 'converact_voice_profile_b',
      adapter: 'controlled',
      secret_refs: {}
    }]);
    ...
    try {
      ...
      await firstWorker.query(`SELECT set_config('app.current_tenant', 'converact_rls_a', true)`);
      await secondWorker.query(`SELECT set_config('app.current_tenant', 'converact_rls_a', true)`);
      const firstClaim = await firstWorker.query(`
        WITH candidate AS (
          SELECT id
          FROM ivekit_voice_configuration_commands
          WHERE id = 'converact_voice_config_a_race' AND state = 'pending'
      ...
      `);
      assert.deepEqual(firstClaim.rows, [{ id: 'converact_voice_config_a_race' }]);
    ...
    await withPgTenant(runtime, 'converact_rls_a', async (tenantPg) => {
      const reclaimed = await tenantPg.query<{ id: string; worker_id: string }>(`
        WITH candidate AS (
          SELECT id
          FROM ivekit_voice_configuration_commands
          WHERE id = 'converact_voice_config_a_expired'
      ...
      assert.deepEqual(reclaimed.rows, [{
        id: 'converact_voice_config_a_expired',
        worker_id: 'voice-recovery-worker'
      }]);
    });

    const configurationStore = new PostgresVoiceConfigurationStore(runtime);
    const profile = await configurationStore.getProfile('converact_rls_a', 'converact_voice_profile_a');
    ...
    const voiceCall = await callStore.get('converact_rls_a', 'converact_voice_call_a');
    ...
    const mediaBridge = await mediaCallService.ensureVoiceBridge({
      tenant_id: 'converact_rls_a', voice_call_id: voiceCall!.id, initiated_by: 'postgres-store-test',
      participant_identity: 'voice-sip-postgres-a', idempotency_key: 'voice-bridge-postgres-a',
      business_ref: { tenant_id: 'converact_rls_a', type: 'order', id: 'VOICE-BRIDGE-A' }
    });
    ...
    process.env.CONVERACT_API_KEY = 'converact-voice-http-postgres-key';
    try {
      ...
      const authHeaders = {
        'x-api-key': 'converact-voice-http-postgres-key',
        'x-tenant-id': 'converact_rls_a',
      ...
      const preflight = await invokeVoice(
        'POST', '/api/ivekit/voice/profiles/converact_voice_profile_a/preflight'
      );
      ...
      const created = await invokeVoice('POST', '/api/ivekit/voice/calls', {
        profile_id: 'converact_voice_profile_a',
      ...
      await withPgTenant(runtime, 'converact_rls_a', (tenantPg) => tenantPg.query(
        `UPDATE ivekit_voice_call_commands
         SET state = 'succeeded', completed_at = CURRENT_TIMESTAMP
         WHERE tenant_id = $1 AND id = $2`,
        ['converact_rls_a', created.data.command.id]
      ));
    } finally {
    ...
    const durableCallCommand: VoiceCallCommand = {
      id: 'converact_voice_store_call_command',
      tenant_id: 'converact_rls_a',
      call_id: 'converact_voice_call_a',
      kind: 'hangup',
      state: 'pending',
      idempotency_key: 'converact-voice-store-call-command',
    ...
    const providerEvent: VoiceProviderEvent = {
      id: 'converact_voice_store_event',
      tenant_id: 'converact_rls_a',
      profile_id: 'converact_voice_profile_a',
      call_id: 'converact_voice_call_a',
      external_event_id: 'converact-provider-event-store-a',
    ...
    const recording: VoiceRecording = {
      id: 'converact_voice_store_recording',
      tenant_id: 'converact_rls_a',
      call_id: 'converact_voice_call_a',
      profile_id: 'converact_voice_profile_a',
    ...
    await assert.rejects(
      () => withPgTenant(runtime, 'converact_rls_a', (tenantPg) => tenantPg.query(`
        ...
        VALUES ('converact_cross_tenant_voice_profile', 'converact_rls_b', 'Cross tenant', 'controlled')
      `)),
      /row-level security|policy/i
    );
    await assert.rejects(
      () => withPgTenant(runtime, 'converact_rls_a', (tenantPg) => tenantPg.query(`
        INSERT INTO ivekit_voice_calls
          (id, tenant_id, business_ref_type, business_ref_id, provider_profile_id,
           ...
           to_address_redacted, idempotency_key)
        VALUES
          ('converact_bad_hmac', 'converact_rls_a', 'order', 'BAD-HMAC', 'converact_voice_profile_a',
      ...
      () => withPgTenant(runtime, 'converact_rls_a', (tenantPg) => tenantPg.query(`
        INSERT INTO ivekit_voice_calls
          (id, tenant_id, business_ref_type, business_ref_id, provider_profile_id,
           ...
           to_address_redacted, idempotency_key)
        VALUES
          ('converact_duplicate_call', 'converact_rls_a', 'order', 'DUPLICATE', 'converact_voice_profile_a',
    ...
    await assert.rejects(
      () => withPgTenant(runtime, 'converact_rls_a', (tenantPg) => tenantPg.query(`
        ...
        VALUES
          ('converact_bad_attempts', 'converact_rls_a', 'converact_voice_call_a', 'hangup',
    ...
    await assert.rejects(
      () => withPgTenant(runtime, 'converact_rls_a', (tenantPg) => tenantPg.query(`
        INSERT INTO ivekit_ivr_flow_versions
          (id, tenant_id, flow_id, version, graph, graph_hash, published_by)
        VALUES
          ('converact_duplicate_flow_version', 'converact_rls_a', 'converact_ivr_flow_a', 1,
    ...
    await assert.rejects(
      () => withPgTenant(runtime, 'converact_rls_a', (tenantPg) => tenantPg.query(`
        ...
        VALUES
          ('converact_cross_tenant_session', 'converact_rls_a', 'converact_voice_call_b',
           'converact_ivr_flow_a', 1)
      `)),
    ...
    const deployedRouteVersion = await admin.query<{ deployment_state: string; provider_revision: string }>(`
      ...
      WHERE tenant_id = 'converact_rls_a' AND id = 'converact_voice_route_version_a'
    ...
    for (const statement of [
      ...
      `UPDATE ivekit_ivr_flow_versions SET graph = '{}'::JSONB
       WHERE tenant_id = 'converact_rls_a' AND id = 'converact_ivr_flow_version_a'`,
      `DELETE FROM ivekit_ivr_flow_versions
       WHERE tenant_id = 'converact_rls_a' AND id = 'converact_ivr_flow_version_a'`,
      `UPDATE ivekit_ivr_session_steps SET branch_taken = 'changed'
       WHERE tenant_id = 'converact_rls_a' AND id = 'converact_ivr_step_a'`,
      `DELETE FROM ivekit_ivr_session_steps
       WHERE tenant_id = 'converact_rls_a' AND id = 'converact_ivr_step_a'`
    ]) await assert.rejects(() => admin.query(statement), /immutable/i);

    await admin.query(`INSERT INTO tenants (id, name) VALUES ('converact_rls_cascade', 'Cascade')`);
    await seedVoiceIvrTenant(admin, 'cascade');
    await admin.query(`DELETE FROM tenants WHERE id = 'converact_rls_cascade'`);
    assert.equal((await admin.query(
      `SELECT id FROM ivekit_ivr_session_steps WHERE tenant_id = 'converact_rls_cascade'`
    )).rowCount, 0);
    ...
    await admin.query(`
      ...
      VALUES
        ('converact_rls_a', TRUE, 'translation-a', 'admin'),
        ('converact_rls_b', TRUE, 'translation-b', 'admin')
    `);
    ...
    await admin.query(`
      INSERT INTO collaboration_sessions
        (id, tenant_id, business_ref_type, business_ref_id, title)
      VALUES ('converact_worker_recovery_session', 'converact_rls_a', 'order', 'RECOVERY-1', 'Worker recovery');
      INSERT INTO collaboration_messages
        (id, tenant_id, session_id, sender_identity, message_type, body)
      VALUES
        ('converact_worker_attachment_message', 'converact_rls_a', 'converact_worker_recovery_session', 'agent-a', 'file', ''),
        ('converact_worker_quality_message', 'converact_rls_a', 'converact_worker_recovery_session', 'agent-a', 'text', '${qualityBody}'),
        ('converact_worker_translation_message', 'converact_rls_a', 'converact_worker_recovery_session', 'agent-a', 'text', '${translationBody}');
      INSERT INTO collaboration_message_attachments
        (id, tenant_id, session_id, message_id, kind, storage_url, filename,
         content_type, size_bytes, checksum, processing_status)
      VALUES
        ('converact_worker_attachment', 'converact_rls_a', 'converact_worker_recovery_session',
         'converact_worker_attachment_message', 'image', 'ivekit://controlled/recovery',
         'recovery.txt', 'text/plain', 19, 'sha256-worker-recovery', 'pending');
      INSERT INTO collaboration_attachment_processing_jobs
        (id, tenant_id, session_id, message_id, attachment_id, processor, status,
         attempt_count, max_attempts, lease_until, worker_id)
      VALUES
        ('converact_worker_attachment_job', 'converact_rls_a', 'converact_worker_recovery_session',
         'converact_worker_attachment_message', 'converact_worker_attachment', 'ocr', 'processing',
      ...
      VALUES
        ('converact_worker_translation_job', 'converact_rls_a', 'converact_worker_recovery_session',
         'converact_worker_translation_message', 'message', 'converact_worker_translation_message',
         'auto', 'zh-CN', '${translationHash}', 'processing', 1, 3,
         '2026-07-12T11:59:00.000Z', 'crashed-translation-worker',
         'converact-worker-translation-recovery', '${'d'.repeat(64)}', FALSE)
    `);
    ...
    assert.deepEqual(await attachmentRecovery.runDue({ tenant_id: 'converact_rls_a', limit: 10 }), {
      candidates: 1, claimed: 1, succeeded: 1, retry_wait: 0, failed: 0
    });
    const queuedQuality = await qualityRecovery.enqueueMessage({
      tenant_id: 'converact_rls_a',
      message_id: 'converact_worker_quality_message'
    }, { automatic: false });
    assert.ok(queuedQuality);
    await admin.query(`
      UPDATE collaboration_quality_review_jobs
      SET id = 'converact_worker_quality_job', status = 'processing', attempt_count = 1,
          lease_until = '2026-07-12T11:59:00.000Z', worker_id = 'crashed-quality-worker'
      WHERE tenant_id = 'converact_rls_a' AND message_id = 'converact_worker_quality_message'
    `);
    assert.deepEqual(await qualityRecovery.runDue({ tenant_id: 'converact_rls_a', limit: 10 }), {
      candidates: 1, claimed: 1, succeeded: 1, retry_wait: 0, failed: 0
    });
    assert.deepEqual(await translationRecovery.runDue({ tenant_id: 'converact_rls_a', limit: 10 }), {
      candidates: 1, claimed: 1, succeeded: 1, retry_wait: 0, failed: 0
    });
    const recoveredJobs = await admin.query<{
      ...
      FROM (
        ...
        WHERE id = 'converact_worker_attachment_job'
        UNION ALL
        SELECT id, status, attempt_count, worker_id, lease_until
        FROM collaboration_quality_review_jobs
        WHERE id = 'converact_worker_quality_job'
        UNION ALL
        SELECT id, status, attempt_count, worker_id, lease_until
        FROM collaboration_translation_jobs
        WHERE id = 'converact_worker_translation_job'
      ) recovered
      ORDER BY id
    `);
    assert.deepEqual(recoveredJobs.rows, [
      { id: 'converact_worker_attachment_job', status: 'succeeded', attempt_count: 2, worker_id: '', lease_until: null },
      { id: 'converact_worker_quality_job', status: 'succeeded', attempt_count: 2, worker_id: '', lease_until: null },
      { id: 'converact_worker_translation_job', status: 'succeeded', attempt_count: 2, worker_id: '', lease_until: null }
    ]);
    await admin.query(`
      INSERT INTO rustdesk_devices
        (id, tenant_id, business_ref_type, business_ref_id, rustdesk_id, display_name)
      VALUES ('converact_recovery_device', 'converact_rls_a', 'order', 'A-1', '123456789', 'Recovery device');
      INSERT INTO rustdesk_gateway_sessions
        (external_id, tenant_id, target_id, actor_identity)
      VALUES
        ('converact_recovery_executed', 'converact_rls_a', 'converact_recovery_device', 'tester'),
        ('converact_recovery_uncertain', 'converact_rls_a', 'converact_recovery_device', 'tester');
      INSERT INTO rustdesk_device_commands
        (id, tenant_id, device_id, external_id, requested_by, requested_reason)
      VALUES
        ('converact_recovery_command_executed', 'converact_rls_a', 'converact_recovery_device', 'converact_recovery_executed', 'tester', 'gateway_ended'),
        ('converact_recovery_command_uncertain', 'converact_rls_a', 'converact_recovery_device', 'converact_recovery_uncertain', 'tester', 'gateway_ended')
    `);
    await withPgTenant(runtime, 'converact_rls_a', async (tenantPg) => {
      ...
      await assert.rejects(
        () => tenantPg.query(`
          ...
          WHERE tenant_id = 'converact_rls_b'
          RETURNING tenant_id
        `).then((result) => {
      ...
      const executedClaim = (await commands.claimNext({
        tenant_id: 'converact_rls_a',
        device_id: 'converact_recovery_device',
        edge_instance_id: 'converact-recovery-edge',
        lease_ms: 1_000,
        now: '2026-07-12T00:00:00.000Z'
      }))!;
      assert.equal(executedClaim.command.id, 'converact_recovery_command_executed');
      ...
      assert.equal(uncertainClaim.command.id, 'converact_recovery_command_uncertain');
    ...
    await admin.query(`
      ...
      VALUES ('converact_rls_session_b', 'converact_rls_b', 'order', 'B-1', 'private B')
    `);
    ...
    await quality.reportQuality({
      tenant_id: 'converact_rls_a',
      call_id: 'converact_quality_call_a',
    ...
    assert.equal(await withPgTenant(runtime, 'converact_rls_a', async (tenantPg) => {
      const foreignQuality = await tenantPg.query(
        `SELECT id FROM ivekit_media_quality_snapshots WHERE tenant_id = 'converact_rls_b'`
      );
    ...
    const eventStore = new ConveractFabricTenantEventStore(runtime, {
      cursor_secret: 'standalone-postgres-event-secret'
    });
    const eventHead = await eventStore.headCursor('converact_rls_a');
    ...
    const foreignReplay = await eventStore.list({
      ...
      role: 'admin',
      cursor: await eventStore.headCursor('converact_rls_b'),
      limit: 10
    });
    assert.deepEqual(foreignReplay.items, []);

    let retentionNow = new Date('2026-07-12T10:00:00.000Z');
    const retentionStore = new ConveractFabricTenantEventStore(runtime, {
    ...
    try {
      ...
      await client.query(`SELECT set_config('app.current_tenant', 'converact_rls_a', true)`);
      await client.query(`SELECT set_config('app.bypass_rls', 'on', true)`);
      const foreign = await client.query(
        `SELECT id FROM collaboration_sessions WHERE tenant_id = 'converact_rls_b'`
      );
      ...
      await assert.rejects(
        () => client.query(`
          INSERT INTO collaboration_sessions
            (id, tenant_id, business_ref_type, business_ref_id)
          VALUES ('converact_cross_tenant_write', 'converact_rls_b', 'order', 'B-2')
        `),
        /row-level security|policy/i
      );
    ...
    const preserved = await admin.query(
      `SELECT id FROM collaboration_sessions WHERE id = 'converact_rls_session_b'`
    );
...
upgradeTest('existing Converact schema upgrades through standalone runner without product or communication data loss', async () => {
  ...
  const migrations = standaloneMigrations();
  const legacyMigrations = converactMigrationsWithoutVoiceFoundation();
  try {
    await runMigrations(admin, {
      directory: legacyMigrations.directory,
      advisoryLockName: 'converact_test_repository_root_migrations'
    });
    await admin.query(`INSERT INTO tenants (id, name) VALUES ('converact_upgrade_tenant', 'Upgrade tenant')`);
    await admin.query(`
      INSERT INTO campaigns (id, tenant_id, name)
      VALUES ('converact_upgrade_campaign', 'converact_upgrade_tenant', 'Preserved campaign')
    `);
    await admin.query(`
      INSERT INTO collaboration_sessions (id, tenant_id, business_ref_type, business_ref_id, title)
      VALUES ('converact_upgrade_session', 'converact_upgrade_tenant', 'order', 'UP-1', 'Preserved session')
    `);
    await admin.query(`
      INSERT INTO ivekit_media_calls
        (id, tenant_id, room_name, media, initiated_by,
         business_ref_type, business_ref_id, title)
      VALUES
        ('converact_upgrade_media', 'converact_upgrade_tenant', 'converact-upgrade-room', 'video',
    ...
    await admin.query(`
      ...
      VALUES
        ('converact_upgrade_remote', 'converact_upgrade_tenant', 'converact_upgrade_session',
         'order', 'UP-1', 'active', 'platform_remote_control', 'upgrade-test')
    `);
    await admin.query(`
      INSERT INTO collaboration_intelligence_policies
        (tenant_id, translation_enabled, translation_profile_id, updated_by)
      VALUES ('converact_upgrade_tenant', TRUE, 'upgrade-translation', 'upgrade-test')
    `);
    const preservedBefore = await admin.query<{ snapshot: Record<string, unknown> }>(`
      SELECT jsonb_build_object(
        'campaign', (SELECT to_jsonb(c) FROM campaigns c WHERE id = 'converact_upgrade_campaign'),
        'session', (SELECT to_jsonb(s) FROM collaboration_sessions s WHERE id = 'converact_upgrade_session'),
        'media', (SELECT to_jsonb(m) FROM ivekit_media_calls m WHERE id = 'converact_upgrade_media'),
        'remote', (SELECT to_jsonb(r) FROM remote_assistance_sessions r WHERE id = 'converact_upgrade_remote'),
        'intelligence', (SELECT to_jsonb(i)
          - 'ocr_profile_ids' - 'asr_profile_ids' - 'quality_profile_ids' - 'translation_profile_ids'
          FROM collaboration_intelligence_policies i WHERE tenant_id = 'converact_upgrade_tenant')
      ) AS snapshot
    `);
    ...
    await applyConveractFabricMigrations(admin, {
      directory: migrations.directory,
      advisoryLockName: 'converact_test_upgrade_migrations'
    });
    await applyConveractFabricMigrations(admin, {
      directory: migrations.directory,
      advisoryLockName: 'converact_test_upgrade_migrations'
    ...
    assert.equal((await admin.query(
      `SELECT id FROM campaigns WHERE id = 'converact_upgrade_campaign'`
    )).rowCount, 1);
    assert.equal((await admin.query(
      `SELECT id FROM collaboration_sessions WHERE id = 'converact_upgrade_session'`
    )).rowCount, 1);
    ...
    const upgradedPolicy = await admin.query<{ translation_profile_ids: string[] }>(`
      ...
      FROM collaboration_intelligence_policies
      WHERE tenant_id = 'converact_upgrade_tenant'
    `);
    ...
    await assert.rejects(
      () => runtime.query('CREATE TABLE converact_runtime_must_not_create (id TEXT)'),
      /permission denied/i
    );
  } finally {
