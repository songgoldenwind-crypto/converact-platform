...
} from '../scripts/ivekit-stage2-release-evidence.js';
...
function input() {
  return {
    ...
    configurationArtifacts: [
      ...
      { profile: 'file_security' as const, path: 'deploy/kubernetes/ivekit/templates/clamav.yaml', sha256: '8'.repeat(64) }
    ]
  };
}

test('stage 2 release evidence binds source, image, required migrations, and configuration profiles', () => {
  const evidence = createIveKitStage2ReleaseEvidence(input());
  ...
  assert.doesNotThrow(() => validateIveKitStage2ReleaseEvidence(evidence));
});

test('stage 2 release evidence rejects missing migrations and tampering', () => {
  assert.throws(
    () => createIveKitStage2ReleaseEvidence({ ...input(), migrations: input().migrations.slice(0, 2) }),
