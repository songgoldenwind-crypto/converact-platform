...
} from '../scripts/converact-rtpengine-supply-chain.js';
...
describe('Converact Fabric RTPengine supply-chain evidence', () => {
  it('binds both SBOM formats and scanner results into provenance', () => {
    ...
    assert.deepEqual(
      ...
      [
        {
          uri: `git+https://github.com/songgoldenwind-crypto/converact-platform@${SOURCE_COMMIT}`,
  ...
  it('hashes input files and writes exclusive mode-0600 evidence', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'converact-supply-chain-'));
    ...
    try {
      ...
      const evidence = await collectRtpengineSupplyChainEvidence({
        CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_IDENTITY: paths.identity,
        CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_CYCLONEDX: paths.cyclonedx,
        CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_SPDX: paths.spdx,
        CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_TRIVY: paths.trivy,
        CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_SECRET_SCAN: paths.secrets,
        CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_TRIVY_VERSION:
          input.trivy.tool_version,
        CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_TRIVY_IMAGE_DIGEST:
          input.trivy.scanner_image_digest,
        CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_TRIVY_DB_UPDATED_AT:
          input.trivy.database_updated_at,
        CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_SECRET_SCANNER_VERSION:
          input.secret_scan.tool_version,
        CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_SECRET_SCANNER_IMAGE_DIGEST:
          input.secret_scan.scanner_image_digest,
        CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_SIGNATURE_NOT_RUN_REASON:
          'No keyless CI identity or cosign key is available',
        CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_GENERATED_AT: input.generated_at,
        CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_EVIDENCE_OUTPUT: paths.evidence,
        CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_PROVENANCE_OUTPUT: paths.provenance
      });
  ...
  it('rejects mutable image and malformed SBOM identities', () => {
    assert.throws(
      () => buildRtpengineSupplyChainEvidence({
        ...
        identity: {
          ...validInput().identity,
          image_digest: 'converact/rtpengine:latest'
        }
      }),
...
function validInput(): RtpengineSupplyChainInput {
  return {
    ...
    identity: {
      ...
      image_reference: 'converact/rtpengine',
      ...
      build_arguments: {
        TARGETARCH: 'amd64',
        CONVERACT_FABRIC_RTPENGINE_BUILD_JOBS: '8'
      }
    },
    cyclonedx: {
      sha256: '2'.repeat(64),
    ...
    spdx: {
      ...
      document: {
        ...
        name: 'converact-rtpengine',
        documentNamespace: 'https://converact.invalid/spdx/task10',
        packages: [{
    ...
    trivy: {
      ...
      document: {
        SchemaVersion: 2,
        ArtifactName: 'converact/rtpengine',
