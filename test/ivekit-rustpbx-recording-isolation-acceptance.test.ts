...
import type { RustPbxRecordingIsolationResult } from
  '../scripts/ivekit-rustpbx-recording-isolation-acceptance.js';

const SCRIPT_PATH = new URL(
  '../scripts/ivekit-rustpbx-recording-isolation-acceptance.ts',
  import.meta.url
);
...
test('RustPBX recording isolation acceptance is a packaged repeatable command', () => {
  ...
  assert.equal(
    ...
    'tsx scripts/ivekit-rustpbx-recording-isolation-acceptance.ts'
  );
  ...
  assert.match(
    ...
    /ivekit-rustpbx-recording-isolation-acceptance\.ts/
  );
  const bundle = readFileSync(
    new URL('../scripts/ivekit-delivery-bundle.ts', import.meta.url),
    'utf8'
  );
...
test('RustPBX recording isolation runtime plan fails closed on mutable inputs', async () => {
  ...
  const valid = {
    ...
    IVEKIT_RUSTPBX_RECORDING_ISOLATION_IMAGE:
      'ivekit/rustpbx:0.4.11-ivekit.25-6c49ee76',
    ...
    IVEKIT_RUSTPBX_RECORDING_ISOLATION_PYTHON_IMAGE:
      'python@sha256:' + 'c'.repeat(64),
    IVEKIT_RUSTPBX_RECORDING_ISOLATION_FAULT_IMAGE:
      'alpine@sha256:' + 'd'.repeat(64),
    IVEKIT_RUSTPBX_RECORDING_ISOLATION_SIPP_IMAGE:
      'alpine@sha256:' + 'e'.repeat(64),
    IVEKIT_SIPP_BINARY: '/tmp/sipp'
  };

  assert.deepEqual(createRustPbxRecordingIsolationRuntimePlan(valid), {
    compose_file: '/tmp/docker-compose.yml',
    runtime_directory: '/tmp/ivekit-recording-isolation',
    result_file: '/tmp/ivekit-recording-isolation/result.json',
    project_name: 'ivekit-rustpbx-recording-isolation',
    rustpbx_image: valid.IVEKIT_RUSTPBX_RECORDING_ISOLATION_IMAGE,
    postgres_image: valid.IVEKIT_RUSTPBX_RECORDING_ISOLATION_POSTGRES_IMAGE,
    node_image: valid.IVEKIT_RUSTPBX_RECORDING_ISOLATION_NODE_IMAGE,
    python_image: valid.IVEKIT_RUSTPBX_RECORDING_ISOLATION_PYTHON_IMAGE,
    fault_image: valid.IVEKIT_RUSTPBX_RECORDING_ISOLATION_FAULT_IMAGE,
    sipp_image: valid.IVEKIT_RUSTPBX_RECORDING_ISOLATION_SIPP_IMAGE,
    sipp_binary: '/tmp/sipp',
    network: 'ivekit-rustpbx-recording-isolation',
  ...
  assert.throws(
    () => createRustPbxRecordingIsolationRuntimePlan({
      ...valid,
      IVEKIT_RUSTPBX_RECORDING_ISOLATION_NODE_IMAGE: 'node:latest'
    }),
    /digest/i
  );
});
...
test('RustPBX recording isolation result writer creates a private sanitized artifact', async () => {
  ...
  } = await import('../scripts/ivekit-rustpbx-recording-isolation-acceptance.js');
  const path = `/tmp/ivekit-rustpbx-recording-isolation-${process.pid}.json`;
