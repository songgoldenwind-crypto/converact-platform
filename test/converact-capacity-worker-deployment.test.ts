...
test('capacity tools image and package expose the restart-safe worker process', () => {
  ...
  assert.match(dockerfile, /COPY scripts\/converact-capacity-worker\.ts/i);
  assert.match(dockerfile, /COPY --chmod=0755 scripts\/converact-capacity-tinode-composite\.ts/i);
  assert.match(dockerfile, /COPY --chmod=0755 scripts\/converact-capacity-tinode-provision\.ts/i);
  assert.match(dockerfile, /COPY scripts\/converact-capacity-controller\.ts/i);
  assert.match(dockerfile, /COPY scripts\/converact-capacity-finalizer\.ts/i);
  assert.match(dockerfile, /COPY scripts\/converact-capacity-scaling-finalizer\.ts/i);
  assert.match(dockerfile, /COPY scripts\/converact-capacity-platform-finalizer\.ts/i);
  assert.match(dockerfile, /COPY src\/infra\/nats-connection-options\.ts/i);
  assert.match(
    dockerfile,
    /COPY services\/converact-service\/acceptance\/sipp\/answer-bye-uac\.xml \/opt\/converact\/scenarios\/answer-bye-uac\.xml/i
  );
  ...
  assert.match(
    String(packageJson.scripts['converact:capacity:worker']),
    /converact-capacity-worker\.ts/
  );
  assert.match(
    String(packageJson.scripts['converact:capacity:controller']),
    /converact-capacity-controller\.ts/
  );
  assert.match(
    String(packageJson.scripts['converact:capacity:finalizer']),
    /converact-capacity-finalizer\.ts/
  );
  assert.match(
    String(packageJson.scripts['converact:capacity:scaling-finalizer']),
    /converact-capacity-scaling-finalizer\.ts/
  );
  assert.match(
    String(packageJson.scripts['converact:capacity:platform-finalizer']),
    /converact-capacity-platform-finalizer\.ts/
  );
  assert.match(
    String(packageJson.scripts['converact:capacity:tinode-composite']),
    /converact-capacity-tinode-composite\.ts/
  );
  assert.match(
    String(packageJson.scripts['converact:capacity:tinode-provision']),
    /converact-capacity-tinode-provision\.ts/
  );
  assert.match(
  ...
  assert.match(runtimeTsconfig, /converact-capacity-tinode-composite\.ts/);
  assert.match(runtimeTsconfig, /converact-capacity-tinode-provision\.ts/);
  assert.match(
    String(packageJson.scripts['typecheck:converact:capacity-runtime']),
...
test('capacity scaling finalizer mounts immutable contract inputs and verifies S3 run evidence', () => {
  ...
  assert.match(yaml, /kind: Job/i);
  assert.match(yaml, /converact-capacity-scaling-finalizer\.ts/i);
...
test('capacity platform finalizer mounts the full release gate and verified source evidence', () => {
  ...
  assert.match(yaml, /kind: Job/i);
  assert.match(yaml, /converact-capacity-platform-finalizer\.ts/i);
...
test('capacity worker deployment uses stable identity, one in-flight shard and durable evidence', () => {
  ...
  assert.match(yaml, /name: network-impairment[\s\S]*converact-capacity-network-impairment\.ts/i);
...
test('capacity controller deployment uses two fenced replicas and an immutable manifest volume', () => {
  ...
  assert.match(yaml, /persistentVolumeClaim:[\s\S]*converact-capacity-manifests/i);
...
test('Tinode Cell deployment uses a three-node stable cluster and local owner sidecars', () => {
  ...
  assert.match(yaml, /IVEKIT_COMPONENT_NODE_ID[\s\S]*fieldPath: metadata\.name/i);
  assert.match(yaml, /CONVERACT_FABRIC_COMPONENT_NODE_ID[\s\S]*fieldPath: metadata\.name/i);
  assert.match(yaml, /IVEKIT_TINODE_OWNER_API_TOKEN/);
...
test('RustDesk Cell deployment keeps each hbbs and hbbr pair on one stable owner ordinal', () => {
  ...
  assert.match(yaml, /converact-rustdesk-owner-binding\.ts/i);
...
test('controlled Compose worker is opt-in and mounts an immutable driver bundle read-only', () => {
  ...
  assert.match(
    compose,
    /CONVERACT_FABRIC_CAPACITY_WORKER_BUNDLE_HOST_PATH[\s\S]*\/opt\/converact-capacity-worker:ro/i
  );
  assert.match(compose, /scripts\/converact-capacity-worker\.ts/i);
});
