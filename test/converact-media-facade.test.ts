...
import { createServer as createConveractServer } from '../src/http.js';
import { signAccessToken } from '../src/middleware/auth.js';
import { createTenant } from '../src/platform/tenant-core.js';

const API_KEY = 'test-converact-media-key';
...
async function route(
  ...
  path: string,
  body: unknown,
  headers: Record<string, string> = authHeaders('tenant_converact_media')
) {
...
test('Converact Fabric media facade exposes deployment capabilities through platform auth', async () => {
...
test('Converact Fabric media facade creates rooms, prepares joins, lists participants, and starts recordings', async () => {
  process.env.CONVERACT_API_KEY = API_KEY;
  clearLiveKitEnv();
  const db = createDatabase(':memory:');
  const tenantId = createTenant(db, { name: 'Converact Fabric Media Tenant' }).id;
...
test('Converact Fabric media facade keeps rooms tenant scoped', async () => {
  process.env.CONVERACT_API_KEY = API_KEY;
  clearLiveKitEnv();
  const db = createDatabase(':memory:');
...
test('legacy room join is system-only so JWT users cannot mint arbitrary identities or agent roles', async () => {
  ...
  process.env.CONVERACT_API_KEY = API_KEY;
  process.env.CONVERACT_JWT_SECRET = 'converact-media-facade-jwt-secret';
  clearLiveKitEnv();
  const db = createDatabase(':memory:');
...
test('Converact Fabric media facade accepts LiveKit webhook raw bodies without platform auth', async () => {
  process.env.CONVERACT_API_KEY = API_KEY;
  clearLiveKitEnv();
  const db = createDatabase(':memory:');
  const tenantId = createTenant(db, { name: 'Converact Fabric Webhook Tenant' }).id;
  try {
    await route(
      db,
      'POST',
      ...
      {
        purpose: 'video_service',
        room_name: 'converact-webhook-room',
    ...
    const rawBody = JSON.stringify({
      ...
      room: { name: 'converact-webhook-room' },
      participant: {
        identity: 'customer-webhook',
        metadata: JSON.stringify({ role: 'customer' })
      }
    });
    const result = await routeConveractFabricMediaApi(
      db,
      'POST',
    ...
    const participants = createLiveKitMediaModule({ db }).participants.listByRoom('converact-webhook-room');
...
test('Converact Fabric media webhook journals room and participant lifecycle events', async () => {
  process.env.CONVERACT_API_KEY = API_KEY;
  clearLiveKitEnv();
  const db = createDatabase(':memory:');
  const tenantId = createTenant(db, { name: 'Converact Fabric Webhook Event Tenant' }).id;
  const events: Array<{
    tenant_id: string;
    type: string;
    data: unknown;
  ...
  try {
    await route(
      ...
      {
        purpose: 'video_service',
        room_name: 'converact-webhook-event-room',
    ...
    for (const payload of [
      {
        ...
        event: 'room_started',
        room: {
          name: 'converact-webhook-event-room',
          sid: 'RM_event',
      ...
      await routeConveractFabricMediaApi(
        tenantGuardedDb,
        'POST',
        '/api/ivekit/media/webhooks/livekit',
        new URL('http://localhost/api/ivekit/media/webhooks/livekit'),
...
test('Converact Fabric media webhook completes recording evidence through the facade hook', async () => {
  process.env.CONVERACT_API_KEY = API_KEY;
  clearLiveKitEnv();
  const db = createDatabase(':memory:');
  const tenantId = createTenant(db, { name: 'Converact Fabric Webhook Evidence Tenant' }).id;
  try {
    await route(
      db,
      'POST',
      ...
      {
        purpose: 'video_service',
        room_name: 'converact-webhook-evidence-room',
    ...
    const started = await route(
      db,
      'POST',
      '/api/ivekit/media/rooms/converact-webhook-evidence-room/recordings/start',
      {
        business_ref: { type: 'service_order', id: 'order-webhook-evidence' },
        format: 'mp4',
        has_video: true
    ...
    const rawBody = JSON.stringify({
      event: 'egress_ended',
      room: { name: 'converact-webhook-evidence-room' },
      egressInfo: {
...
test('Converact Fabric media webhook releases each terminal Egress job and hides placement internals', async () => {
  process.env.CONVERACT_API_KEY = API_KEY;
  clearLiveKitEnv();
  const db = createDatabase(':memory:');
  const tenantId = createTenant(db, { name: 'Converact Fabric Egress Placement Tenant' }).id;
  ...
  try {
    await route(db, 'POST', '/api/ivekit/media/rooms', {
      purpose: 'video_service', room_name: 'converact-egress-placement-room',
      business_ref: { type: 'service_order', id: 'order-egress-placement' }
    }, authHeaders(tenantId));
    const started = await route(db, 'POST',
      '/api/ivekit/media/rooms/converact-egress-placement-room/recordings/start', {
        business_ref: { type: 'service_order', id: 'order-egress-placement' },
    ...
    const jobsResult = await routeConveractFabricMediaApi(
    ...
    for (const job of jobs) {
      const rawBody = JSON.stringify({
        event: 'egress_ended', room: { name: 'converact-egress-placement-room' },
...
test('Converact Fabric media facade is registered in the main HTTP router', async () => {
  process.env.CONVERACT_API_KEY = API_KEY;
  clearLiveKitEnv();
  const db = createDatabase(':memory:');
  const tenantId = createTenant(db, { name: 'HTTP Router Media Tenant' }).id;
  const server = createConveractServer(db);
