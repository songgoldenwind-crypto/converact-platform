...
describe('Converact Fabric media control deployment', () => {
  it('runs as non-root with a pinned base and deterministic entrypoint', () => {
    ...
    assert.match(
      dockerfile,
      /ENTRYPOINT \["node", "--import", "tsx", "scripts\/converact-media-control-agent\.ts"\]/
    );
  ...
  it('keeps component leases alive through the Cell admission leader', () => {
    ...
    assert.equal(
      ...
      '${CONVERACT_FABRIC_CELL_NODES_JSON:-${RUSTPBX_CELL_NODES_JSON:?RUSTPBX_CELL_NODES_JSON is required}}'
    );
    assert.equal(
  ...
  it('uses the production hybrid media router with mTLS and loopback internals', () => {
    const service = compose.services['media-control'];
    assert.equal(
      service.environment.CONVERACT_FABRIC_MEDIA_CONTROL_PRODUCTION,
      'true'
    );
    assert.equal(
      service.environment.CONVERACT_FABRIC_MEDIA_CONTROL_TRANSPORT,
      'hybrid'
    );
    assert.equal(
      service.environment.CONVERACT_FABRIC_MEDIA_CONTROL_REQUIRE_MTLS,
      'true'
    );
    assert.equal(
      service.environment.CONVERACT_FABRIC_MEDIA_CONTROL_ADMISSION_REQUIRE_MTLS,
      'false'
    );
    assert.equal(
      service.environment.CONVERACT_FABRIC_MEDIA_CONTROL_ADMISSION_ENDPOINT,
      'http://127.0.0.1:3210'
    );
    assert.equal(
    ...
    assert.equal(
      service.environment.CONVERACT_FABRIC_MEDIA_CONTROL_REQUIRE_PRODUCTION_TRANSPORT,
      undefined
    );
    assert.match(
      entrypoint,
    ...
    assert.doesNotMatch(
      entrypoint,
      /CONVERACT_FABRIC_MEDIA_CONTROL_REQUIRE_PRODUCTION_TRANSPORT/
    );
    assert.match(
      entrypoint,
      /CONVERACT_FABRIC_MEDIA_CONTROL_TLS_KEY_FILE/
    );
    assert.match(
      entrypoint,
      /CONVERACT_FABRIC_MEDIA_CONTROL_TLS_CERT_FILE/
    );
    assert.match(
      entrypoint,
      /CONVERACT_FABRIC_MEDIA_CONTROL_TLS_CA_FILE/
    );
    assert.match(
      entrypoint,
