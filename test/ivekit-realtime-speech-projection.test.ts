...
test('iveKit SDK exposes paged final realtime speech projections', async () => {
  const calls: URL[] = [];
  const sdk = createIveKitHttpSdk({
    baseUrl: 'https://ivekit.example.test', tenantId: 'tenant-a', apiKey: 'sdk-key',
    fetch: async (input) => {
...
test('realtime speech HTTP projection is call-member scoped and preserves paging input', async () => {
  ...
  function mediaRoute(
    ...
    return routeIveKitMediaApi(db, method, url.pathname, url, body, '', headers, {
      pg,
      realtimeSpeechStore
    });
  }
