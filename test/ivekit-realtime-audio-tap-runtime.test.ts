...
test('LiveKit audio tap derives isolated signing keys for each gateway instance', () => {
  ...
  const podA = deriveLiveKitAudioTapInstanceSecret(clusterSecret, 'ivekit-api-0');
  const podAReplay = deriveLiveKitAudioTapInstanceSecret(clusterSecret, 'ivekit-api-0');
  const podB = deriveLiveKitAudioTapInstanceSecret(clusterSecret, 'ivekit-api-1');
...
test('configured runtime owns and removes the local Unix socket', async (t) => {
  const directory = await mkdtemp(join(tmpdir(), 'ivekit-tap-runtime-'));
...
test('configured runtime can isolate the RustPBX gateway in a Pod-local sidecar', async (t) => {
  const directory = await mkdtemp(join(tmpdir(), 'ivekit-tap-runtime-rustpbx-'));
...
test('configured runtime rolls back the Unix socket when LiveKit gateway startup fails', async (t) => {
  const directory = await mkdtemp(join(tmpdir(), 'ivekit-tap-runtime-rollback-'));
  const socketPath = join(directory, 'tap.sock');
