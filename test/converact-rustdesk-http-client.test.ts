...
test('Converact Fabric RustDesk HTTP client calls the reusable facade lifecycle', async () => {
  ...
  const responses = [
    ...
    { id: 'rdesk_1', runtime_status: 'online' },
    {
      ...
      provider: 'rustdesk',
      external_id: 'rdgw_1',
      launch_url: 'https://converact.example.com/remote/rustdesk/launch?session_id=rdgw_1'
    },
  ...
  for (const call of calls) {
    assert.equal(call.headers['x-api-key'], 'converact-api-key');
...
test('Converact Fabric RustDesk HTTP client rejects bad configuration before calling fetch', async () => {
  assert.throws(
    () => createConveractFabricRustDeskHttpClient({ baseUrl: 'file:///tmp/converact', apiKey: 'k', tenantId: 't' }),
    /baseUrl must use http\(s\)/
  );
  for (const baseUrl of [
    'https://url-user:url-password@converact.example.com',
    'https://converact.example.com?token=query-secret',
    'https://converact.example.com#fragment-secret'
  ]) {
    assert.throws(
      () => createConveractFabricRustDeskHttpClient({ baseUrl, apiKey: 'k', tenantId: 't' }),
      (error) => {
  ...
  assert.throws(
    () => createConveractFabricRustDeskHttpClient({ baseUrl: 'https://converact.example.com/platform/', apiKey: 'k', tenantId: 't' }),
    /baseUrl must not include a path/
  );
  assert.throws(
    () => createConveractFabricRustDeskHttpClient({ baseUrl: 'https://converact.example.com', apiKey: '', tenantId: 't' }),
  ...
  assert.throws(
    () => createConveractFabricRustDeskHttpClient({ baseUrl: 'https://converact.example.com', apiKey: 'k', tenantId: '' }),
    /tenantId is required/
  );
});

test('Converact Fabric RustDesk HTTP client exposes explicit emergency fallback authorization', async () => {
  const calls: FetchCall[] = [];
  const client = createConveractFabricRustDeskHttpClient({
    baseUrl: 'https://converact.example.com',
...
test('Converact Fabric RustDesk HTTP client keeps Bearer identity authoritative', async () => {
...
test('Converact Fabric RustDesk HTTP client exposes status, method, path, and upstream error', async () => {
  const client = createConveractFabricRustDeskHttpClient({
    baseUrl: 'https://converact.example.com',
    apiKey: 'converact-api-key',
    tenantId: 'tenant_led',
  ...
  await assert.rejects(
    ...
    (error) => {
      assert.equal(error instanceof ConveractFabricRustDeskHttpError, true);
      const httpError = error as ConveractFabricRustDeskHttpError;
...
test('Converact Fabric RustDesk HTTP client applies the configured request timeout', async () => {
  const client = createConveractFabricRustDeskHttpClient({
    baseUrl: 'https://converact.example.com',
    apiKey: 'converact-api-key',
    tenantId: 'tenant_led',
  ...
  await assert.rejects(
    ...
    (error) => {
      assert.equal(error instanceof ConveractFabricRustDeskHttpError, true);
      const timeout = error as ConveractFabricRustDeskHttpError;
...
test('Converact Fabric RustDesk HTTP client requests and projects a pinned client distribution profile', async () => {
  const calls: string[] = [];
  const now = Date.now();
...
test('Converact Fabric RustDesk client profile preserves the placement-enabled native control v2 protocol', async () => {
...
test('Converact Fabric RustDesk profile projection accepts the five official 1.4.9 desktop asset names', async () => {
  ...
  for (const [platform, architecture, filename] of officialAssets) {
    ...
    assert.equal(projected.install_source.state, 'configured');
    if (projected.install_source.state === 'configured') assert.equal(projected.install_source.filename, filename);
  }
});

test('Converact Fabric RustDesk profile projection rejects an invalid validation clock', async () => {
  await assert.rejects(
...
test('Converact Fabric RustDesk client profile projection rejects drift, expiry, and malformed responses', async () => {
  const base = expectedClientDistributionProfile();
  const expected = {
...
test('Converact Fabric RustDesk client profile binds the returned public key to both fingerprints', async () => {
  const profile = {
    ...expectedClientDistributionProfile(),
    manual_fields: {
...
for (const [name, key] of [
  ...
  test(`Converact Fabric RustDesk profile projection rejects ${name} public key`, async () => {
    const profile = {
      ...expectedClientDistributionProfile(),
...
test('Converact Fabric RustDesk HTTP client rejects missing profile pins before fetch', async () => {
  let calls = 0;
  const client = createConveractFabricRustDeskHttpClient({
    baseUrl: 'https://converact.example.com',
    accessToken: 'short-lived-browser-token',
...
for (const [name, filename, urlFilename] of [
  ...
  test(`Converact Fabric RustDesk profile projection rejects artifact ${name}`, async () => {
    const profile = {
      ...expectedClientDistributionProfile(),
      manual_fields: {
        ...expectedClientDistributionProfile().manual_fields,
...
test('Converact Fabric RustDesk profile projection rejects inexact release paths and contradictory identity tokens', async () => {
...
test('Converact Fabric RustDesk profile projection rejects noncanonical installer filenames', async () => {
...
test('Converact Fabric RustDesk profile projection rejects lifetime below 60 seconds', async () => {
  const profile = {
    ...expectedClientDistributionProfile(),
...
for (const [name, timestamps] of [
  ...
  test(`Converact Fabric RustDesk profile projection rejects ${name}`, async () => {
    const profile = {
      ...expectedClientDistributionProfile(),
      ...timestamps,
      manual_fields: {
...
test('Converact Fabric RustDesk HTTP client projects complete terminal profiles from every device response path', async () => {
...
test('Converact Fabric RustDesk HTTP client rejects invalid terminal profile fields', async () => {
...
test('Converact Fabric RustDesk terminal profile enforces requested, consented, and granted subsets', async () => {
  const base = expectedTerminalProfile('rdesk_1');
...
test('Converact Fabric RustDesk terminal profile is bound to the enclosing device identifiers', async () => {
...
test('Converact Fabric RustDesk HTTP client fails closed on invalid terminal evidence', async () => {
...
test('Converact Fabric RustDesk HTTP client projects operation evidence onto browser-safe keys', async () => {
  const observed = {
...
test('Converact Fabric RustDesk HTTP client fails closed on invalid evidence and disconnect provenance', async () => {
  ...
  const clientFor = (payload: unknown) => createConveractFabricRustDeskHttpClient({
    baseUrl: 'https://converact.example.com',
    accessToken: 'short-lived-browser-token',
    tenantId: 'tenant_led',
...
test('Converact Fabric RustDesk HTTP client enforces non-contradictory disconnect observations', async () => {
