...
import { validateConveractFabricBackupSet } from '../src/agent-runtime/converact/operations/backup.js';

test('backup runner creates a validated database and streamed object backup set', async (t) => {
  const root = mkdtempSync(join(tmpdir(), 'converact-backup-runner-'));
  const output = join(root, 'backup-a');
  t.after(() => rm(root, { recursive: true, force: true }));
  const invocations: Array<{ executable: string; args: string[]; env: NodeJS.ProcessEnv }> = [];
  const runner: ConveractFabricProcessRunner = async (executable, args, options) => {
  ...
  assert.equal((await validateConveractFabricBackupSet({ directory: output })).backup_id, 'backup-a');
...
test('failed backup remains explicitly partial and is never accepted as complete', async (t) => {
  const root = mkdtempSync(join(tmpdir(), 'converact-backup-partial-'));
  const output = join(root, 'backup-a');
  t.after(() => rm(root, { recursive: true, force: true }));
  await assert.rejects(() => runConveractFabricBackup({
    directory: output,
    backup_id: 'backup-a',
  ...
  await assert.rejects(() => validateConveractFabricBackupSet({ directory: output }));
});

test('restore defaults to checksum-only validation and executes guarded database and object restore', async (t) => {
  const root = mkdtempSync(join(tmpdir(), 'converact-restore-runner-'));
  const backup = join(root, 'backup-a');
  t.after(() => rm(root, { recursive: true, force: true }));
  const dumpRunner: ConveractFabricProcessRunner = async (_executable, args) => {
    const file = args.find((arg) => arg.startsWith('--file='))?.slice('--file='.length);
    assert.ok(file);
    writeFileSync(file, 'database-dump');
    return { stdout: '', stderr: '' };
  };
  await runConveractFabricBackup({
    directory: backup,
    backup_id: 'backup-a',
    env: { PGHOST: 'db', PGDATABASE: 'ivekit', PGUSER: 'user' },
  ...
  let processCalls = 0;
  const dryRun = await runConveractFabricRestore({
    directory: backup,
    processRunner: async () => {
      processCalls += 1;
      return { stdout: '', stderr: '' };
  ...
  const result = await runConveractFabricRestore({
    ...
    env: {
      ...
      CONVERACT_FABRIC_RESTORE_CONFIRM: 'RESTORE:backup-a',
...
test('backup and restore keep Converact Fabric, Tinode and RustPBX databases isolated', async (t) => {
  const root = mkdtempSync(join(tmpdir(), 'converact-dependent-databases-'));
  const backup = join(root, 'backup-a');
  t.after(() => rm(root, { recursive: true, force: true }));
  const backupDatabases: string[] = [];
  const backupResult = await runConveractFabricBackup({
    directory: backup,
    backup_id: 'backup-a',
    env: {
      PGHOST: 'db', PGDATABASE: 'ivekit', PGUSER: 'converact-admin',
  ...
  const restored = await runConveractFabricRestore({
    directory: backup,
    execute: true,
    env: {
      PGHOST: 'restore-db', PGDATABASE: 'converact-new', PGUSER: 'converact-admin',
      ...
      CONVERACT_FABRIC_RESTORE_CONFIRM: 'RESTORE:backup-a', CONVERACT_FABRIC_RESTORE_TARGET_EMPTY: '1'
    },
  ...
  assert.deepEqual(operations.slice(0, 3), [
    { executable: 'psql', database: 'converact-new' },
...
test('restore refuses a non-empty database before pg_restore', async (t) => {
  const root = mkdtempSync(join(tmpdir(), 'converact-restore-nonempty-'));
  const backup = join(root, 'backup-a');
  t.after(() => rm(root, { recursive: true, force: true }));
  await runConveractFabricBackup({
    directory: backup,
  ...
  let calls = 0;
  await assert.rejects(() => runConveractFabricRestore({
    directory: backup,
    execute: true,
    env: {
      PGHOST: 'db', PGDATABASE: 'ivekit', PGUSER: 'user',
