...
} from '../scripts/converact-capacity.js';

test('capacity CLI functions compile and validate an immutable Cell-10K bundle', () => {
  const directory = mkdtempSync(join(tmpdir(), 'converact-capacity-cli-'));
  ...
  writeFileSync(runConfigPath, JSON.stringify({
    run: {
      ...
      sutReleaseId: 'converact@0123456789abcdef0123456789abcdef01234567',
...
test('capacity bundle validation rejects post-compile mutation', () => {
  const directory = mkdtempSync(join(tmpdir(), 'converact-capacity-cli-tamper-'));
