...
import {
  PostgresConveractFabricRetentionStore,
  type ConveractFabricRetentionClaim
...
test('Postgres retention store claims policies with skip-locked leases and durable runs', async () => {
  ...
  const claims = await new PostgresConveractFabricRetentionStore(pg, { id: () => 'run-a' }).claimDue({
...
test('Postgres retention store enforces terminal notification deletion and legal holds', async () => {
  const pg = new RecordingPg((sql) => /WITH candidates AS MATERIALIZED/i.test(sql)
    ...
    : []);
  const summary = await new PostgresConveractFabricRetentionStore(pg).deleteExpired(claim());
...
test('Postgres audit retention separates absolute deadlines from policy age', async () => {
  ...
  const summary = await new PostgresConveractFabricRetentionStore(pg).deleteExpired({
    ...claim(),
...
test('Postgres retention store writes optimistic policies and idempotent legal holds', async () => {
  ...
  const store = new PostgresConveractFabricRetentionStore(pg, { id: () => 'hold-a' });
...
function claim(): ConveractFabricRetentionClaim {
  return {
