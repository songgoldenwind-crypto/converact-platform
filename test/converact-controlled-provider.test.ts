...
} from '../scripts/converact-controlled-provider.js';
...
test('controlled provider implements health, OCR, ASR, quality, and translation contracts', async () => {
  ...
  assert.deepEqual(JSON.parse(health.body), { status: 'ok', service: 'converact-controlled-provider' });
...
test('controlled provider mode changes require the control token and package script is exposed', async () => {
  ...
  assert.equal(pkg.scripts['converact:controlled-provider'], 'tsx scripts/converact-controlled-provider.ts');
});
