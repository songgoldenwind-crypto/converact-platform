...
test('HOMER image workflow builds exact source and delegates its digest to the shared release gate', () => {
  const workflow = readFileSync('.github/workflows/converact-homer-image.yml', 'utf8');
  ...
  assert.match(workflow, /HOMER_BUILDER_IMAGE: \$\{\{ vars\.CONVERACT_FABRIC_HOMER_BUILDER_IMAGE \}\}/);
  assert.match(workflow, /HOMER_NODE_IMAGE: \$\{\{ vars\.CONVERACT_FABRIC_HOMER_NODE_IMAGE \}\}/);
  assert.match(workflow, /HOMER_RUNTIME_IMAGE: \$\{\{ vars\.CONVERACT_FABRIC_HOMER_RUNTIME_IMAGE \}\}/);
  ...
  assert.equal(parsed.jobs.publish.env?.DOCKER_CONFIG, '${{ runner.temp }}/converact-docker-config');
