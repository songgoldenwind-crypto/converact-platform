...
import {
  createPostgresConveractFabricRetentionCategoryHandlers,
...
test('secure-file retention skips legal holds and deletes every object before expiring state', async () => {
  ...
  const handlers = createPostgresConveractFabricRetentionCategoryHandlers({ pg, storage });
...
test('media-recording retention deletes the object before terminal state and skips holds', async () => {
  ...
  const handlers = createPostgresConveractFabricRetentionCategoryHandlers({
    pg,
...
function claim(category: ConveractFabricRetentionClaim['policy']['category']): ConveractFabricRetentionClaim {
  return {
    run_id: `run-${category}`,
