...
import type { ConveractFabricUploadTransport } from '../sdk/converact/src/upload-transport.js';

test('attachment progress upload preserves binary, auth, monotonic progress, and abort', async () => {
  const requests: Parameters<ConveractFabricUploadTransport['upload']>[0][] = [];
  let aborted = false;
  const transport: ConveractFabricUploadTransport = {
    upload(request) {
...
test('attachment fetch fallback returns structured errors and supports timeout abort', async () => {
  const rejected = createConveractFabricHttpSdk({
    baseUrl: 'https://fabric.converact.example.com',
    tenantId: 'tenant-upload',
    apiKey: 'upload-key',
  ...
  const timedOut = createConveractFabricHttpSdk({
    baseUrl: 'https://fabric.converact.example.com',
    tenantId: 'tenant-upload',
    apiKey: 'upload-key',
    timeoutMs: 100,
