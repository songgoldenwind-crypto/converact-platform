...
test('Converact Fabric application starts and stops every worker once', async () => {
  const events: string[] = [];
...
test('Converact Fabric application injects SIP placement into the voice command runtime', async () => {
...
test('Converact Fabric application stops remaining workers after one stop failure', async () => {
  ...
  await assert.rejects(() => application.stop(), /failed to stop 1 Converact Fabric worker/);
...
test('Converact Fabric application publishes worker events and requeues attachment quality review', async () => {
...
test('default provider event publication persists one durable event', async (t) => {
  ...
  const events = new ConveractFabricTenantEventStore(pg, { env });
...
test('Converact server delegates collaboration workers to Converact Fabric application', () => {
