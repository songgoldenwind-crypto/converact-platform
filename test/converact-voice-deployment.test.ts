...
const HELM_CONVERACT = new URL('../infra/k8s/templates/converact-deployment.yaml', import.meta.url);
...
const STANDALONE_CONTEXT_VERIFIER = new URL('../scripts/verify-converact-standalone-context.ts', import.meta.url);

const SECRET_VALUES = {
  RUSTPBX_DATABASE_URL: 'postgresql://rustpbx_app:database-secret@postgres:5432/rustpbx',
  RUSTPBX_IMAGE: 'ghcr.io/songgoldenwind-crypto/converact-rustpbx@sha256:2dc00f409f49bf48a23de6101d9d7371692eb7f067e70f4d449f16e158302526',
  ...
  RUSTPBX_ROUTER_URL: 'http://converact-api:3000/api/ivekit/voice/providers/profile/router',
  RUSTPBX_CDR_WEBHOOK_URL: 'http://converact-api:3000/api/ivekit/voice/providers/profile/cdrs',
...
test('standalone Helm exposes SIP/VoLTE activation without enabling it by default', () => {
  ...
  assert.match(values, /repository: ghcr\.io\/songgoldenwind-crypto\/converact-rustpbx/);
  assert.match(helpers, /Converact Fabric-patched RustPBX image/);
...
test('standalone service forwards SIP control secrets through the optional runtime env file', () => {
  ...
  assert.match(converact, /env_file:/);
  assert.match(converact, /CONVERACT_FABRIC_VOICE_RUNTIME_ENV_FILE:-\.\/voice-runtime\.env/);
  assert.match(converact, /required: false/);
  assert.match(converact, /RUSTPBX_RWI_URL: \$\{RUSTPBX_RWI_URL:-\}/);
});
...
test('RustPBX renderer accepts only immutable PostgreSQL production inputs', () => {
  ...
  assert.throws(
    () => renderRustPbxConfig({
      ...SECRET_VALUES,
      RUSTPBX_IMAGE: 'ghcr.io/songgoldenwind-crypto/converact-rustpbx:latest'
    }),
    /immutable/i
  );
  assert.throws(
    () => renderRustPbxConfig({ ...SECRET_VALUES, RUSTPBX_IMAGE: 'ghcr.io/restsend/rustpbx' }),
    /Converact Fabric-patched/i
  );
  assert.throws(
    () => renderRustPbxConfig({
      ...SECRET_VALUES,
...
test('RustPBX renderer emits a usable config and a secret-free summary', () => {
  ...
  assert.match(rendered.config, /realtime_audio_tap_socket_path = "\/run\/converact\/realtime-audio-tap\.sock"/);
  ...
  assert.match(rendered.config, /issuer = "converact"/);
...
test('RustPBX renderer accepts profile-tuned bounded SIP capacity', () => {
  const rendered = renderRustPbxConfig({
    ...
    RUSTPBX_REALTIME_AUDIO_TAP_SOCKET_PATH: '/run/converact/custom-audio-tap.sock',
  ...
  assert.match(rendered.config, /realtime_audio_tap_socket_path = "\/run\/converact\/custom-audio-tap\.sock"/);
...
test('standalone Voice overlay isolates RustPBX data and exposes only SIP and RTP', () => {
  ...
  assert.match(voice, /scripts\/converact-component-node-admission\.ts/);
  ...
  assert.match(voice, /network_mode: service:rustpbx/);
  assert.match(voice, /scripts\/converact-rustpbx-recovery\.ts/);
  ...
  assert.doesNotMatch(serviceBlock(voice, 'converact'), /RUSTPBX_DB_PASSWORD/);
  assert.doesNotMatch(serviceBlock(core, 'converact'), /RUSTPBX_DB_PASSWORD/);
...
test('WebPhone production deployment shares one JWT authority and exposes only the WSS path', () => {
  ...
  const platformApi = readFileSync(HELM_CONVERACT, 'utf8');
  ...
  for (const compose of [readFileSync(VOICE_COMPOSE, 'utf8'), serviceVoice]) {
    ...
    const api = serviceBlock(compose, 'converact');
...
test('standalone service Voice overlay uses only compiled image entrypoints', () => {
  ...
  assert.match(voice, /^  converact:/m);
  assert.doesNotMatch(voice, /^\s+ivekit:/m);
  assert.match(voice, /http:\/\/converact:3000\/api\/ivekit\/voice\/providers/);
...
test('production compose has no floating or SQLite RustPBX deployment', () => {
  ...
  assert.match(compose, /scripts\/converact-component-node-admission\.ts/);
  ...
  assert.doesNotMatch(converact, /RUSTPBX_DB_PASSWORD/);
  ...
  assert.match(converact, /CONVERACT_FABRIC_VOICE_SECRET_ENV_NAMES: \$\{CONVERACT_FABRIC_VOICE_SECRET_ENV_NAMES:-RUSTPBX_MANAGEMENT_TOKEN,RUSTPBX_RWI_TOKEN\}/);
  assert.match(converact, /CONVERACT_FABRIC_VOICE_RUNTIME_ENV_FILE/);
});
...
test('RustPBX Helm templates optionally co-locate the fenced component-node agent', () => {
  ...
  for (const template of [serviceTemplate, platformTemplate]) {
    ...
    assert.match(template, /converact-component-node-admission/);
...
test('RustPBX deployments co-locate the bounded recording spool sidecar and shared durable volume', () => {
  ...
  for (const template of [serviceTemplate, platformTemplate]) {
    ...
    assert.match(template, /mountPath: \/run\/converact-recording-secrets/);
...
test('RustPBX capacity limits and overload telemetry are consistent across deployment surfaces', () => {
  ...
  assert.match(platformTemplate, /ConveractFabricRustPbxSipOverloadRejections/);
  ...
  assert.match(prometheusRules, /ConveractFabricRustPbxSipTransactionSaturation/);
  assert.match(prometheusRules, /ConveractFabricRustPbxSipOverloadRejections/);
  assert.match(platformTemplate, /ConveractFabricRustPbxRecordingQueueDrops/);
  assert.match(prometheusRules, /ConveractFabricRustPbxRecordingQueueDrops/);
  assert.match(prometheusRules, /rustpbx_media_recording_queue_drops_total/);
  assert.match(platformTemplate, /ConveractFabricRustPbxSessionCleanupDegraded/);
  assert.match(prometheusRules, /ConveractFabricRustPbxSessionCleanupDegraded/);
...
test('Helm Voice workload is opt-in, immutable, isolated, and operationally bounded', () => {
  ...
  const converact = readFileSync(HELM_CONVERACT, 'utf8');
  ...
  assert.match(secrets, /converact-voice-address-key:/);
  ...
  assert.doesNotMatch(converact, /rustpbx-database-url/);
  assert.doesNotMatch(converact, /RUSTPBX_DB_PASSWORD/);
  assert.match(converact, /name: RUSTPBX_MANAGEMENT_TOKEN[\s\S]*?key: rustpbx-management-token/);
});
...
test('Voice reconciliation scheduler discovers call and configuration unknowns', () => {
  ...
  const worker = runtime.slice(
    runtime.indexOf('export function startConveractFabricVoiceReconciliationWorker'),
