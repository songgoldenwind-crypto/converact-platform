...
} from '../scripts/converact-voice-media-goal2-finalize.js';
...
describe('Converact Fabric RTPengine Goal 2 finalizer', () => {
  it('is part of the Goal 2 gate and has a runnable CLI', async () => {
    ...
    assert.match(
      packageJson.scripts['test:converact:voice-media-goal2'] || '',
      /test\/converact-voice-media-goal2-finalizer\.test\.ts/
    );
    assert.equal(
      packageJson.scripts['converact:voice-media-goal2:finalize'],
      'node --import tsx scripts/converact-voice-media-goal2-finalize.ts'
    );
  });

  it('hashes retained attempts and writes an exclusive mode-0600 result', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'converact-goal2-finalizer-'));
    ...
    try {
      ...
      const result = await collectVoiceMediaGoal2FinalEvidence({
        CONVERACT_FABRIC_RTPENGINE_GOAL2_CONTRACT: paths.contract,
        CONVERACT_FABRIC_RTPENGINE_GOAL2_SUPPLY_CHAIN: paths.supplyChain,
        CONVERACT_FABRIC_RTPENGINE_GOAL2_STAGES: paths.stages,
        CONVERACT_FABRIC_RTPENGINE_GOAL2_ATTEMPTS: paths.attempts,
        CONVERACT_FABRIC_RTPENGINE_GOAL2_FAILURE_EVIDENCE: paths.failures,
        CONVERACT_FABRIC_RTPENGINE_GOAL2_GENERATED_AT: input.generated_at,
        CONVERACT_FABRIC_RTPENGINE_GOAL2_CAPACITY_CLAIM: 'none',
        CONVERACT_FABRIC_RTPENGINE_GOAL2_OUTPUT: paths.output
      });
