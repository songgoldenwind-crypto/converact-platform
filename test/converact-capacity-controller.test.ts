...
} from '../scripts/converact-capacity-controller.js';
...
test('capacity controller entrypoint loads only a hash-bound immutable manifest bundle', () => {
  const directory = mkdtempSync(join(tmpdir(), 'converact-capacity-controller-'));
...
test('capacity controller config keeps polling comfortably inside its lease', () => {
  assert.deepEqual(capacityControllerConfig({
    ...
    CONVERACT_FABRIC_CAPACITY_MANIFEST_PATH: '/run/converact-capacity/manifest.json',
    ...
    manifest_path: '/run/converact-capacity/manifest.json',
    lease_ttl_ms: 15000,
