...
test('iveKit SDK exposes the complete IVR control plane with tenant auth and idempotency', async () => {
  ...
  const sdk = createIveKitHttpSdk({
    baseUrl: 'https://ivekit.example/', tenantId: 'tenant-a', apiKey: 'api-key', fetch: fetchImpl
  });
  for (const method of [
