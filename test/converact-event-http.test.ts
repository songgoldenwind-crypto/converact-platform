...
import { routeConveractFabricEventApi } from '../src/agent-runtime/converact/event-http.js';
...
test('Converact Fabric event HTTP exposes head and explicit replay recovery states', async () => {
  process.env.CONVERACT_JWT_SECRET = 'converact-event-http-secret';
  const pg = new MemoryPg();
  const tenantId = 'tenant_event_http';
  const headers = authHeaders(tenantId, 'viewer-1');

  const initial = (await routeConveractFabricEventApi(
    pg,
    'GET',
    '/api/ivekit/events',
  ...
  const replay = (await routeConveractFabricEventApi(
    pg,
    'GET',
    replayUrl.pathname,
    replayUrl,
  ...
  const converged = (await routeConveractFabricEventApi(
    pg,
    'GET',
    replayUrl.pathname,
    replayUrl,
  ...
  const invalid = (await routeConveractFabricEventApi(
    pg,
    'GET',
    replayUrl.pathname,
    replayUrl,
  ...
  assert.equal(await routeConveractFabricEventApi(
    pg,
    'POST',
    '/api/ivekit/events',
    new URL('http://localhost/api/ivekit/events'),
...
test('Converact Fabric event replay can be disabled independently', async () => {
  process.env.CONVERACT_JWT_SECRET = 'converact-event-http-secret';
