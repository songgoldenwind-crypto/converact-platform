...
const API_KEY = 'test-converact-chat-key';
...
async function route(
  ...
  path: string,
  body: unknown,
  headers: Record<string, string> = authHeaders('tenant_converact_chat'),
...
test('Converact Fabric chat facade exposes capabilities without leaking Tinode server credentials', async () => {
...
test('Converact Fabric chat capabilities fail closed when browser and root API keys are identical', async () => {
...
test('Converact Fabric chat facade exposes AI quality review jobs and findings for LED', async () => {
  const snapshot = snapshotEnv(['CONVERACT_API_KEY', ...TINODE_ENV_KEYS]);
  process.env.CONVERACT_API_KEY = API_KEY;
  clearTinodeEnv();
  const pg = new MemoryPg();
  const tenantId = 'tenant_converact_quality';
  const provider: QualityReviewProvider = {
    name: 'converact-quality',
    mode: 'self_hosted',
    review: async () => ({
      findings: [{
...
test('Converact Fabric chat facade exposes attachment upload, processing, status, and retry routes', async () => {
  const snapshot = snapshotEnv([
    'CONVERACT_API_KEY',
  ...
  process.env.CONVERACT_API_KEY = API_KEY;
  process.env.CONVERACT_UPLOAD_DIR = mkdtempSync(join(tmpdir(), 'converact-attachment-'));
  process.env.CONVERACT_COLLABORATION_ATTACHMENT_MAX_BYTES = '1024';
  clearTinodeEnv();
  const pg = new MemoryPg();
  const tenantId = 'tenant_converact_attachment';
...
test('Converact Fabric chat facade provides the local session, participant, message, policy, and snapshot workflow', async () => {
  const snapshot = snapshotEnv(['CONVERACT_API_KEY', ...TINODE_ENV_KEYS]);
  process.env.CONVERACT_API_KEY = API_KEY;
  clearTinodeEnv();
  const pg = new MemoryPg();
...
test('Converact Fabric chat facade keeps sessions tenant scoped', async () => {
  const snapshot = snapshotEnv(['CONVERACT_API_KEY', ...TINODE_ENV_KEYS]);
  process.env.CONVERACT_API_KEY = API_KEY;
  clearTinodeEnv();
  const pg = new MemoryPg();
...
test('Converact Fabric chat facade is registered in the main HTTP router', async () => {
  const snapshot = snapshotEnv(['CONVERACT_API_KEY', ...TINODE_ENV_KEYS]);
  process.env.CONVERACT_API_KEY = API_KEY;
  clearTinodeEnv();
  const db = createDatabase(':memory:');
  const server = createConveractServer(db, new MemoryPg());
