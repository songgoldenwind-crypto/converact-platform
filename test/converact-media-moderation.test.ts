...
const JWT_SECRET = 'converact-media-moderation-secret-32-bytes';

test('Converact Fabric media enforces host moderation and revokes provider before terminal state', async () => {
  const previous = {
  ...
  process.env.CONVERACT_API_KEY = 'converact-media-moderation-system-key';
  ...
  try {
    ...
    await assert.rejects(
      () => route(
        ...
        {
          'X-API-Key': 'converact-media-moderation-system-key',
...
test('Converact Fabric media moderation audit migration is tenant scoped', () => {
...
test('Converact Fabric media moderation recovers provider success after a failed audit write', async () => {
...
test('Converact Fabric media moderation recovery requires a root PostgreSQL pool even with no pending work', async () => {
  const scopedClient = {
