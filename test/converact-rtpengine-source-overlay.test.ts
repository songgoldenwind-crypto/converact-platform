...
test('RTPengine fetch refuses an archive with the wrong identity', () => {
  const root = mkdtempSync(join(tmpdir(), 'converact-rtpengine-fetch-'));
...
test('pinned patch application is idempotent', async () => {
  const root = mkdtempSync(join(tmpdir(), 'converact-rtpengine-overlay-'));
  ...
  execFileSync(
    ...
    [
      ...
      source,
      '-c',
      'user.name=converact-test',
      '-c',
      'user.email=converact-test@localhost',
      'commit',
      '-qm',
      'fixture'
    ]
...
test('pinned patch-set identity survives overlapping patch context', async () => {
  const root = mkdtempSync(join(tmpdir(), 'converact-rtpengine-patch-set-'));
  const source = join(root, 'source');
...
test('patch-set identity refuses symlink writes', async () => {
  const root = mkdtempSync(join(tmpdir(), 'converact-rtpengine-symlink-'));
  const source = join(root, 'source');
  const patch = join(root, 'change.patch');
  const identityPath = join(source, 'patch-set.json');
...
test('source identity assertion rejects an unpinned tree', async () => {
  const root = mkdtempSync(join(tmpdir(), 'converact-rtpengine-identity-'));
  writeFileSync(
...
test('metrics patch exports a fixed low-cardinality Converact Fabric metric set', () => {
  const patch = readFileSync(
