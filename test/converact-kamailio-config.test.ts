...
const TOPOH_KEY = 'converact-topology-mask-key-1234567890abcdef';
const RPC_TOKEN = 'converact-kamailio-rpc-token-1234567890abcdef';
const WEBPHONE_JWT_SECRET = 'converact-webphone-jwt-secret-1234567890abcdef';
...
test('Kamailio renderer protects trust boundaries, RPC and topology data', () => {
  ...
  assert.match(cfg, /\$hdr\(Authorization\) != "Bearer converact-kamailio-rpc-token/);
  assert.match(cfg, /jsonrpc_dispatch\(\)/);
  assert.match(cfg, /prom_dispatch\(\)/);
  assert.match(cfg, /modparam\("topoh", "mask_key", "converact-topology-mask-key/);
...
test('Kamailio renderer closes WebPhone WSS authentication and registration routing', () => {
  ...
  assert.match(cfg, /jwt_verify\("\/run\/secrets\/webphone-jwt-secret", "HS256",\s*"iss='converact';aud='rustpbx-webphone'"/);
...
test('Kamailio renderer replicates only authenticated WebPhone locations across Edge nodes', () => {
  ...
  assert.match(cfg, /modparam\("dmq", "notification_address", "sip:converact-kamailio-0\.converact-kamailio-headless:5066"\)/);
  assert.match(cfg, /modparam\("dmq", "notification_address", "sip:converact-kamailio-1\.converact-kamailio-headless:5066"\)/);
...
test('Kamailio renderer rejects unsafe listeners, ACLs, secrets and capacity values', () => {
  ...
  assert.throws(
    () => renderKamailioConfig({
      ...
      dmq: {
        ...baseConfig().dmq,
        notification_addresses: ['sip:converact-kamailio-0.converact-kamailio-headless:5066']
      }
    }, secrets()),
    /at least two/i
  );
    ...
    () => renderKamailioConfig({
      ...
      dmq: {
        ...
        notification_addresses: [
          'sip:converact-kamailio-0.converact-kamailio-headless:5067',
          'sip:converact-kamailio-1.converact-kamailio-headless:5067'
        ]
      }
    }, secrets()),
    /server_port/i
...
test('Kamailio runtime reads bounded secret files and writes generated configs atomically', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'converact-kamailio-render-'));
...
test('Kamailio runtime supports host-side rendering into container runtime paths', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'converact-kamailio-host-render-'));
  const configFile = join(directory, 'config.json');
  const topohFile = join(directory, 'topoh-key');
  const rpcFile = join(directory, 'rpc-token');
  const webphoneJwtFile = join(directory, 'webphone-jwt-secret');
...
test('Kamailio renderer has a file-only CLI and documented deployment contract', async () => {
  ...
  assert.match(packageJson, /"converact:kamailio:render"/);
  for (const name of [
  ...
  assert.match(referenceConfig, /converact:kamailio:render/);
...
function baseConfig() {
  return {
    ...
    dmq: {
      ...
      server_port: 5066,
      notification_addresses: [
        'sip:converact-kamailio-0.converact-kamailio-headless:5066',
        'sip:converact-kamailio-1.converact-kamailio-headless:5066'
      ],
      num_workers: 4,
