...
test('stage 2 deployment gate renders Compose and Helm with immutable images', () => {
  const script = readFileSync('scripts/verify-converact-stage2-deployment.sh', 'utf8');
  const workflow = readFileSync('.github/workflows/converact-stage2-ci.yml', 'utf8');
  const packageJson = JSON.parse(readFileSync('package.json', 'utf8')) as {
    scripts: Record<string, string>;
  };

  assert.equal(
    packageJson.scripts['verify:converact:stage2-deployment'],
    'sh scripts/verify-converact-stage2-deployment.sh'
  );
  assert.equal(
    packageJson.scripts['verify:converact:livekit-redis-topology'],
  ...
  assert.match(script, /test\/fixtures\/converact-kamailio-values\.yaml/);
  ...
  assert.match(script, /registry\.example\.com\/converact\/kamailio@sha256:c{64}/);
  assert.match(script, /image\.digest=sha256:/);
  assert.match(script, /converact:\n[\s\S]*?digest: sha256:d{64}/);
  ...
  assert.match(script, /registry\.example\.invalid\/converact\/platform@sha256:/);
  assert.match(script, /registry\.example\.invalid\/converact\/ai-agent@sha256:/);
  assert.match(script, /registry\.example\.invalid\/converact\/frontend@sha256:/);
  ...
  assert.match(script, /media\.egress\.image\.repository=converact\/livekit-egress/);
  assert.match(
    script,
    /media\.egress\.image\.repository=registry\.example\.invalid\/converact\/livekit-egress/
  );
  assert.match(
    script,
  ...
  assert.match(script, /untrusted\.example\.invalid\/converact\/livekit-egress/);
  ...
  assert.match(script, /test\/converact-stage2-release-evidence\.test\.ts/);
  assert.match(workflow, /azure\/setup-helm@v5\.0\.0/);
  assert.match(workflow, /version: v3\.18\.4/);
  assert.match(workflow, /npm run verify:converact:stage2-deployment/);
});
