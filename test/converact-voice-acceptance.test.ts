...
import {
  ...
  runConveractFabricVoiceAcceptance,
  runConveractFabricVoiceAcceptanceFromEnv,
  writeConveractFabricVoiceAcceptanceTemplate
} from '../scripts/converact-voice-acceptance.js';

test('Voice acceptance passes a complete source-bound real environment report', () => {
  const dir = mkdtempSync(join(tmpdir(), 'converact-voice-acceptance-'));
  try {
    const reportFile = join(dir, 'report.json');
    writeFileSync(reportFile, `${JSON.stringify(completeReport(dir), null, 2)}\n`);

    const result = runConveractFabricVoiceAcceptance({ reportFile });
...
test('Voice acceptance template is incomplete and covers every required data-plane check', () => {
  const template = createConveractFabricVoiceAcceptanceTemplate({
...
test('Voice acceptance writes a template and a runbook for every capability group', () => {
  const dir = mkdtempSync(join(tmpdir(), 'converact-voice-template-'));
  try {
    const templateFile = join(dir, 'voice-template.json');
    const result = writeConveractFabricVoiceAcceptanceTemplate({
      templateFile,
      runId: 'voice-run-template',
      environmentId: 'voice-staging',
    ...
    const runbook = renderConveractFabricVoiceAcceptanceRunbook();
...
test('Voice acceptance rejects controlled evidence and missing real observations', () => {
  const dir = mkdtempSync(join(tmpdir(), 'converact-voice-controlled-'));
  try {
...
test('Voice acceptance rejects incomplete, duplicated, secret-bearing, or escaped evidence', () => {
  const dir = mkdtempSync(join(tmpdir(), 'converact-voice-invalid-'));
  try {
    const report = completeReport(dir);
...
test('Voice acceptance rejects an evidence directory symlink that escapes the report root', () => {
  const root = mkdtempSync(join(tmpdir(), 'converact-voice-parent-symlink-'));
...
test('Voice acceptance enforces capability, performance, source, and QA semantics', () => {
  const dir = mkdtempSync(join(tmpdir(), 'converact-voice-semantics-'));
  try {
    const report = completeReport(dir);
...
test('Voice acceptance reports not_run without a real report', () => {
  assert.deepEqual(runConveractFabricVoiceAcceptanceFromEnv({}), {
    ok: false,
    status: 'not_run',
...
function completeReport(dir: string) {
  ...
  return {
    ...
    versions: {
      ivekit_image: `registry.voice.internal/converact/service@sha256:${'c'.repeat(64)}`,
      rustpbx_image: `ghcr.io/songgoldenwind-crypto/converact-rustpbx@sha256:${'d'.repeat(64)}`,
      rustpbx: '0.4.11',
      postgres: '16.14',
      livekit_sip: '1.6.0',
