...
const legacyRoomJoinInput: ConveractFabricMediaRoomJoinInput = { identity: 'customer-defaults' };
const legacyFlatRoomRef: ConveractFabricCreateMediaRoomInput = {
  business_ref_type: 'service_order',
  business_ref_id: 'SO-FLAT-ROOM',
  business_ref_metadata: { source: 'legacy' }
};
const legacyFlatRecordingRef: ConveractFabricStartMediaRecordingInput = {
  business_ref_type: 'service_order',
...
test('Converact Fabric HTTP SDK is extractable and exposes the complete Media and Chat facade', async () => {
  ...
  const sdk = module.createConveractFabricHttpSdk({
    baseUrl: 'https://converact.example.com/root/',
    apiKey: 'converact-key',
    tenantId: 'tenant-led',
    userId: 'agent-led',
    fetch: fetchImpl
  });
  ...
  for (const call of calls) {
    assert.equal(call.headers['x-api-key'], 'converact-key');
...
test('Converact Fabric HTTP SDK maps reaction and pin commands', async () => {
  const calls: Array<{ method: string; url: string }> = [];
  const sdk = (await import('../sdk/converact/src/http-sdk.js')).createConveractFabricHttpSdk({
...
test('Converact Fabric media SDK maps child Egress job inspection and export paths', async () => {
...
test('Converact Fabric media SDK maps durable call and moderation commands', async () => {
  const calls: FetchCall[] = [];
  const sdk = (await import('../sdk/converact/src/http-sdk.js')).createConveractFabricHttpSdk({
    baseUrl: 'https://fabric.converact.example.com',
...
test('Converact Fabric HTTP SDK exposes cursor session and message history requests', async () => {
  const calls: string[] = [];
  const sdk = (await import('../sdk/converact/src/http-sdk.js')).createConveractFabricHttpSdk({
    baseUrl: 'https://fabric.converact.example.com',
...
test('Converact Fabric HTTP SDK exposes durable event pages, bounded replay, and snapshot fallback', async () => {
  const calls: string[] = [];
  const responses = [
...
test('Converact Fabric HTTP SDK keeps Bearer identity authoritative and exposes structured errors', async () => {
  const sdkPath = 'src/agent-runtime/converact/http-sdk.ts';
  assert.equal(existsSync(sdkPath), true);
  const module = await import('../src/agent-runtime/converact/http-sdk.js');
  ...
  await assert.rejects(
    ...
    (error: unknown) => {
      assert.equal(error instanceof module.ConveractFabricHttpSdkError, true);
  ...
  assert.throws(
    () => module.createConveractFabricHttpSdk({ baseUrl: 'file:///tmp/converact', apiKey: 'k', tenantId: 't' }),
    /baseUrl must use http\(s\)/
  );
  assert.throws(
    () => module.createConveractFabricHttpSdk({ baseUrl: 'https://converact.example.com', tenantId: 't' }),
    /exactly one of apiKey or accessToken is required/
  );
  assert.throws(
    () => module.createConveractFabricHttpSdk({
      baseUrl: 'https://converact.example.com', apiKey: 'k', accessToken: 'jwt', tenantId: 't'
    }),
    /exactly one of apiKey or accessToken is required/
  );
});

test('Converact Fabric SDK exports named browser-safe chat DTOs', () => {
  ...
  const chatInterface = sdk.match(/export interface ConveractFabricChatHttpClient \{([\s\S]*?)\n\}/)?.[1] || '';

  for (const name of [
    'ConveractFabricChatSession',
    'ConveractFabricChatParticipant',
    'ConveractFabricChatMessage',
    'ConveractFabricChatAttachment',
    'ConveractFabricChatDelivery',
...
test('Converact Fabric SDK exports named browser-safe media DTOs without untyped returns', () => {
  ...
  const mediaInterface = sdk.match(/export interface ConveractFabricMediaHttpClient \{([\s\S]*?)\n\}/)?.[1] || '';
  const callJoinInterface = types.match(/export interface ConveractFabricMediaJoinInput \{([\s\S]*?)\n\}/)?.[1] || '';
  ...
  for (const name of [
    ...
    'ConveractFabricMediaCursorPage'
  ]) assert.match(types, new RegExp(`export (?:interface|type) ${name}`));

  assert.match(sharedTypes, /export interface ConveractFabricSdkBusinessRef/);
  assert.doesNotMatch(sdk, /export interface ConveractFabricSdkBusinessRef/);
...
test('Converact Fabric LED handoff artifacts cover SDK, extraction, deployment, and validation boundaries', () => {
  const guidePath = 'docs/converact-led-integration-guide.md';
  const apiPath = 'docs/converact-openapi.md';
  const examplePath = 'scripts/converact-led-integration-example.ts';
  ...
  for (const marker of [
    ...
    'RLS', 'LED', 'Converact', '真实环境', '030_collaboration_message_state.sql'
  ...
  assert.match(example, /createConveractFabricHttpSdk/);
  assert.match(example, /createConveractFabricRustDeskLedSdk/);
  assert.match(example, /postMessage/);
  assert.match(example, /createJoinPlan/);
  assert.match(pkg, /"converact:led-example"/);
});

test('Converact Fabric HTTP SDK exposes the unified business context endpoint', async () => {
  const calls: FetchCall[] = [];
  const sdk = (await import('../sdk/converact/src/http-sdk.js')).createConveractFabricHttpSdk({
    baseUrl: 'https://fabric.converact.example.com/',
    tenantId: 'tenant-led',
