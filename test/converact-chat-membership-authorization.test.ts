...
const API_KEY = 'test-converact-membership-key';
const JWT_SECRET = 'test-converact-membership-jwt-secret-32-bytes';

test('Converact Fabric chat enforces active membership participant RBAC and non-revivable client plans', async () => {
...
test('Converact Fabric fails participant leave fast during a Tinode client-plan grant and succeeds on retry', async () => {
  const previous = {
