...
import {
  inspectConveractFabricVoice,
  type ConveractFabricVoicePreflightReport
...
test('Voice preflight keeps disabled workers configuration-safe without address keys or profiles', async () => {
  const report = await inspectConveractFabricVoice({
...
test('Voice preflight returns stable issue codes for unsafe role stale capabilities and invalid worker config', async () => {
  ...
  const missingMigration = await inspectConveractFabricVoice({
...
function assertSecretSafe(report: ConveractFabricVoicePreflightReport, secrets: string[]): void {
