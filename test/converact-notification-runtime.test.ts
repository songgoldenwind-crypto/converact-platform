...
test('Converact Fabric application owns notification worker lifecycle when configured', async () => {
  const events: string[] = [];
...
test('Converact Fabric application owns active notification health worker independently', async () => {
  const events: string[] = [];
  const application = startConveractFabricApplication({
    pg: new MemoryPg(),
