...
test('realtime recovery acceptance is a declared reproducible entrypoint', () => {
  ...
  assert.equal(
    pkg.scripts['converact:realtime-recovery-acceptance'],
...
test('realtime recovery PostgreSQL topology is isolated and immutable', () => {
  ...
  assert.match(compose, /name: \$\{COMPOSE_PROJECT_NAME:-converact-realtime-recovery\}-network/);
  assert.match(compose, /name: \$\{COMPOSE_PROJECT_NAME:-converact-realtime-recovery\}-data/);
