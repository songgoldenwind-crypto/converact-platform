...
test('Voice controller exposes every WebPhone action and publishes stable state', async () => {
  ...
  let keySequence = 0;
  const controller = createIveKitVoiceController({
    client,
...
test('Voice controller reuses an idempotency key only after ambiguous failure', async () => {
  ...
  const client = voiceClient({
    async enqueueCallAction(_callId, input, options) {
      ...
      attempts += 1;
      if (attempts === 1) {
        throw new IveKitHttpSdkError('request timed out', 0, 'POST', '/actions', null);
      }
      return command(input.action);
    }
  });
  ...
  const controller = createIveKitVoiceController({
    client: voiceClient({
      async enqueueCallAction(_callId, input, options) {
        ...
        attempts += 1;
        if (attempts === 1) {
          throw new IveKitHttpSdkError('invalid call state', 422, 'POST', '/actions', null);
        }
        return command(input.action);
      }
    }),
...
test('Voice controller prepares extension sessions only when the capability is available', async () => {
  ...
  const unavailable = createIveKitVoiceController({
...
function voiceClient(input: {
  ...
  enqueueCallAction?: IveKitVoiceControllerClient['enqueueCallAction'];
...
function outboundInput(): IveKitVoiceCreateOutboundCallInput {
  return {
...
function command(kind: IveKitVoiceCommandKind): IveKitVoiceCallCommand {
  return {
