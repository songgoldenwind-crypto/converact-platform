...
test('Voice HTTP uses the signed tenant and returns stable async command contracts', async (t) => {
  ...
  const listed = await routeConveractFabricVoiceApi(
  ...
  await assert.rejects(() => routeConveractFabricVoiceApi(
  ...
  const created = await routeConveractFabricVoiceApi(
    null, 'POST', '/api/ivekit/voice/calls',
  ...
  const action = await routeConveractFabricVoiceApi(
...
test('Voice HTTP system credentials require an explicit tenant header', async (t) => {
  ...
  await routeConveractFabricVoiceApi(
    null, 'GET', path, url, {}, '',
...
test('RustPBX recording spool provider routes derive tenant from auth and keep binary parts fenced', async () => {
  ...
  const initialized = await routeConveractFabricVoiceApi(
  ...
  const uploaded = await routeConveractFabricVoiceApi(
  ...
  const finalized = await routeConveractFabricVoiceApi(
    pool, 'POST',
...
test('Voice extension sessions bind operators to active WebRTC-enabled extensions', async (t) => {
  ...
  const invoke = (token: string) => routeConveractFabricVoiceApi(
