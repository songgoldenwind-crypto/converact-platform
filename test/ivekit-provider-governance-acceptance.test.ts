...
import {
  runIveKitProviderGovernanceAcceptance
} from '../scripts/ivekit-provider-governance-acceptance.js';
...
test('controlled Provider acceptance covers protocol and governance failure modes', async () => {
  ...
  assert.equal(
    pkg.scripts['ivekit:provider-governance-acceptance'],
    'node --import tsx scripts/ivekit-provider-governance-acceptance.ts'
  );
});
