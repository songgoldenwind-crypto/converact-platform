...
test('event HTTP exposes the catalog and admin subscription lifecycle', async () => {
  ...
  const catalog = await routeIveKitEventApi(
  ...
  const created = await routeIveKitEventApi(
  ...
  const updated = await routeIveKitEventApi(
  ...
  const archived = await routeIveKitEventApi(
...
test('OPC embedded HTTP entrypoint forwards safe event administration context', () => {
  const source = readFileSync('src/http.ts', 'utf8');
  assert.match(source, /routeIveKitEventApi\(pg, method, path, url, headers, body\)/);
  assert.match(source, /'x-opc-source-ip': req\.socket\.remoteAddress \|\| ''/);
  assert.match(source, /'retry-after': retryAfterSeconds/);
});

function moduleFixture(calls: Array<{ name: string; input: any }>): IveKitEventHttpModule {
  return {
