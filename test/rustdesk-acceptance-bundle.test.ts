...
test('RustDesk acceptance bundle writes the server handoff artifact set', () => {
  ...
  assert.match(manifest.expected_artifacts.client_config_pack.command, /CONVERACT_RUSTDESK_CLIENT_CONFIG_PACK_FILE=<bundle>\/client-config-pack\.md/);
  ...
  assert.match(manifest.expected_artifacts.filled_client_acceptance_report.command, /CONVERACT_RUSTDESK_ACCEPTANCE_AUDIT_FILE=<bundle>\/audit-export\.jsonl/);
  assert.match(manifest.expected_artifacts.audit_export.command, /CONVERACT_RUSTDESK_AUDIT_EXPORT_FILE=<bundle>\/audit-export\.jsonl/);
  assert.match(manifest.expected_artifacts.audit_export.command, /npm run rustdesk:audit-export/);
  assert.match(manifest.expected_artifacts.audit_coverage_report.command, /CONVERACT_RUSTDESK_AUDIT_COVERAGE_FILE=<bundle>\/audit-export\.jsonl/);
  ...
  assert.match(serverRunbook, /CONVERACT_RUSTDESK_PREFLIGHT_REPORT_FILE=.*preflight\.json/);
  assert.match(serverRunbook, /CONVERACT_RUSTDESK_SERVER_EVIDENCE_FILE=.*server-evidence\.json/);
  assert.match(serverRunbook, /CONVERACT_RUSTDESK_READINESS_REPORT_FILE=.*readiness\.json/);
  assert.match(serverRunbook, /CONVERACT_RUSTDESK_CLIENT_CONFIG_PACK_FILE=.*client-config-pack\.md/);
  ...
  assert.match(serverRunbook, /CONVERACT_RUSTDESK_AUDIT_EXPORT_EXTERNAL_ID=<rustdesk-gateway-external-id>/);
  assert.match(serverRunbook, /npm run rustdesk:audit-export/);
  assert.match(serverRunbook, /CONVERACT_RUSTDESK_AUDIT_COVERAGE_FILE=.*audit-export\.jsonl/);
  assert.match(serverRunbook, /CONVERACT_RUSTDESK_AUDIT_COVERAGE_REPORT_FILE=.*audit-coverage\.json/);
  assert.match(serverRunbook, /npm run rustdesk:audit-coverage/);
  assert.match(serverRunbook, /CONVERACT_RUSTDESK_EVIDENCE_DEPLOYMENT_COMMANDS_FILE=.*deployment-commands\.md/);
  assert.match(serverRunbook, /CONVERACT_RUSTDESK_EVIDENCE_CLIENT_CONFIG_PACK_FILE=.*client-config-pack\.md/);
  assert.match(serverRunbook, /CONVERACT_RUSTDESK_EVIDENCE_CLIENT_ACCEPTANCE_AUDIT_FILE=.*audit-export\.jsonl/);
  assert.match(serverRunbook, /CONVERACT_RUSTDESK_EVIDENCE_AUDIT_COVERAGE_REPORT_FILE=.*audit-coverage\.json/);
  ...
  assert.match(eventRunbook, /CONVERACT_RUSTDESK_EVENT_TEMPLATE_FILE=.*events-template\.jsonl/);
  assert.match(eventRunbook, /CONVERACT_RUSTDESK_EVENT_FILE=.*events-template\.jsonl/);
  assert.match(eventRunbook, /CONVERACT_RUSTDESK_EVENT_VALIDATE_ONLY=1/);
  assert.match(eventRunbook, /CONVERACT_RUSTDESK_EVENT_DEAD_LETTER_FILE=.*event-forwarder-dead-letter\.jsonl/);
  assert.match(eventRunbook, /CONVERACT_RUSTDESK_EVENT_REPLAY_DEAD_LETTER_FILE=.*event-forwarder-dead-letter\.jsonl/);
  assert.match(eventRunbook, /CONVERACT_RUSTDESK_EVENT_REPLAY_REMAINING_FILE=.*event-forwarder-remaining\.jsonl/);
  assert.match(eventRunbook, /CONVERACT_RUSTDESK_AUDIT_EXPORT_FILE=.*audit-export\.jsonl/);
  assert.match(eventRunbook, /CONVERACT_RUSTDESK_AUDIT_COVERAGE_FILE=.*audit-export\.jsonl/);
...
test('RustDesk acceptance bundle CLI writes a bundle directory and exposes package/env wiring', () => {
  ...
  for (const key of [
    ...
    'CONVERACT_RUSTDESK_ACCEPTANCE_BUNDLE_TITLE='
  ]) {
...
function bundleEnv(): NodeJS.ProcessEnv {
  return {
    ...
    CONVERACT_RUSTDESK_ACCEPTANCE_EXTERNAL_ID: 'rdgw_bundle_1',
    CONVERACT_RUSTDESK_ACCEPTANCE_RUSTDESK_ID: '123456789',
    CONVERACT_RUSTDESK_ACCEPTANCE_OPERATOR: 'qa_operator',
    CONVERACT_RUSTDESK_EVENT_TEMPLATE_TARGET: '123456789'
  };
}
