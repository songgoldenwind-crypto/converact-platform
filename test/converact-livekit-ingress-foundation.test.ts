...
test('LiveKit Ingress exact source image is offline, immutable and release gated', () => {
  ...
  const publicBuild = source('infra/converact/livekit-ingress/build-converact.sh');
  const workflow = source('.github/workflows/converact-livekit-ingress-image.yml');
  ...
  assert.match(publicBuild, /CONVERACT_FABRIC_LIVEKIT_INGRESS_BUILDER_IMAGE/);
  assert.match(publicBuild, /CONVERACT_FABRIC_LIVEKIT_INGRESS_RUNTIME_IMAGE/);
  ...
  assert.match(
    workflow,
    /CONVERACT_FABRIC_LIVEKIT_INGRESS_BUILDER_IMAGE: \$\{\{ vars\.CONVERACT_FABRIC_LIVEKIT_INGRESS_BUILDER_IMAGE \}\}/
  );
  assert.match(
    workflow,
    /CONVERACT_FABRIC_LIVEKIT_INGRESS_RUNTIME_IMAGE: \$\{\{ vars\.CONVERACT_FABRIC_LIVEKIT_INGRESS_RUNTIME_IMAGE \}\}/
  );
  assert.match(workflow, /for arch in amd64 arm64/);
  assert.match(workflow, /ghcr\.io\/songgoldenwind-crypto\/converact-livekit-ingress/);
...
test('LiveKit Ingress Helm pool isolates public media and admission capacity', () => {
  ...
  assert.match(deployment, /redis:[\s\S]*include "converact\.livekitRedisConfig"/);
  ...
  assert.match(values, /repository: ghcr\.io\/songgoldenwind-crypto\/converact-livekit-ingress/);
