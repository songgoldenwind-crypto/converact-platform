...
test('standalone Helm exposes an opt-in digest-pinned compact Tinode workload', () => {
  ...
  assert.match(values, /repository: ghcr\.io\/songgoldenwind-crypto\/converact-tinode-server/);
  assert.match(values, /digest: ""/);
  assert.match(helpers, /define "converact\.tinodeImage"/);
  ...
  assert.match(deployment, /image: \{\{ include "converact\.tinodeImage" \. \| quote \}\}/);
});
...
test('bundled Tinode has service disruption and optional network isolation contracts', () => {
  ...
  assert.match(apiDeployment, /include "converact\.tinodeFullname"/);
});
