...
import { IvrError, routeConveractFabricIvrApi, type IvrHttpModule } from '../src/agent-runtime/converact/ivr/index.js';
...
test('IVR Step HTTP authenticates profile before passing trusted tenant context to the service', async () => {
  ...
  const result = await routeConveractFabricIvrApi(
...
test('IVR Step HTTP rejects non-RustPBX profiles and leaves unrelated routes untouched', async () => {
  assert.equal(await routeConveractFabricIvrApi(
    pg, 'GET', url.pathname, url, null
  ), undefined);
  await assert.rejects(() => routeConveractFabricIvrApi(
    pg, 'POST', url.pathname, url, {}, '{}', {}, {
      webhook_authenticator: {
        authenticate: async () => ({
...
test('IVR HTTP exposes flow release, simulation, and session surfaces under signed tenant authority', async (t) => {
  ...
  const listed = await routeConveractFabricIvrApi(
  ...
  await routeConveractFabricIvrApi(
  ...
  const simulated = await routeConveractFabricIvrApi(
