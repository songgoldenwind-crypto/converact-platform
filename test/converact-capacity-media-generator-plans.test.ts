...
test('RTP media twin plan binds SIP session manifest, packet rate and immutable binary identity', () => {
  const plan = buildRtpMediaTwinPlan({
    binary: '/opt/converact/bin/converact-rtp-twin',
    ...
    session_manifest_path: '/var/lib/converact/run-rtp-001/sip-sessions.jsonl',
    codec: 'pcmu',
    payload_type: 0,
    clock_rate_hz: 8_000,
    packetization_ms: 20,
    ...
    result_path: '/var/lib/converact/run-rtp-001/rtp-evidence.json',
...
test('LiveKit plan models many rooms, screen, TURN and Egress instead of one giant room', () => {
  const plan = buildLiveKitCapacityPlan({
    binary: '/opt/converact/bin/converact-livekit-loadgen',
    ...
    result_path: '/var/lib/converact/run-livekit-001/evidence.json'
  });
...
test('RustDesk synthetic plan requires native protocol driver and separates Windows correctness', () => {
  const plan = buildRustDeskSyntheticPlan({
    binary: '/opt/converact/bin/converact-rustdesk-synthetic',
    ...
    office_trace_path: '/opt/converact/traces/office.trace',
    office_trace_sha256: 'e'.repeat(64),
    high_motion_trace_path: '/opt/converact/traces/high-motion.trace',
    high_motion_trace_sha256: 'f'.repeat(64),
    file_fixture_path: '/opt/converact/fixtures/10mb.bin',
    ...
    duration_seconds: 300,
    result_path: '/var/lib/converact/run-rustdesk-001/evidence.json',
...
test('external generator bounds result files and terminates on lease cancellation', async () => {
  const directory = mkdtempSync(join(tmpdir(), 'converact-capacity-generator-'));
