...
import {
  ConveractFabricRetentionError,
  routeConveractFabricRetentionApi,
  type ConveractFabricRetentionHttpModule
...
test('retention HTTP manages optimistic policies and records safe audit metadata', async () => {
  ...
  const result = await routeConveractFabricRetentionApi(
...
test('retention HTTP creates legal holds idempotently and hides idempotency internals', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof ConveractFabricRetentionError
      && error.code === 'compliance_denied'
  );
});

function moduleStub(overrides: Partial<ConveractFabricRetentionHttpModule> = {}): ConveractFabricRetentionHttpModule {
  return {
