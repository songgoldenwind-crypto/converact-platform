...
test('SIPp RTP-check Docker plan isolates media endpoints and preserves raw evidence', () => {
  ...
  assert.equal(plan.uac_container, 'converact-rtp-uac-pcmu-10');
  assert.equal(plan.uas_container, 'converact-rtp-uas-pcmu-10');
  ...
  assert.match(uac, /--network converact-rtp --ip 172\.30\.44\.20/);
  ...
  assert.match(uas, /run -d --name converact-rtp-uas-pcmu-10/);
  assert.match(uas, /-w \/results\/uas/);
  assert.match(uas, /--network converact-rtp --ip 172\.30\.44\.22/);
