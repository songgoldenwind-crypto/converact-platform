...
test('Linux process observer CLI is packaged for repeatable capacity campaigns', () => {
  ...
  assert.equal(
    packageJson.scripts['ivekit:observe:linux'],
    'tsx scripts/ivekit-linux-process-observer.ts'
  );
  assert.equal(
    existsSync(new URL('../scripts/ivekit-linux-process-observer.ts', import.meta.url)),
    true
  );
});

test('Linux process observer CLI parses fenced PID and command modes', async () => {
  const module = await import('../scripts/ivekit-linux-process-observer.js');
...
test('Linux process observer brackets a real child and writes private non-overwriting evidence', {
  ...
  const module = await import('../scripts/ivekit-linux-process-observer.js');
  const directory = mkdtempSync(join(tmpdir(), 'ivekit-linux-process-observer-'));
