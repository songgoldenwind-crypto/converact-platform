...
test('event webhook store inserts idempotently and rejects changed payloads', async () => {
  const insertedPg = new RecordingPg((sql) => /INSERT INTO ivekit_event_webhook_subscriptions/i.test(sql)
    ? [subscriptionRow()]
    : []);
  const inserted = await new PostgresIveKitEventWebhookStore(insertedPg).insert(subscriptionRow());
  ...
  await assert.rejects(
    new PostgresIveKitEventWebhookStore(conflictPg).insert(subscriptionRow()),
...
test('event webhook store claims subscriptions scans events and fences cursor updates', async () => {
  ...
  const store = new PostgresIveKitEventWebhookStore(pg);
...
function subscriptionRow(): IveKitEventWebhookSubscription {
  return {
