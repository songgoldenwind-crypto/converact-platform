...
const rustdeskRenameMigration = JSON.parse(
  readFileSync('docs/capacity/forks/converact-rustdesk-client-rename-v2.json', 'utf8')
) as ForkSourceMigration;
...
test('fork manifest patch paths match v1 bytes or an explicitly bound successor', () => {
  for (const component of manifest.components) {
    for (const patch of component.patches || []) {
      assert.equal(existsSync(patch.path), true, `${component.component_id}: ${patch.path}`);
      if (patch.path === rustdeskRenameMigration.path) {
        assert.equal(component.component_id, rustdeskRenameMigration.component_id);
        assert.equal(patch.sha256, rustdeskRenameMigration.previous_sha256);
        assert.equal(
          sha256(
            execFileSync('git', [
              'show',
              `${rustdeskRenameMigration.freeze_commit}:${patch.path}`
            ])
          ),
          rustdeskRenameMigration.previous_sha256,
          `${component.component_id}: frozen ${patch.path}`
        );
        assert.equal(
          sha256(readFileSync(patch.path)),
          rustdeskRenameMigration.current_sha256,
          `${component.component_id}: current ${patch.path}`
        );
        continue;
      }
      assert.equal(
...
test('RustDesk rename migration binds the immutable v1 manifest and one-root fallback', () => {
  assert.equal(rustdeskRenameMigration.migration_id, 'converact-rustdesk-client-rename-v2');
  assert.equal(rustdeskRenameMigration.source_manifest, 'docs/capacity/forks/ivekit-forks-v1.json');
  assert.equal(
    sha256(readFileSync(rustdeskRenameMigration.source_manifest)),
    rustdeskRenameMigration.source_manifest_sha256
  );
  assert.equal(rustdeskRenameMigration.current_install_root, '%ProgramData%/Converact/RustDesk');
  assert.equal(rustdeskRenameMigration.legacy_install_root, '%ProgramData%/iveKit/RustDesk');

  const source = readFileSync(rustdeskRenameMigration.path, 'utf8');
  assert.match(source, /current_roots_file\.is_file\(\)/);
  assert.match(source, /!legacy_roots_file\.is_file\(\)/);
  assert.doesNotMatch(source, /chain\(|extend\(/);
});
...
test('RustPBX fork manifest tracks the ivekit.41 candidate patchset', () => {
  const rustpbx = manifest.components.find((component) => component.component_id === 'rustpbx');
  assert.ok(rustpbx);

  const expectedReference = 'ivekit/rustpbx:0.4.11-ivekit.41-6c49ee76';
...
test('LiveKit SIP build script rejects source drift and verifies its runtime identity', () => {
  ...
  const workflow = readFileSync('.github/workflows/converact-livekit-sip-image.yml', 'utf8');
  ...
  assert.match(build, /io\.converact\.component=livekit-sip/);
  ...
  assert.match(workflow, /CONVERACT_FABRIC_LIVEKIT_SIP_BUILDER_IMAGE/);
  assert.match(workflow, /CONVERACT_FABRIC_LIVEKIT_SIP_RUNTIME_IMAGE/);
  assert.match(workflow, /uses: \.\/\.github\/workflows\/converact-oci-release-gate\.yml/);
