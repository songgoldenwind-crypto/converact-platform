...
import {
  ...
  type ConveractFabricSipAudioElement,
  type ConveractFabricSipWebPhoneEngine,
  type ConveractFabricSipWebPhoneEngineEvents
...
test('extension session plans reject unsafe transports, expiry, and missing ephemeral credentials', () => {
  assert.equal(parseConveractFabricVoiceExtensionSessionPlan(sessionPlan(), { now: () => NOW }).transport, 'wss');
  assert.throws(
    () => parseConveractFabricVoiceExtensionSessionPlan(
...
test('SIP WebPhone drives registration, calling, media controls, and devices through one state model', async () => {
  ...
  let events!: ConveractFabricSipWebPhoneEngineEvents;
  const engine: ConveractFabricSipWebPhoneEngine = {
  ...
  const phone = createConveractFabricSipWebPhone({
  ...
  phone.attachRemoteAudio({} as ConveractFabricSipAudioElement);
...
test('SIP WebPhone surfaces an output-device binding failure', async () => {
  const phone = createConveractFabricSipWebPhone({ plan: sessionPlan(), now: () => NOW });
...
function sessionPlan(
  patch: Partial<ConveractFabricVoiceExtensionSessionPlan> = {}
