...
test('route snapshot runtime accepts direct IVEKIT compatibility keys', () => {
  const config = rustPbxRouteSnapshotRuntimeConfig({
    DATABASE_URL: 'postgresql://runtime:secret@postgres.internal:5432/ivekit',
    IVEKIT_RUSTPBX_ROUTE_TENANT_ID: 'tenant-a',
    IVEKIT_RUSTPBX_ROUTE_PROFILE_ID: 'profile-a',
    IVEKIT_RUSTPBX_ROUTE_SNAPSHOT_FILE: '/var/lib/ivekit/routes.json',
    IVEKIT_RUSTPBX_ROUTE_SNAPSHOT_HMAC_KEY: SIGNING_KEY,
    IVEKIT_RUSTPBX_ROUTE_SNAPSHOT_INTERVAL_MS: '2000',
    IVEKIT_RUSTPBX_ROUTE_SNAPSHOT_TTL_MS: '15000',
    IVEKIT_RUSTPBX_ROUTE_DEPENDENCIES: 'start_ivr,bridge_livekit'
  });

  assert.equal(config.tenant_id, 'tenant-a');
  assert.equal(config.profile_id, 'profile-a');
  assert.equal(config.output_path, '/var/lib/ivekit/routes.json');
  assert.equal(config.interval_ms, 2_000);
  assert.equal(config.ttl_ms, 15_000);
  assert.deepEqual(config.available_dependencies, ['start_ivr', 'bridge_livekit']);
});
...
test('route snapshot projector signs a canonical HMAC-only snapshot and advances sequence', async () => {
  const directory = await mkdtemp(join(tmpdir(), 'converact-route-snapshot-'));
