...
test('RWI acceptance recognizes bounded RustPBX terminated dialog display states', () => {
  ...
  assert.equal(
    isTerminalRustPbxHangupState('ivekit-call-local-remote(Terminated UacBye)'),
    true
  );
  assert.equal(isTerminalRustPbxHangupState('ivekit-call-local-remote(Confirmed)'), false);
...
test('RWI acceptance runs preflight, deterministic originate, AMI reconciliation, and hangup', async () => {
  ...
  try {
    const report = await runRustPbxRwiAcceptance({
      IVEKIT_ACCEPTANCE_RUN_ID: 'controlled-a',
    ...
    assert.equal(report.ami_reconciliation.provider_call_id, 'ivekit-rwi-controlled-a');
