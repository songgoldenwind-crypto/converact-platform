...
const events = new ConveractFabricTenantEventStore(pg, { cursor_secret: 'event-ws-secret' });
...
test('durable Converact Fabric replay does not absorb unrelated call-center broadcasts', async () => {
  const before = await events.headCursor(tenantId);
