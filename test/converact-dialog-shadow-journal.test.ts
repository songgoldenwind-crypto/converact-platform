...
async function temporaryJournal(
  ...
  const directory = await mkdtemp(path.join(os.tmpdir(), 'converact-dialog-shadow-'));
...
test('dialog shadow WAL rejects symlink targets and permissive files', async (t) => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'converact-dialog-shadow-safe-'));
