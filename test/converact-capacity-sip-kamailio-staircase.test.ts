...
test('SIP Kamailio staircase config requires ascending controlled points and immutable inputs', () => {
  const config = buildSipKamailioStaircaseConfig({
    ...
    duration_seconds: 20,
    sipp_binary: '/opt/converact-capacity-benchmark/bin/sipp-3.7.7',
    rustpbx_image: 'converact/rustpbx:0.4.11-ivekit.16-6c49ee76',
    kamailio_image: `converact/kamailio@sha256:${'a'.repeat(64)}`,
    postgres_image: `postgres@sha256:${'b'.repeat(64)}`,
    python_image: `python@sha256:${'c'.repeat(64)}`,
    capacity_tools_image: 'converact/capacity-tools:0.1.0-rustpbx-router-v1',
  ...
  assert.throws(
    () => buildSipKamailioStaircaseConfig({
      ...config,
      kamailio_image: 'converact/kamailio:latest'
    }),
    /immutable/i
  );
  assert.throws(
...
test('SIP Kamailio resource parser preserves per-container maxima', () => {
  const samples = parseSipDockerStatsCsv([
    ...
    '2026-07-23T00:00:00Z,converact-rustpbx-baseline-kamailio-1,12.50%,20MiB / 8GiB,1kB / 2kB,0B / 0B,19',
    '2026-07-23T00:00:01Z,converact-rustpbx-baseline-kamailio-1,31.25%,24MiB / 8GiB,2kB / 3kB,0B / 0B,20',
    '2026-07-23T00:00:01Z,converact-rustpbx-baseline-rustpbx-1,44.50%,120MiB / 8GiB,5kB / 6kB,0B / 0B,15'
  ].join('\n'));
  const summary = summarizeSipDockerResources(samples);

  assert.equal(samples.length, 3);
  assert.deepEqual(summary['converact-rustpbx-baseline-kamailio-1'], {
    sample_count: 2,
  ...
  assert.equal(
    summary['converact-rustpbx-baseline-rustpbx-1']?.memory_max_bytes,
    120 * 1024 * 1024
  );
});
...
test('SIP Kamailio staircase has CLI and capacity package entrypoints', async () => {
  const [cli, rootPackage, capacityPackage, tsconfig, dockerfile] = await Promise.all([
    readFile(new URL('../scripts/converact-capacity-sip-kamailio-staircase.ts', import.meta.url), 'utf8'),
  ...
  assert.match(rootPackage, /converact:capacity:sip-kamailio-staircase/);
  assert.match(capacityPackage, /sip-kamailio-staircase/);
  assert.match(tsconfig, /converact-capacity-sip-kamailio-staircase\.ts/);
  assert.match(dockerfile, /COPY --chmod=0755 scripts\/converact-capacity-sip-kamailio-staircase\.ts/);
});
