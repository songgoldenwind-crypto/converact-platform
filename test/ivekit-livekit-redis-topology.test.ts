...
test('all LiveKit services render the shared Redis config and TLS secret mount', () => {
  for (const path of [
    ...
    assert.match(template, /include "opc\.livekitRedisConfig"/, path);
    assert.match(template, /include "opc\.livekitRedisTLSVolumeMount"/, path);
    assert.match(template, /include "opc\.livekitRedisTLSVolume"/, path);
  }
});

test('OPC application renders the same Redis topology, ACL and TLS contract', () => {
  const deployment = source('infra/k8s/templates/opc-deployment.yaml');
  const secrets = source('infra/k8s/templates/secrets.yaml');

  assert.match(deployment, /include "opc\.redisClientEnv"/);
  assert.match(deployment, /include "opc\.livekitRedisTLSVolumeMount"/);
  assert.match(deployment, /include "opc\.livekitRedisTLSVolume"/);
  for (const key of [
    'redis-username',
    'redis-password',
