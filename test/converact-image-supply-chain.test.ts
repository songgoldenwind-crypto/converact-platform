...
const workflow = readFileSync('.github/workflows/converact-oci-release-gate.yml', 'utf8');
...
test('core source images share one digest-only build and release workflow', () => {
  const reusable = readFileSync(
    '.github/workflows/converact-source-image-release.yml',
    'utf8'
  );
  const caller = readFileSync('.github/workflows/converact-core-images.yml', 'utf8');
  ...
  assert.match(reusable, /uses: \.\/\.github\/workflows\/converact-oci-release-gate\.yml/);
  ...
  for (const image of [
    ...
    'converact-console',
    'converact-fabric',
    'converact-capacity-tools',
    'converact-kamailio',
    'converact-agent-runtime'
  ]) {
    assert.match(caller, new RegExp(`ghcr\\.io/songgoldenwind-crypto/${image}`));
  }
  assert.match(caller, /uses: \.\/\.github\/workflows\/converact-source-image-release\.yml/);
  assert.match(caller, /strategy:[\s\S]*matrix:[\s\S]*include:/);
});

test('Converact Fabric service image is built from its generated standalone source context', () => {
  const reusable = readFileSync(
    '.github/workflows/converact-source-image-release.yml',
    'utf8'
  );
  ...
  assert.match(reusable, /none\|converact-fabric-standalone/);
  assert.match(reusable, /scripts\/converact-standalone-build-context\.ts/);
  ...
  const serviceEntry = caller.match(/- id: converact-fabric[\s\S]*?(?=\n          - id:)/)?.[0] || '';
  assert.match(serviceEntry, /context: \.tmp\/converact-fabric-standalone-context/);
  assert.match(serviceEntry, /dockerfile: \.tmp\/converact-fabric-standalone-context\/Dockerfile/);
  assert.match(serviceEntry, /prepare: converact-fabric-standalone/);
  assert.match(caller, /prepare: \$\{\{ matrix\.prepare \}\}/);
});

test('LiveKit Server release rebuilds and gates the exact maintained fork', () => {
  const release = readFileSync(
    '.github/workflows/converact-livekit-server-image.yml',
    'utf8'
  );
  ...
  assert.match(release, /ghcr\.io\/songgoldenwind-crypto\/converact-livekit-server/);
  assert.match(release, /uses: \.\/\.github\/workflows\/converact-oci-release-gate\.yml/);
...
test('Tinode release builds an exact multi-architecture non-root fork and gates its digest', () => {
  const release = readFileSync(
    '.github/workflows/converact-tinode-server-image.yml',
    'utf8'
  );
  ...
  assert.match(release, /CONVERACT_FABRIC_TINODE_BUILDER_IMAGE: \$\{\{ vars\.CONVERACT_FABRIC_TINODE_BUILDER_IMAGE \}\}/);
  assert.match(release, /CONVERACT_FABRIC_TINODE_RUNTIME_IMAGE: \$\{\{ vars\.CONVERACT_FABRIC_TINODE_RUNTIME_IMAGE \}\}/);
  assert.match(release, /for arch in amd64 arm64/);
  assert.match(release, /ghcr\.io\/songgoldenwind-crypto\/converact-tinode-server/);
  assert.match(release, /uses: \.\/\.github\/workflows\/converact-oci-release-gate\.yml/);
  assert.match(release, /digest: \$\{\{ needs\.publish\.outputs\.digest \}\}/);
...
test('LiveKit Egress release removes mutable build inputs and gates a multi-architecture digest', () => {
  const release = readFileSync(
    '.github/workflows/converact-livekit-egress-image.yml',
    'utf8'
  );
  ...
  for (const input of [
    ...
    assert.match(
      release,
      new RegExp(`CONVERACT_FABRIC_LIVEKIT_EGRESS_${input}_IMAGE: \\$\\{\\{ vars\\.CONVERACT_FABRIC_LIVEKIT_EGRESS_${input}_IMAGE \\}\\}`)
    );
  }
  assert.match(release, /for arch in amd64 arm64/);
  assert.match(release, /ghcr\.io\/songgoldenwind-crypto\/converact-livekit-egress/);
  assert.match(release, /uses: \.\/\.github\/workflows\/converact-oci-release-gate\.yml/);
  assert.match(release, /digest: \$\{\{ needs\.publish\.outputs\.digest \}\}/);
...
test('LiveKit Ingress release uses exact source, immutable inputs and the shared digest gate', () => {
  ...
  for (const input of ['BUILDER', 'RUNTIME']) {
    assert.match(
      release,
      new RegExp(`CONVERACT_FABRIC_LIVEKIT_INGRESS_${input}_IMAGE: \\$\\{\\{ vars\\.CONVERACT_FABRIC_LIVEKIT_INGRESS_${input}_IMAGE \\}\\}`)
    );
  }
  assert.match(release, /for arch in amd64 arm64/);
  assert.match(release, /ghcr\.io\/songgoldenwind-crypto\/converact-livekit-ingress/);
  assert.match(release, /uses: \.\/\.github\/workflows\/converact-oci-release-gate\.yml/);
  assert.match(release, /digest: \$\{\{ needs\.publish\.outputs\.digest \}\}/);
