...
test('standalone Converact Fabric server exposes only approved routes', async (t) => {
  const calls: string[] = [];
...
test('standalone Converact Fabric server exposes Prometheus metrics', async (t) => {
  const db = createDatabase(':memory:');
  const server = createConveractFabricHttpServer({ db, pg: null });
  t.after(async () => {
    await new Promise<void>((resolve, reject) => server.close((error) => error ? reject(error) : resolve()));
    db.close();
  });
...
test('standalone Converact Fabric server separates liveness from dependency readiness', async (t) => {
  const db = createDatabase(':memory:');
  let probes = 0;
  const ready = {
...
test('standalone Converact Fabric server preserves bounded attachment bytes', async (t) => {
...
test('standalone Converact Fabric server preserves bounded secure file bytes', async (t) => {
...
test('standalone Converact Fabric server parses JSON request bodies', async (t) => {
  const db = createDatabase(':memory:');
  const server = createConveractFabricHttpServer({
    db,
    pg: null,
...
test('standalone Converact Fabric server preserves LiveKit webhook raw body', async (t) => {
  const db = createDatabase(':memory:');
...
test('standalone Converact Fabric server enters the tenant PostgreSQL context', async (t) => {
...
test('standalone Converact Fabric media calls receive the request-scoped PostgreSQL client', async (t) => {
  const previousApiKey = process.env.CONVERACT_API_KEY;
...
test('standalone Converact Fabric server preserves HTML and binary responses', async (t) => {
  const db = createDatabase(':memory:');
  const server = createConveractFabricHttpServer({
    db,
    pg: null,
...
test('standalone Converact Fabric server returns safe structured errors', async (t) => {
  const db = createDatabase(':memory:');
  const server = createConveractFabricHttpServer({
    db,
    pg: null,
...
test('standalone Converact Fabric server returns retry-after for distributed rate limits', async (t) => {
  const db = createDatabase(':memory:');
  const server = createConveractFabricHttpServer({
    db,
    pg: null,
    routes: {
      notifications: async () => { throw new ConveractFabricRateLimitError(17, 'recipient'); }
    }
  });
  t.after(async () => {
    await new Promise<void>((resolve, reject) => server.close((error) => error ? reject(error) : resolve()));
...
test('standalone Converact Fabric event administration returns structured rate-limit errors', async (t) => {
  const db = createDatabase(':memory:');
  const server = createConveractFabricHttpServer({
    db,
    pg: null,
    routes: {
      events: async () => { throw new ConveractFabricRateLimitError(23, 'actor'); }
    }
  });
  t.after(async () => {
    await new Promise<void>((resolve, reject) => server.close((error) => error ? reject(error) : resolve()));
...
test('standalone Converact Fabric server bounds every non-attachment request body', async (t) => {
...
test('standalone Converact Fabric server rejects malformed JSON before routing', async (t) => {
  let routeCalls = 0;
  const db = createDatabase(':memory:');
  const server = createConveractFabricHttpServer({
    db,
...
test('standalone Converact Fabric server handles configured browser CORS preflight', async (t) => {
...
test('standalone Converact Fabric server injects standalone media hooks', async (t) => {
  const db = createDatabase(':memory:');
  const pg = new MemoryPg();
  const server = createConveractFabricHttpServer({
    db,
