...
test('OpenTelemetry is explicit, trace-only, bounded and secret-safe', () => {
  assert.deepEqual(resolveTelemetryConfig({}, 'converact-api'), {
    enabled: false,
    service_name: 'converact-api',
    endpoint: '',
    sample_ratio: 0.1,
    max_queue_size: 2048,
  ...
  assert.equal(enabled.service_name, 'converact-worker');
...
test('Converact Fabric workloads preload trace SDK only when telemetry is enabled', () => {
