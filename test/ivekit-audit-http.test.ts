...
test('audit HTTP lists tenant-bound events with filters and pagination', async () => {
  let captured: IveKitAuditListInput | null = null;
  const result = await routeIveKitAuditApi(
...
test('audit HTTP exports JSONL and denies non-administrative roles', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof IveKitOperationsError
      && error.code === 'compliance_denied'
  );
});

function moduleStub(overrides: Partial<IveKitAuditHttpModule> = {}): IveKitAuditHttpModule {
  return {
