...
import {
  createAiAgentConveractCallbackSmokeConfigFromEnv,
  runAiAgentConveractCallbackSmoke
} from '../scripts/ai-agent-converact-callback-smoke.js';

test('ai agent converact callback smoke validates legacy business dispatch with tenant context', async () => {
  ...
  const result = await runAiAgentConveractCallbackSmoke(
    {
      baseUrl: 'http://converact.test',
      converactApiKey: 'converact-key',
  ...
  assert.equal(dispatchCall?.headers.get('x-api-key'), 'converact-key');
...
test('ai agent converact callback smoke config requires base url api keys and tenant', () => {
  assert.throws(
    () => createAiAgentConveractCallbackSmokeConfigFromEnv({}),
    /CONVERACT_BASE_URL/
  );
  assert.throws(
    () => createAiAgentConveractCallbackSmokeConfigFromEnv({ CONVERACT_BASE_URL: 'http://localhost:3000' }),
    /CONVERACT_API_KEY/
  );
  assert.throws(
    () =>
...
test('ai agent converact callback smoke cleans up the legacy room when dispatch fails', async () => {
