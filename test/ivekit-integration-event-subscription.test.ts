...
test('event webhook subscription rejects incompatible notification endpoints', async () => {
  const repository = new MemorySubscriptionRepository();
  const create = (endpoint: NotificationEndpoint | null) => new IveKitEventWebhookSubscriptionService({
    repository,
...
class MemorySubscriptionRepository implements IveKitEventWebhookSubscriptionRepository {
  item: IveKitEventWebhookSubscription | null = null;

  async insert(subscription: IveKitEventWebhookSubscription) {
  ...
  async update(subscription: IveKitEventWebhookSubscription) {
    this.item = subscription;
    return subscription;
  }
}
...
function webhookEndpoint(): NotificationEndpoint {
  return {
    ...
    provider_kind: 'webhook', status: 'active', endpoint_url: 'https://led.example.com/ivekit-events',
