...
test('Voice LiveKit bridge creates Media Core first, persists pending, and dials once', async () => {
  ...
  const service = new VoiceLiveKitBridgeService({
    media_calls: {
      async ensureVoiceBridge(input) {
        ...
        return { media_call_id: 'media-call-a', room_name: 'converact-pstn-room-a' };
      }
    },
    bridge: adapter
  });
  ...
  assert.equal(roomName, 'converact-pstn-room-a');
...
test('LiveKit SIP transfer uses the SDK without persisting the clear target', async () => {
  ...
  const result = await adapter.transfer({
    tenant_id: 'tenant-a', bridge_id: 'bridge-a', room_name: 'converact-pstn-room-a',
  ...
  assert.deepEqual(sip.transferCalls, [[
    'converact-pstn-room-a', 'voice-sip-call-a', 'sip:1002@pbx.internal', { playDialtone: false }
  ]]);
...
function fixedMediaCalls() {
  return {
    async ensureVoiceBridge() { return { media_call_id: 'media-call-a', room_name: 'converact-pstn-room-a' }; }
  };
}
  ...
  const result = {
    ...
    async create(...args: unknown[]) {
      ...
      return {
        ...
        roomName: String(args[2] ?? 'converact-pstn-room-a'), sipCallId: 'sip-call-a'
      };
    },
...
function bridgeRecord(patch: Partial<VoiceLiveKitBridge> = {}): VoiceLiveKitBridge {
  return {
    ...
    sip_participant_id: '', room_name: 'converact-pstn-room-a', provider_bridge_id: '', status: 'pending',
