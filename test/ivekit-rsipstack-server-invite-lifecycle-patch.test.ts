...
test('ivekit.40 applies the server INVITE lifecycle and owner patches in order', () => {
  ...
  assert.match(
    build,
    /rsipstack-ivekit-single-trying\.patch"[\s\S]*rsipstack-ivekit-server-invite-lifecycle\.patch"[\s\S]*rustrtc-ivekit-udp-socket-capacity\.patch"/
  );
  assert.match(
    build,
...
test('the fork manifest binds both lifecycle patches and their evidence', () => {
  assert.equal(manifest.revision, 60);
