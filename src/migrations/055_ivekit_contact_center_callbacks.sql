ALTER TABLE ivekit_cc_callbacks
  ADD COLUMN IF NOT EXISTS requested_by TEXT NOT NULL DEFAULT '',
  ADD COLUMN IF NOT EXISTS cancelled_by TEXT NOT NULL DEFAULT '';

CREATE INDEX IF NOT EXISTS idx_ivekit_cc_callbacks_reconcile
  ON ivekit_cc_callbacks(tenant_id, updated_at, id)
  WHERE state IN ('dialing', 'connected') AND outbound_call_id IS NOT NULL;

DROP TRIGGER IF EXISTS ivekit_cc_callbacks_immutable_delete ON ivekit_cc_callbacks;
CREATE TRIGGER ivekit_cc_callbacks_immutable_delete
BEFORE DELETE ON ivekit_cc_callbacks
