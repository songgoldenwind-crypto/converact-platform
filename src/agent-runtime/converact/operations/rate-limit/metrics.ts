...
const checks = new Counter({
  ...
  help: 'Total distributed Converact Fabric rate limit decisions',
...
const rejections = new Counter({
  ...
  help: 'Total distributed Converact Fabric rate limit rejections by bounded scope',
  labelNames: ['route', 'scope'],
  registers: [metricsRegistry]
});

export const converactFabricRateLimitMetricDefinitions = [
...
export function observeConveractFabricRateLimit(input: {
  route_group: string;
  allowed: boolean;
