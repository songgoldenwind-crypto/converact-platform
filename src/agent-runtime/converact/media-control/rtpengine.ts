  ...
  releaseSession(
    ...
    return this.#serial(session.media_reservation_id, async () => {
      ...
      const outcome = await this.#execute({
        ...
        tenant_id: current.tenant_id ?? 'converact-system',
        call_id: current.call_id,
        leg_id: current.leg_id ?? 'converact-system',
        cell_id: current.cell_id ?? 'converact-system',
        owner_node_id: current.owner_node_id ?? 'converact-system',
...
function commandFromRecord(
  ...
  return {
    ...
    command_id: record.command_id,
    tenant_id: record.tenant_id ?? 'converact-recovery',
    call_id: record.transport_call_id,
    leg_id: record.leg_id ?? 'converact-recovery',
    cell_id: record.cell_id ?? 'converact-recovery',
    owner_node_id: record.owner_node_id ?? 'converact-recovery',
    owner_epoch: record.owner_epoch,
