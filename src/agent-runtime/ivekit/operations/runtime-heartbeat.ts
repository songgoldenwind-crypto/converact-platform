...
export function startIveKitRuntimeHeartbeat(input: {
  ...
  const sourceCommit = sourceCommitValue(env.OPC_IVEKIT_SOURCE_COMMIT || env.GIT_COMMIT);
...
export function iveKitRuntimeHeartbeatConfig(
  ...
  return {
    enabled: booleanEnv(env.OPC_IVEKIT_RUNTIME_HEARTBEAT_ENABLED, false),
    interval_ms: integerEnv(env.OPC_IVEKIT_RUNTIME_HEARTBEAT_INTERVAL_MS, 10_000, 1_000, 300_000),
    stale_after_ms: integerEnv(env.OPC_IVEKIT_RUNTIME_HEARTBEAT_STALE_AFTER_MS, 45_000, 5_000, 900_000)
  };
}
...
export function iveKitRuntimeComponents(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  const flags: Array<[string, string]> = [
    ['notification_worker', 'OPC_IVEKIT_NOTIFICATION_WORKER_ENABLED'],
    ['event_webhook_worker', 'OPC_IVEKIT_EVENT_WEBHOOK_WORKER_ENABLED'],
    ['notification_health_worker', 'OPC_IVEKIT_NOTIFICATION_HEALTH_WORKER_ENABLED'],
    ['retention_worker', 'OPC_IVEKIT_RETENTION_WORKER_ENABLED'],
    ['contact_center_worker', 'OPC_IVEKIT_CONTACT_CENTER_WORKER_ENABLED'],
    ['ivr_workers', 'OPC_IVEKIT_IVR_WORKERS_ENABLED'],
    ['voice_workers', 'OPC_IVEKIT_VOICE_WORKERS_ENABLED'],
    ['translation_worker', 'OPC_TRANSLATION_WORKER_ENABLED'],
    ['quality_worker', 'OPC_QUALITY_REVIEW_WORKER_ENABLED'],
    ['file_scan_worker', 'OPC_FILE_SECURITY_SCAN_WORKER_ENABLED'],
    ['file_cleanup_worker', 'OPC_FILE_CLEANUP_WORKER_ENABLED']
  ];
  for (const [component, key] of flags) {
    if (env[key] === '1' || env[key] === 'true') components.push(component);
  }
  return components;
}
