...
export async function applyDialogShadowEnvelope(
  value: DialogShadowJetStreamEnvelope,
  input: {
    subject_prefix: string;
    local_identity: {
      cell_id: string;
...
): Promise<DialogShadowReplicaAck | null> {
  try {
...
function decodeEnvelope(value: unknown): DialogShadowJetStreamEnvelope {
