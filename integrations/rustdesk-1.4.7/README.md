# iveKit RustDesk 1.4.7 Native Control and Evidence Overlay
...
Apply it only to the pinned upstream `1.4.7` source tree:

```sh
node integrations/rustdesk-1.4.7/apply-overlay.mjs /path/to/rustdesk-1.4.7
```

The patcher fails closed if the upstream `src/lib.rs` or `src/ui_cm_interface.rs` anchors drift. Build the Windows x86_64 client with the normal RustDesk build process, sign it, publish it to the controlled artifact repository, and record `native_control_protocol: ivekit-rustdesk-native-control-v2` beside its SHA-256 in the iveKit Windows client profile. Placement-enabled package version 6 rejects installers without v2. The companion still understands v1 only for a rolling deployment in which Cell placement is explicitly disabled.
