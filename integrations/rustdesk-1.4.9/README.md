# Converact Fabric RustDesk 1.4.9 Native Control and Evidence Overlay
...
in the Converact Fabric Windows client profile. Placement-enabled package version 6
...
`.github/workflows/converact-rustdesk-windows-ci.yml` checks out the pinned upstream source with submodules, applies this overlay, installs the upstream vcpkg manifest, and runs a Windows `cargo check`. A signed release binary and two-machine behavior remain real-environment acceptance items.
