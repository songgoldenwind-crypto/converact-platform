import type { IveKitWebhookDeliveryEnvelope } from './event-types.js';

export interface IveKitWebhookReplayClaim {
  key: string;
  expires_at: string;
  body_sha256: string;
  delivery_id: string;
  event_id: string;
  tenant_id: string;
  envelope: IveKitWebhookDeliveryEnvelope<unknown>;
}

export interface IveKitWebhookReplayStore {
  claim(input: IveKitWebhookReplayClaim): Promise<boolean>;
}

export interface VerifyIveKitWebhookInput {
  ...
  now?: Date;
  replayStore?: IveKitWebhookReplayStore;
}

export interface VerifyIveKitWebhookResult<T = unknown> {
  envelope: IveKitWebhookDeliveryEnvelope<T>;
  duplicate: boolean;
}
...
export async function verifyIveKitWebhook<T = unknown>(
  input: VerifyIveKitWebhookInput
): Promise<VerifyIveKitWebhookResult<T>> {
  ...
  if (!(rawBody instanceof Uint8Array) || rawBody.byteLength < 2 || rawBody.byteLength > 1_048_576) {
    throw new Error('iveKit webhook body is invalid');
  }
  ...
  if (!(secret instanceof Uint8Array) || secret.byteLength < 32 || secret.byteLength > 4_096) {
    throw new Error('iveKit webhook secret is invalid');
  }
  const tolerance = boundedTolerance(input.toleranceSeconds);
  const now = input.now || new Date();
  if (!Number.isFinite(now.getTime())) throw new Error('iveKit webhook timestamp is invalid');
  if (!/^\d{10,11}$/.test(input.timestamp)) throw new Error('iveKit webhook timestamp is invalid');
  const timestampSeconds = Number(input.timestamp);
  if (!Number.isSafeInteger(timestampSeconds)
    || Math.abs(Math.floor(now.getTime() / 1000) - timestampSeconds) > tolerance) {
    throw new Error('iveKit webhook timestamp is outside the replay window');
  }
  const match = input.signature.match(/^v1=([a-f0-9]{64})$/);
  if (!match) throw new Error('iveKit webhook signature is invalid');
  const crypto = globalThis.crypto;
  if (!crypto?.subtle) throw new Error('Web Crypto is required to verify iveKit webhooks');
  ...
  if (!valid) throw new Error('iveKit webhook signature is invalid');

  let parsed: unknown;
  try {
    parsed = JSON.parse(decoder.decode(rawBody));
  } catch {
    throw new Error('iveKit webhook envelope is invalid');
  }
  ...
  if (input.replayStore) {
    ...
    const claimed = await input.replayStore.claim({
      ...
      envelope: envelope as IveKitWebhookDeliveryEnvelope<unknown>
    });
    duplicate = !claimed;
  }
...
function validateEnvelope<T>(value: unknown): IveKitWebhookDeliveryEnvelope<T> {
  ...
  if (data.schema_version !== 1) throw new Error('iveKit webhook envelope schema is invalid');
  ...
  return {
    ...
    data: {
      ...
      visibility: {
        scope: scope as IveKitWebhookDeliveryEnvelope['data']['visibility']['scope'],
        ref_id: refId,
...
function eventType(value: unknown): string {
  ...
  if (!/^[a-z][a-z0-9_]*(?:\.[a-z][a-z0-9_]*)+$/.test(result)) {
    throw new Error('iveKit webhook envelope event is invalid');
  }
  return result;
}
...
function isoTimestamp(value: unknown): string {
  ...
  if (!text || !Number.isFinite(parsed)) throw new Error('iveKit webhook envelope timestamp is invalid');
...
function boundedTolerance(value?: number): number {
  ...
  if (!Number.isInteger(result) || result < 30 || result > 3_600) {
    throw new Error('iveKit webhook tolerance is invalid');
  }
  return result;
}
...
function boundedReplayRetention(value: number | undefined, tolerance: number): number {
  ...
  if (!Number.isInteger(result) || result < Math.max(3_600, tolerance) || result > 7_776_000) {
    throw new Error('iveKit webhook replay retention is invalid');
  }
  return result;
}
