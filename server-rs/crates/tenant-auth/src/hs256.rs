...
const MAX_HS256_TOKEN_BYTES: usize = 65_536;
...
impl Hs256PlatformTokenVerifier {
    ...
    pub fn new(
        ...
        let expected_key_id = JsText::from(expected_key_id);
        if secret.is_empty()
            || secret.len() > MAX_HS256_SECRET_BYTES
            || !bounded_text(&expected_issuer)
            || !bounded_text(&expected_audience)
            || !bounded_text(&expected_key_id)
    ...
    pub fn verify(
        ...
        if token.is_empty() || token.len() > MAX_HS256_TOKEN_BYTES {
            return Err(PlatformTokenVerificationError::EncodingInvalid);
        }
        let [header_raw, payload_raw, signature_raw] = split_token(token)?;
        self.verify_signature(header_raw, payload_raw, signature_raw)?;
        self.verify_header(header_raw)?;
        let payload = decode_payload(payload_raw)?;
        self.verify_claims(payload, wall_now_epoch_ms)
    }
    ...
    fn verify_header(&self, header_raw: &str) -> Result<(), PlatformTokenVerificationError> {
        let bytes = decode_canonical_base64url(header_raw)?;
        let header: TokenHeader = serde_json::from_slice(&bytes)
            .map_err(|_| PlatformTokenVerificationError::HeaderInvalid)?;
        let key_id = JsText::from(header.kid);
        if header.alg != "HS256" || header.typ != "JWT" || key_id != self.expected_key_id {
    ...
    fn verify_claims(
        ...
        self.verify_exact_claim_bindings(&payload, &tenant_id)?;
    ...
    fn verify_exact_claim_bindings(
        ...
        if payload.sub != payload.identity_id
            || payload.iss != self.expected_issuer
            || payload.issuer != self.expected_issuer
            || payload.key_id != self.expected_key_id
...
fn split_token(token: &str) -> Result<[&str; 3], PlatformTokenVerificationError> {
...
fn decode_canonical_base64url(input: &str) -> Result<Vec<u8>, PlatformTokenVerificationError> {
    if input.is_empty()
        || !input
            .bytes()
