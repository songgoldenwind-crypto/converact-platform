# rvoip × Converact Fabric 通信底座整合设计
...
> 日期：2026-07-30
>
> 适用范围：Converact Platform/Converact Fabric、Converact Fabric RustPBX fork、rsipstack、rustrtc、RTPengine、`voice-media-rs`
> 与 `eisenzopf/rvoip`
>
...
本次整合不是在 Converact Platform 与 rvoip 之间二选一，也不是把 rvoip 当成只供参考的测试仓库。
最终决策是：

1. **rvoip 进入 Converact Platform 的通信协议底座候选层。** 只把 SIP parser/serializer、Protocol
...
> rvoip 是可逐模块采用的 SIP/RTP 协议底座与工程资产；RustPBX 是 Converact Platform 的业务软交换
...
WebRTC wire state 仍由 LiveKit 独占，但 Converact Platform 的 generation contract 必须完整约束它：
...
| 事实 | 唯一 Authority |
| --- | --- |
| Call/Leg、Business Dialog、路由、计费/录音决策、Logical Media Graph | RustPBX/Converact Fabric |
| Room、WebRTC Participant/Track、ICE/DTLS/SRTP、SFU route | LiveKit |
| Voice↔LiveKit logical bridge、correlation、generation/attempt、command/receipt、handoff decision 与 tombstone | Converact Platform Bridge Coordinator store |
...
| rating 和最终客户账单 | Converact Platform/Converact Fabric billing |
...
### 2.1 Converact Platform/Converact Fabric 提供的能力

Converact Platform/Converact Fabric 当前不是原始 RustPBX。它已经围绕固定 RustPBX、rsipstack 和 rustrtc 源码
形成维护分支，并补齐：
...
### 2.4 为什么上游没有天然合并，以及 Converact Platform 何时才会更好
...
- 两边都必须保留自己可独立发布的 domain model 和高层 API，因此上游不可能天然采用
  Converact Platform 的 owner fencing、Cell placement、CDR、Media Plan 和 VOS-EQ 约束；
...
```text
                         Converact Platform / Converact Fabric Control Plane
...
| 用户、租户、业务策略、计费规则 | Converact Platform/Converact Fabric | 无 |
| Call、Leg、Business Dialog | RustPBX | 提供协议事件，不保存业务最终事实 |
| owner epoch、route revision、command sequence | RustPBX/Converact Fabric owner contract | 携带并校验，不自行生成业务 Authority |
...
| 租户身份、credential 事实和 placement | Converact Platform identity | rvoip 只通过最小 credential port 读取 |
...
| Voice↔LiveKit logical bridge、generation/attempt、command/receipt、handoff decision 与 tombstone | Converact Platform Bridge Coordinator store；Edge/writer decision 仍归 Media Engine Facade | 不创建第二 Call/Room/CDR/媒体 Authority |
| `VOICE-LIVEKIT-BRIDGE-V1` admission、profile 与 evidence finalization | Converact Fabric capacity ledger/finalizer | 不继承 ordinary RTP、LiveKit-only 或 bridge-excluded 证据 |
...
| IM | Tinode | 不接管 |
| vCon | Converact Platform export/exchange adapter | 不成为 durable session Authority |
...
这是目标模块结构，不授权立即移动 Converact Platform 仓库或重写已完成的 RustPBX patch queue。第一步
...
当前 `converact_guard_entry` 仍以 SIP call-id 为主索引，无法区分同一 Call-ID 下的多个
...
- Converact Platform identity/tenant store 继续承担用户、租户和 credential reference Authority；
...
### 7.6 Converact Platform-owned Protocol Session façade

`SipProtocolCommand`、`SipFoundationEvent` 和 Protocol Dialog snapshot 是 Converact Platform 自己
...
也不成为 Converact Platform 的 façade 或持久模型。它们只可作为测试/API 语义参考。
...
rvoip 的 SIP/RTP foundation 不等于完整媒体服务器。以下能力继续由 Converact Platform 媒体体系负责：
...
voice-media-rs Codec Interface
             |
      Converact Fabric G729 Adapter
             |
   exact rvoip G729 source crate
```

一帧 codec Interface 固定 10 ms。10/20/30/40/50/60 ms RTP packet 由 Converact Fabric Adapter
...
`RecordingSink` 的依赖反转方向，但保留 Converact Fabric：
...
- STIR/SHAKEN 进入 Goal 6，作为 signing/verifying Adapter；认证、证书运营和 carrier
  certification 另行签署；
- SCIM 进入 Converact Platform 企业身份路线；不进入 SIP/RTP packet path；
...
- rvoip Conversation/Session/Participant 作为 Converact Platform 业务模型；
...
- 用 rvoip 上游 2K/一小时结果宣称 Converact Platform 容量；
...
“Rust 编写”或“上游测过”都不能替代 Converact Platform 同源码、同硬件证据。
...
- 2/4/8 节点扩展效率分别签署；
- rvoip 上游 CPS 只作为 generator/harness 参考，不能转成 Converact Platform claim。
...
8. Converact Platform-owned Protocol Session façade semantics（高层 rvoip runtime 明确拒绝）；
...
- RustPBX 当前仍由 `infra/converact/rustpbx` 的 pinned upstream + patch queue 构建，不是
...
Converact Platform 不再把 rvoip 简化为“只拿 G.729 和测试”的外部参考，也不把它误当成可直接替换
...
资格轨道，避免架构分叉。这样既兑现“不重复造 SIP/RTP 基础设施”，又不丢失 Converact Platform
