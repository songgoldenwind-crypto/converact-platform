# Converact Fabric Goal 4 Media Processing Implementation Plan

> 相关文档：
> [`整合设计 Revision 3`](rvoip-converact-communication-foundation-integration-design.md) ·
...
| `src/agent-runtime/converact/media-control/processing.ts` | Current HTTP transport client；迁移后仅诊断/兼容 |
| `src/agent-runtime/converact/media-control/router.ts` | Current profile 路由；Target 为 Rust Media Plan/Edge Backend selector |
| `infra/converact/voice-media/*` | benchmark/soak/诊断镜像和未来 scale-out 研究配置 |
| `test/converact-voice-media-goal4-*.test.ts` | 合同、集成、部署和 finalizer 门禁 |
...
- [x] 验证：
  `node --import tsx --test test/converact-voice-media-goal4-contract.test.ts`。
...
- [ ] 只使用隔离的 Converact Platform project/network/container，不修改 LED 服务。
