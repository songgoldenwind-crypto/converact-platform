# Converact Fabric V5 阶段三 RustDesk Windows 闭环实施计划

更新日期：2026-07-15

## 1. 目标与边界

在不实现 Converact Platform/LED 业务逻辑、不开发移动端、不重写 RustDesk 远程桌面协议的前提下，完成
可复用的 Windows 远控闭环：
...
客户端录屏字节。Converact Fabric 不复制这些数据，也不把 RustDesk 变成浏览器内远桌面；Converact Fabric 负责
...
- RustDesk OSS server 提供 `hbbs` rendezvous/signaling 和 `hbbr` relay；OSS 不提供 Pro Web
  Console/API，因此 Converact Fabric 保持独立控制面：<https://rustdesk.com/docs/en/self-host/rustdesk-server-oss/>。
...
Converact Fabric 不保存或下发 RustDesk permanent/temporary password。有人值守使用客户 consent、Converact Fabric
...
4. Windows 客户端 capability 配置与 Converact Fabric scope 尚未形成一份机器可校验的 effective policy。
5. 当前 release 需要重新绑定 Stage 3 migration/config/Windows package 的 release evidence。

## 4. 架构

```text
LED/Converact Platform customer UI                 LED/Converact Platform operator UI
       | create/display code                | verify code + launch
       +------------------+-----------------+
                          v
              Converact Fabric RustDesk control plane
        consent / auth code / policy / lock / audit
                          |
               device-bound edge token
                          v
              Converact Fabric Windows companion
...
- [x] 核对 OSS server/client 官方能力与 Converact Fabric ownership boundary。
...
- migration 064 在 standalone PostgreSQL 全新建库、旧 Converact Platform 库升级、幂等和 FORCE RLS 验证中通过。
- 部署、交付、OpenAPI、standalone source graph 等契约测试 `57/57` 通过。
- 根项目 `npm run typecheck`、Converact Fabric SDK `npm run build` 和 `git diff --check` 通过。
...
- 运行手册见 `docs/converact-rustdesk-windows-deployment.md`；真实双 Windows 终端仍为 `not_run`。
...
- [x] RustDesk 原生直接文件传输明确标记 `native_unscanned`，不冒充 Converact Fabric 已扫描文件。
...
物理 Windows 已通过；这些项目继续按 `docs/converact-fabric-v6-real-environment-acceptance.md` 保持 `not_run`。
...
5. Converact Fabric 不存屏幕像素、按键、剪贴板正文、文件内容、绝对路径或录屏字节。
...
3. LED 只依赖 SDK/API/edge package，不导入 Converact Platform call-center 代码或读取 PostgreSQL。
