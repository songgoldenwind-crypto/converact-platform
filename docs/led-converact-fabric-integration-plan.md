# LED × Converact Fabric 通信能力完整对接修改方案

> 文档日期：2026-07-28
> 面向：LED 架构、后端、Web 前端、Windows 客户端、测试与运维研发
> 对接范围：视频、应用内语音、IM、文件、RustDesk 远控、WebPhone/SIP、事件与审计
> Converact Fabric 基线：`@converact/sdk` 与 `/api/ivekit/*` v1 additive contract
> 业务主引用：`business_ref.type=order_assignment`，`business_ref.id=<LED assignment id>`

## 1. 结论

LED 不需要复制 LiveKit、Tinode、RustDesk、RustPBX、Kamailio 或 Converact Fabric 的内部实现。LED 需要补的是：

1. 一个服务端 Converact Fabric BFF/adapter，统一处理鉴权、幂等键、资源绑定和错误映射。
2. 一个按 assignment 聚合的通信绑定模型，保存 Converact Fabric 资源 ID 和安全状态，不保存短期凭据。
...
| Converact Fabric IM 底座 | 真实双用户 Tinode WebSocket、JRP 禁止直发、Converact Fabric 双向投递、关闭幂等、2/2 撤权、关闭后 inbound cursor 暂停均通过 | LED 实现页面/BFF 并做浏览器 UI 验收 |
...
LED 研发应按下表区分“LED 要开发”和“等待 Converact Fabric/Converact Platform 修复或交付”。不能为了赶联调绕过
Converact Fabric facade 直连底层组件。
...
| `POST /api/ivekit/chat/sessions` RLS 边界 | Converact Fabric/Converact Platform 已修复并完成真实 PostgreSQL 回归 | LED 按原合同调用；若再失败提交 tenant/request-id，不增加绕过分支 | 改 tenant、关 RLS、直写 Converact Fabric 数据库 |
| remote-session/consent 正式 facade | Converact Fabric/Converact Platform | 接入固定版 SDK；不自行伪造资源 ID | 直写旧表或在 LED 生成假 consent |
| RustDesk 页面和 attended 用户交互 | LED | 实现授权、拉起、控制和断开状态 | 把一次性 code 当 RustDesk password |
| Windows companion 精准断开 | Converact Fabric/Converact Platform | LED 只消费 disconnect state 并展示 | 默认重启整机 RustDesk 服务 |
| LiveKit/TURN/Tinode/RustDesk/Kamailio 公网端点与证书 | Converact Fabric/Converact Platform 运维 | 配置允许的 origin，并执行端到端验收 | 把服务端 secret 放入 LED |
| OCR/ASR/翻译/AI Provider 未配置 | Converact Fabric/Converact Platform 运维 | 根据 capability 显示不可用/待处理 | 把“未配置”显示成“未发现风险” |
| 双 Windows、双浏览器和弱网验收 | 双方 | LED 提供真实用户流程，Converact Platform 提供底座环境 | 用单浏览器合成流宣称最终验收完成 |

当前 Converact Platform 缺陷必须由 Converact Platform 修复后给出 commit、部署版本和回归证据。LED 只需保留明确错误状态，
不得增加与缺陷绑定的永久兼容分支。
...
- 把 LED 用户映射为稳定 Converact Fabric identity。
- 调用 Converact Fabric facade/SDK，不直连底层组件。
- 在 LED 数据库保存资源绑定、展示投影和 Webhook inbox。
- 浏览器 UI、用户授权交互、错误提示和断线恢复。
- Windows 客户端安装引导与 `rustdesk://` 用户点击拉起。

### 3.2 Converact Fabric 负责
...
- Converact Fabric/PostgreSQL、Tinode 数据库、Redis、NATS。
...
flowchart LR
  ...
  BFF --> IVE["Converact Fabric HTTP / WebSocket"]
  IVE --> LK
  IVE --> TN
  IVE --> RDCP["RustDesk control plane"]
  IVE --> RP["RustPBX / Kamailio"]
  IVE --> PG["Converact Fabric PostgreSQL / RLS"]
...
- 业务写请求先到 LED BFF，再调用 Converact Fabric。
- LiveKit/Tinode/RustDesk/SIP 的短期连接计划由 Converact Fabric 签发。
...
```text
CONVERACT_FABRIC_BASE_URL
CONVERACT_FABRIC_API_KEY
CONVERACT_FABRIC_TENANT_ID
CONVERACT_FABRIC_EVENT_WEBHOOK_SECRET
CONVERACT_FABRIC_REQUEST_TIMEOUT_MS
CONVERACT_FABRIC_BROWSER_AUTH_MODE
CONVERACT_FABRIC_ALLOWED_EVENT_PATTERNS
```
...
| 配置 | 规则 |
| --- | --- |
| `CONVERACT_FABRIC_BASE_URL` | HTTPS origin root，不能带 `/api/ivekit` 或其它 path |
| `CONVERACT_FABRIC_API_KEY` | 仅 LED 后端 secret，不进入浏览器、日志、错误或数据库 |
| `CONVERACT_FABRIC_TENANT_ID` | 固定映射 LED 当前租户；禁止由浏览器 body 决定 |
| `CONVERACT_FABRIC_EVENT_WEBHOOK_SECRET` | 至少 32 bytes，单独轮换，不能复用 API key |
| `CONVERACT_FABRIC_REQUEST_TIMEOUT_MS` | 建议普通读 5-10 秒，写 10-15 秒 |
| `CONVERACT_FABRIC_BROWSER_AUTH_MODE` | 推荐 `jwks`；过渡期可使用 BFF-only |
| `CONVERACT_FABRIC_ALLOWED_EVENT_PATTERNS` | 显式 allowlist，不订阅无关租户事件 |

浏览器鉴权推荐顺序：

1. **推荐：LED 自有 RS256/JWKS**。Converact Fabric 配置 LED issuer，JWT 至少包含
   `sub`、`tenant_id`、`role`、`iss`、`iat`、`exp`。
2. **可用：BFF-only**。浏览器不直调 Converact Fabric HTTP，所有业务请求由 LED 后端代表用户调用。
3. **不推荐：共享 HS256 secret**。如过渡期使用，claim 为 `sub/tid/role/iat/exp`，必须使用独立
   secret 和短 TTL。

禁止把 `CONVERACT_FABRIC_API_KEY` 打进 Next/Vite public env。

版本和启动门禁：

- `@converact/sdk` 必须锁定精确交付版本或制品 digest，不能使用 `^`、`~` 或运行时自动升级。
...
| `resource_id` | Converact Fabric opaque ID |
| `status/revision` | 最后一次服务端投影 |
| `started_at/ended_at` | 生命周期 |
| `created_by` | LED 用户 ID |
| `last_event_id` | 最后应用事件，用于诊断 |

`communication_bindings` 保存当前指针，`communication_resources` 保存历史，两者都不能替代 Converact Fabric
权威状态。并发创建时用 assignment 行锁或数据库唯一约束收敛，不能依赖前端按钮禁用。

对需要后台重试的 Converact Fabric 写操作，可增加 `converact_command_outbox`，至少保存
...
使用 `@converact/sdk`，不要复制 SDK 源码或手写底层 URL。

后端 client 创建规则：

```ts
const converact = createConveractFabricClient({
  baseUrl: env.CONVERACT_FABRIC_BASE_URL,
  apiKey: env.CONVERACT_FABRIC_API_KEY,
  tenantId: env.CONVERACT_FABRIC_TENANT_ID,
  userId: authenticatedLedUser.id,
  timeoutMs: env.CONVERACT_FABRIC_REQUEST_TIMEOUT_MS
});
```
...
```ts
const snapshot = await converact.media.createCall({
  media: "video",
...
- LED 只展示 Converact Fabric recording/egress 状态，不直接操作对象存储。
- 录制失败、对象存储不可用、转码失败不得结束正在进行的 WebRTC/语音会话。
- UI 必须区分“通话中”和“录制失败”。
- 导出只能通过 Converact Fabric 鉴权 facade，不能展示 object key 或 storage URL。
...
6. 建立 Converact Fabric WebSocket，保存 opaque cursor。
...
typing、presence 都走 Converact Fabric API/SDK。

LED 不得自行逐个删除 Tinode subscription 来代替 `chat.closeSession()`。Converact Fabric 关闭合同会先验证全部
...
UI 以 Converact Fabric snapshot/event 为权威，不以 Tinode local cache 覆盖服务端版本。
...
LED 可直接复用 `clients/converact-reference` 的 Remote 工作区流程，或集成
...
adapter，应由 Converact Fabric/Converact Platform 侧提供正式 remote-session/consent facade；LED 不应绕过 SDK 直连旧表或
伪造 ID。
...
| `native_unscanned` | RustDesk 原生传输未被 Converact Fabric 扫描 |
| `local_only` | 录屏只留在终端 |
...
6. 呼叫控制使用 Converact Fabric voice call/action，不直接调用 RustPBX RWI。
...
```text
POST /internal/webhooks/converact
```
...
3. 使用 `CONVERACT_FABRIC_EVENT_WEBHOOK_SECRET` 校验 `x-ivekit-signature`。
...
这些路径是 LED BFF 设计建议，不是 Converact Fabric 新 API。BFF 必须从 URL assignment 反查当前用户权限和
Converact Fabric business ref，不能透传前端给出的 tenant、actor 或任意资源 ID。
...
| 401 | 刷新 LED/Converact Fabric 短 token；失败回登录 |
| 403 | 显示无权限，不隐藏成网络错误 |
...
- 文件下载只经过 Converact Fabric facade。
...
### Converact Fabric/Converact Platform 研发交付
...
10. LED 与 Converact Fabric 的责任边界、部署参数和故障联系人写入运维手册。

## 21. 参考合同

- `docs/converact-openapi.md`
- `docs/converact-led-integration-guide.md`
- `docs/converact-fabric-video-im-capability-design.md`
- `sdk/converact`
- `clients/converact-reference`

本方案是 LED 的实施清单；字段和方法的最终类型以固定版本 `@converact/sdk`、运行时
`/api/ivekit/*/capabilities` 与 `/api/ivekit/events/catalog` 为准。

## 22. IM 会话关闭的 LED 处理规则

1. LED 只能调用 SDK/HTTP 的统一 `closeSession()`，不得直接修改 Converact Platform 数据库或调用 Tinode 删除
   topic/ACL。
