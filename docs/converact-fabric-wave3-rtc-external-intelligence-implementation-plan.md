# Converact Fabric Wave 3 RTC 与外部智能能力实施计划

更新日期：2026-07-24
状态：实施中
适用范围：Converact Platform 与 LED 共用通信底座，不包含 LED 业务逻辑，不部署自建模型
...
- 供应商专用协议以后只增加 adapter，不改变 Converact Platform/LED API、事件或数据库。
...
建立阶段按租户有序 route failover。已建立 session 不在一个音频帧中间切换 Provider；仅在连接失效后以新 provider session、连续的 Converact Fabric sequence 和新的 provider revision 重建。
...
- `src/agent-runtime/converact/intelligence-http.ts`
- `src/agent-runtime/converact/intelligence-preflight.ts`
- `sdk/converact/src/intelligence-types.ts`
...
- `src/agent-runtime/converact/voice/adapters/external-realtime-speech.ts`
- `src/agent-runtime/converact/voice/realtime-speech-routing.ts`
- `test/converact-external-realtime-speech.test.ts`
- `test/converact-realtime-speech-routing.test.ts`

修改：

- `src/agent-runtime/converact/voice/realtime-speech-translation.ts`
...
- `test/converact-tts-provider.test.ts`
- `test/converact-model-gateway-provider.test.ts`
...
- `src/agent-runtime/converact/voice/realtime-speech-store.ts`
- `src/agent-runtime/converact/voice/realtime-speech-projection.ts`
- `src/migrations/098_ivekit_realtime_speech_projection.sql`
- `test/converact-realtime-speech-projection.test.ts`

修改：

- `src/agent-runtime/converact/application.ts`
- `src/agent-runtime/converact/operations/retention/runtime.ts`
- `sdk/converact/src/media-types.ts`
- `sdk/converact/src/http-sdk.ts`
...
- `test/converact-rustpbx-audio-tap.test.ts`

修改：

- `infra/converact/rustpbx/build.sh`
- `infra/converact/rustpbx/Cargo.lock`
- `infra/converact/rustpbx/README.md`
...
- `npm run converact:realtime-speech-provider-acceptance` 已使用正式 WSS adapter、正式策略路由和治理
...
- `npm run converact:realtime-recovery-acceptance` 已在隔离服务器停止并恢复实际 PostgreSQL 16
...
- 浏览器 evidence 已升级到 schema `1.7.0`，把连接准备模式、主媒体发布完成、远端轨道
  ...
  `rtc.pli_throttle`，Converact Fabric 已验证 profile 为 low/mid/high `100/100/100 ms`，每项
...
所有测试先同步到 `/opt/converact-wave123-validation-20260722/source`，然后在服务器执行：
