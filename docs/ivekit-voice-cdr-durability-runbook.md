# iveKit Voice CDR 跨 Zone 耐久性运行手册
...
3. `OPC_IVEKIT_CDR_REGION_ID` 必须等于当前 API 实例所在 Region。未配置时严格保持
...
10. `VOICE-HA-T1` 清理必须先提交双腿 `terminating` shadow。该状态只能执行终态收敛，
    ...
    RustPBX 配置的 `IVEKIT_RUSTPBX_CDR_REGION_ID`。
...
启用 voice 的 API 必须显式配置 `OPC_IVEKIT_CDR_REGION_ID`。这个身份独立于 placement
...
`IVEKIT_RUSTPBX_CDR_REGION_ID`，服务密钥只通过
...
SELECT pg_advisory_xact_lock(
  hashtextextended('ivekit:voice-cdr-durability:' || :'region_id', 0)
);
...
BEGIN;
SELECT pg_advisory_xact_lock(
  hashtextextended('ivekit:voice-cdr-durability:region-a', 0)
);
UPDATE ivekit_voice_cdr_durability_contracts
