...
**状态：进行中。** M5.1 统一业务上下文、M5.2 统一导航/深链接和 M5.3 参与人/授权摘要已完成。聚合接口、SDK `context` 客户端、JWT 成员过滤、参考客户端业务摘要、资源级 URL、浏览器历史恢复、跨业务旧资源清理，以及 Chat/Media/Remote 只读授权投影均有自动化覆盖；受控三工作区 E2E 4/4 通过。
