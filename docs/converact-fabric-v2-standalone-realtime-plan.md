# Converact Fabric V2 独立服务与可靠实时同步实施计划

更新日期：2026-07-13

开发分支：`codex/converact-v2-realtime-standalone`

基线提交：`488f7b3`

## 1. 目标

Converact Fabric V2 要把 V1 已完成的 IM、LiveKit 音视频、RustDesk 远程协助和统一协作客户端，收敛为真正可独立构建、部署、升级和交付的通用服务，同时补齐实时链路中仍依赖内存或全量快照的可靠性缺口。

本 Goal 完成后，LED 或其他项目应只依赖以下交付物：

1. Converact Fabric 独立服务镜像和可审计的独立构建上下文。
2. Converact Fabric PostgreSQL fresh-install foundation schema 和后续 migration。
3. `@converact/sdk`、参考客户端 dist 和稳定 `/api/ivekit/*` 契约。
...
### 1.1 当前执行状态（2026-07-13）

| 里程碑 | 状态 | 已有证据 | 剩余工作 |
| --- | --- | --- | --- |
| M6.1 独立构建边界 | 已完成 | 当前为 91 个白名单源码文件、6 个运行依赖；隔离 `npm ci`/build 通过；服务器 Docker build 和 `/health` 通过 | 无 |
| M6.2 Standalone PostgreSQL | 已完成 | 最终 fresh schema 46 表、32 个 checksum migration、0 RLS gap、0 Converact Platform 业务表；existing Converact Platform 数据无损升级；runtime 跨租户读写、DDL 和迁移账本访问均被拦截；失败后前向重试通过 | 无 |
...
### 2.1 独立服务仍依赖完整 Converact Platform 源码

当前 `src/converact-server.ts` 已经是 Converact Fabric-only 运行入口，`src/agent-runtime/converact/http-server.ts` 也只开放 Converact Fabric、LiveKit webhook、RustDesk launch、health 和 metrics 路由，但构建边界仍不独立：

1. 根 `Dockerfile` 会复制完整 `src/`、`scripts/`、`shared/`、`public/` 和 `config/`。
2. Converact Fabric Compose 仍依赖从完整 Converact Platform 仓库构建出的应用镜像。
3. 服务入口仍从根模块读取 PostgreSQL、认证、WebSocket、metrics、对象存储和兼容数据库 helper。
4. V1 交付包只交付镜像名，没有交付可在独立目录构建该镜像的 source/build context。

因此 V1 证明了独立运行入口，尚未证明服务端可以脱离完整 Converact Platform 源码构建。

### 2.2 fresh database 仍创建完整 Converact Platform schema

当前 `scripts/run-postgres-migrations.ts` 调用根 `runMigrations()`，fresh database 会执行 `005_full_schema.sql`。该 migration 明确创建 Converact Platform 应用所需的 137 张表，然后再执行 collaboration/media/RustDesk migration。

Converact Fabric 运行时实际需要的基础对象包括：
...
独立部署不能再以完整 `005_full_schema.sql` 作为基础，但现有 Converact Platform 数据库升级路径必须保持不变。
...
2. fresh standalone PostgreSQL foundation 和 existing Converact Platform upgrade 两条迁移路径。
...
8. 为后续 OCR、ASR、AI 质检、翻译和 SIP/VoLTE 共用模块建立独立 service extension boundary，禁止这些能力反向依赖 Converact Platform call-center 业务。
...
4. Converact Platform call-center、IVR、CRM、外呼和坐席业务功能。

### 3.3 已纳入共用底座后续目标

以下能力必须由 Converact Fabric 共用层提供给 LED 和其他项目，不放进 LED 业务代码，也不再作为长期排除项：
...
4. 新 standalone schema 与现有 Converact Platform schema 使用相同表名和 migration version，避免双模型漂移。
...
```text
LED / other host
  -> @converact/sdk / reference client
  -> Converact Fabric standalone HTTP + WebSocket
...
目标：生成一个离开 Converact Platform 仓库后仍可执行 `npm ci && npm run build` 和 `docker build` 的 Converact Fabric service context。

任务：

1. 增加 `services/converact-service/` 作为独立 package/build owner。
2. 建立显式 source manifest，只允许 Converact Fabric、collaboration、LiveKit、media gateway、tenant/RLS、auth、object storage、metrics 和 WebSocket 所需模块。
3. 将共享依赖收敛到 Converact Fabric-owned adapter；禁止 import call-center、IVR、campaign、billing、CRM 和通用 Converact Platform server。
...
- 删除/隔离 Converact Platform 根源码后，独立 package 仍可 build 和启动 health endpoint。
- OCI build context 不引用 `../..`。
- V1 URL、SDK 和 reference client contract tests 不回归。

### M6.2 Standalone PostgreSQL foundation

目标：fresh database 不再执行 137 表完整 Converact Platform schema。

任务：

1. 增加 Converact Fabric foundation migration，创建 tenant、RLS helper、audit、LiveKit room、recording、evidence 等通信域前置对象。
2. 建立 standalone migration manifest，显式包含 foundation 和 `011` 到后续 Converact Fabric migration，排除 IVR `023`、legacy runtime `031` 等非通信 migration。
3. 保留根 Converact Platform migration runner，existing Converact Platform database 不重复创建或重编号。
...
7. 增加 fresh install、existing Converact Platform upgrade、重复执行、回滚前向恢复和跨租户 RLS 集成测试。

验收：

- fresh standalone schema 只包含 Converact Fabric 所需表。
- migration 后所有 tenant 表 `ENABLE/FORCE RLS`。
- runtime role 跨租户读写失败。
- existing Converact Platform schema 执行 standalone runner 不破坏数据且不重复版本。

### M6.3 Tinode inbound synchronization

目标：任何通过允许的 Tinode 客户端写入 topic 的业务事件，最终都进入 Converact Fabric 本地镜像、policy/audit 和统一事件流。
...
2. Converact Fabric outbound publish 必须携带稳定 Converact Platform message ID；inbound 发现本地 ID 时只确认 provider seq，不复制消息。
...
- 服务器隔离项目：`converact-v2-2fb74f4`，HTTP 仅绑定 `127.0.0.1:18303`。
...
- 停止 Converact Fabric 后向真实 Tinode 发布 seq 4，重启后 cursor 从 3 追到 4；再次重启 message/inbox/policy/quality 数量不增长，worker failure 为 0。
- 最终 PostgreSQL 为 31 个 checksum migration、45 张表、0 RLS gap；runtime role 四项管理员标志均为 false，跨租户读取为 0、写入被拒绝，长驻容器只使用 `opc_runtime`。
- 服务器证据文件：`/opt/converact-v2-validation/2fb74f4/tinode-inbound-acceptance-result.json` 与 `/opt/converact-v2-validation/2fb74f4/final-audit-result.txt`，权限均为 `600`。
...
- 独立开关为 `CONVERACT_FABRIC_EVENT_REPLAY_ENABLED`；retention、payload 上限和 WS replay 上限分别由 `CONVERACT_FABRIC_EVENT_RETENTION_MS`、`CONVERACT_FABRIC_EVENT_MAX_PAYLOAD_BYTES`、`CONVERACT_FABRIC_WS_REPLAY_MAX_EVENTS` 控制；过期清理 worker 使用安全 tenant 枚举函数并在 runtime RLS 事务内分批删除。
- fresh standalone PostgreSQL 当前为 32 个 checksum migration、46 张表；事件 append/head/list、RLS、runtime sequence 权限和 existing Converact Platform 无损升级通过。服务器提交 `823d28a` 的 HTTP/WS 重放各恢复 2 个事件，撤权后 0、定向 admin 0、跨租户读 0/写阻断、迁移和重启前后 event 计数均为 `6,6`，retention 删除通过且 worker failure 为 0。
...
2. spool 目录通过 `CONVERACT_RUSTDESK_EDGE_SPOOL_DIR` 显式配置；默认关闭命令执行或使用平台私有 application-data 目录，禁止写当前工作目录。
...
- 提交 `27575fe` 完成 edge spool/recovery，`c13f503` 补齐 standalone Compose 的 edge token secret 传递；均已推送 `codex/converact-v2-realtime-standalone`。
...
- 服务器 `64.225.122.227` 使用隔离 project `converact-v2-c13f503`、独立 fresh PostgreSQL volume 和 `127.0.0.1:18306`。镜像 `converact-service:c13f503` 健康，源码提交为 `c13f50396b4e98ac9aaff4c9992bb51c7fbeaa51`。
...
- 证据位于 `/opt/converact-v2-validation/c13f503/`：`rustdesk-recovery-result.json`、`rustdesk-edge-linux-test.txt`、`rustdesk-recovery-audit.txt`、`final-m6.5-audit.txt`，全部 mode `0600`。context archive SHA-256 为 `633d3f62e4fb70ba078722f7c0bae01893521b571d2054e8f35066edf4c4ccfd`，无 AppleDouble 和 build residue。
...
3. Compose 使用独立 Converact Fabric image、standalone migration job 和 provider plane。
4. 交付包增加 service source/build context、standalone migration manifest、image metadata 和 SBOM/checksum。
5. 文档补充 fresh install、existing Converact Platform upgrade、rollback、Tinode cursor recovery、event retention 和 edge spool 运维。
6. 交付生成器继续使用文件白名单、secret scan、所有权 marker 和 SHA-256。

验收：

- LED 仅凭交付包可以 build Converact Fabric image，不需要 Converact Platform 根源码。
...
- 提交 `d21a2eb` 已推送；SDK 新增 `events.getHeadCursor()`、`events.listPage()` 和有界 `events.replay()`，HTTP 409 被保留为类型化 `snapshot_required` 页，不会被误判为普通网络异常。
...
9. 独立 build context 在临时目录构建，禁止回读 Converact Platform root。
...
- 参考客户端 unit/component 117/117、production build 与 5 个 JS chunk budget 通过；服务器使用 `mcr.microsoft.com/playwright:v1.61.1-noble` 对当前客户端源码执行受控 IM/Media/RustDesk/移动端/深链 E2E，9/9 通过，证据为 `/opt/converact-v2-validation/d21a2eb/reference-client-e2e.txt`。
...
- Converact Fabric application、production、LiveKit edge 和 storage overlay 共 4 组 Compose `config --quiet` 通过；Go provider gateway、Python AI worker 和 Rust voice media sidecar 检查通过。
...
7. standalone foundation migration 不允许对已有 Converact Platform schema执行 down migration；回滚服务版本使用 forward-compatible schema。
...
| standalone schema 与 Converact Platform schema 漂移 | 同表同 migration version；schema contract diff test |
...
1. Converact Fabric service build context 离开 Converact Platform 仓库后可以独立 build。
2. fresh PostgreSQL 不创建 call-center/IVR/CRM 表。
3. existing Converact Platform database 无破坏升级。
...
8. standalone dependency policy 为 intelligence、translation、SIP 模块保留 Converact Fabric-owned extension boundary，且不允许导入 Converact Platform call-center/IVR。
...
1. **当前 Goal：Converact Fabric V2 独立服务与可靠实时同步闭环。** 解决代码、数据、实时和边缘可靠性。
2. **Converact Fabric V3 多模态智能与翻译。** 完成 OCR、ASR、AI 质检、人工复核和消息翻译，统一支持 self-hosted/third-party provider，并交付 LED SDK/API/UI 参考模块。
3. **Converact Fabric V4 SIP/VoLTE 通信。** 完成 LiveKit SIP trunk/dispatch、号码、入呼/外呼、DTMF、录制、状态机和 LED 接入，不引入 Converact Platform 坐席、CRM、营销外呼业务。
4. **最终 Goal：Converact Fabric Release 1.0 与真实环境验收。** 发布独立 image/SDK、部署目标环境，完成真实 Tinode/LiveKit/RustDesk/OCR/ASR/AI/translation/SIP、对象存储、网络、双浏览器/双桌面、容量、弱网、升级和回滚证据。

第四个 Goal 完成后冻结 Converact Fabric 共用底座并开始 Converact Platform 主项目架构设计。RTMP/HLS 直播和数字人继续作为未来可选扩展，不阻塞 Converact Platform 主架构。
