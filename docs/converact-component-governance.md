# Converact Fabric 组件治理与部署 Profile

> 状态：V1 已落地
> 范围：Converact Platform 与 LED 共用的 IM、音视频、语音呼叫、远程协助及其异步支撑能力
...
`component-authority-matrix-v1.json` 回答“谁负责、在哪个 Profile、默认是否启动、怎么开关、怎么回滚、资源预算是什么”。技术基线的 46 条决策必须在矩阵中各出现一次，不能遗漏或重复。矩阵另外记录 Converact Fabric API 控制面，因此当前共有 47 个治理条目。
...
`core` 是唯一默认 Profile，也是其他 Profile 的强制基础。默认 Chart 只启动 Converact Fabric API，不自动启动 ClamAV、RustPBX、Kamailio、Tinode 或任何 AI/监控/benchmark 组件。
...
生产 Chart 当前管理的 Converact Fabric API、RustPBX、Kamailio、Tinode、ClamAV 和 SIP exporter 都必须使用 `repository@sha256:digest`。Helm helper 对 digest 格式执行强校验，tag 不能作为生产身份。
...
8. Converact Platform 与 LED 是否仍通过稳定 API/SDK/事件边界使用能力，而不是依赖项目私有接口？
...
这个机制允许继续大胆修改开源源码，但修改发生在明确的主责组件内。外部项目可以持续提供算法、协议和性能优化思路，而不会把 Converact Platform/LED 底座变成多套系统永久并行的集合。
