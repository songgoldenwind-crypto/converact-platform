# iveKit V5 阶段二 IM、LiveKit 与文件安全生产化实施计划

更新日期：2026-07-15

## 1. 目标

在不实现 OPC/LED 业务逻辑、不开发移动端的前提下，完成以下共享底座：
...
4. API、SDK、事件、OpenAPI、Compose/Helm、运维和自动化验收可被 OPC、LED 及后续产品直接复用。
...
`npm run verify:ivekit:stage2-deployment` 将 Compose、Helm lint/template、不可变应用/ClamAV
...
显式使用同一 Redis address/认证/TLS，且双池只接受批准仓库 `ivekit/livekit-egress` 的不可变
...
- PostgreSQL harness：6 项全部通过；覆盖 fresh、OPC upgrade/数据保留、FORCE RLS、Tinode
...
6. 自动化只产生状态、finding 和事件，不执行 OPC/LED 订单处罚或业务处置。
