...
1. 用 W3C TraceContext 串联 iveKit HTTP、PostgreSQL、NATS、外部 Provider 和异步 worker 边界。
...
| 业务指标 | iveKit `prom-client` 与各通信组件原生 Prometheus 指标 | Collector 不复制业务指标模型 |
...
- `OPC_OTEL_ENABLED=0` 时零 exporter、零网络连接，并保留原启动命令。
...
服务器源码目录为 `/opt/opc-wave123-validation-20260722/source`，Node 为 `/opt/opc-wave123-validation-20260722/cache/toolchain/bin/node`，Helm 使用缓存镜像 `alpine/helm:3.18.4` 并显式覆盖 entrypoint。
