...
**技术栈：** TypeScript、Node.js、PostgreSQL FORCE RLS、现有 durable worker、`@opc/ivekit-sdk`、Node test runner、Prometheus。
...
- `scripts/ivekit-provider-governance-acceptance.ts`：受控协议与治理矩阵验收，明确不作为真实厂商证据。
- `test/ivekit-intelligence-provider-governance.test.ts`：状态机和 SQL 合同测试。
- `test/ivekit-intelligence-provider-route.test.ts`：四种 capability 的 route/failover 测试。
...
- `src/agent-runtime/ivekit/intelligence-http.ts`：运行状态和 route-safe API。
- `src/agent-runtime/ivekit/application.ts`：注入 governance store、指标和 route executor。
- `src/agent-runtime/collaboration/index.ts`：导出新增公共类型。
- `sdk/ivekit/src/intelligence-types.ts`、`sdk/ivekit/src/http-sdk.ts`：策略 route 和运行状态 SDK。
- `services/ivekit-service/source-policy.json`：加入 migration 059。
- `services/ivekit-service/env.example`、`services/ivekit-service/README.md`：运行预算和运维说明。
- `docs/ivekit-openapi.md`、`docs/openapi.yaml`、`docs/ivekit-v3-intelligence-operations.md`：API、兼容和故障处置文档。
...
- [x] 在 `test/ivekit-intelligence-provider-governance.test.ts` 写失败测试，断言 migration 059 包含四个 route columns、runtime/lease 表、唯一约束、due index、FORCE RLS 和 `opc_runtime` 权限。
- [x] 运行 `node --import tsx --test test/ivekit-intelligence-provider-governance.test.ts`，确认因 migration 缺失失败。
- [x] 创建 `src/migrations/059_ivekit_provider_governance.sql`。先增加 route columns，并用旧 primary profile 回填；再创建 runtime/lease 表及 RLS。
- [x] 将 migration 加入 `services/ivekit-service/source-policy.json`，位置在 058 后、090 前。
- [x] 更新 `test/ivekit-standalone-migrations.test.ts`，断言 059 顺序和 standalone inclusion。
- [x] 运行 migration focused tests 和 `npm run typecheck`。

### Task 2：Profile 预算和租户 route

- [x] 在 `test/ivekit-intelligence-provider-registry.test.ts` 增加预算默认值、上下界、未知字段和 `reservation_ttl_ms > timeout_ms` 测试。
- [x] 在 `test/ivekit-intelligence-policy.test.ts` 增加 route 排序保留、重复 profile、capability 错配、third-party 未授权、旧 policy 回退测试。
...
- [x] 完成 `verify:ivekit:foundation` 的全部组成门禁：foundation tests 在允许监听端口的全仓库测试中通过，SDK build 和 dry-run pack 通过；受限沙箱内直接 wrapper 仅因 `listen EPERM` 不能原命令复现。
...
- [x] 更新 `docs/ivekit-v5-shared-foundation-design.md` 证据矩阵，只把有当前 source-bound 证据的项目改为 `implemented`。
