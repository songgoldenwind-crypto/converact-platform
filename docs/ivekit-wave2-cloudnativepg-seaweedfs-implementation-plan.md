# iveKit Wave 2 CloudNativePG 与 SeaweedFS 实施计划
...
3. 保留统一 S3 provider 边界，新增 SeaweedFS `4.40` 自建对象存储兼容配置，不让 OPC、LED 或 LiveKit 依赖 SeaweedFS 私有 API。
...
采用“外部平台 profile”，不把 operator 安装清单塞进 OPC 或 iveKit 应用 Chart。
...
- OPC/LED 使用 `S3_*`/`AWS_*` 通用配置；`MINIO_*` 仅保留为有截止日期的兼容输入。
...
- `infra/k8s/templates/opc-deployment.yaml`：通过统一 helper 引用 database URL Secret。
...
- `infra/k8s/templates/opc-deployment.yaml`：优先注入 `S3_*`/`AWS_*`，保留 `MINIO_*` 兼容期输入。
...
- `.env.example`、`infra/env.example`、`services/ivekit-service/env.example`：说明通用 S3 配置优先级与兼容截止。
...
4. OPC 附件、录音对象、备份 runner 与 LiveKit Egress 使用同一协议契约但可以使用不同 bucket/凭据。

### 工作包 C：SeaweedFS 4.40 服务器验收

新增：

- `services/ivekit-service/acceptance/seaweedfs-s3/docker-compose.yml`：隔离 master、volume、filer、S3 gateway，不暴露公网端口。
- `services/ivekit-service/acceptance/seaweedfs-s3/probe.ts`：复用生产 `ObjectStorage` provider 执行 S3 矩阵。
- `services/ivekit-service/acceptance/seaweedfs-s3/accept.sh`：服务器约束、随机凭据、健康门、故障注入、清理和证据输出。
...
服务器源码目录固定为 `/opt/opc-wave123-validation-20260722/source`。Node 测试使用 `/opt/opc-wave123-validation-20260722/cache/toolchain/bin/node`。Helm 通过缓存镜像 `alpine/helm:3.18.4` 执行并显式设置 `--entrypoint /bin/sh`。
