# Converact Fabric V3 多模态智能与翻译部署运维手册

更新日期：2026-07-15。本文对应 Converact Fabric V3 的 OCR、ASR、AI 防绕单质检、人工复核、录制源导入和消息/附件翻译；第 14 节补充 V5 Stage 2 的 Tinode、LiveKit 与文件安全生产部署要求。它只描述可复用 Converact Fabric 边界，不包含 SIP/VoLTE、RTMP/HLS、数字人或 Converact Platform call-center 业务编排。
...
LED 和 Converact Platform 都是租户客户端。它们只能通过 `@converact/sdk`、`/api/ivekit/*` 和租户事件接入，不得直连 PostgreSQL、MinIO/S3 管理 API、Provider 或 worker 表。

## 2. 运行架构

```text
LED/Converact Platform backend or short-lived browser JWT
                 |
                 | @converact/sdk / HTTPS / tenant WebSocket
                 v
       Converact Fabric facade + RBAC + tenant policy
          |             |             |
          v             v             v
...
[
  {
    ...
    "token_env": "CONVERACT_FABRIC_OCR_TOKEN",
    "timeout_ms": 30000,
    "name": "internal-ocr"
  },
  {
    ...
    "endpoint": "/v1/asr",
    "health_endpoint": "/health",
    "token_env": "CONVERACT_FABRIC_ASR_TOKEN",
    "timeout_ms": 60000,
    "name": "internal-asr"
  }
]
...
自建 HTTP 只允许 localhost、私网 IP 或容器/内部主机名；公网主机应使用 HTTPS。Converact Fabric 不负责部署具体 OCR/ASR 模型，Provider 可以由 PaddleOCR、Whisper 类服务或公司内部推理网关实现，只要满足第 4 节协议。
...
[
  {
    ...
    "health_endpoint": "/health",
    "token_env": "CONVERACT_FABRIC_QUALITY_TOKEN",
    "timeout_ms": 30000,
    "name": "vendor-quality"
  },
  {
    ...
    "health_endpoint": "/health",
    "token_env": "CONVERACT_FABRIC_TRANSLATION_TOKEN",
    "timeout_ms": 30000,
...
旧的 `CONVERACT_OCR_*`、`CONVERACT_ASR_*`、`CONVERACT_QUALITY_REVIEW_*`、`CONVERACT_TRANSLATION_*` 仍可生成兼容 profile；新部署应使用 profile JSON，避免能力配置继续分散。
...
Converact Fabric 不信任 Provider 的执行建议。AI finding 的业务动作固定进入人工 `review`，模型不能直接封禁、处罚、删除内容或关闭订单。

OCR/ASR 的文件字节由 Converact Fabric 从私有对象存储读取后转发；请求不包含 S3/MinIO 凭据或 `storage_url`，Provider 不应回源 Converact Fabric 数据库或对象存储。

`media_mode=text` 用于图片/文档；`media_mode=video_frame_sampling` 用于视频和屏幕录制，默认每 2,000 ms 一帧、最多 120 帧。Provider 可返回最多 500 个 observation：`type=qr_code|barcode|text_region`、`value`、可选 `symbology/confidence/frame_timestamp_ms/page/metadata`。`value` 最大 4,096 字节，只能在当前处理内存中用于检测，不得写数据库、日志、事件、错误消息或 API；Converact Fabric 仅保存 SHA-256。
...
| --- | --- | --- | --- | --- | --- | --- |
| Attachment OCR/ASR | `CONVERACT_ATTACHMENT_PROCESSING_WORKER_ENABLED` | `...INTERVAL_MS` | `...BATCH_SIZE` | `...MAX_ATTEMPTS` | `...CLAIM_LEASE_MS` | `...RETRY_DELAYS_MS` |
| Quality | `CONVERACT_QUALITY_REVIEW_WORKER_ENABLED` | `...INTERVAL_MS` | `...BATCH_SIZE` | `...MAX_ATTEMPTS` | `...CLAIM_LEASE_MS` | `...RETRY_DELAYS_MS` |
| Translation | `CONVERACT_TRANSLATION_WORKER_ENABLED` | `...INTERVAL_MS` | `...BATCH_SIZE` | `...MAX_ATTEMPTS` | `...CLAIM_LEASE_MS` | `...RETRY_DELAYS_MS` |

建议初始值：interval `5000`、batch `25`、attempts `3`、lease `120000`、retry `5000,30000`。批量和副本数应按 Provider QPS/并发限制扩容，不能只提高 worker 数量。

`CONVERACT_QUALITY_REVIEW_AUTO_ENQUEUE=0` 是部署安全默认；真正是否自动质检还受租户 `auto_quality_review` 控制。自动翻译还要求 `translation_enabled=true`、`auto_translation=true` 和非空 `translation_target_languages`。
...
1. 复制 `infra/converact/env.example` 到权限为 `0600` 的环境文件。
2. 填写 profile JSON 与独立 token；保持 worker 为 `0`。
3. 渲染：

```bash
docker compose --project-name converact-app \
  --env-file /secure/path/application.env \
  -f infra/converact/docker-compose.yml config --quiet
```

4. 启动 migration 和应用，确认 one-shot 服务退出码为 0。
5. 运行 preflight 和 Provider health。
6. 配置租户 policy，再逐个将所需 worker 改成 `1` 并重建 `converact` 服务。
...
```bash
CONVERACT_FABRIC_CONTROLLED_HOST=127.0.0.1 \
CONVERACT_FABRIC_CONTROLLED_PORT=8790 \
CONVERACT_FABRIC_CONTROLLED_TOKEN=replace_with_test_service_token \
CONVERACT_FABRIC_CONTROL_TOKEN=replace_with_test_control_token \
  npm run converact:controlled-provider
```
...
```bash
npm run converact:realtime-speech-provider-acceptance
```
...
| 环境变量 | 默认值 | 合法范围 | 语义 |
| --- | ---: | ---: | --- |
| `CONVERACT_FABRIC_REALTIME_PROJECTION_QUEUE_MAX_ITEMS` | `4096` | `1..100000` | 等待投影项上限，不含当前正在执行的 1 项 |
| `CONVERACT_FABRIC_REALTIME_PROJECTION_SHUTDOWN_TIMEOUT_MS` | `1000` | `10..30000` | 关闭时等待投影排空的硬上限 |
...
进程恢复回归入口为 `npm run converact:realtime-recovery-acceptance`。它使用唯一 Compose project
...
```bash
npm run verify:converact:stage2-deployment
npm run verify:converact:standalone-context
npm run test:converact:delivery
npm run typecheck
npm run build:converact-sdk
npm_config_cache=.tmp/npm-cache npm run pack:converact-sdk
```
...
- `CONVERACT_MEDIA_CONFIG_REDIS_ADDRESS` 是 Redis URL 或 `host:port`，报告中不得输出凭据。
- TURN TLS/UDP 端口有效；RTC UDP 起止端口有效且 start 不大于 end。
- `CONVERACT_MEDIA_EGRESS_ENABLED=1`，S3/MinIO endpoint 和 bucket 合法，access/secret 已配置。
- `CONVERACT_MEDIA_CONFIG_WEBHOOK_URL` 在生产使用 HTTPS。
- `CONVERACT_LIVEKIT_TIME_SYNC_STATUS=synchronized`，绝对时钟偏移不超过
...
```bash
helm upgrade --install converact services/converact-service/helm/converact \
...
清理 worker 只有在 `CONVERACT_FILE_CLEANUP_WORKER_ENABLED=1` 和
`CONVERACT_FILE_CLEANUP_CONFIRM=1` 同时设置时才执行删除。首次上线应先保持两个值为 `0`，核对
