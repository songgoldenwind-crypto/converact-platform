# Converact Fabric LED 集成与抽离指南

> 版本：2026-07-23。面向 LED 项目架构师、后端、前端、部署和 QA。真实服务器证据见《Converact Fabric服务器部署验收报告-2026-07-11》、V2 M6.3-M6.6 记录和 `docs/evidence/`；物理 RustDesk 客户端、真实 Voice/PSTN/RTP、真实 LiveKit 实时音频旁路和未配置的外部 OCR/ASR/AI provider 仍按人工/外部依赖项处理。

## 1. 交付目标

Converact Fabric 将 Converact Platform 已有通信能力分成四个可复用域：
...
LED 不应复制 Converact Platform 的 call-center 业务代码。稳定边界是 `/api/ivekit/context/*`、`/api/ivekit/media/*`、`/api/ivekit/chat/*`、`/api/ivekit/rustdesk/*`、`/api/ivekit/voice/*`、`/api/ivekit/ivr/*`、`/api/ivekit/contact-center/*` 和标准租户事件。SDK 只封装这些 HTTP 契约，后续把能力搬到独立服务时，LED 只需要更换 `baseUrl`。

## 2. 当前完成度

| 范围 | 代码状态 | 真实环境状态 |
| --- | --- | --- |
| Media Core | 房间、join、参与人、录制生命周期、对象读/导出/retention、preflight，以及 LiveKit/RustPBX 到统一实时 ASR/翻译 Provider 路由的有界 PCM 旁路已完成 | 既有 LiveKit/Egress/MinIO、TURN、双浏览器音视频、屏幕共享、DataChannel 和录制受控验收保留；本轮真实 LiveKit `AudioStream -> Converact Fabric -> 外部 Provider` 尚未运行 |
...
2026-07-11 的最终部署把 PostgreSQL 角色初始化、advisory-locked migration 和 Tinode 服务账号 bootstrap 拆成一次性任务。长驻 Converact Fabric 仅持有 `opc_runtime`，Tinode 仅持有 `tinode_app`；LED 不得获取 `opc_admin`、PostgreSQL 连接密码、LiveKit API secret、MinIO root 或桶级 service secret。MinIO 根账号只用于初始化，Converact Fabric/Egress 只使用限定 `recordings` 桶的业务账号。

## 3. 推荐部署拓扑

### 3.1 推荐：独立 Converact Fabric 服务

`infra/converact/docker-compose.yml` 运行 `npm run start:converact`，启动可复用 HTTP、WebSocket、Media/Chat/RustDesk/Voice/IVR/Contact Center 模块和已启用的 worker，不启动 Converact Platform 历史 call-center 或 SQLite runtime。Converact Platform 和 LED 都通过公网 base URL 调用它。

无论嵌入还是抽离，LED 只能通过 Converact Fabric facade/SDK 和标准事件访问能力，不直连 PostgreSQL、Tinode 数据库或 MinIO 管理 API。浏览器只接收短期 LiveKit/Tinode 用户凭据，不接收服务端 provider secret。
...
LED backend/frontend
        ...
        | HTTPS /api/ivekit/*
        v
Converact Fabric process + PostgreSQL/RLS
   |          |          |
LiveKit     Tinode    RustDesk control plane
```

### 3.2 兼容：嵌入 Converact Platform

现有 Converact Platform 进程仍导出相同 `/api/ivekit/*` 路由和兼容 SDK symbol，可作为迁移期入口。LED 只要保持 `baseUrl` 可配置，就能在嵌入式和独立部署间切换，不需要修改业务 payload。

### 3.3 第三阶段：共享通信平台

独立服务按 tenant 提供 Media/Chat/Remote，Converact Platform 和 LED 使用各自 API key/JWT。LiveKit、Tinode、RustDesk 可共享集群，但业务数据必须按 PostgreSQL RLS 和 provider 命名空间隔离。
...
npm run build:converact-sdk
npm run pack:converact-sdk
```

`pack:converact-sdk` 是 dry-run，不产生 tarball，用于确认发布物没有服务端源码、测试和凭据。LED 可从私有 registry 安装 `npm install @converact/sdk`，或在联调阶段安装本地 `sdk/converact` 目录。
...
```ts
import { createConveractFabricClient } from '@converact/sdk';
...
const browserSdk = createConveractFabricClient({
  baseUrl: 'https://converact.example.com',
...
### 4.3 Converact Platform 迁移期兼容导出

现有 Converact Platform 内部调用可暂时保持原路径：
...
- Media/Chat 非 2xx 响应抛出 `ConveractFabricHttpSdkError`，包含 `status`、`method`、`path`、`payload`；网络错误和 `timeoutMs` 超时的 `status=0`。
- RustDesk 非 2xx 响应抛出 `ConveractFabricRustDeskHttpError`，字段结构相同。
...
统一客户端的 `converact.rustdesk` 同时包含底层 HTTP 方法和 LED 高层流程：
...
const requested = await customerConveract.rustdesk.requestAuthorizationCode({
...
await converact.rustdesk.verifyAuthorizationCode(requested.authorization.id, {
  code: requested.code!
});
const remote = await converact.rustdesk.startSession({
  businessRef: orderRef,
  deviceId: device.id,
...
await converact.rustdesk.recordControlAction(remote.gatewaySession.external_id, {
  operationId: 'op-1',
  actorIdentity: 'agent_1001',
  action: 'mouse_click',
  permission: 'control_mouse_keyboard'
});
await converact.rustdesk.endGatewaySession(remote.gatewaySession.external_id, {
...
```bash
CONVERACT_FABRIC_LED_BASE_URL=https://converact.example.com \
CONVERACT_FABRIC_LED_API_KEY=... \
CONVERACT_FABRIC_LED_TENANT_ID=tenant_led \
CONVERACT_FABRIC_LED_USER_ID=agent_1001 \
CONVERACT_FABRIC_LED_BUSINESS_REF_ID=SO-1001 \
npm run converact:led-example
```

示例创建 collaboration session、参与人、LiveKit room、join plan 和幂等 IM 消息。只有额外提供 `CONVERACT_FABRIC_LED_REMOTE_SESSION_ID` 以及 RustDesk device/runtime ID 时才启动已有授权范围内的 RustDesk session。

### 4.7 Durable event replay

统一客户端的 `converact.events` 提供：
...
`npm run converact:delivery-bundle` 生成的目录是 LED/其它项目的可移交边界，不要求接收方持有 Converact Platform 根仓库。关键内容如下：
...
| `deploy/application/` | standalone Compose/Voice overlay；交付版要求 `CONVERACT_FABRIC_SERVICE_IMAGE` |
| `deploy/kubernetes/converact/` | digest-pinned standalone Helm Chart、迁移 hook 和可选 RustPBX |
...
sequenceDiagram
  ...
  participant IVE as Converact Fabric
...
业务消息必须先走 Converact Fabric。Tinode 客户端 mode 为 `JRP`，没有 `W`；`direct_client_publish=false`。这样文本、附件、OCR/ASR、AI 质检和审计不会被绕开。
...
4. 发送者在 `CONVERACT_CHAT_MESSAGE_MUTATION_WINDOW_MS` 内可编辑或软删除文本消息。
5. Converact Fabric 在本地 edit/delete 权威事务中同时创建 Tinode mutation outbox；edit 使用 replacement publish，delete 使用目标 sequence delete，SDK 响应暴露 `provider_mutation.status`。
6. LED UI 仍以 Converact Fabric snapshot 和 `collaboration.message.edited/deleted` 为权威；`pending|processing|retry_wait` 仅表示执行面尚未收敛。
7. edit 已发出但 ACK 丢失，或 worker 接管一个过期 `processing` edit lease 时，都直接成为 `provider_outcome_uncertain` dead letter，不自动重发；过期 delete 仍可自动重试。owner/admin 先核对 Tinode，再调用 `listTinodeMutationDeadLetters()` / `replayTinodeMutationDeadLetter()` 人工对账与重放。若之后到达可验证的原生 echo，Converact Fabric 会在纠正 outbox 的同一事务持久化 `provider_mutation_updated(status=delivered,reconciled_from_status=dead_letter)`；LED 应通过实时事件、Webhook 或 replay 覆盖本地旧死信投影，实时广播失败不会丢失该纠正。
...
RustDesk 前置条件是 collaboration remote session 已创建且授权 scope 已 grant。LED 使用 `createConveractFabricRustDeskLedSdk`：
...
过期前完成注册，过期或 capability 关闭时重新向 Converact Fabric 申请，不得在 LED 数据库或日志保存 credential。
...
交付包的 `database/migrations/` 是通信域 overlay，不是通用空库初始化脚本；其中录制表、`tenants` 和 RLS helper 依赖 Converact Fabric 应用镜像的 foundation migration。生产环境应由 Compose 中同一版本镜像的 `postgres-migrate` job 按数字顺序执行，不能挑选 SQL 直接套到无基础表的 LED 数据库。

## 7. 配置与依赖

### Converact Fabric HTTP

- `CONVERACT_FABRIC_ALLOWED_ORIGINS`：允许浏览器直连的逗号分隔 HTTPS origin，不含路径；未列出的跨域请求返回 403。
- `CONVERACT_FABRIC_HTTP_BODY_MAX_BYTES`：普通 JSON 和 webhook body 上限，默认 1 MiB；附件另用 `CONVERACT_COLLABORATION_ATTACHMENT_MAX_BYTES`。
...
- LiveKit Ingress 管理面由 `CONVERACT_LIVEKIT_INGRESS_ENABLED=1` 启用；API 超时使用 `CONVERACT_LIVEKIT_INGRESS_REQUEST_TIMEOUT_MS`
- URL pull 必须配置 `CONVERACT_LIVEKIT_INGRESS_PULL_HOST_ALLOWLIST`；默认仅 HTTPS，HTTP 兼容开关 `CONVERACT_LIVEKIT_INGRESS_ALLOW_HTTP_URL` 不应在公网生产启用
- `CONVERACT_MEDIA_CONFIG_RTC_TCP_PORT`，默认 `7881`
- `CONVERACT_MEDIA_CONFIG_RTC_UDP_PORT`，默认 `7882-7892`，生产防火墙必须开放同一 UDP 范围
- `CONVERACT_MEDIA_CONFIG_USE_EXTERNAL_IP=true` 用于生产公网 ICE 候选；本地固定配置保持 `false`
- MinIO/S3 endpoint、bucket、key、secret
- 客户邀请和 Web Assist join 签名 secret
- SIP/VoLTE 默认关闭；LED 需要该入口时设置 `CONVERACT_SIP_VOLTE_ENABLED=1`，并完整配置 `LIVEKIT_SIP_BRIDGE_TARGET`、`RUSTPBX_LIVEKIT_TRUNK`、`RUSTPBX_RWI_URL`、`RUSTPBX_RWI_TOKEN`
...
- 实时字幕/翻译通过 `CONVERACT_FABRIC_REALTIME_AUDIO_TAP_ENABLED=1` 显式启用；HMAC 根密钥使用 `CONVERACT_FABRIC_REALTIME_AUDIO_TAP_HMAC_SECRET_B64`，必须是 canonical base64 编码的 32 至 128 字节随机值
- `CONVERACT_FABRIC_REALTIME_AUDIO_TAP_TOKEN_TTL_SECONDS` 默认 60 秒；生产不应为了掩盖网络错误而放宽到长时 token
- LiveKit gateway 使用 `CONVERACT_FABRIC_LIVEKIT_AUDIO_TAP_LISTEN_PORT`（默认 3010）和 `CONVERACT_FABRIC_LIVEKIT_AUDIO_TAP_PATH`；Kubernetes 的 `GATEWAY_URL`、`INSTANCE_ID` 和 Pod DNS 由 Helm 自动生成，不能手工改成负载均衡 Service
- Kubernetes 将两类 gateway 拆开：API Pod 只启用 LiveKit WebSocket gateway；每个 RustPBX Pod 内同置独立 `realtime-audio-tap-gateway` sidecar，并通过 memory `emptyDir` 共享 `/run/converact/realtime-audio-tap.sock`。RustPBX 解码 PCM 不先跨节点或经过 Service
- Compose 是单实例边界，Converact Fabric 进程可同时启用两类 gateway；RustPBX 与 Converact Fabric 必须挂载同一 `realtime_audio_tap` 私有卷。`CONVERACT_FABRIC_RUSTPBX_AUDIO_TAP_GATEWAY_ENABLED` 与 `CONVERACT_FABRIC_LIVEKIT_AUDIO_TAP_GATEWAY_ENABLED` 用于显式拆分进程职责，二者不能同时关闭
...
- `services/converact-service/helm/converact/profiles/ai.values.yaml` 才会启用旁路；最小 core profile 默认关闭。Secret key 名默认为 `realtime-audio-tap-hmac-secret-b64`
...
- 录制或远控录屏由 LED 传 business/session 标识后调用 `importSource`，Converact Fabric 负责生成受控 message/attachment、ASR job、finding 和审计链。
- Provider 部署、密钥、worker、重试和真实验收见 `docs/converact-fabric-v3-intelligence-operations.md`；未完成真实厂商测试时状态必须保持 `not_run`。
...
3. Converact Fabric 会在 provider 前写 durable moderation command；运维恢复任务使用 system API key 调 `/api/ivekit/media/moderation/recover`，按 tenant 最终化崩溃窗口中的 pending command。
...
6. SDK 抛出 `ConveractFabricHttpSdkError`，包含 `status/method/path/payload`；网络/超时 status 为 0。
...
- `converact.media.call.created/updated/ended`
- `converact.media.participant.updated/moderated`
...
Webhook 使用 `x-ivekit-timestamp`、`x-ivekit-signature: v1=<hex>`、`x-ivekit-delivery`、`x-ivekit-event` 和 `x-ivekit-event-id`。签名输入是 `${timestamp}.${rawBody}`。LED Node 后端调用 SDK `verifyConveractFabricWebhook()`，并把 `ConveractFabricWebhookReplayStore.claim()` 实现为 PostgreSQL/Redis 的原子 durable inbox：claim 同时持久化已验证 envelope 和 body SHA-256，重复 delivery 返回 false。签名时间窗默认 5 分钟，防重放记录默认保留 7 天；两者不能混为一谈，也不能使用进程内 Set。

参考接收器位于 `sdk/converact/examples/webhook-receiver.ts`。它对重复投递返回 200，对签名密钥服务或 inbox 存储故障返回 503 触发安全重试，对已经取得密钥但验签失败的请求统一返回 401；实际 LED 业务 Worker 从自己的 durable inbox 异步消费并用 `tenant_id + business_ref` 绑定订单/工单，不读取 Converact Fabric 内部表。

生产启用 `CONVERACT_FABRIC_EVENT_WEBHOOK_WORKER_ENABLED=1` 时必须同时启用 Notification Worker 并配置独立的 Notification encryption/HMAC key。最老事件延迟和固定结果计数由 `opc_ivekit_event_webhook_oldest_event_age_seconds`、`opc_ivekit_event_webhook_operations_total` 暴露，告警和处置见 `docs/converact-monitoring-runbook.md`。
...
```bash
CONVERACT_FABRIC_DELIVERY_DIR=/absolute/output/converact-led-delivery \
CONVERACT_FABRIC_DELIVERY_CONTROLLED_ACCEPTANCE_DIR=/absolute/input/controlled-acceptance \
CONVERACT_FABRIC_DELIVERY_IMAGE_REFERENCE=converact-service:<release-commit> \
CONVERACT_FABRIC_DELIVERY_IMAGE_DIGEST=sha256:<64-hex> \
  npm run converact:delivery-bundle
```
...
- `deploy/application/` 和 `deploy/livekit/`：应用面与媒体面分离 Compose；应用面必须设置 digest 固定的 `CONVERACT_FABRIC_SERVICE_IMAGE`，不依赖 Converact Platform 源码目录。
- `deploy/kubernetes/converact/`：部署 standalone Converact Fabric、可选 RustPBX，以及可选 bundled Tinode compact 单节点或 cluster 三节点；PostgreSQL、Redis、LiveKit 仍通过外部服务接入，Tinode 也可选择外部集群，所有凭据来自接收方已有 Secret。
...
- `acceptance/voice-real-template.json`、`voice-real-runbook.md` 和 `tools/converact-voice-acceptance.ts`：绑定本次 source commit 的 45 项真实 Voice 验收合同；模板初始必须是 `incomplete`。
- `acceptance/v6-real-template.json` 和 `tools/converact-v6-real-acceptance.ts`：固定八组真实环境合同；交付生成时八组必须全部为 `not_run`，后续结果逐项绑定环境、制品 digest、operator/QA、observation bytes 和 SHA-256。
...
生成器不会复制 Converact Platform call-center、legacy IVR 或产品前端源码，但会保留 standalone `agent-runtime/converact/ivr`；它不接受符号链接或清单外文件，并扫描常见私钥、云密钥、GitHub token、OpenAI key 和 JWT Authorization。受控证据不能提升真实 LiveKit/Tinode/RustDesk/RustPBX 客户端或真实 OCR/ASR/AI/翻译厂商状态；`ready_for_handoff` 仅表示工程交付包完整，不代表 release 已有 digest 或所有生产验收通过。
...
6. production base 的 `postgres-bootstrap` 会在健康 PostgreSQL 上幂等确认 `keycloak` 数据库；自建 Tinode overlay 把集合扩展为 `keycloak,tinode`。脚本拒绝任意数据库标识和非 `converact` owner，不会删除或重建已有数据库。
7. `minio-init` 会有限重试 MinIO endpoint，幂等创建录制 bucket，关闭匿名访问，回读确认 private 后再执行 `stat`。Egress 和 Converact Platform 存储入口只有在该 one-shot 成功后才允许启动；RustPBX 与 LiveKit Server 不依赖 `minio-init`，确保存储不可用时既有 RTP/WebRTC 媒体仍可继续。
8. PgBouncer 固定为 `edoburu/pgbouncer:v1.25.2-p0@sha256:7d7a27d...`，使用 `scram-sha-256` 与 transaction pooling；必须通过 6432 端口的认证 `psql SELECT 1` 后，Converact Platform 才启动。只接受连接但凭证/数据库不可用不会标记健康。固定 digest 已在隔离 Docker 网络中连接临时 PostgreSQL 并返回认证查询结果；这不是生产容量或故障切换证据。Keycloak 和 Tinode 同样等待数据库 bootstrap 成功，而不是只等待 PostgreSQL container 进程存在。
9. Chatwoot 由 `omnichannel` profile 显式启用，不属于默认 Converact Fabric 生产链路，也不计入当前 readiness。启用前仍需单独完成固定版本、pgvector、`db:chatwoot_prepare`、Rails/Sidekiq、升级与回滚设计。
10. LiveKit 地址已拆成服务端 `LIVEKIT_URL` 和浏览器 `LIVEKIT_PUBLIC_URL`。生产 Join Plan 只返回显式 `wss://` 公网地址；缺失或误配明文 `ws://` 时直接拒绝浏览器 join。
11. `infra/livekit/` 提供可从 Converact Platform 独立运行的 Linux VM Media Core 包：LiveKit、Redis、Egress 和 Caddy L4 使用 host networking；Caddy 按两个 SNI 域名分流 WSS 与 TURN/TLS；LiveKit 使用内置 TURN，不增加 coturn。
...
13. 生产 Compose 默认按 `CONVERACT_LIVEKIT_DEPLOYMENT_MODE=external` 消费外置 Media Core，内置 LiveKit/SIP/Egress 只在显式 `media-bundled` profile 下启用，且只用于联调。Kubernetes 同样默认 `livekit.enabled=false`，生产媒体节点应使用 LiveKit 官方 Helm chart 独立部署。
14. 媒体镜像已固定为 Converact Fabric LiveKit Server `v1.13.4-ivekit.1`、Egress `v1.13.0`、SIP `v1.7.0`、Caddy L4 `v2.11.3`、Redis `7.4.9`；MinIO Server 固定为 `RELEASE.2025-09-07T16-13-09Z@sha256:14cea493...`，MinIO Client 固定为 `RELEASE.2025-08-13T08-35-41Z@sha256:a7fe349e...`，PgBouncer 固定为 `v1.25.2-p0@sha256:7d7a27d...`。独立存储覆盖层要求同时提供 tag 与完整 manifest digest；升级必须成组回归，不使用 `latest`。当前 LiveKit Server 只有服务器构建候选，GHCR digest/签名与真实媒体仍为 `not_run`。
...
17. root Kubernetes Chart 的 Converact Platform、AI Agent、Frontend 镜像全部要求完整 SHA-256 digest，缺 digest 或继续使用 tag 时 Helm fail-closed。RustDesk 部署命令必须通过 `CONVERACT_RUSTDESK_DEPLOYMENT_HELM_VALUES_FILE` 指向生产 values 文件；文件内需给出实际应用镜像 digest、Secret 引用和环境参数，不能直接使用仓库默认空值。
...
Chatwoot 是独立可选项，不属于 Converact Fabric readiness：
...
1. 先用 `CONVERACT_LIVEKIT_ACCEPTANCE_BUNDLE_DIR=<evidence-dir> npm run livekit:acceptance-bundle` 固定本轮路径，再按 manifest 运行 `livekit:deployment-preflight`、`livekit:server-evidence`、`smoke:media:readiness`、真实客户端验收和 `livekit:evidence-pack`；最终必须为 `ready_for_customer_review`。
2. `npm run tinode:deployment-preflight`、`npm run smoke:chat:tinode`、双浏览器 SDK join/data/info/presence/read note。
   - 本地先运行 `npm run verify:converact:im-client` 和 `npm run test:e2e:converact-im`；受控 E2E 覆盖两身份、合法 token/topic 订阅、Tinode-only `{data}` 触发 HTTP 镜像收敛、客户端零 publish、附件进度、receipt、typing/presence、edit/delete、reply/forward、reaction/pin、finding 人审、断网重连、会话关闭和桌面/手机布局。
   - 用 `CONVERACT_FABRIC_IM_ACCEPTANCE_TEMPLATE_FILE=<path> npm run converact:im-client-acceptance` 生成故意不完整的真实环境模板。两台真实浏览器和真实 Tinode 完成后，设置 `CONVERACT_FABRIC_IM_ACCEPTANCE_REPORT_FILE` 再运行 validator；没有报告时结果必须为 `not_run`，受控截图不得改写为真实证据。
...
6. RustDesk server evidence/readiness/client acceptance/audit coverage/evidence pack，包含真实物理断开观察。
   - 本地先运行 `npm run test:e2e:converact-rustdesk`；受控 E2E 覆盖设备、授权 scope、gateway、宿主协议拉起、控制权、转交、操作审计、结束/撤权、断开进度、旧链接失效、租户/参与人隔离、幂等重试、token 零持久化和响应式布局。
...
CONVERACT_LIVEKIT_STORAGE_ISOLATION_COMPOSE_PROJECT=<isolated-compose-project> \
CONVERACT_LIVEKIT_STORAGE_ISOLATION_COMPOSE_FILE=docker-compose.callcenter.yml \
CONVERACT_LIVEKIT_STORAGE_ISOLATION_OUTPUT_FILE=/secure/evidence/storage-isolation.json \
...
```
   ...
   LED 交付包不依赖 Converact Platform 源码树：进入 `acceptance/livekit-storage-isolation/`，先运行
   `npm ci --ignore-scripts` 和 `npm run install:chromium`，再按目录内 README 设置 LiveKit 凭据、专用
   Compose project、`CONVERACT_LIVEKIT_STORAGE_ISOLATION_COMPOSE_FILES` 有序 JSON 数组、可选 env file 和
...
node --import tsx scripts/converact-v6-real-acceptance.ts \
  ...
  --manifest /secure/evidence/converact-v6-real/report.json
```

八组固定为 Provider、Tinode、LiveKit/TURN/Egress、RustDesk 双 Windows、Voice/PSTN、商业通知、生产对象存储和 Kubernetes。`passed/failed` 必须绑定不可变 artifact digest、环境、run ID、不同 operator/QA 和逐项 JSON observation；`not_run` 必须无 evidence。交付包已包含模板、校验器及其 SHA-256 绑定，完整合同、字段、命令和当前原因见 `docs/converact-fabric-v6-real-environment-acceptance.md`。
...
1. Converact Fabric API 第一版保持 additive evolution；删除/改名必须先发布 deprecation。
2. `capabilities` 是运行时能力协商入口，LED 不应硬猜 provider 是否配置。
3. LED 负责业务对象、页面流程、审核工作台和业务处置；Converact Fabric 负责通信 session、媒体、IM、远协、审计和 provider adapter。
4. 当前官方浏览器依赖为 `tinode-sdk@0.25.1`；升级前必须重跑 adapter、真实 server 和权限门禁。
5. 详细数据/API/验收证据继续以 `converact-fabric-video-im-capability-design.md`、`livekit-im-full-capability-plan.md` 和 `审核文档.md` 为准。
