# Converact Fabric V5 Stage 4 语音、通知与安全运维实施计划

> 状态：代码、部署配置和本地自动化门禁完成；真实通信与目标集群验收保持 `not_run`  
> 范围：Converact Platform、LED 及其他业务可共同复用的通信协作底座
> 不包含：Converact Platform/LED 业务逻辑、移动推送、真实运营商/PSTN 或物理音频设备采购与验收
...
新增独立 `converact/notifications` 深模块，核心实体如下：
...
- [x] capability policy 与统一授权 helper 已接入 Converact Fabric 新路由；
...
- foundation 分量门禁：standalone HTTP `16/16`、其余 core `96/96`，SDK TypeScript build 和 dry-run pack 通过；聚合 `npm run verify:converact:foundation` 在当前 managed sandbox 内经 npm 子进程监听 `127.0.0.1` 时被 `EPERM` 拒绝，因此采用同一测试清单的直接分量命令取证，不把 wrapper 失败写成 passed；
