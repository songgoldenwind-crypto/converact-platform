...
> Contract freeze: 2026-07-22. This matrix is the Converact Fabric real-terminal V1
...
| Platform | Converact Fabric V1 architecture | Package expectation | Platform-specific limitation |
| --- | --- | --- | --- |
| Windows | `x86_64` | `rustdesk-1.4.9-x86_64.exe` (EXE) | Windows `x86` is an upstream legacy build and is not in the Converact Fabric V1 support window. Windows ARM64 has no pinned V1 artifact. Service/UAC and login-screen behavior require real-machine acceptance. |
...
| Linux | `x86_64`, `aarch64` | `rustdesk-1.4.9-x86_64.deb`, `rustdesk-1.4.9-aarch64.deb` (DEB) | Wayland support is experimental and a Wayland login screen is not supported; login-screen access after logout/reboot requires X11. `armv7` is outside the Converact Fabric V1 support window. See the [official Linux guide](https://rustdesk.com/docs/en/client/linux/). |
...
| `granted` | Requested scopes were bounded by active consent and Converact Fabric policy. | The native client exposed or successfully used the capability. |
...
file bytes, clipboard contents, and client-side recording bytes. Converact Fabric receives
only tenant/business binding, state, scopes, ownership, disconnect state, audit
metadata, checksums, and evidence references.

Terminal profiles and operation evidence must not include Converact Fabric API keys,
