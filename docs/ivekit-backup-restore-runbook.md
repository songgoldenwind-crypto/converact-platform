...
本文定义 OPC、LED 及其他业务共同使用的 iveKit 通信底座的备份、校验、恢复和演练流程。命令面向平台运维人员，不向租户或普通业务服务开放。

一次 iveKit 备份集可以包含：

| 数据 | 默认状态 | 说明 |
| --- | --- | --- |
| iveKit PostgreSQL 主库 | 必选 | IM 映射、媒体、文件、远控、语音控制面、通知、审计、保留策略等 |
| Tinode PostgreSQL | 可选 | 设置 `OPC_IVEKIT_TINODE_ADMIN_DATABASE_URL` 后纳入同一清单 |
| RustPBX PostgreSQL | 可选 | 设置 `OPC_IVEKIT_RUSTPBX_ADMIN_DATABASE_URL` 后纳入同一清单 |
...
`.ivekit-backup` 在任务开始时是 `partial`，只有所有数据写入且完整性复验通过后才原子更新为 `complete`。失败目录保留用于诊断，但不能用于恢复。
...
```bash
export OPC_IVEKIT_ADMIN_DATABASE_URL='postgresql://admin:***@db/ivekit?sslmode=require'
```

或标准 `PGHOST`、`PGPORT`、`PGDATABASE`、`PGUSER`、`PGPASSWORD`、`PGSSLMODE`。可选依赖库只接受独立 URL：

```bash
export OPC_IVEKIT_TINODE_ADMIN_DATABASE_URL='postgresql://admin:***@db/tinode?sslmode=require'
export OPC_IVEKIT_RUSTPBX_ADMIN_DATABASE_URL='postgresql://admin:***@db/rustpbx?sslmode=require'
```
...
export MINIO_BUCKET='ivekit-production'
export MINIO_ACCESS_KEY='***'
export MINIO_SECRET_KEY='***'
```

也支持 `S3_BUCKET`、`S3_REGION`、`S3_ENDPOINT`、`AWS_ACCESS_KEY_ID` 和 `AWS_SECRET_ACCESS_KEY`。`OPC_IVEKIT_BACKUP_OBJECT_PREFIX` 可限制到 iveKit 专属前缀；未设置时备份整个配置桶。

没有 S3/MinIO 桶时，命令读取 `OPC_UPLOAD_DIR`，默认是当前目录下的 `data/uploads`。生产环境必须明确挂载持久目录，源目录不存在会让备份失败，不会静默当作空目录。
...
```bash
OPC_IVEKIT_BACKUP_ROOT=/secure/ivekit-backups npm run ivekit:backup
```

指定确切目录和可追踪 ID：

```bash
npm run ivekit:backup -- \
  --output /secure/ivekit-backups/ivekit-20260715-a \
  --backup-id ivekit-20260715-a
```
...
Compose 的 `backup` 服务默认不随应用启动，只能通过 `operations` profile 显式执行。其输出目录由 `OPC_IVEKIT_BACKUP_HOST_DIR` 挂载到容器 `/backups`。
...
backup:
  ...
  schedule: "0 2 * * *"
  persistence:
    existingClaim: ivekit-backups
```

启用后 Chart 创建 `concurrencyPolicy: Forbid` 的 CronJob。同一时刻不会并行创建两份备份，任务有截止时间、失败重试和历史保留上限。S3 凭据从 `secrets.runtimeEnvironmentSecret` 注入；主库管理员 URL 从 iveKit Secret 的 `admin-database-url` 键注入。
...
```bash
npm run ivekit:restore -- --backup /secure/ivekit-backups/<backup-id>
```
...
docker compose --profile operations run --rm \
  ...
  dist/ivekit-restore.js --backup /backups/<backup-id>
```
...
2. 恢复组织密钥管理系统中的 Secret，但先不要启动 iveKit、Tinode、RustPBX 或任何 worker。
...
```bash
export IVEKIT_RESTORE_CONFIRM='RESTORE:<backup-id>'
export IVEKIT_RESTORE_TARGET_EMPTY=1
npm run ivekit:restore -- \
  --backup /secure/ivekit-backups/<backup-id> \
  --execute
```

`IVEKIT_RESTORE_TARGET_EMPTY=1` 只是操作员声明，不能跳过程序对所有目标库的实际空库查询。对象目标中只要有同名键，恢复就会停止，不覆盖现有数据。
