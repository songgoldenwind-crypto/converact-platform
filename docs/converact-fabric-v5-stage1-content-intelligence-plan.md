# Converact Fabric V5 阶段一内容智能增强实施计划
...
**技术栈：** TypeScript、Node.js、PostgreSQL FORCE RLS、现有 Provider route/governance、tenant events、`@converact/sdk`、Node test runner。
...
- `test/converact-content-intelligence.test.ts`：detector、observation、聚合、版本和 secret-safety 合同。
...
- `scripts/converact-controlled-provider.ts`：受控二维码、条码和视频帧 fixtures。
- `scripts/converact-delivery-bundle.ts`、`services/converact-service/source-policy.json`：交付 migration 060。
- `docs/openapi.yaml`、`docs/converact-openapi.md`、`docs/converact-fabric-v3-intelligence-operations.md`：合同、边界和运维说明。
- `sdk/converact/src/chat-types.ts`：finding/observation/版本字段。
...
- [x] 在 `test/converact-content-intelligence.test.ts` 写失败测试，覆盖全角数字、空格/点/短横线、中文数字口述、`wx/微 信/V信/加v`、WhatsApp、Telegram、QQ 和直接支付意图。
...
- [x] 运行 `node --import tsx --test test/converact-content-intelligence.test.ts test/collaboration-policy-finding.test.ts`。
...
- [x] 更新 `docs/converact-fabric-v5-shared-foundation-design.md`：代码合同有证据的标为 implemented；真实厂商、真实视频抽帧效果和业务准确率继续标为 `not_run`。
...
- Converact Fabric 交付契约 23 项、standalone source graph/context 10 项通过；独立 context 编译 218 个源文件。
