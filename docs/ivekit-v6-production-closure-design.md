...
本轮只关闭 iveKit/LED 共享通信底座的已知技术尾项，不实现 LED/OPC 的订单、支付、人员、价格、评价、投诉或业务后台，也不开发移动端。iveKit 继续通过 `tenant_id + business_ref` 提供产品无关能力。
...
iveKit 仍是会话、消息状态、授权、审计、证据与业务引用的权威控制面。Tinode、RustDesk、LiveKit、RustPBX 和内容 Provider 是可替换执行面：执行面故障不能回滚已经成立的 iveKit 权威状态，而是进入 durable retry、dead letter 或明确的 degraded 状态。
...
iveKit 的 `collaboration_messages` 与 `collaboration_message_mutations` 继续是权威。针对已绑定 Tinode 且已有 provider sequence 的消息：

- edit 使用 Tinode publish replacement 语义，引用原消息 sequence；
- delete 使用 Tinode message delete 语义，只删除目标 sequence；
- provider payload 携带稳定的 iveKit mutation ID，用于审计和入站回环识别；
- mutation API 成功表示 iveKit 权威状态已提交，不表示 Tinode 已完成；返回值和事件暴露 provider sync 状态。
...
- 外部 Tinode 客户端发起的合法 mutation 继续投影为新的 iveKit mutation；
- 旧版本、重复包和乱序包不得覆盖更新版本；
