# Converact Fabric V5 Stage 5 全链路集成与交付计划

> 状态：代码、部署配置、自动化验收和交付文档完成；真实环境验收保持 `not_run`  
> 范围：让 Converact Platform、LED 和后续产品只通过稳定 API、SDK、事件、Webhook 与部署包消费 Converact Fabric
> 不包含：LED/Converact Platform 订单、工程师、支付、质检处置等业务逻辑，也不补做已标记为 `not_run` 的真实外部环境验收

## 1. 完成目标

Stage 5 结束时，接入研发应能完成以下工作而无需读取 Converact Fabric 数据库、Provider 配置或 Converact Platform call-center 代码：

1. 使用 `@converact/sdk` 调用全部 V5 公共能力；
...
5. 从 source-bound delivery bundle 部署 standalone Converact Fabric，执行 migration、升级、备份恢复和验收；
...
- 根 `npm run typecheck` 与 `@converact/sdk` build 通过；SDK dry-run pack 为 83 个文件，包含编译后的 Webhook verifier 和 LED receiver 示例；
- standalone context 验证通过：309 个允许源码文件、8 个 runtime package、7 个编译入口，未带入 Converact Platform 产品域；
- production、infra/converact、standalone service 三套 Compose `config --quiet` 通过；
- 本机隔离 PostgreSQL harness `6/6` 通过，覆盖 fresh migration、Converact Platform upgrade、RLS、Tinode inbox/projector、IVR 和受控 RustPBX；
