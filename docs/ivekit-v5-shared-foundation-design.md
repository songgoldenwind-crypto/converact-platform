...
iveKit V5 是 OPC、LED 及后续产品共同复用的通信协作底座。它提供 IM、实时音视频、远程协助、语音呼叫、内容智能、文件安全、通知、审计和运维能力，但不实现任何产品业务流程。

明确排除：

1. LED/OPC 业务域，包括订单、工程师、定价、支付、评价、退款、仲裁和业务后台。
...
```text
OPC backend          LED backend          future products
      \                  |                    /
       \        @opc/ivekit-sdk + HTTPS      /
        +--------------- iveKit API --------+
                           |
          +----------------+----------------+
          |                |                |
...
- 产品业务对象只使用不透明的 `business_ref`，iveKit 不解释其类型和值。
- OPC、LED 通过 SDK、API、租户事件和签名 Webhook 接入，不直连 Provider、Tinode、LiveKit、RustDesk、RustPBX 或 iveKit 数据库。
- 业务处罚、订单状态和支付动作不在 iveKit 内执行。AI 只产生 finding，最终动作由业务服务或人工审核决定。
...
2. 一次性授权、attended/unattended policy、控制权租约和二次确认统一由 iveKit 控制面裁决。
...
1. `@opc/ivekit-sdk`、OpenAPI、事件 schema 和 Webhook schema 覆盖全部 V5 能力。
2. Webhook 使用时间戳、event id 和 HMAC 签名；接收方可防重放。
3. OPC/LED 示例只使用 `tenant_id`、`business_ref` 和 SDK，不引用内部表或 Provider。
