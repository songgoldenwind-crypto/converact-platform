...
| `npm run rustdesk:client-config-pack` | RustDesk 客户端安装配置交接包；读取 iveKit base URL/API key/tenant，通过 `/api/ivekit/rustdesk/client-config` 拉取 ID server、relay server、API server、public key 和 fingerprint；可选读取 `OPC_RUSTDESK_CLIENT_CONFIG_EXTERNAL_ID` 对应 launch plan，把 launch URL、protocol URL 和目标 RustDesk ID 写入 Markdown。它用于给现场/LED 研发配置 RustDesk 客户端，不证明客户端已经连接或远控成功 |
