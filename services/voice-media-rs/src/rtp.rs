...
impl DtmfTracker {
    fn observe(
        ...
        if !same_event {
            ...
            return Ok(Some(dtmf_event(ingress_leg, packet, phase)));
        }
        ...
        if packet.end {
            ...
            return Ok(Some(dtmf_event(ingress_leg, packet, DtmfPhase::Completed)));
        }
        Ok(None)
    }
}
    ...
    fn drain<S: RtpProcessSink>(
        ...
        for _ in 0..self.max_drain_per_datagram {
            match self.jitter.pop() {
                ...
                JitterPop::Gap { .. } => match self.pipeline.conceal() {
                    Ok(processed) => {
                        self.emit(
                            processed.frame,
                            EmissionKind::Audio {
