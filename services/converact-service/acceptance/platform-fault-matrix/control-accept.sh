...
CAPACITY_PROBE="$ACCEPTANCE_DIR/capacity-probe.ts"
...
CONFIRMATION=${CONVERACT_G02_CONTROL_CONFIRM:-}
RUN_ID=${CONVERACT_G02_FAULT_RUN_ID:-}
SOURCE_COMMIT=${CONVERACT_G02_SOURCE_COMMIT:-}
OPERATIONS=${CONVERACT_G02_CAPACITY_OPERATIONS:-1000000}

if [[ "$CONFIRMATION" != "G02_PLATFORM_CONTROL_EVIDENCE" ]]; then
  printf '%s\n' 'CONVERACT_G02_CONTROL_CONFIRM must equal G02_PLATFORM_CONTROL_EVIDENCE' >&2
  exit 2
fi
...
if [[ ! "$OPERATIONS" =~ ^[0-9]+$ ]] || (( OPERATIONS < 10000 || OPERATIONS > 5000000 )); then
  printf '%s\n' 'CONVERACT_G02_CAPACITY_OPERATIONS must be between 10000 and 5000000' >&2
  exit 2
fi
...
if [[ ! "$NODE_VERSION" =~ ^v24\.[0-9]+\.[0-9]+$ ]]; then
  printf '%s\n' 'control campaign requires Node v24' >&2
  exit 2
fi
...
CAPACITY_RESULT="$EVIDENCE_DIR/capacity-result.json"
IDENTITY_RESULT="$EVIDENCE_DIR/evidence-identity.json"
FINAL_RESULT="$EVIDENCE_DIR/capacity-controlled-evidence.json"
...
timeout -k 5 120 "$NODE_BIN_PATH" --import tsx "$CAPACITY_PROBE" run \
  "$CAPACITY_RESULT" "$OPERATIONS" >"$EVIDENCE_DIR/capacity-run.log" 2>&1
snapshot_containers >"$AFTER_CONTAINERS"
if ! cmp -s "$BEFORE_CONTAINERS" "$AFTER_CONTAINERS"; then
  printf '%s\n' 'unrelated container state changed during control campaign' >&2
  exit 1
fi
...
export CONVERACT_G02_WORKLOAD="$OPERATIONS production BoundedWorkGate admissions; active=64; pending=256; retry=3; fanout=8; fixed host; synthetic control-plane only"
export CONVERACT_G02_SEED="$RUN_ID"
export CONVERACT_G02_STARTED_AT

"$NODE_BIN_PATH" "$IDENTITY_PROBE" "$IDENTITY_RESULT"
"$NODE_BIN_PATH" --import tsx "$CAPACITY_PROBE" finalize \
  "$IDENTITY_RESULT" "$CAPACITY_RESULT" "$FINAL_RESULT" \
  >"$EVIDENCE_DIR/capacity-finalize.log" 2>&1
