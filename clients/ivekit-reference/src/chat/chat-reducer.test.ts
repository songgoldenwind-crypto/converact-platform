...
import type { IveKitChatMessage, IveKitChatPin, IveKitChatReaction, IveKitChatRealtimeState } from '@opc/ivekit-sdk';
...
test('chat reducer loads, prepends without duplication, and suppresses stale requests', () => {
  ...
  state = chatReducer(state, { type: 'history_prepended', messages: [message('a', 1), message('b', 2)] });
...
test('chat reducer handles optimistic send, retryable delivery, terminal failure, and realtime dedupe', () => {
  ...
  state = chatReducer(state, { type: 'send_failed', localId: 'local', retryable: true, error: 'provider pending' });
  assert.equal(state.messages[0].client_state, 'retry_wait');
  state = chatReducer(state, { type: 'send_failed', localId: 'local', retryable: false, error: 'terminal' });
...
test('chat reducer projects unread receipts, presence expiry, typing expiry, edits, deletes, reactions, and pins', () => {
  ...
  state = chatReducer(state, { type: 'message_state_updated', unreadCount: 4 });
  assert.equal(state.unreadCount, 4);
  state = chatReducer(state, { type: 'message_edited', message: { ...message('a', 1), body: 'edited', edit_version: 1 } });
  assert.equal(state.messages[0].body, 'edited');
  state = chatReducer(state, { type: 'reactions_updated', messageId: 'a', reactions: [reaction('a', 'like')] });
  assert.equal(state.messages[0].reactions?.[0].emoji, 'like');
  state = chatReducer(state, { type: 'pins_updated', pins: [pin('a')] });
  ...
  state = chatReducer(state, { type: 'message_deleted', message: { ...message('a', 1), body: '', deleted_at: new Date().toISOString() } });
