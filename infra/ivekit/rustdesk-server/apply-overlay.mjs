...
export function patchCargoToml(source) {
  if (source.includes('ivekit-component-hook =')) return source;
  return replaceOnce(
    source,
    '[dependencies]\n' +
    ...
    '[dependencies]\n' +
      'hbb_common = { path = "libs/hbb_common" }\n' +
      'ivekit-component-hook = { path = "ivekit/component-hook-rs" }\n',
    'RustDesk Server Cargo dependency anchor'
  );
}

export function patchCargoLock(source) {
  const packageIdentity = 'name = "ivekit-component-hook"\nversion = "0.1.0"';
  const dependencyIdentity = ' "ivekit-component-hook",';
