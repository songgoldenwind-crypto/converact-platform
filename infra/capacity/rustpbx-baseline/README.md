...
Converact Fabric-patched RustPBX process on `172.30.44.0/24`. It exposes no host ports.
...
python3 infra/capacity/rustpbx-baseline/prepare.py \
  /opt/converact-capacity-benchmark/runtime/rustpbx-baseline
```
...
```bash
docker compose \
  --env-file /opt/converact-capacity-benchmark/runtime/rustpbx-baseline/.env \
...
installing the pinned SIPp binary at `/opt/converact-capacity-benchmark/bin/sipp-3.7.7`:
...
The baseline sets `callrecord.worker_threads = 1`. CDR delivery therefore runs
outside the SIP runtime while the same bounded queue and exact CDR accounting
remain under test.
