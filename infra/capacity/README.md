# Converact Fabric Capacity Runtime Deployment
...
docker build \
  ...
  -t converact/capacity-tools:source-candidate \
  .
```
...
`manifest.json.contents.capacity_runtime`; no Converact Platform business-domain source is required.
...
```json
{
  ...
  "executable": "/opt/converact-capacity-worker/bin/tinode-loadgen",
  "binary_version": "tinode-loadgen@replace-with-commit",
  "binary_sha256": "replace-with-64-lowercase-hex",
  "result_directory": "/var/lib/converact-capacity/results",
  "timeout_ms": 600000,
  "args": ["--profile", "cell-10k-v1"],
  "static_input": {
    "credential_bundle_path": "/opt/converact-capacity-worker/secrets/legacy.json",
    "composite_credential_bundles": [
      {
        "run_id": "replace-with-run-id",
        "phase_id": "steady",
        "shard_id": "connection/tinode_websocket/3000-6000",
        "path": "/opt/converact-capacity-worker/secrets/steady-3000-6000.json",
...
```bash
node --import tsx scripts/converact-capacity-tinode-staircase.ts \
  --output-file /absolute/path/tinode-strict-staircase.json \
  --tinode-image converact/tinode:v0.25.3-ivekit.3-22a7c18e-amd64 \
...
After all point runs are terminal, `converact:capacity:scaling-finalizer` or
...
terminal, `converact:capacity:platform-finalizer` or `kubernetes/platform-finalizer-job.yaml` applies
...
- Secret `converact-capacity-runtime` with `database-url`, multi-node TLS `nats-url`, `nats-user`, and
  `nats-password`;
- Secret `converact-capacity-nats-tls` with `ca.crt`, `tls.crt`, and `tls.key` for client mTLS;
...
- ConfigMap `converact-capacity-worker` and driver spec ConfigMap;
