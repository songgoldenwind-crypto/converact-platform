...
git -C "$BUILD_ROOT/rustpbx" apply --check "$PATCH_DIR/rustpbx-converact-recovered-controller-fault-boundary.patch"
git -C "$BUILD_ROOT/rustpbx" apply "$PATCH_DIR/rustpbx-converact-recovered-controller-fault-boundary.patch"
git -C "$BUILD_ROOT/rustpbx" apply --check "$PATCH_DIR/rustpbx-converact-recovered-admission-cleanup-deadline.patch"
git -C "$BUILD_ROOT/rustpbx" apply "$PATCH_DIR/rustpbx-converact-recovered-admission-cleanup-deadline.patch"
git -C "$BUILD_ROOT/rustpbx" apply --check "$PATCH_DIR/rustpbx-converact-recovered-dialog-worker-supervision.patch"
git -C "$BUILD_ROOT/rustpbx" apply "$PATCH_DIR/rustpbx-converact-recovered-dialog-worker-supervision.patch"
git -C "$BUILD_ROOT/rustpbx" apply --check "$PATCH_DIR/rustpbx-converact-recovered-worker-registry-isolation.patch"
git -C "$BUILD_ROOT/rustpbx" apply "$PATCH_DIR/rustpbx-converact-recovered-worker-registry-isolation.patch"
git -C "$BUILD_ROOT/rustpbx" apply --check "$PATCH_DIR/rustpbx-converact-active-call-agent-session.patch"
git -C "$BUILD_ROOT/rustpbx" apply "$PATCH_DIR/rustpbx-converact-active-call-agent-session.patch"
git -C "$BUILD_ROOT/rustpbx" apply --check "$PATCH_DIR/rustpbx-converact-call-inspect.patch"
git -C "$BUILD_ROOT/rustpbx" apply "$PATCH_DIR/rustpbx-converact-call-inspect.patch"
...
if [[ "${CONVERACT_FABRIC_RUSTPBX_VERIFY_ONLY:-0}" == "1" ]]; then
  mapfile -t RUSTPBX_FORMAT_FILES < <(
    git -C "$BUILD_ROOT/rustpbx" apply --numstat \
      ...
      "$PATCH_DIR/rustpbx-converact-recovered-controller-fault-boundary.patch" \
      "$PATCH_DIR/rustpbx-converact-recovered-admission-cleanup-deadline.patch" \
      "$PATCH_DIR/rustpbx-converact-recovered-dialog-worker-supervision.patch" \
      "$PATCH_DIR/rustpbx-converact-recovered-worker-registry-isolation.patch" \
      "$PATCH_DIR/rustpbx-converact-active-call-agent-session.patch" \
      "$PATCH_DIR/rustpbx-converact-call-inspect.patch" |
