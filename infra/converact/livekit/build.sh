...
: "${CONVERACT_FABRIC_LIVEKIT_IMAGE:?CONVERACT_FABRIC_LIVEKIT_IMAGE is required}"
...
LIVEKIT_GO_CACHE="${GOCACHE:-/tmp/converact-livekit-go-cache}"
...
docker build \
  ...
  --label "io.converact.component=livekit-server" \
  --label "io.ivekit.owner-contract=component-node-v1" \
  --tag "${CONVERACT_FABRIC_LIVEKIT_IMAGE}" \
  "${LIVEKIT_SOURCE_DIR}"

printf '%s\n' "${CONVERACT_FABRIC_LIVEKIT_IMAGE}"
