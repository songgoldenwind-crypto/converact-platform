...
: "${CONVERACT_FABRIC_LIVEKIT_SIP_IMAGE:?CONVERACT_FABRIC_LIVEKIT_SIP_IMAGE is required}"
...
build_args=(
  ...
  --label "io.converact.component=livekit-sip"
  --label "io.ivekit.build-contract=livekit-sip-v1"
  --tag "${CONVERACT_FABRIC_LIVEKIT_SIP_IMAGE}"
)
if [[ -n "${CONVERACT_FABRIC_LIVEKIT_SIP_PLATFORM:-}" ]]; then
  build_args+=(--platform "${CONVERACT_FABRIC_LIVEKIT_SIP_PLATFORM}")
fi
...
image_revision="$(
  "${DOCKER_COMMAND}" image inspect "${CONVERACT_FABRIC_LIVEKIT_SIP_IMAGE}" \
...
runtime_version="$(
  "${DOCKER_COMMAND}" run --rm "${CONVERACT_FABRIC_LIVEKIT_SIP_IMAGE}" --version
)"
...
printf '%s\n' "${CONVERACT_FABRIC_LIVEKIT_SIP_IMAGE}"
