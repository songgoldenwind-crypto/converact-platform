# Converact Fabric OpenTelemetry Collector Gateway
...
1. Create Secret `converact-otel-backend` with key `endpoint`. The value is the
   external OTLP/HTTP base endpoint consumed by the Collector exporter; do not
   put credentials in this repository.
2. Apply the profile in the same namespace as the Converact Fabric workloads, or narrow
   the NetworkPolicy selectors for a dedicated observability namespace.
3. Point the Chart at
   `http://converact-otel-collector:4318/v1/traces` and enable telemetry only in
...
must not change Converact Fabric readiness or terminate active communication.
