...
| target | The complete interface and corpus are frozen; `.73` adds the default-disabled Rust matched-CANCEL pair, `.74` adds ordinary initial inbound-INVITE response authority, and `.75` fences failure teardown to the exact admitted Call. `.76` adds a conservative Rust restart-reconstruction seam: the closed recovery binding and transaction are validated, a durable Oracle must atomically fence the predecessor and prove `NoVisibleEffect`, and the exact successor is revalidated before capabilities can be installed. The installed gate is permanently bound to that full successor identity and rechecks it before every prepare path and after asynchronous durable preparation. Visible/ambiguous history, replacement during the probe and later same-provider-ID replacement all fail closed without successor mutation or a new effect. Focused `.76` checks pass Native SIP capability `25/25`, registry `24/24`, full local RustPBX `2082/2082` with 9 external-prerequisite cases ignored, locked check and rustfmt. Earlier server evidence is not inherited. The real PostgreSQL Oracle, live recovery holder, physical PostgreSQL, live Endpoint, isolated-server `.76`, remaining transports, process-crash/two-node, Linux full, all performance work and Native Authority remain `not_run` |
