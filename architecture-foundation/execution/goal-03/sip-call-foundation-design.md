...
The `.81` delta adds the local recovered Active Call composition; `.82` adds
its local child-unwind boundary, `.83` closes the unbounded media wait on
recovered admission rejection, `.84` makes loss of either recovered Dialog
worker visible to the sole controller, and current `.85` proves exact registry
isolation for that local fault path. It does not prove actual live-task panic
cleanup, external media orphan reconciliation,
