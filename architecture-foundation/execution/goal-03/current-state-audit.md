...
| RustPBX/rsipstack runtime | `infra/converact/rustpbx/build.sh` and patch queue | RustPBX `6c49ee76…`, rsipstack `8318e97b…`, rustrtc `166c6d22…`, patchset `.75` is pinned. `.73` retains the matched-CANCEL pair; `.74` adds one Native Call-owned, default-disabled transaction-local gate for ordinary initial inbound-INVITE responses. `.75` binds failure cleanup to the exact `NativeCallIdentity` and Native Call cell, and holds the provider slot through all secondary-index cleanup so a reused Call-ID successor is never removed by an older failure path. The builder field remains `None`; no product config or Endpoint-global gate activates it | `.75` focused functional checks pass Native SIP capability `19/19` and Active Call registry `24/24`; exact local full RustPBX passes `2076/2076` with 9 external-prerequisite cases ignored, and locked library check plus scoped rustfmt pass. Physical PostgreSQL, live Endpoint and isolated server functional verification remain `not_run`. Earlier `.73`/`.74` server attempts are not inherited. Restart capability reconstruction, all performance/load/capacity/soak work and production eligibility remain `not_run` |
...
9. `.73` wires the matched-CANCEL capability holder into the Rust session path
   ...
   `.75` now closes the successor-replacement cleanup race with one sealed,
   non-cloneable identity/cell fence consumed exactly once by value and
   provider-slot-scoped secondary-index teardown; stale
   cleanup is a no-op and exact cleanup removes every owned index. Next close
   capability reconstruction after restart, UAS-2xx and reconciliation resume.
