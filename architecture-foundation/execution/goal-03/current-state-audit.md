...
| RustPBX/rsipstack runtime | `infra/converact/rustpbx/build.sh` and patch queue | RustPBX `6c49ee76…`, rsipstack `8318e97b…`, rustrtc `166c6d22…`, patchset `.76` is pinned. `.73` retains the matched-CANCEL pair; `.74` adds the Native Call-owned ordinary-response gate; `.75` fences failure cleanup to the exact admitted Call. `.76` adds a default-disabled Rust recovery seam that accepts only one exact `NativeCallRecoveryBinding`, validates the recovered Native Call and server-INVITE transaction, awaits an oracle that atomically fences the predecessor and proves `NoVisibleEffect`, then revalidates the successor before installing capabilities. The installed transaction gate remains bound to that full `NativeCallIdentity`; it checks the identity before every prepare path and again after asynchronous durable preparation, so a later Call reusing the provider Call-ID cannot inherit or be mutated by the stale gate. `VisibleOrAmbiguous` fails closed without mutation. The real PostgreSQL oracle and product wiring do not exist, and no Endpoint-global gate activates it | `.76` focused Native SIP capability passes `25/25`, Active Call registry remains `24/24`, exact local full RustPBX passes `2082/2082` with 9 external-prerequisite cases ignored, and locked library check plus scoped rustfmt pass. Physical PostgreSQL, live Endpoint and isolated server `.76` functional verification remain `not_run`; earlier server attempts are not inherited. Durable recovery Oracle/wiring, all performance/load/capacity/soak work and production eligibility remain `not_run` |
...
9. `.73` wires the matched-CANCEL capability holder into the Rust session path
   ...
   successor replacement fails closed without installing intents. Next supply
   the real PostgreSQL Oracle and live holder, then close UAS-2xx and
