...
| RustPBX/rsipstack runtime | `infra/converact/rustpbx/build.sh` and patch queue | RustPBX `6c49ee76…`, rsipstack `8318e97b…`, rustrtc `166c6d22…`, patchset `.72` is pinned; `.71` adds the fixed observer supervisor and `.72` adds the exact-target reconciler component without touching product configuration, `SipServerBuilder` or live Endpoint composition | `.72` exact-source reconciler tests pass `28/28`, the affected SipEffect suite passes `87 passed / 0 failed / 8 ignored`, the two privacy UI probes fail with their exact expected compiler codes, and locked library check plus Rust formatting pass. The two test counts and scoped formatting also pass in the controlled offline Linux focused run. The controlled Linux full-suite result remains bound to `.71`; `.72` Linux full, exact-image traffic/performance requalification and production activation remain `not_run` |
