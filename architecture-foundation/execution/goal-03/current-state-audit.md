...
| Durable effect ledger | `effect-oracle.ts`; `postgres-effect-store.ts`; migrations `107`, `113`, `114`, `115` and `116`; native `.59` protocol-observation through `.72` exact-target reconciler supervision and `.77` capability-recovery Oracle | Reference and native implementations use closed v1/v2 wire-attempt facts, separate transport and peer terminal meanings, atomic observation transactions, repair fences and bounded shards. `.72` retains its sealed exact-target repair grant. `.77` adds a default-disabled Rust/PostgreSQL recovery Oracle: one transaction advances the exact tenant/session fence, probes only the deterministic 200-CANCEL and 487-INVITE predecessor effect IDs, and persists an immutable replayable receipt. Migration 116 rejects both stale owner/generation inserts and a stale pre-existing effect's first transition into `send_attempted`; later real observations remain writable. The raw transaction key is hashed and never stored | local exact-source SipEffect tests pass `121 passed / 0 failed / 10 physical tests ignored`; the isolated PostgreSQL 16 migration/SQL harness passes session fencing, exact two-key visibility, receipt replay/immutability, stale insert and pre-existing-send-attempt rejection, and tenant RLS under `evidence/raw/capability-recovery-oracle-204f4d5-17/`. This does not promote `G03-E16`. Rust-adapter physical PostgreSQL tests, live issuer/holder, live Endpoint, real process-crash/two-node recovery, fault campaigns, production and every deferred performance item remain `not_run` |
...
| RustPBX/rsipstack runtime | `infra/converact/rustpbx/build.sh` and patch queue | RustPBX `6c49ee76…`, rsipstack `8318e97b…`, rustrtc `166c6d22…`, patchset `.77` is pinned. `.73`–`.76` retain the matched-CANCEL pair, ordinary-response authority, exact cleanup fence and conservative restart seam. `.77` implements that seam's Oracle in Rust over PostgreSQL without introducing a TypeScript runtime authority. `VisibleOrAmbiguous`, stale session ownership, receipt drift and successor replacement fail closed. No Endpoint-global gate activates it | local exact-source SipEffect tests pass `121/121` with 10 physical PostgreSQL cases explicitly ignored. The isolated server ran only the migration and SQL functional harness; its pre-existing container stayed healthy and only the exact `network=none` temporary test container/tmpfs were removed. Rust adapter physical tests, product wiring, live Endpoint, real process restart and production remain `not_run`. No load, CPS, latency, concurrency, capacity, soak or 100K command ran |
...
9. `.73` wires the matched-CANCEL capability holder into the Rust session path
   ...
   harness proves its SQL boundary without touching the existing service. Next
   supply the live holder and Rust-adapter physical test, then close UAS-2xx and
