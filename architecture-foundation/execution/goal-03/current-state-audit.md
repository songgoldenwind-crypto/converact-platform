...
| RustPBX/rsipstack runtime | `infra/converact/rustpbx/build.sh` and patch queue | RustPBX `6c49ee76…`, rsipstack `8318e97b…`, rustrtc `166c6d22…`, patchset `.64` is pinned; `.64` layers product retention and typed failure handling over the `.63` bounded UAS 2xx owner, uses the shared timer heap, and adds no Call authority, task, global scan or duplicate wire allocation | exact-source local and authorized-server component suites pass rsipstack `311/311`, doctest `67/67` and RustPBX `2,008/0/8`; `.63` remains rejected because its RustPBX compile failed on the missing typed outcome; `.64` exact image/wire/latency/peer/long-call/capacity evidence and production activation remain `not_run` |
