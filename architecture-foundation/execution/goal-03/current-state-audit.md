...
| RustPBX/rsipstack runtime | `infra/converact/rustpbx/build.sh` and patch queue | RustPBX `6c49ee76…`, rsipstack `8318e97b…`, rustrtc `166c6d22…`, patchset `.70` is pinned; `.66` layers default-disabled indexed stale-effect recovery over `.65`, `.67` makes clock aging revision-safe, `.68` removes the administrator effect write, `.69` replaces clock rewriting with the real minimum stale wait, and `.70` removes the ambiguous recovery candidate identifier | `.70` local exact-patch gates pass `187/187`, affected TypeScript gates pass `121/121`, the G03 contract passes `9/9`, and typecheck passes. Exact `.69` Linux exposed the ambiguous `UPDATE ... RETURNING` (`1/2`); exact `.70` passed the focused PostgreSQL 16.14 case `2/2` in `31.25s`, without PostgreSQL errors or OOM. The retained `.65` full component candidate remains RustPBX `2,015/0/8`, dialog-shadow `20/20`, rsipstack `311/311` and doctests `67/67`; `.70` full Linux suite, image/wire/latency/peer/long-call/capacity evidence and production activation remain `not_run` |
...
9. Wire the current `.70` matched-CANCEL capability holder and UAS-2xx owner through the
   live Endpoint path and reconciliation resume; close parent-Unknown and
   stale-nonterminal/UAS-owner crash recovery before the default-disabled `.70`
