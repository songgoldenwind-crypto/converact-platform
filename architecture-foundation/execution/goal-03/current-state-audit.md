...
| RustPBX/rsipstack runtime | `infra/converact/rustpbx/build.sh` and patch queue | RustPBX `6c49ee76…`, rsipstack `8318e97b…`, rustrtc `166c6d22…`, patchset `.64` is pinned; `.64` layers product retention and typed failure handling over the `.63` bounded UAS 2xx owner, uses the shared timer heap, and adds no Call authority, task, global scan or duplicate wire allocation | local exact-source rsipstack `311/311` plus doctest `67/67` and RustPBX `2,008/0/8` pass; `.63` authorized-server rsipstack passed but its RustPBX compile failed on the missing typed outcome, so it is superseded; authorized-server `.64` full suites and exact image/wire/latency/peer/long-call/capacity evidence remain `not_run`; production activation remains `not_run` |
...
tested locally and passes rsipstack `311/311`, doctest `67/67`, and RustPBX
`2,008/0/8`; its authorized-server run remains `not_run`. The following remain
`not_run` for the current `.64` candidate at this update:
...
9. Wire the `.64` matched-CANCEL capability holder and UAS-2xx owner through the
   live Endpoint path and reconciliation resume; close parent-Unknown and
   stale-nonterminal/UAS-owner crash recovery before the default-disabled `.64` gate
