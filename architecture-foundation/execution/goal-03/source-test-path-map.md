...
| native rsipstack runtime and bounded mailboxes | `.75` build: the `.73` matched-CANCEL pair, `.74` ordinary-response gate, and `.75` exact Native Call failure-cleanup fence. Failure teardown consumes one sealed, non-cloneable `NativeCallIdentity` plus Native Call cell-pointer fence by value, then holds the provider slot through provider/native/dialog index cleanup. Existing bounded mailboxes, protocol observation, derived ACK, private peer proof, UAS-2xx owner/retention, recovery identity and directional model remain intact | local exact-source focused Native SIP capability `19/19`, Active Call registry `24/24`, full RustPBX `2076/2076` with 9 external-prerequisite cases ignored, locked Rust library check and scoped rustfmt; isolated-server `.75` verification remains `not_run` | compiled default-disabled `.75` functional path only. Earlier isolated server evidence is retained but not inherited. Restart reconstruction, physical PostgreSQL, live Endpoint/product activation, remaining transports, all performance work and `G03-E16` remain `not_run` |
| native durable rsipstack egress adapter | protocol-observation through `.75`: `.73` retains two separate matched-CANCEL effects; `.74` reserves each ordinary response in the sole Native Call authority and orders durable prepare before Call-state commit and transport; `.75` makes each setup failure remove only the exact original Call and preserves any reused-ID successor with all its indexes | focused Native SIP capability `19/19`, registry `24/24`, full RustPBX `2076/2076` with 9 ignored external prerequisites, locked check and rustfmt; physical PostgreSQL and server functional verification remain `not_run` | compiled default-disabled Rust path only; prior component evidence is not inherited. Capability reconstruction, durable recovery resumption, physical PostgreSQL, product activation and `G03-E16` remain `not_run` |
