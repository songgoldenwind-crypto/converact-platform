...
| native rsipstack runtime and bounded mailboxes | `.72` build: the `.71` fixed observer tail plus the separately default-disabled exact-target reconciler, bounded mailbox, protocol observation, derived non-2xx ACK, peer-ingress proof, peer-derived matched-CANCEL, bounded UAS-2xx owner/retention, Native Call recovery identity, durable gate and directional model patches | `.72` reconciler tests `28/28`, affected SipEffect suite `87 passed / 0 failed / 8 ignored`, two expected-code sibling privacy UI probes, locked library check and Rust formatting; the same focused counts and scoped formatting pass offline on controlled Linux under `evidence/raw/focused-linux-sip-effect-b3c9da0-14/`; controlled Linux full-suite counts remain bound to `.71`; `.53` remains the controlled image/wire/latency/interop/capacity source | `.72` local plus focused-Linux component result only; physical PostgreSQL, 10K/100K query plan, issuer/sink, Linux full, live Native Authority, exact-image traffic/performance requalification and `G03-E16` remain `not_run` |
| native durable rsipstack egress adapter | protocol-observation through `.72` reconciliation-supervision patches: closed v2 facts, distinct terminal meanings, bounded observation ownership, fixed workers and bounded queue, opaque/sealed crate-private 1..100 target grants scoped by tenant/session/generation/successor epoch, exact claimed/exhausted IDs, parent-bound ACK, private peer proof, matched-CANCEL capability and retained UAS-2xx owner | `.72` reconciler tests `28/28`, affected SipEffect tests `87 passed / 0 failed / 8 ignored`, expected `E0603`/`E0451` sibling compile failures, locked check and Rust formatting; controlled `.72` focused Linux and `.71` full Linux component suites; prior `.60` physical PostgreSQL atomic child prepare `1/1` | compiled default-disabled components only; authoritative issuer, durable completion sink, physical exact-target claim/query plan, real Call Core registration, live Endpoint wiring, transport flow-generation binding, parent-Unknown/stale/UAS crash recovery and `G03-E16` remain `not_run` |
