...
| native rsipstack runtime and bounded mailboxes | `.60` build plus bounded mailbox, protocol-observation, derived non-2xx ACK, durable gate and directional Native Call patches | exact patch gates, fresh full-queue apply, rsipstack `302/302`, and direction-keyed UAS/UAC conformance; `.53` remains the exact controlled image/wire/latency/interop/capacity source | `.59` protocol evidence remains under `native-protocol-observation-fe4c38b-05`; `.60` exact-source evidence is under `derived-non-2xx-ack-9fc99ee-06`; live Native Authority and `.60` image/traffic requalification remain `G03-E16/not_run` |
| native durable rsipstack egress adapter | protocol-observation patches plus `rsipstack-converact-derived-non-2xx-ack.patch` and `rustpbx-converact-derived-non-2xx-ack.patch`: closed v2 facts, separate terminal meanings, bounded observation ownership and one stable parent-bound non-2xx ACK child | focused derived/cancellation/replay tests; complete RustPBX library suite locally and on Linux (`2,002` passed, `0` failed, `8` ignored); physical PostgreSQL atomic child prepare `1/1`; static exact-patch gates | compiled default-disabled adapter only; automatic 200-CANCEL, UAS-2xx ownership, parent-Unknown/stale-nonterminal recovery, live endpoint activation and `G03-E16` remain `not_run` |
...
The local source/unit suite, controlled `.59` and `.60` Linux component bundles,
...
`.60`. Fault/OOM, allocation, multi-core scaling, real-peer, long-call and
