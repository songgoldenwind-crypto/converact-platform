...
| native rsipstack runtime and bounded mailboxes | `.74` build: the `.73` matched-CANCEL pair plus one Native Call-owned default-disabled transaction-local ordinary-response gate. It freezes Call-ID, INVITE CSeq, From, top Via, To base and one authority-generated local To tag, then authorizes exact 101..699 wire images; multiple provisional responses may precede exactly one final. Existing bounded mailboxes, protocol observation, derived ACK, private peer proof, UAS-2xx owner/retention, recovery identity and directional model remain intact | local exact-source focused Native response capability `17/17`, Native Call `13/13`, Active Call registry `24/24`, durable gate `39/39`, locked Rust library check and scoped rustfmt; full RustPBX `.74` and isolated-server `.74` verification remain `not_run` | compiled default-disabled `.74` functional path only. The `.73` isolated Linux `32/32` evidence is retained but not inherited. Successor-safe cleanup fencing, restart reconstruction, physical PostgreSQL, live Endpoint/product activation, remaining transports, all performance work and `G03-E16` remain `not_run` |
| native durable rsipstack egress adapter | protocol-observation through `.74`: `.73` retains two separate matched-CANCEL effects; `.74` reserves each ordinary response in the sole Native Call authority, registers exact intent, durably prepares, commits Call state and only then permits transport. Dialog drift fails before store work; cancellation/panic ambiguity retains the exact binding, while a concurrent Call revision after durable prepare records `TransportUnknown` | focused durable gate `39/39`, Native response capability `17/17`, Native Call `13/13`, registry `24/24`, locked check and rustfmt; full RustPBX, physical PostgreSQL and server functional verification remain `not_run` | compiled default-disabled Rust path only; prior component evidence is not inherited. Durable recovery resumption, physical PostgreSQL, product activation and `G03-E16` remain `not_run` |
