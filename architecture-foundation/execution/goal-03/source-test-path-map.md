...
| native rsipstack runtime and bounded mailboxes | `.70` build plus the `.66` indexed stale-nonterminal runtime, superseded `.67`/`.68` fixture guards, `.69` database-clock fixture and `.70` recovery SQL alias, bounded mailbox, protocol-observation, derived non-2xx ACK, peer-ingress proof, peer-derived matched-CANCEL, bounded UAS-2xx owner/retention, Native Call recovery identity, durable gate and directional Native Call patches | `.70` local exact-patch gates pass `187/187`, affected TypeScript gates pass `121/121`, G03 contract passes `9/9`, and typecheck passes; exact `.69` exposed the ambiguous recovery candidate id (`1/2`), while exact `.70` passed the controlled PostgreSQL 16.14 case `2/2` in `31.25s`; retained `.65` authorized-server `1d05333…` passed RustPBX `2,015/0/8`, dialog-shadow `20/20`, rsipstack `311/311` and doctests `67/67`; `.53` remains the exact controlled image/wire/latency/interop/capacity source | `.70` controlled recovery evidence is under `stale-nonterminal-recovery-6abf714-11`; live Native Authority, full `.70` Linux suite, image/traffic/performance requalification and `G03-E16` remain `not_run` |
...
| durable effects/receipts reference | `effect-oracle.ts`; `postgres-effect-store.ts`; migrations 107, 113, 114 and 115; `.66` native stale-effect recovery; `.69` database-clock fixture; `.70` recovery SQL alias | effect-oracle tests, v1/v2 migration gates, bounded successor-fence static/unit tests, six prior controlled physical cases, and exact `.70` stale-nonterminal recovery `2/2`; real crash/live writer remains unproved | `G03-E04-EFFECT`, `G03-E05-POSTGRES`; native writer activation is `G03-E16/not_run` |
