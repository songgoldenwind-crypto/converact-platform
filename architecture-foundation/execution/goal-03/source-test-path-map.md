...
Current patchset is `.84`. Rows ending at `.83` describe its retained substrate;
the first row below records additive recovered-Dialog worker supervision and
does not promote live recovery or Native Authority.
...
| native rsipstack runtime and bounded mailboxes | `.84` build retains `.73` matched-CANCEL through `.83` bounded recovered admission and supervises the two recovered Dialog event workers with one bounded exit channel. The Rust app keeps one default-disabled store/owner Authority; one owner snapshot feeds registry and Oracle where applicable, stale cleanup uses exact pointer fences, and the recovered control mailbox is bounded | exact-source SipEffect `135/135` with 11 ignored; native SIP effect `40/40` with one ignored; recovered handle `1/1`; dialog shadow `13/13`; Active Call registry `25/25`; full RustPBX `2115/2115` with 12 ignored; locked check, rustfmt, typecheck and static contracts pass | compiled default-disabled `.84` component path only. Live worker-fault injection, process abort/OOM, orphan reconciliation, process restart, live Endpoint, remaining transports, Linux process execution, all performance work and `G03-E16` remain `not_run` |
| native durable rsipstack egress adapter | protocol-observation through `.84`: `.77` enforces exact-key original-INVITE recovery and session high-water marks; `.78` binds the store to the Rust app lifecycle; `.79` selects installation from authenticated owner proof; `.80` provides the capsule issuer; `.81` refuses to manufacture original transaction facts from a confirmed Dialog; `.82` contains recovered-controller unwind; `.83` bounds admission cleanup; `.84` supervises its Dialog event workers. Disabled mode performs no database work; malformed, missing, duplicate or live-reloaded configuration fails closed. Cold startup is bounded at 2 s without widening the 250 ms Call-store deadline | local SipEffect `135/135`; native SIP effect `40/40`; full RustPBX `2115/2115`. Historical migration 001–116 and exact `.78` Rust physical adapter `1/1` passed on isolated PostgreSQL 16; `.84` used no server | default-disabled Rust components only. Real process restart, original-INVITE Oracle activation, live Endpoint activation and `G03-E16` remain `not_run` |
| durable effects/receipts reference | `effect-oracle.ts`; `postgres-effect-store.ts`; migrations 107, 113, 114, 115 and 116; `.66` stale-effect recovery; `.71` fixed observer; `.72` exact-target reconciler; `.77` capability-recovery Oracle; `.78` Rust composition root; `.79` authenticated invocation; `.80` issuer; `.81` confirmed-dialog boundary; `.82` child fault boundary; `.83` admission cleanup deadline; `.84` Dialog worker supervision | existing controlled evidence plus `.84` local SipEffect `135/135`, native SIP effect `40/40` and full RustPBX `2115/2115`. TypeScript remains a contract/conformance and authenticated control-plane compatibility layer; server runtime, workers, owner registry and durable store composition are Rust | existing `G03-E04-EFFECT` and `G03-E05-POSTGRES` are not promoted to live authority by this component evidence; original-INVITE Oracle invocation, issuer/sink, live Endpoint, process-crash/two-node and native activation remain `not_run`; 10K/100K and every performance item are deferred |
...
| controlled PostgreSQL evidence | prior PostgreSQL bundles plus `evidence/raw/capability-recovery-oracle-204f4d5-17/`; `.78` bundle under `evidence/raw/durable-sip-runtime-composition-2ecfb72-18/`; `.79` through `.84` local-only bundles | prior restart/observation/derived-ACK cases plus full migration 001–116 and exact `.78` Rust startup-contract adapter `1/1`; `.84` reuses this physical contract but makes no new physical claim and did not contact the server | `G03-E05-POSTGRES` remains scoped component evidence; no live Endpoint, Linux product process, native authority or production promotion |
