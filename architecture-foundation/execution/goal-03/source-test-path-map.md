...
| native rsipstack runtime and bounded mailboxes | `.77` build: `.73` matched-CANCEL, `.74` ordinary responses, `.75` exact failure cleanup, `.76` conservative recovery seam and `.77` Rust/PostgreSQL recovery Oracle. Recovery binds the closed Native Call capsule and exact server-INVITE transaction; one transaction advances the exact session fence, probes only the deterministic 200-CANCEL/487-INVITE effect keys and writes an immutable receipt. Existing bounded mailboxes, protocol observation, derived ACK, private peer proof, UAS-2xx owner/retention and directional model remain intact | local exact-source SipEffect `121 passed / 0 failed / 10 physical tests ignored`; isolated PostgreSQL 16 migration/SQL harness passes exact fencing, two-key visibility, replay/immutability, stale insert and pre-existing-send-attempt rejection, and tenant RLS | compiled default-disabled `.77` component path only. Evidence: `capability-recovery-oracle-204f4d5-17/`. Rust-adapter physical tests, live wiring, product activation, remaining transports, process restart, all performance work and `G03-E16` remain `not_run` |
| native durable rsipstack egress adapter | protocol-observation through `.77`: `.76` refuses blind reconstruction; `.77` implements its exact-key durable Oracle and enforces the owner/generation high-water mark in both Rust and migration 116 so an older binary cannot insert a stale effect or advance a pre-existing effect into its first `send_attempted` state after takeover. A later same-provider-ID successor, visible/ambiguous predecessor, receipt mismatch or stale owner fails closed without Call mutation or a new effect | local SipEffect `121/121`; migration/SQL physical contract passed on the isolated server with `network=none`, no host ports and zero impact to the existing service | compiled default-disabled Rust component only. Live recovery holder, Rust-adapter physical PostgreSQL tests, real process restart, product activation and `G03-E16` remain `not_run` |
| durable effects/receipts reference | `effect-oracle.ts`; `postgres-effect-store.ts`; migrations 107, 113, 114, 115 and 116; `.66` stale-effect recovery; `.71` fixed observer; `.72` exact-target reconciler; `.77` capability-recovery Oracle | existing `.72` evidence plus `.77` local SipEffect `121/121` and isolated PostgreSQL session-fence/exact-probe/receipt/RLS harness. TypeScript remains a contract/conformance harness; the new server runtime implementation is Rust | existing `G03-E04-EFFECT` and `G03-E05-POSTGRES` are not promoted to live authority by this component evidence; issuer/sink, Rust-adapter physical tests, live Endpoint, process-crash/two-node and native activation remain `not_run`; 10K/100K and every performance item are deferred |
...
| controlled PostgreSQL evidence | prior PostgreSQL bundles plus `evidence/raw/capability-recovery-oracle-204f4d5-17/` | prior restart/observation/derived-ACK cases plus `.77` migration 116 and exact capability-recovery SQL boundary | `G03-E05-POSTGRES` remains scoped component evidence; no live authority or production promotion |
