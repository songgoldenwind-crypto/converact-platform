...
| native rsipstack runtime and bounded mailboxes | `.78` build: `.73` matched-CANCEL, `.74` ordinary responses, `.75` exact failure cleanup, `.76` conservative recovery seam, `.77` Rust/PostgreSQL recovery Oracle and `.78` product composition. The default-disabled Rust app constructs one verified PostgreSQL store before SIP startup, shares it with Gate/observer/recovery, owns the fixed supervisor, injects it exactly once into either builder path and rejects runtime config reload | exact-source SipEffect `133 passed / 0 failed / 11 physical tests ignored`; native composition `38 / 0 / 1 ignored`; exact isolated PostgreSQL adapter `1 / 0`; locked check, rustfmt and static composition contracts pass. Evidence: `durable-sip-runtime-composition-2ecfb72-18/` | compiled default-disabled `.78` component path plus physical startup-contract proof only. Recovered-Call invocation, live Endpoint, remaining transports, Linux process execution, process restart, all performance work and `G03-E16` remain `not_run` |
| native durable rsipstack egress adapter | protocol-observation through `.78`: `.77` enforces exact-key recovery and session high-water marks; `.78` binds that durable store to the real Rust app/SIP builder lifecycle without adding a second store or Authority. Disabled mode performs no database work; malformed, missing, duplicate or live-reloaded configuration fails closed. Cold startup is bounded at 2 s without widening the 250 ms Call-store deadline | local SipEffect `133/133`; native composition `38/38`; current migration 001–116 and exact Rust physical adapter `1/1` passed on isolated PostgreSQL 16 with no host ports, tmpfs data and zero impact to the existing service | default-disabled Rust composition only. Recovered-Call invocation, real process restart, live Endpoint activation and `G03-E16` remain `not_run` |
| durable effects/receipts reference | `effect-oracle.ts`; `postgres-effect-store.ts`; migrations 107, 113, 114, 115 and 116; `.66` stale-effect recovery; `.71` fixed observer; `.72` exact-target reconciler; `.77` capability-recovery Oracle; `.78` Rust composition root | existing controlled evidence plus `.78` local SipEffect `133/133`, composition `38/38` and exact physical adapter `1/1`. TypeScript remains a contract/conformance harness; server runtime, workers and durable store composition are Rust | existing `G03-E04-EFFECT` and `G03-E05-POSTGRES` are not promoted to live authority by this component evidence; issuer/sink, live Endpoint, process-crash/two-node and native activation remain `not_run`; 10K/100K and every performance item are deferred |
...
| controlled PostgreSQL evidence | prior PostgreSQL bundles plus `evidence/raw/capability-recovery-oracle-204f4d5-17/`; current `.78` bundle under `evidence/raw/durable-sip-runtime-composition-2ecfb72-18/` | prior restart/observation/derived-ACK cases plus current full migration 001–116 and exact `.78` Rust startup-contract adapter `1/1`; the Rust binary ran locally through an SSH tunnel to a no-host-port, tmpfs-only server container because the server had no safe Linux compile floor | `G03-E05-POSTGRES` remains scoped component evidence; no live Endpoint, Linux product process, native authority or production promotion |
