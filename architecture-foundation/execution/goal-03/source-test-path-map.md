...
| durable effects/receipts reference | `effect-oracle.ts`; `postgres-effect-store.ts`; migrations 107, 113, 114 and 115; `.66` native stale-effect recovery patch | effect-oracle tests, v1/v2 migration gates, bounded successor-fence static/unit tests, and six prior controlled physical PostgreSQL cases; `.66` physical crash-window case remains `not_run` | `G03-E04-EFFECT`, `G03-E05-POSTGRES`; native writer activation is `G03-E16/not_run` |
...
| recovery/clock | `sip-foundation/recovery.ts`; TypeScript v2 recovery capsule; Rust `.65` reciprocal dialog takeover/shadow and `NativeCallRecoveryBinding`; `.66` exact-session stale SipEffect recovery using the database clock | recovery/takeover tests, v1 fail-closed and shared Rust/TypeScript SHA-256 golden, plus bounded batch/index/static Rust tests | local component proof only; `.66` physical PostgreSQL and live successor wiring plus real process-crash/two-node takeover remain `G03-E08-RECOVERY/not_run` |
