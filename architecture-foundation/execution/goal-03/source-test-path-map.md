...
Current patchset is `.85`. Rows ending at `.84` describe its retained substrate;
the first row below records exact recovered-worker registry-isolation evidence
and does not promote live recovery or Native Authority.

| Domain | Current/target implementation | Focused tests | Evidence ID |
| --- | --- | --- | --- |
| recovered worker exact registry isolation | `.85` adds a component fault test only. It registers two distinct recovered Calls and four Dialog indexes using the real registry, injects one worker Future panic through `.84` reporting, releases the affected controller cleanup lease and proves only that Call/pair disappears while the unrelated Call/pair survives | registry isolation `1/1`; dialog shadow `14/14`; full RustPBX `2116/2116` with 12 ignored; exact patch replay, locked check, rustfmt, static contracts `240/240` across 61 files and G03 machine contracts `9/9` | local bundle `recovered-worker-registry-isolation-8ee4ec0-25/`; live Endpoint fault injection, process abort/OOM, blocking external dependency isolation, external media orphan reconciliation and `G03-E15/E16` remain `not_run` |
| recovered Dialog worker supervision | `.84` keeps exactly two event forwarders and adds one capacity-2 exit channel. Unexpected receiver close, forwarding stop or caught unwind reports to the sole recovered controller and terminates only the exact Call; intentional cancellation is silent, controller exit cancels both workers, and a successfully forwarded terminal Dialog event cannot race a second worker-loss reason | worker supervision `1/1`; dialog shadow `13/13`; full RustPBX `2115/2115` with 12 ignored; exact patch replay, locked check, rustfmt, static contracts `236/236` across 60 files and G03 machine contracts `9/9` | local bundle `recovered-dialog-worker-supervision-ae4665f-24/`; live Endpoint worker fault injection, process abort/OOM, external media orphan reconciliation and `G03-E15/E16` remain `not_run` |
| recovered-admission cleanup deadline | `.83` uses one private deadline helper for both controller and admission-error media cleanup; invalid recovered input or registry rejection waits at most 2 s before returning the original error so outer Dialog/owner cleanup proceeds | deadline behavior `1/1`; dialog shadow `12/12`; full RustPBX `2114/2114` with 12 ignored; exact patch replay, locked check, rustfmt, static contracts `232/232` and G03 machine contracts `9/9` | local bundle `recovered-admission-cleanup-d86e23a-23/`; external media orphan reconciliation, live rejection, process fault/OOM and `G03-E15/E16` remain `not_run` |
| recovered-controller child fault boundary | `.82` catches unwind inside the existing recovered controller Future, performs one separately unwind-contained exact-Call termination attempt with an 8 s total deadline, caps each media close at 2 s, and then releases the existing exact cleanup lease; it creates no second task, registry or Call authority | focused unwind helper `1/1`; dialog shadow `11/11`; full RustPBX `2113/2113` with 12 ignored; exact patch replay, locked check, scoped rustfmt, static contracts `229/229` and G03 machine contracts `9/9` | local bundle `recovered-controller-fault-a989c37-22/`; live Endpoint controller fault injection, process abort/OOM, external media orphan reconciliation and `G03-E15/E16` remain `not_run` |
| recovered confirmed-dialog Active Call | `.81` reuses the sole `ActiveProxyCallRegistry`, publishes one confirmed inbound Native Leg plus the exact reciprocal Dialog pair, rolls back partial registration, exposes one bounded recovered handle with whole-Call Hangup only, and drops an exact identity/native-cell cleanup lease; it never fabricates original server-INVITE facts | recovered handle `1/1`; dialog shadow `10/10`; Active Call registry `25/25`; full RustPBX `2112/2112` with 12 ignored; exact patch replay, locked check, scoped rustfmt, static contracts and G03 machine contracts | local bundle `recovered-active-call-7d09469-21/`; real restart, live peer, original-INVITE Oracle activation, live Endpoint and `G03-E16` remain `not_run` |
...
| native rsipstack runtime and bounded mailboxes | `.85` build retains `.73` matched-CANCEL through `.84` recovered Dialog worker supervision and adds exact registry-isolation evidence without changing runtime. The Rust app keeps one default-disabled store/owner Authority; one owner snapshot feeds registry and Oracle where applicable, stale cleanup uses exact pointer fences, and the recovered control mailbox is bounded | exact-source SipEffect `135/135` with 11 ignored; native SIP effect `40/40` with one ignored; recovered handle `1/1`; dialog shadow `14/14`; Active Call registry `25/25`; full RustPBX `2116/2116` with 12 ignored; locked check, rustfmt, typecheck and static contracts pass | compiled default-disabled `.85` component path only. Live Endpoint fault injection, process abort/OOM, blocking external dependencies, orphan reconciliation, process restart, remaining transports, Linux process execution, all performance work and `G03-E16` remain `not_run` |
| native durable rsipstack egress adapter | protocol-observation through `.85`: `.77` enforces exact-key original-INVITE recovery and session high-water marks; `.78` binds the store to the Rust app lifecycle; `.79` selects installation from authenticated owner proof; `.80` provides the capsule issuer; `.81` refuses to manufacture original transaction facts from a confirmed Dialog; `.82` contains recovered-controller unwind; `.83` bounds admission cleanup; `.84` supervises its Dialog event workers; `.85` adds component registry-isolation evidence. Disabled mode performs no database work; malformed, missing, duplicate or live-reloaded configuration fails closed. Cold startup is bounded at 2 s without widening the 250 ms Call-store deadline | local SipEffect `135/135`; native SIP effect `40/40`; full RustPBX `2116/2116`. Historical migration 001–116 and exact `.78` Rust physical adapter `1/1` passed on isolated PostgreSQL 16; `.85` used no server | default-disabled Rust components only. Real process restart, original-INVITE Oracle activation, live Endpoint activation and `G03-E16` remain `not_run` |
| durable effects/receipts reference | `effect-oracle.ts`; `postgres-effect-store.ts`; migrations 107, 113, 114, 115 and 116; `.66` stale-effect recovery; `.71` fixed observer; `.72` exact-target reconciler; `.77` capability-recovery Oracle; `.78` Rust composition root; `.79` authenticated invocation; `.80` issuer; `.81` confirmed-dialog boundary; `.82` child fault boundary; `.83` admission cleanup deadline; `.84` Dialog worker supervision; `.85` exact registry-isolation evidence | existing controlled evidence plus `.85` local SipEffect `135/135`, native SIP effect `40/40` and full RustPBX `2116/2116`. TypeScript remains a contract/conformance and authenticated control-plane compatibility layer; server runtime, workers, owner registry and durable store composition are Rust | existing `G03-E04-EFFECT` and `G03-E05-POSTGRES` are not promoted to live authority by this component evidence; original-INVITE Oracle invocation, issuer/sink, live Endpoint, process-crash/two-node and native activation remain `not_run`; 10K/100K and every performance item are deferred |
...
| controlled PostgreSQL evidence | prior PostgreSQL bundles plus `evidence/raw/capability-recovery-oracle-204f4d5-17/`; `.78` bundle under `evidence/raw/durable-sip-runtime-composition-2ecfb72-18/`; `.79` through `.85` local-only bundles | prior restart/observation/derived-ACK cases plus full migration 001–116 and exact `.78` Rust startup-contract adapter `1/1`; `.85` reuses this physical contract but makes no new physical claim and did not contact the server | `G03-E05-POSTGRES` remains scoped component evidence; no live Endpoint, Linux product process, native authority or production promotion |
...
The `.81` bundle is also local functional evidence only. It proves one
confirmed recovered projection in the sole Active Call registry, exact pair
registration/rollback, bounded whole-Call Hangup and pointer-fenced cleanup on
the pinned sources. It makes no physical PostgreSQL claim and deliberately does
not relabel confirmed-dialog recovery as original-INVITE Oracle execution. The
server was not contacted. Real restart, live peer behavior, controller-panic
media cleanup, two-node recovery, live Endpoint, fault/OOM, production and all
deferred performance work remain `not_run`.

The `.82` bundle is local functional and source evidence only. It proves the
unwind helper and the presence of separately bounded controller/media cleanup
on the exact pinned source without adding a second task or Authority. It does
not inject the panic through a live Endpoint controller and does not prove
external media orphan reconciliation after a deadline. The server was not
contacted. Real task/process panic, process abort/OOM, real restart/two-node,
live Endpoint, fault/OOM, production and all deferred performance work remain
`not_run`.

The `.83` bundle is local functional and source evidence only. It proves the
shared media-deadline helper completes ordinary Futures and cancels a pending
Future at its test deadline, and that admission rejection uses the frozen
2-second production deadline. It does not prove cleanup inside an external
media service after cancellation. The server was not contacted. Live rejection,
external media orphan reconciliation, process abort/OOM, real restart/two-node,
live Endpoint, production and all deferred performance work remain `not_run`.

The `.84` bundle is local functional and source evidence only. It proves the
bounded two-worker supervisor reports unexpected close/stop/panic to one
controller, terminates only the exact recovered Call, cancels both workers on
controller exit and preserves terminal-event precedence. It does not inject a
worker fault through a live Endpoint or prove process-level fault survival. The
server was not contacted. Live worker fault injection, process abort/OOM,
external media orphan reconciliation, real restart/two-node, live Endpoint,
production and all deferred performance work remain `not_run`.

The `.85` bundle is local component evidence only. It combines the real worker
panic report, two real recovered registry entries and their exact cleanup
fences to prove the affected Call/pair is removed while the unrelated Call/pair
survives. It does not run the live Endpoint controller, inject a process abort
or OOM, block an external dependency, or reconcile external media. The server
was not contacted. Those campaigns, real restart/two-node, production and all
deferred performance work remain `not_run`.
