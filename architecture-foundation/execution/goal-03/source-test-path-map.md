...
| native rsipstack runtime and bounded mailboxes | `.57` build plus `rsipstack-ivekit-bounded-protocol-mailboxes.patch` and `rustpbx-ivekit-bounded-call-mailboxes.patch` | exact patch gates; locked `.57` Rust library suite 1977 passed + 5 ignored; `.53` remains the exact controlled image/wire/latency/interop/capacity source | controlled host evidence at source `b63383b`; `.57` host requalification and live Native Authority remain `G03-E16/not_run` |
| native durable rsipstack egress adapter | `rustpbx-converact-rsipstack-sip-effect-gate.patch`: bounded semantic-intent registry, exact canonical wire persistence, non-replayed send-attempt permit and fixed-shard non-blocking observation | 10 focused native tests; complete locked RustPBX library suite; `test/converact-rustpbx-rsipstack-sip-effect-gate-patch.test.ts` | compiled default-disabled adapter only; live endpoint activation, all-direction intent wiring, observer supervision and `G03-E16` remain `not_run` |
