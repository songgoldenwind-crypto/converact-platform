...
test('SipFoundation freezes one authority, exact current pins and bounded SLOs', () => {
  ...
  assert.equal(
    contract.native_call_recovery.recovered_active_call_installation,
    'one_existing_registry_slot_one_confirmed_inbound_native_leg_two_protocol_dialogs',
  );
  assert.equal(
    contract.native_call_recovery.recovered_control_scope,
    'bounded_whole_call_hangup_only_unsupported_commands_rejected_before_enqueue',
  );
  assert.equal(
    contract.native_call_recovery.recovered_controller_fault_boundary,
    'catch_unwind_inside_child_task_then_best_effort_terminate_exact_call',
  );
  assert.equal(contract.native_call_recovery.recovered_media_cleanup_deadline_ms, 2000);
  assert.equal(
    contract.native_call_recovery.recovered_controller_panic_cleanup_deadline_ms,
    8000,
  );
  assert.equal(
    contract.native_call_recovery.recovered_admission_rejection_cleanup,
    'bounded_by_recovered_media_cleanup_deadline_before_error_return',
  );
  assert.equal(contract.native_call_recovery.recovered_dialog_worker_exit_capacity, 2);
  assert.equal(
    contract.native_call_recovery.recovered_dialog_worker_supervision,
  ...
  assert.deepEqual(
    contract.native_matched_cancel_effects.server_functional_verification,
    capabilityRecoveryServerVerification,
  );
  assert.equal(
  ...
  assert.equal(
    contract.native_ordinary_response_effects.authority,
    'Unified_RustPBX_Native_Call_registry',
  );
  assert.equal(
    contract.native_ordinary_response_effects.implementation_status,
    'component_implemented_default_disabled',
  );
  assert.equal(
    contract.native_ordinary_response_effects.authority_order,
    'reserve_exact_Call_capability_then_register_exact_intent_then_durable_prepare_then_commit_Call_state_then_transport_send',
  );
  assert.deepEqual(
    contract.native_ordinary_response_effects.frozen_dialog_identity,
    [
      'SIP_Call_ID',
  ...
  assert.equal(
    contract.native_ordinary_response_effects.durable_prepare_cancellation_or_panic,
    'retain_exact_binding_for_query_reconcile',
  );
  assert.equal(
    contract.native_ordinary_response_effects.call_revision_race_after_durable_prepare,
  ...
  assert.equal(
    contract.native_call_cleanup_fencing.fence_cloning,
    'forbidden',
  );
  assert.equal(
  ...
  assert.equal(
    contract.native_call_capability_recovery.performance_verification,
    'not_run',
  );
  assert.deepEqual(
    contract.native_ordinary_response_effects.server_functional_verification,
    capabilityRecoveryServerVerification,
  );
  assert.equal(
    contract.native_ordinary_response_effects.performance_verification,
    'not_run',
  );
  assert.equal(
    contract.native_call_recovery.current_status,
    'component_implemented_local_exact_source_requalified_real_restart_and_live_takeover_not_run',
  );
