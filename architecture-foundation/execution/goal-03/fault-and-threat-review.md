...
bounds media cleanup before any recovered admission error returns. The
...
10. `.76` prevents empty-memory restart state from being mistaken for proof that
    ...
    rejection admission cleanup. This is source/local component evidence, not a
    live controller fault or orphan-media reconciliation campaign. No performance command ran; `.79`–`.83`
