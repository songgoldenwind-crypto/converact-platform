...
| queue/connection exhaustion | hard capacities, deterministic 503/Retry-After, no unbounded waiter/task | `.43` native suites pass locally; host saturation distribution `not_run` |
...
2. `.43` has not yet repeated the controlled rsipstack wire campaign.
...
5. SIPp/real peers, long call and host performance evidence have not run for
   the exact `.43` candidate; retained `.42` results are historical only.
