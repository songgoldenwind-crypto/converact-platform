...
5. Native Call/Leg/effect binding tests; the `.62` RustPBX port includes a
   ...
   requires one pre-registered peer-derived capability and passes exact-source
   suites. The gate remains default-disabled; retain activation `not_run` until
   its real Call Core holder, live Endpoint composition and reconciliation
   resume are wired, and until UAS-2xx ownership, parent-Unknown and stale-
