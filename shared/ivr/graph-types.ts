/**
 * Minimal IVR graph types for shared validation (designer + server).
 */
...
export function normalizeGraphForValidation(graph: IvrFlowGraph): IvrFlowGraph {
  return {
    ...
    nodes: graph.nodes.map((n) => {
      const node = n as IvrNodeBase & Record<string, unknown>;
      const { id, type, name, position, data: rawData, ...rest } = node;
      const baseData =
