...
export interface KamailioAcceptanceReport {
  schema_version: 1;
  suite: 'Converact Fabric Kamailio SIP Edge controlled acceptance';
...
function main(): void {
  ...
  const report = buildKamailioAcceptanceReport({
    ...
    kamailio_image: String(resolveFabricEnv(process.env, 'KAMAILIO_IMAGE') || ''),
    rustpbx_image: String(resolveFabricEnv(process.env, 'RUSTPBX_IMAGE') || ''),
