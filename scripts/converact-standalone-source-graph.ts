...
export interface ConveractFabricStandaloneSourceGraph {
  repoRoot: string;
  entrypoints: string[];
  files: string[];
  packages: string[];
...
export interface ConveractFabricStandaloneSourcePolicy {
  entrypoints: string[];
...
export function analyzeConveractFabricStandaloneSourceGraph(input: {
  repoRoot: string;
  entrypoints: string[];
}): ConveractFabricStandaloneSourceGraph {
...
export function assertConveractFabricStandaloneBoundary(
  graph: ConveractFabricStandaloneSourceGraph,
  forbiddenPrefixes: string[] = DEFAULT_FORBIDDEN_PREFIXES
): void {
  if (graph.unresolved.length > 0) {
    throw new Error(`Converact Fabric standalone source graph has unresolved imports:\n${graph.unresolved.join('\n')}`);
  }
  const forbidden = graph.files.filter((path) => forbiddenPrefixes.some((prefix) =>
    prefix.endsWith('/') ? path.startsWith(prefix) : path === prefix
  ));
  if (forbidden.length > 0) {
    throw new Error(`Converact Fabric standalone source graph crosses forbidden boundaries:\n${forbidden.join('\n')}`);
  }
}

export function readConveractFabricStandaloneSourcePolicy(repoRoot: string): ConveractFabricStandaloneSourcePolicy {
  return JSON.parse(readFileSync(
    join(repoRoot, 'services', 'converact-service', 'source-policy.json'),
    'utf8'
  )) as ConveractFabricStandaloneSourcePolicy;
}
...
function resolveInside(root: string, repoPath: string): string {
  ...
  if (absolute !== root && !absolute.startsWith(`${root}${sep}`)) {
    throw new Error(`Converact Fabric source path escapes repository: ${repoPath}`);
  }
  return absolute;
}
