      ...
      predicate: {
        buildDefinition: {
          ...
          resolvedDependencies: [
            {
              uri: `git+https://github.com/songgoldenwind-crypto/converact-platform@${
                identity.source_commit
              }`,
...
export async function collectRtpengineSupplyChainEvidence(
  ...
  const identity = await jsonFile<RtpengineSupplyChainInput['identity']>(
    requiredPath(env, 'CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_IDENTITY')
  );
  const cycloneDxPath = requiredPath(
    env,
    'CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_CYCLONEDX'
  );
  const spdxPath = requiredPath(env, 'CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_SPDX');
  const trivyPath = requiredPath(env, 'CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_TRIVY');
  const secretPath = requiredPath(
    env,
    'CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_SECRET_SCAN'
  );
  ...
  const exceptionsPath = optionalPath(
    env,
    'CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_EXCEPTIONS'
  );
  ...
  const signatureReference =
    env.CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_SIGNATURE_REFERENCE?.trim();
  const signatureBundlePath = optionalPath(
    env,
    'CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_SIGNATURE_BUNDLE'
  );
  ...
  const evidence = buildRtpengineSupplyChainEvidence({
    generated_at: required(env, 'CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_GENERATED_AT'),
    identity,
    cyclonedx: {
      sha256: cycloneDx.sha256,
  ...
  await writeNewJson(
    requiredPath(env, 'CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_PROVENANCE_OUTPUT'),
    evidence.provenance
  );
  await writeNewJson(
    requiredPath(env, 'CONVERACT_FABRIC_RTPENGINE_SUPPLY_CHAIN_EVIDENCE_OUTPUT'),
    evidence
  );
  return evidence;
}
...
if (process.argv[1] &&
  ...
  try {
    ...
    process.stderr.write(
      `Converact Fabric RTPengine supply-chain evidence failed: ${
        message.replace(/[\0\r\n]+/g, ' ').slice(0, 512)
      }\n`
    );
    process.exitCode = 1;
