...
export async function runCellAdmission(
  ...
  console.log(
    `[converact-cell-admission] standby on ${config.host}:${config.port} ` +
    ...
    leaseMaintainer = await startCellLeaseMaintainer({
      ...
      on_waiting(error) {
        ...
        if (waitingAttempts === 1 || waitingAttempts % 30 === 0) {
          console.log(
            '[converact-cell-admission] waiting for active Cell lease:',
      ...
      on_lost(error) {
        ...
        console.error(
          '[converact-cell-admission] cell lease lost; admission is draining:',
          error instanceof Error ? error.message : String(error)
        );
        stopAfterLeaseLoss?.();
      }
  ...
  const sweepTimer = setInterval(() => {
    ...
    sweepInFlight = reservationLedger.expireDue({
      ...
      controller?.startDrain(new Date());
      console.error(
        '[converact-cell-admission] reservation ledger sweep failed; admission is draining:',
        error instanceof Error ? error.message : String(error)
      );
    }).finally(() => {
      sweepInFlight = null;
    ...
    ? setInterval(() => {
        ...
        heartbeatInFlight = nodeSynchronizer.syncLeases({
          ...
          for (const failure of result.failed) {
            ...
            console.error(
              `[converact-cell-admission] component node heartbeat failed for ` +
    ...
    stopping = (async () => {
      ...
      if (nodeSynchronizer) {
        ...
        if (result.failed.length > 0) {
          console.error(
            '[converact-cell-admission] component node drain heartbeat failed:',
      ...
      try {
        ...
        console.error(
          '[converact-cell-admission] failed to release cell lease:',
          error instanceof Error ? error.message : String(error)
        );
        process.exitCode = 1;
      } finally {
      ...
      if (closeError) {
        console.error(
          '[converact-cell-admission] failed to close HTTP server:',
          closeError.message
        );
        process.exitCode = 1;
      }
  ...
  console.log(
    `[converact-cell-admission] listening on ${config.host}:${config.port} ` +
    `for ${config.region_id}/${config.zone_id}/${config.cell_id} ` +
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  runCellAdmission(cellAdmissionRuntimeConfig()).catch((error) => {
    console.error(
      '[converact-cell-admission] FATAL:',
      error instanceof Error ? error.message : String(error)
    );
    process.exitCode = 1;
  });
