...
import { buildConveractFabricStandaloneContext } from './converact-standalone-build-context.js';

const repoRoot = resolve(fileURLToPath(new URL('..', import.meta.url)));
const outputDir = resolve(
  resolveFabricEnv(process.env, 'STANDALONE_CONTEXT_DIR') || join(repoRoot, '.tmp', 'converact-standalone-context')
);
...
const entrypoints = [
  ...
  'converact-init-audit-runtime-role.js',
...
for (const entrypoint of entrypoints) {
  if (!existsSync(entrypoint)) {
    throw new Error(`Converact Fabric standalone build did not emit ${entrypoint}`);
  }
}
