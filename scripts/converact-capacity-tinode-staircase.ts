...
function parseArguments(argv: string[], env: NodeJS.ProcessEnv): TinodeStaircaseConfigInput {
  ...
  const rawPoints = String(
    values.get('--points') || resolveConveractEnv(
      env,
      'CONVERACT_FABRIC_TINODE_STAIRCASE_POINTS'
    ) || ''
  ).trim();
  return {
    output_file: required('--output-file', 'CONVERACT_FABRIC_TINODE_STAIRCASE_OUTPUT_FILE'),
    tinode_image: required('--tinode-image', 'CONVERACT_FABRIC_TINODE_IMAGE'),
    postgres_image: required('--postgres-image', 'CONVERACT_FABRIC_POSTGRES_IMAGE'),
    ...(rawPoints
      ? {
      ...
      : {}),
    ...optional('--tinode-port', 'CONVERACT_FABRIC_TINODE_STAIRCASE_PORT', 'tinode_port'),
    ...optional(
      '--connection-ramp-per-second',
      'CONVERACT_FABRIC_TINODE_CONNECTION_RAMP_PER_SECOND',
      'connection_ramp_per_second'
    ),
    ...optional(
      '--interaction-start-rate-per-second',
      'CONVERACT_FABRIC_TINODE_INTERACTION_START_RATE_PER_SECOND',
      'interaction_start_rate_per_second'
    ),
    ...optional(
      '--sample-interval-ms',
      'CONVERACT_FABRIC_TINODE_SAMPLE_INTERVAL_MS',
      'sample_interval_ms'
    )
  };
