...
export function createRtpengineAcceptanceCommand(input: {
  ...
  for (const [label, value] of [
    ...
    ['tenant ID', input.tenant_id ?? 'converact-acceptance'],
    ...
    ['Cell ID', input.cell_id ?? 'converact-acceptance-cell'],
    ['owner node ID', input.owner_node_id ?? 'converact-acceptance-node']
  ] as const) {
  ...
  return checkedMediaControlCommand({
    ...
    tenant_id: input.tenant_id ?? 'converact-acceptance',
    call_id: input.call_id,
    leg_id: 'converact-acceptance-leg',
    cell_id: input.cell_id ?? 'converact-acceptance-cell',
    owner_node_id: input.owner_node_id ?? 'converact-acceptance-node',
...
export async function runRtpengineMediaScenario(input: {
  ...
  let authority: RtpengineAcceptanceAdmissionIdentity = {
    ...
    tenant_id: 'converact-acceptance',
    cell_id: 'converact-acceptance-cell',
    owner_node_id: 'converact-acceptance-node',
    owner_epoch: input.owner_epoch
  };
