...
export interface ConveractFabricRustDeskLedExampleConfig {
  baseUrl: string;
  apiKey: string;
  tenantId: string;
  userId?: string;
...
export interface ConveractFabricRustDeskLedExampleResult {
  deviceId: string;
  rustdeskId: string;
  externalId: string;
  launchUrl: string;
...
export function createConveractFabricRustDeskLedExampleConfigFromEnv(env: NodeJS.ProcessEnv): ConveractFabricRustDeskLedExampleConfig {
  const rawBaseUrl =
  ...
  const remoteSessionId =
    ...
    resolveBrandEnv(env, 'RUSTDESK_FABRIC_REMOTE_SESSION_ID') ||
    resolveBrandEnv(env, 'RUSTDESK_IVEKIT_REMOTE_SESSION_ID') ||
    resolveBrandEnv(env, 'REMOTE_SESSION_ID') ||
    '';
  const deviceId = normalizedOptional(
    resolveBrandEnv(env, 'RUSTDESK_LED_EXAMPLE_DEVICE_ID') ||
    resolveBrandEnv(env, 'RUSTDESK_FABRIC_DEVICE_ID') ||
    resolveBrandEnv(env, 'RUSTDESK_IVEKIT_DEVICE_ID')
  );
  const rustdeskId = normalizedOptional(
    resolveBrandEnv(env, 'RUSTDESK_LED_EXAMPLE_RUSTDESK_ID') ||
    resolveBrandEnv(env, 'RUSTDESK_FABRIC_RUSTDESK_ID') ||
    resolveBrandEnv(env, 'RUSTDESK_IVEKIT_RUSTDESK_ID') ||
    resolveBrandEnv(env, 'RUSTDESK_EDGE_RUSTDESK_ID')
  );

  if (!rawBaseUrl) throw new Error('CONVERACT_RUSTDESK_LED_EXAMPLE_BASE_URL or CONVERACT_RUSTDESK_FABRIC_BASE_URL or CONVERACT_BASE_URL is required');
  if (!apiKey) throw new Error('CONVERACT_RUSTDESK_LED_EXAMPLE_API_KEY or CONVERACT_RUSTDESK_FABRIC_API_KEY or CONVERACT_COLLABORATION_API_KEY or CONVERACT_API_KEY is required');
  if (!tenantId) throw new Error('CONVERACT_RUSTDESK_LED_EXAMPLE_TENANT_ID or CONVERACT_RUSTDESK_FABRIC_TENANT_ID or CONVERACT_REMOTE_GATEWAY_TENANT_ID is required');
  ...
  return {
    ...
    userId: normalizedOptional(
      resolveBrandEnv(env, 'RUSTDESK_LED_EXAMPLE_USER_ID') ||
      resolveBrandEnv(env, 'RUSTDESK_FABRIC_USER_ID') ||
      resolveBrandEnv(env, 'RUSTDESK_IVEKIT_USER_ID')
    ),
    ...
    businessRef: {
      type: requiredString(
        ...
        resolveBrandEnv(env, 'RUSTDESK_FABRIC_BUSINESS_REF_TYPE') ||
      ...
      id: requiredString(
        ...
        resolveBrandEnv(env, 'RUSTDESK_FABRIC_BUSINESS_REF_ID') ||
      ...
      display_name: normalizedOptional(
        ...
        resolveBrandEnv(env, 'RUSTDESK_FABRIC_BUSINESS_REF_DISPLAY_NAME') ||
    ...
    deviceDisplayName: requiredString(
      ...
      resolveBrandEnv(env, 'RUSTDESK_FABRIC_DEVICE_DISPLAY_NAME') ||
    ...
    actorIdentity: requiredString(
      ...
      resolveBrandEnv(env, 'RUSTDESK_FABRIC_ACTOR_IDENTITY') ||
    ...
    permissions: splitScopes(
      resolveBrandEnv(env, 'RUSTDESK_LED_EXAMPLE_PERMISSIONS') ||
      resolveBrandEnv(env, 'RUSTDESK_FABRIC_CONSENT_SCOPES') ||
      resolveBrandEnv(env, 'RUSTDESK_IVEKIT_CONSENT_SCOPES')
    ),
...
export async function runConveractFabricRustDeskLedExample(
  config: ConveractFabricRustDeskLedExampleConfig,
  client: ConveractFabricRustDeskHttpClient = createConveractFabricRustDeskHttpClient({
    baseUrl: config.baseUrl,
    apiKey: config.apiKey,
    tenantId: config.tenantId,
    userId: config.userId
  })
): Promise<ConveractFabricRustDeskLedExampleResult> {
  ...
  const device = config.deviceId
    ...
    : await client.registerDevice({
      business_ref: {
        ...
        ...config.businessRef,
        metadata: {
          source: 'converact-rustdesk-led-example',
      ...
      metadata: { source: 'converact-rustdesk-led-example' }
    });
  ...
  if (config.postAuditProbe) {
    ...
    await client.recordGatewayEvent(externalId, {
      ...
      actor_identity: config.actorIdentity,
      target: rustdeskId,
      idempotency_key: `converact-rustdesk-led-example:${externalId}:control-action`,
      metadata: {
        source: 'converact-rustdesk-led-example',
        operation_id: `converact-rustdesk-led-example-${externalId}`,
...
}): ConveractFabricRustDeskLedExampleResult['clientConfig'] {
...
async function main(): Promise<void> {
  const result = await runConveractFabricRustDeskLedExample(createConveractFabricRustDeskLedExampleConfigFromEnv(process.env));
