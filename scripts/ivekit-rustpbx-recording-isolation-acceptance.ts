...
export function createRustPbxRecordingIsolationRuntimePlan(
  ...
  const runtimeDirectory = absolutePath(
    env.IVEKIT_RUSTPBX_RECORDING_ISOLATION_DIR,
    'runtime directory'
  );
  return {
    compose_file: absolutePath(
      env.IVEKIT_RUSTPBX_RECORDING_ISOLATION_COMPOSE_FILE,
      'Compose file'
    ),
    ...
    rustpbx_image: exactRustPbxImage(
      env.IVEKIT_RUSTPBX_RECORDING_ISOLATION_IMAGE
    ),
    postgres_image: digestImage(
      env.IVEKIT_RUSTPBX_RECORDING_ISOLATION_POSTGRES_IMAGE,
      'PostgreSQL image'
    ),
    node_image: digestImage(
      env.IVEKIT_RUSTPBX_RECORDING_ISOLATION_NODE_IMAGE,
      'Node image'
    ),
    python_image: digestImage(
      env.IVEKIT_RUSTPBX_RECORDING_ISOLATION_PYTHON_IMAGE,
      'Python image'
    ),
    fault_image: digestImage(
      env.IVEKIT_RUSTPBX_RECORDING_ISOLATION_FAULT_IMAGE,
      'fault injector image'
    ),
    sipp_image: digestImage(
      env.IVEKIT_RUSTPBX_RECORDING_ISOLATION_SIPP_IMAGE,
      'SIPp runtime image'
    ),
    sipp_binary: absolutePath(env.IVEKIT_SIPP_BINARY, 'SIPp binary'),
    network: 'ivekit-rustpbx-recording-isolation',
    rustpbx_ip: '172.30.45.10',
    uac_ip: '172.30.45.20',
    uas_ip: '172.30.45.21'
...
export async function runRustPbxRecordingIsolationAcceptance(
  ...
  const composeEnv = {
    ...
    IVEKIT_RUSTPBX_RECORDING_ISOLATION_NETWORK: plan.network,
...
description = "iveKit recording isolation"
...
function exactRustPbxImage(value: string | undefined): string {
  ...
  if (/\s/.test(image)
    ...
    throw new Error('RustPBX image must use an exact iveKit release tag or digest');
  }
  return image;
}
