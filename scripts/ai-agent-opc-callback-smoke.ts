...
export interface AiAgentOpcCallbackSmokeStep {
  name: string;
  status: number;
}

export interface AiAgentOpcCallbackSmokeResult {
  roomName: string;
  actionTaken: string;
  steps: AiAgentOpcCallbackSmokeStep[];
}
...
export async function runAiAgentOpcCallbackSmoke(
  config: AiAgentOpcCallbackSmokeConfig,
  fetchImpl: FetchLike = fetch
): Promise<AiAgentOpcCallbackSmokeResult> {
  const steps: AiAgentOpcCallbackSmokeStep[] = [];
...
async function main(): Promise<void> {
  const config = createAiAgentOpcCallbackSmokeConfigFromEnv(process.env);
  const result = await runAiAgentOpcCallbackSmoke(config);
