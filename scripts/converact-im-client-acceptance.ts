...
export const CONVERACT_IM_REQUIRED_ACCEPTANCE_CHECKS = Object.freeze([
...
type CheckId = typeof CONVERACT_IM_REQUIRED_ACCEPTANCE_CHECKS[number];

export interface ConveractFabricImAcceptanceFailure { id: string; reason: string; }
export interface ConveractFabricImAcceptanceResult {
  ok: boolean;
  status: 'ready_for_review' | 'incomplete';
  run_id: string;
  environment_id: string;
  summary: { required: number; passed: number; failed: number };
  failures: ConveractFabricImAcceptanceFailure[];
}

export function createConveractFabricImAcceptanceTemplate(): {
  schema_version: 1;
  ...
  const checks = Object.fromEntries(CONVERACT_IM_REQUIRED_ACCEPTANCE_CHECKS.map((id) => [id, {
    passed: false,
    evidence: {
  ...
  return {
    ...
    endpoints: { converact: 'https://replace-with-converact-host', tinode: 'wss://replace-with-tinode-host/v0/channels' },
    checks
  };
}

export function writeConveractFabricImAcceptanceTemplate(templateFile: string): string {
  ...
  writeFileSync(resolve(templateFile), `${JSON.stringify(createConveractFabricImAcceptanceTemplate(), null, 2)}\n`, 'utf8');
  return resolve(templateFile);
}

export function runConveractFabricImAcceptance(config: { reportFile: string; outputFile?: string }): ConveractFabricImAcceptanceResult {
  ...
  const failures: ConveractFabricImAcceptanceFailure[] = [];
  ...
  const result: ConveractFabricImAcceptanceResult = {
    ok: failures.length === 0,
    status: failures.length ? 'incomplete' : 'ready_for_review',
    run_id: runId,
    environment_id: environmentId,
    summary: { required: CONVERACT_IM_REQUIRED_ACCEPTANCE_CHECKS.length, passed, failed: CONVERACT_IM_REQUIRED_ACCEPTANCE_CHECKS.length - passed },
    failures
  };
...
export function runConveractFabricImAcceptanceFromEnv(env: Record<string, string | undefined>):
  | ConveractFabricImAcceptanceResult
  ...
  if (templateFile) {
    return {
      ...
      template_file: writeConveractFabricImAcceptanceTemplate(templateFile)
    };
  }
  ...
  return runConveractFabricImAcceptance({
    reportFile,
    outputFile: String(resolveFabricEnv(env, 'IM_ACCEPTANCE_OUTPUT_FILE') || '').trim() || undefined
  });
}

export function renderConveractFabricImAcceptanceRunbook(): string {
  return `# Converact Fabric IM real-environment acceptance\n\n` +
    `1. Open two real browsers on different user identities: one agent and one customer.\n` +
    `2. Connect both clients to the deployed Converact Fabric facade and real Tinode WSS endpoint.\n` +
...
function validateIdentities(value: unknown, failures: ConveractFabricImAcceptanceFailure[]) {
...
function validateEndpoints(value: unknown, failures: ConveractFabricImAcceptanceFailure[]) {
  const endpoints = objectValue(value);
  const fabricEndpoint = endpoints.converact ?? endpoints.ivekit;
  if (!/^https:\/\//.test(String(fabricEndpoint || '')) || !/^wss:\/\//.test(String(endpoints.tinode || '')) || containsPlaceholder(endpoints)) {
    fail(failures, 'report.endpoints', 'HTTPS Converact Fabric and WSS Tinode endpoints are required');
  }
}
...
function required(value: unknown, id: string, failures: ConveractFabricImAcceptanceFailure[]): string {
...
function fail(failures: ConveractFabricImAcceptanceFailure[], id: string, reason: string) { failures.push({ id, reason }); }

if (fileURLToPath(import.meta.url) === resolve(process.argv[1] || '')) {
  const result = runConveractFabricImAcceptanceFromEnv(process.env);
