...
export function createRustDeskEdgeAgentConfigFromEnv(env: NodeJS.ProcessEnv): RustDeskEdgeAgentConfig {
  const rawBaseUrl = resolveBrandEnv(env, 'RUSTDESK_EDGE_BASE_URL') || resolveBrandEnv(env, 'BASE_URL') || resolveBrandEnv(env, 'COLLABORATION_BASE_URL') || '';
  const baseUrlEnvName = resolveBrandEnv(env, 'RUSTDESK_EDGE_BASE_URL')
    ? 'CONVERACT_RUSTDESK_EDGE_BASE_URL'
    : resolveBrandEnv(env, 'BASE_URL')
      ? 'CONVERACT_BASE_URL'
      : 'CONVERACT_COLLABORATION_BASE_URL';
  const deviceToken = resolveDeviceToken(env);
  const deviceTokenFile = String(resolveBrandEnv(env, 'RUSTDESK_EDGE_DEVICE_TOKEN_FILE') || '').trim();
  const apiKey = String(
    deviceToken ||
  ...
  const tenantId = String(resolveBrandEnv(env, 'RUSTDESK_EDGE_TENANT_ID') || resolveBrandEnv(env, 'REMOTE_GATEWAY_TENANT_ID') || '').trim();
  const businessRefType = String(resolveBrandEnv(env, 'RUSTDESK_EDGE_BUSINESS_REF_TYPE') || '').trim();
  const businessRefId = String(resolveBrandEnv(env, 'RUSTDESK_EDGE_BUSINESS_REF_ID') || '').trim();
  const rustdeskId = resolveRustDeskId(env);
  const metadata = parseMetadataJson(resolveBrandEnv(env, 'RUSTDESK_EDGE_METADATA_JSON'));
  const clientVersion = edgeVersion(
    resolveBrandEnv(env, 'RUSTDESK_EDGE_CLIENT_VERSION') || metadata.client_version
  );
  const edgeOs = edgeOperatingSystem(resolveBrandEnv(env, 'RUSTDESK_EDGE_OS') || metadata.os);
  const placementEnabled = envFlag(resolveFabricEnv(env, 'PLACEMENT_ENABLED'));
  const nativeControlProtocol = String(
    resolveBrandEnv(env, 'RUSTDESK_NATIVE_CONTROL_PROTOCOL') ||
    (placementEnabled
      ? 'ivekit-rustdesk-native-control-v2'
      : 'ivekit-rustdesk-native-control-v1')
  ...
  if (
    ...
    throw new Error('CONVERACT_RUSTDESK_NATIVE_CONTROL_PROTOCOL is invalid');
  }
  ...
  if (!stripTrailingSlash(rawBaseUrl)) throw new Error('CONVERACT_RUSTDESK_EDGE_BASE_URL or CONVERACT_BASE_URL is required');
  const baseUrl = normalizeHttpBaseUrl(rawBaseUrl, baseUrlEnvName);
  if (!apiKey) throw new Error('CONVERACT_RUSTDESK_EDGE_API_KEY or CONVERACT_COLLABORATION_API_KEY or CONVERACT_API_KEY is required');
  if (!tenantId) throw new Error('CONVERACT_RUSTDESK_EDGE_TENANT_ID is required');
  if (!businessRefType) throw new Error('CONVERACT_RUSTDESK_EDGE_BUSINESS_REF_TYPE is required');
  if (!businessRefId) throw new Error('CONVERACT_RUSTDESK_EDGE_BUSINESS_REF_ID is required');
  if (!rustdeskId) throw new Error('CONVERACT_RUSTDESK_EDGE_RUSTDESK_ID or RUSTDESK_ID is required');

  const heartbeatIntervalMs = parseHeartbeatIntervalMs(resolveBrandEnv(env, 'RUSTDESK_EDGE_HEARTBEAT_INTERVAL_MS'));
  ...
  const commandLeaseMs = parseBoundedInteger(
    resolveBrandEnv(env, 'RUSTDESK_EDGE_COMMAND_LEASE_MS'),
  ...
  const commandTimeoutMs = parseBoundedInteger(
    resolveBrandEnv(env, 'RUSTDESK_EDGE_COMMAND_TIMEOUT_MS'),
  ...
  if (commandLeaseMs < rustDeskMinimumCommandLeaseMs(commandTimeoutMs)) {
    throw new Error(
      'CONVERACT_RUSTDESK_EDGE_COMMAND_LEASE_MS must cover primary and fallback timeouts plus reporting margin'
    );
  }
  const disconnectAdapter = parseEdgeAdapter(
    resolveBrandEnv(env, 'RUSTDESK_EDGE_DISCONNECT_EXECUTABLE'),
    resolveBrandEnv(env, 'RUSTDESK_EDGE_DISCONNECT_ARGS_JSON'),
  ...
  const restartAdapter = parseEdgeAdapter(
    resolveBrandEnv(env, 'RUSTDESK_EDGE_RESTART_EXECUTABLE'),
    resolveBrandEnv(env, 'RUSTDESK_EDGE_RESTART_ARGS_JSON'),
  ...
  if ((disconnectAdapter || restartAdapter) && !commandToken) {
    throw new Error('CONVERACT_RUSTDESK_EDGE_COMMAND_TOKEN is required when a command adapter is configured');
  }
  const spoolDir = String(resolveBrandEnv(env, 'RUSTDESK_EDGE_SPOOL_DIR') || '').trim();
  if ((disconnectAdapter || restartAdapter) && !spoolDir) {
    throw new Error('CONVERACT_RUSTDESK_EDGE_SPOOL_DIR is required when a command adapter is configured');
  }
  if (spoolDir && !isAbsolutePath(spoolDir)) {
    throw new Error('CONVERACT_RUSTDESK_EDGE_SPOOL_DIR must be an absolute path');
  }
  const observationInputDir = String(resolveBrandEnv(env, 'RUSTDESK_EDGE_OBSERVATION_INPUT_DIR') || '').trim();
  const observationSpoolDir = String(resolveBrandEnv(env, 'RUSTDESK_EDGE_OBSERVATION_SPOOL_DIR') || '').trim();
  ...
  if ((observationInputDir || observationSpoolDir) && !deviceTokenFile) {
    throw new Error('CONVERACT_RUSTDESK_EDGE_DEVICE_TOKEN_FILE is required for RustDesk observations');
  }
  for (const [path, name] of [
    [observationInputDir, 'CONVERACT_RUSTDESK_EDGE_OBSERVATION_INPUT_DIR'],
    [observationSpoolDir, 'CONVERACT_RUSTDESK_EDGE_OBSERVATION_SPOOL_DIR']
  ] as const) {
    if (path && !isAbsolutePath(path)) throw new Error(`${name} must be an absolute path`);
  }
  const evidenceInputDir = String(resolveBrandEnv(env, 'RUSTDESK_EDGE_EVIDENCE_INPUT_DIR') || '').trim();
  const evidenceSpoolDir = String(resolveBrandEnv(env, 'RUSTDESK_EDGE_EVIDENCE_SPOOL_DIR') || '').trim();
  ...
  const evidenceConfig = evidenceInputDir
    ? createRustDeskEvidenceUploaderConfigFromEnv({
      ...env,
      CONVERACT_RUSTDESK_EDGE_BASE_URL: baseUrl,
      CONVERACT_RUSTDESK_EDGE_DEVICE_TOKEN_FILE: deviceTokenFile,
      CONVERACT_RUSTDESK_EDGE_OBSERVATION_INPUT_DIR: observationInputDir
    })
    : null;
  const nativeEvidenceEventDir = String(resolveBrandEnv(env, 'RUSTDESK_NATIVE_EVIDENCE_EVENT_DIR') || '').trim();
  const nativeEvidenceCandidateDir = String(
    resolveBrandEnv(env, 'RUSTDESK_NATIVE_EVIDENCE_CANDIDATE_DIR') || ''
  ).trim();
  const nativeEvidenceSpoolDir = String(resolveBrandEnv(env, 'RUSTDESK_NATIVE_EVIDENCE_SPOOL_DIR') || '').trim();
  if (
    new Set([
  ...
  const nativeEvidenceConfig = nativeEvidenceEventDir
    ? createRustDeskNativeEvidenceWatcherConfigFromEnv({
      ...env,
      CONVERACT_RUSTDESK_EDGE_EVIDENCE_INPUT_DIR: evidenceInputDir,
      CONVERACT_RUSTDESK_NATIVE_EVIDENCE_EVENT_DIR: nativeEvidenceEventDir,
      CONVERACT_RUSTDESK_NATIVE_EVIDENCE_SPOOL_DIR: nativeEvidenceSpoolDir
    })
    : null;
  return {
    baseUrl,
    ...
    rustdeskId,
    displayName: String(resolveBrandEnv(env, 'RUSTDESK_EDGE_DEVICE_DISPLAY_NAME') || env.HOSTNAME || rustdeskId).trim(),
    actorIdentity: String(resolveBrandEnv(env, 'RUSTDESK_EDGE_ACTOR_IDENTITY') || 'rustdesk-edge-agent').trim(),
    runtimeStatus: parseRuntimeStatus(resolveBrandEnv(env, 'RUSTDESK_EDGE_RUNTIME_STATUS')),
    heartbeatIntervalMs,
    edgeInstanceId: String(
      resolveBrandEnv(env, 'RUSTDESK_EDGE_INSTANCE_ID') ||
    ...
    ...(spoolDir
      ? {
        ...
        spoolMaxBytes: parseBoundedInteger(
          resolveBrandEnv(env, 'RUSTDESK_EDGE_SPOOL_MAX_BYTES'),
          'CONVERACT_RUSTDESK_EDGE_SPOOL_MAX_BYTES',
          64 * 1_024,
          1_024,
          1_048_576
        ),
        spoolMaxAgeMs: parseBoundedInteger(
          resolveBrandEnv(env, 'RUSTDESK_EDGE_SPOOL_MAX_AGE_MS'),
          'CONVERACT_RUSTDESK_EDGE_SPOOL_MAX_AGE_MS',
          7 * 24 * 60 * 60 * 1_000,
          1_000,
          365 * 24 * 60 * 60 * 1_000
        ),
        spoolMaxQuarantineRecords: parseBoundedInteger(
          resolveBrandEnv(env, 'RUSTDESK_EDGE_SPOOL_MAX_QUARANTINE_RECORDS'),
          'CONVERACT_RUSTDESK_EDGE_SPOOL_MAX_QUARANTINE_RECORDS',
          100,
          1,
          10_000
        )
    ...
    ...(observationInputDir
      ? {
        ...
        observationPollIntervalMs: parseBoundedInteger(
          resolveBrandEnv(env, 'RUSTDESK_EDGE_OBSERVATION_POLL_INTERVAL_MS'),
          'CONVERACT_RUSTDESK_EDGE_OBSERVATION_POLL_INTERVAL_MS',
          2_000,
          250,
          300_000
        ),
        observationBatchSize: parseBoundedInteger(
          resolveBrandEnv(env, 'RUSTDESK_EDGE_OBSERVATION_BATCH_SIZE'),
          'CONVERACT_RUSTDESK_EDGE_OBSERVATION_BATCH_SIZE',
          20,
          1,
          100
        ),
        observationRetryDelayMs: parseBoundedInteger(
          resolveBrandEnv(env, 'RUSTDESK_EDGE_OBSERVATION_RETRY_DELAY_MS'),
          'CONVERACT_RUSTDESK_EDGE_OBSERVATION_RETRY_DELAY_MS',
          5_000,
          0,
          3_600_000
        ),
        observationMaxAttempts: parseBoundedInteger(
          resolveBrandEnv(env, 'RUSTDESK_EDGE_OBSERVATION_MAX_ATTEMPTS'),
          'CONVERACT_RUSTDESK_EDGE_OBSERVATION_MAX_ATTEMPTS',
          10,
          1,
          100
        ),
        observationMaxInputBytes: parseBoundedInteger(
          resolveBrandEnv(env, 'RUSTDESK_EDGE_OBSERVATION_MAX_INPUT_BYTES'),
          'CONVERACT_RUSTDESK_EDGE_OBSERVATION_MAX_INPUT_BYTES',
          64 * 1_024,
          1_024,
          1_048_576
        ),
        observationMaxQuarantineRecords: parseBoundedInteger(
          resolveBrandEnv(env, 'RUSTDESK_EDGE_OBSERVATION_MAX_QUARANTINE_RECORDS'),
          'CONVERACT_RUSTDESK_EDGE_OBSERVATION_MAX_QUARANTINE_RECORDS',
          100,
          1,
          10_000
        )
    ...
    ...(evidenceConfig
      ? {
        ...
        evidencePollIntervalMs: parseBoundedInteger(
          resolveBrandEnv(env, 'RUSTDESK_EDGE_EVIDENCE_POLL_INTERVAL_MS'),
          'CONVERACT_RUSTDESK_EDGE_EVIDENCE_POLL_INTERVAL_MS',
          2_000,
          250,
          300_000
        ),
    ...
    ...(nativeEvidenceConfig
      ? {
        ...
        nativeEvidenceMaxCandidateBytes: parseBoundedInteger(
          resolveBrandEnv(env, 'RUSTDESK_NATIVE_EVIDENCE_MAX_CANDIDATE_BYTES'),
          'CONVERACT_RUSTDESK_NATIVE_EVIDENCE_MAX_CANDIDATE_BYTES',
          64 * 1_024,
          1_024,
          1_048_576
        ),
        nativeEvidenceMaxPendingMs: parseBoundedInteger(
          resolveBrandEnv(env, 'RUSTDESK_NATIVE_EVIDENCE_MAX_PENDING_MS'),
          'CONVERACT_RUSTDESK_NATIVE_EVIDENCE_MAX_PENDING_MS',
          15 * 60_000,
          30_000,
          86_400_000
        )
      ...
      : {}),
    offlineOnExit: envFlag(resolveBrandEnv(env, 'RUSTDESK_EDGE_OFFLINE_ON_EXIT')),
    metadata: {
      ...metadata,
...
function parseHeartbeatIntervalMs(rawIntervalMs: string | undefined): number {
  ...
  if (!Number.isFinite(intervalMs) || intervalMs < 10_000) {
    throw new Error('CONVERACT_RUSTDESK_EDGE_HEARTBEAT_INTERVAL_MS must be a number >= 10000');
  }
  return intervalMs;
}
...
function resolveRustDeskId(env: NodeJS.ProcessEnv): string {
  const direct = String(resolveBrandEnv(env, 'RUSTDESK_EDGE_RUSTDESK_ID') || env.RUSTDESK_ID || '').trim();
  if (direct) return direct;
  const filePath = String(resolveBrandEnv(env, 'RUSTDESK_EDGE_RUSTDESK_ID_FILE') || '').trim();
  if (!filePath) return '';
  let fileValue = '';
  try {
    fileValue = readFileSync(filePath, 'utf8').trim();
  } catch {
    throw new Error(`CONVERACT_RUSTDESK_EDGE_RUSTDESK_ID_FILE cannot be read: ${filePath}`);
  }
  if (!fileValue) throw new Error(`CONVERACT_RUSTDESK_EDGE_RUSTDESK_ID_FILE is empty: ${filePath}`);
  return fileValue;
}

function resolveCommandToken(env: NodeJS.ProcessEnv): string {
  const direct = String(resolveBrandEnv(env, 'RUSTDESK_EDGE_COMMAND_TOKEN') || '').trim();
  if (direct) return direct;
  const filePath = String(resolveBrandEnv(env, 'RUSTDESK_EDGE_COMMAND_TOKEN_FILE') || '').trim();
  if (!filePath) return '';
  let fileValue = '';
  try {
    fileValue = readFileSync(filePath, 'utf8').trim();
  } catch {
    throw new Error(`CONVERACT_RUSTDESK_EDGE_COMMAND_TOKEN_FILE cannot be read: ${filePath}`);
  }
  if (!fileValue) throw new Error(`CONVERACT_RUSTDESK_EDGE_COMMAND_TOKEN_FILE is empty: ${filePath}`);
  return fileValue;
}

function resolveDeviceToken(env: NodeJS.ProcessEnv): string {
  const filePath = String(resolveBrandEnv(env, 'RUSTDESK_EDGE_DEVICE_TOKEN_FILE') || '').trim();
  if (!filePath) return '';
  if (
    String(
      resolveBrandEnv(env, 'RUSTDESK_EDGE_API_KEY') ||
    ...
    throw new Error(
      'CONVERACT_RUSTDESK_EDGE_DEVICE_TOKEN_FILE must not be combined with legacy API or command credentials'
    );
  }
  let fileValue = '';
  try {
    ...
    if ((error as Error).message === 'symbolic-link') {
      throw new Error(`CONVERACT_RUSTDESK_EDGE_DEVICE_TOKEN_FILE must not be a symbolic link: ${filePath}`);
    }
    throw new Error(`CONVERACT_RUSTDESK_EDGE_DEVICE_TOKEN_FILE cannot be read: ${filePath}`);
  }
  if (fileValue.length < 32 || fileValue.length > 4_096 || /\s/.test(fileValue)) {
    throw new Error(`CONVERACT_RUSTDESK_EDGE_DEVICE_TOKEN_FILE content is invalid: ${filePath}`);
  }
  return fileValue;
}
...
function parseMetadataJson(value: string | undefined): Record<string, unknown> {
  ...
  try {
    ...
    throw new Error('CONVERACT_RUSTDESK_EDGE_METADATA_JSON must be a JSON object');
  }
}
...
function parseRuntimeStatus(value: string | undefined): 'online' | 'offline' {
  ...
  throw new Error('CONVERACT_RUSTDESK_EDGE_RUNTIME_STATUS must be online or offline');
}

function edgeVersion(value: unknown): string {
  const version = String(value || '').trim();
  if (version && !/^[a-zA-Z0-9._+-]{1,64}$/.test(version)) {
    throw new Error('CONVERACT_RUSTDESK_EDGE_CLIENT_VERSION must contain 1 to 64 version characters');
  }
  return version;
}
...
function edgeOperatingSystem(value: unknown): string {
  ...
  if (
    ...
    throw new Error('CONVERACT_RUSTDESK_EDGE_OS must be a supported operating system');
  }
  return os;
}
...
async function main(): Promise<void> {
  ...
  if (envFlag(resolveBrandEnv(process.env, 'RUSTDESK_EDGE_ONCE'))) {
...
function requiredCommandToken(value: string | undefined): string {
  ...
  if (!token) throw new Error('CONVERACT_RUSTDESK_EDGE_COMMAND_TOKEN is required');
  return token;
}
