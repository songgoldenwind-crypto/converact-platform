...
const runId = required('CONVERACT_FABRIC_ACCEPTANCE_RUN_ID');
if (!/^[a-z0-9][a-z0-9-]{0,31}$/.test(runId)) {
  throw new Error('invalid CONVERACT_FABRIC_ACCEPTANCE_RUN_ID');
}
...
const providerName = `converact-acceptance-${runId}`;
...
const trunkInput = {
  ...
  desired_state: {
    ...
    name: `Converact Fabric acceptance ${runId}`,
    direction: 'both',
    transport: 'udp',
    codecs: ['PCMU'],
    max_channels: 4,
