...
export interface IveKitEventWsShardInput {
  endpoint: string;
  run_id: string;
  shard_id: string;
...
export interface IveKitEventWsShardResult {
...
export async function runIveKitEventWsShard(
  input: IveKitEventWsShardInput
): Promise<IveKitEventWsShardResult> {
  validateInput(input);
...
function validateInput(input: IveKitEventWsShardInput): void {
  const endpoint = new URL(input.endpoint);
