...
export type ConveractFabricVoiceAcceptanceDeploymentMode =
  | 'standalone-compose'
  | 'standalone-helm'
  | 'external-provider';
...
export interface ConveractFabricVoiceAcceptanceEvidenceReference {
...
export interface ConveractFabricVoiceAcceptanceConfig {
  reportFile: string;
  outputFile?: string;
}

export interface ConveractFabricVoiceAcceptanceTemplateConfig {
  templateFile?: string;
  runId: string;
  environmentId: string;
  deploymentMode: ConveractFabricVoiceAcceptanceDeploymentMode;
...
export interface ConveractFabricVoiceAcceptanceFailure {
  id: string;
  reason: string;
}

export interface ConveractFabricVoiceAcceptanceResult {
  schema_version: 1;
  ok: boolean;
  ...
  failures: ConveractFabricVoiceAcceptanceFailure[];
}
...
const DEPLOYMENT_MODES = new Set<ConveractFabricVoiceAcceptanceDeploymentMode>([
  'standalone-compose',
  'standalone-helm',
  'external-provider'
]);
...
export function createConveractFabricVoiceAcceptanceTemplate(
  config: Omit<ConveractFabricVoiceAcceptanceTemplateConfig, 'templateFile'>
...
export function writeConveractFabricVoiceAcceptanceTemplate(
  config: ConveractFabricVoiceAcceptanceTemplateConfig
  ...
  const template = createConveractFabricVoiceAcceptanceTemplate(config);
...
export function runConveractFabricVoiceAcceptance(
  config: ConveractFabricVoiceAcceptanceConfig
): ConveractFabricVoiceAcceptanceResult {
  const reportFile = resolve(config.reportFile);
  const report = readJsonObject(reportFile);
  const failures: ConveractFabricVoiceAcceptanceFailure[] = [];
  let missing = 0;
  let passedChecks = 0;
  ...
  const deploymentMode = String(report.deployment_mode || '') as ConveractFabricVoiceAcceptanceDeploymentMode;
  ...
  const result: ConveractFabricVoiceAcceptanceResult = {
    schema_version: 1,
...
export function runConveractFabricVoiceAcceptanceFromEnv(
  ...
  return runConveractFabricVoiceAcceptance({
    reportFile,
    outputFile: optional(resolveFabricEnv(env, 'VOICE_ACCEPTANCE_OUTPUT_FILE'))
  });
}

export function renderConveractFabricVoiceAcceptanceRunbook(): string {
  ...
  const lines = [
    '# Converact Fabric Voice Real Environment Acceptance Runbook',
    '',
...
function templateConfigFromEnv(env: NodeJS.ProcessEnv): ConveractFabricVoiceAcceptanceTemplateConfig {
  const mode = String(resolveFabricEnv(env, 'VOICE_ACCEPTANCE_DEPLOYMENT_MODE') || 'standalone-compose') as ConveractFabricVoiceAcceptanceDeploymentMode;
...
if (invokedPath === fileURLToPath(import.meta.url)) {
  try {
    if (resolveFabricEnv(process.env, 'VOICE_ACCEPTANCE_TEMPLATE_FILE')) {
      ...
      const result = writeConveractFabricVoiceAcceptanceTemplate(config);
      ...
      if (runbookFile) {
        ...
        writeFileSync(resolve(runbookFile), renderConveractFabricVoiceAcceptanceRunbook(), 'utf8');
      }
      process.stdout.write(`${JSON.stringify({ ...result, runbook_file: runbookFile || '' }, null, 2)}\n`);
    } else {
      const result = runConveractFabricVoiceAcceptanceFromEnv(process.env);
