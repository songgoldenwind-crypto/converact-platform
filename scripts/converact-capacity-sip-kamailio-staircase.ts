...
export function sipKamailioStaircaseConfigFromEnv(
  ...
  return {
    output_file: absoluteRequired(env, 'CONVERACT_FABRIC_SIP_STAIRCASE_OUTPUT'),
    artifact_root: absoluteRequired(env, 'CONVERACT_FABRIC_SIP_STAIRCASE_ARTIFACT_ROOT'),
    runtime_root: absoluteRequired(env, 'CONVERACT_FABRIC_SIP_STAIRCASE_RUNTIME_ROOT'),
    points: points(resolveConveractEnv(
      env,
      'CONVERACT_FABRIC_SIP_STAIRCASE_POINTS'
    ) || '100,250,500,750,1000'),
    duration_seconds: integer(resolveConveractEnv(
      env,
      'CONVERACT_FABRIC_SIP_STAIRCASE_DURATION_SECONDS'
    ), 20),
    sipp_binary: absoluteRequired(env, 'CONVERACT_FABRIC_SIPP_BINARY'),
    ...
    node_command: resolve(resolveConveractEnv(
      env,
      'CONVERACT_FABRIC_NODE_COMMAND'
    ) || process.execPath),
...
function points(value: string): number[] {
  ...
  if (output.some((entry) => !Number.isInteger(entry))) {
    throw new Error('CONVERACT_FABRIC_SIP_STAIRCASE_POINTS must be comma-separated integers');
  }
  return output;
}
