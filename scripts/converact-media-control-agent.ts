...
const production = booleanEnv('CONVERACT_FABRIC_MEDIA_CONTROL_PRODUCTION', true);
const requireMtls = booleanEnv(
  'CONVERACT_FABRIC_MEDIA_CONTROL_REQUIRE_MTLS',
  production
);
const transportMode = stringEnv(
  'CONVERACT_FABRIC_MEDIA_CONTROL_TRANSPORT',
  production ? 'hybrid' : 'simulator'
);
if (!['simulator', 'rtpengine', 'hybrid'].includes(transportMode)) {
  throw new Error('Converact media control transport is unsupported');
}
if (production && !requireMtls) {
  throw new Error('Converact media control production mTLS cannot be disabled');
}
if (production && transportMode === 'simulator') {
  throw new Error('Converact media control simulator is not production eligible');
}
...
const terminalEventJournal = transportMode === 'hybrid'
  ? await MediaTerminalEventJournal.open({
      path: join(
        requiredEnv('CONVERACT_FABRIC_MEDIA_CONTROL_WAL_DIRECTORY'),
        'media-terminal-event.wal'
      ),
      maxRecords: integerEnv(
        'CONVERACT_FABRIC_MEDIA_CONTROL_EVENT_WAL_MAX_RECORDS',
        1_000_000,
        1,
        10_000_000
      ),
      maxBytes: integerEnv(
        'CONVERACT_FABRIC_MEDIA_CONTROL_EVENT_WAL_MAX_BYTES',
        268_435_456,
        512,
        17_179_869_184
      ),
      maxRecordBytes: integerEnv(
        'CONVERACT_FABRIC_MEDIA_CONTROL_EVENT_WAL_MAX_RECORD_BYTES',
        262_144,
        256,
        4_194_304
      )
...
const processingEventHandoff = transportRuntime.processing
  ? new ProcessingEventHandoff({
      ...
      batch_size: integerEnv(
        'CONVERACT_FABRIC_PROCESSING_EVENT_BATCH_SIZE',
        256,
        1,
        10_000
      ),
      poll_interval_ms: integerEnv(
        'CONVERACT_FABRIC_PROCESSING_EVENT_POLL_INTERVAL_MS',
        100,
        1,
        300_000
      ),
      retry_base_ms: integerEnv(
        'CONVERACT_FABRIC_PROCESSING_EVENT_RETRY_BASE_MS',
        100,
        1,
        300_000
      ),
      retry_max_ms: integerEnv(
        'CONVERACT_FABRIC_PROCESSING_EVENT_RETRY_MAX_MS',
        5_000,
        1,
        3_600_000
      ),
      error_observer: (error) => {
        process.stderr.write(
          `Converact processing event handoff failed: ${safeError(error)}\n`
        );
      }
    })
  : undefined;
...
const serviceToken = secret(
  'CONVERACT_FABRIC_MEDIA_CONTROL_TOKEN',
  'CONVERACT_FABRIC_MEDIA_CONTROL_TOKEN_FILE'
);
const admissionToken = secret(
  'CONVERACT_FABRIC_MEDIA_CONTROL_ADMISSION_TOKEN',
  'CONVERACT_FABRIC_MEDIA_CONTROL_ADMISSION_TOKEN_FILE'
);
const admissionRequireMtls = booleanEnv(
  'CONVERACT_FABRIC_MEDIA_CONTROL_ADMISSION_REQUIRE_MTLS',
  production
);
const admissionEndpoint = requiredEnv(
  'CONVERACT_FABRIC_MEDIA_CONTROL_ADMISSION_ENDPOINT'
);
...
const admission = new HttpComponentNodeAdmissionClient({
  ...
  timeout_ms: integerEnv(
    'CONVERACT_FABRIC_MEDIA_CONTROL_ADMISSION_TIMEOUT_MS',
    2_000,
    100,
    30_000
  )
...
const admissionHealthIntervalMs = integerEnv(
  'CONVERACT_FABRIC_MEDIA_CONTROL_ADMISSION_HEALTH_INTERVAL_MS',
  1_000,
  100,
  60_000
);
...
const agent = new MediaControlAgent({
  ...
  max_reservations: integerEnv(
    'CONVERACT_FABRIC_MEDIA_CONTROL_MAX_RESERVATIONS',
    100_000,
    1,
    1_000_000
  ),
  max_terminal_reservations: integerEnv(
    'CONVERACT_FABRIC_MEDIA_CONTROL_MAX_TERMINAL_RESERVATIONS',
    100_000,
    1,
    1_000_000
  ),
  max_commands_per_reservation: integerEnv(
    'CONVERACT_FABRIC_MEDIA_CONTROL_MAX_COMMANDS_PER_RESERVATION',
    16,
    4,
    256
  ),
  terminal_retention_ms: integerEnv(
    'CONVERACT_FABRIC_MEDIA_CONTROL_TERMINAL_RETENTION_MS',
    300_000,
    1_000,
    86_400_000
  ),
  ...
  authorization_failure_observer: (failure) => {
    process.stderr.write(
      'Converact media authorization rejected ' +
...
const server = createMediaControlHttpServer({
  ...
  error_observer: (failure) => {
    process.stderr.write(
      'Converact media HTTP request rejected ' +
  ...
  max_body_bytes: integerEnv(
    'CONVERACT_FABRIC_MEDIA_CONTROL_MAX_BODY_BYTES',
    262_144,
    1_024,
    1_048_576
  )
});
const host = stringEnv('CONVERACT_FABRIC_MEDIA_CONTROL_HOST', '127.0.0.1');
const port = integerEnv('CONVERACT_FABRIC_MEDIA_CONTROL_PORT', 3_211, 1, 65_535);
const sweepIntervalMs = integerEnv(
  'CONVERACT_FABRIC_MEDIA_CONTROL_SWEEP_INTERVAL_MS',
  1_000,
  100,
  60_000
);
const sweepTimer = setInterval(() => {
  void agent.sweep(new Date()).catch((error) => {
    process.stderr.write(
      `Converact media control sweep failed: ${safeError(error)}\n`
    );
  });
}, sweepIntervalMs);
sweepTimer.unref();
...
const orphanSweepTimer = setInterval(() => {
  ...
  void agent.sweepOrphans(new Date())
    .catch((error) => {
      process.stderr.write(
        `Converact media orphan sweep failed: ${safeError(error)}\n`
      );
    })
    .finally(() => {
...
const terminalEventCompactionTimer = terminalEventJournal
  ? setInterval(() => {
      ...
      void terminalEventJournal.compact(eventReplayCapacity)
        .catch((error) => {
          process.stderr.write(
            `Converact terminal event WAL compaction failed: ${safeError(error)}\n`
          );
        })
        .finally(() => {
      ...
      'CONVERACT_FABRIC_MEDIA_CONTROL_EVENT_WAL_COMPACT_INTERVAL_MS',
      60_000,
      1_000,
      3_600_000
    ))
...
server.listen(port, host, () => {
  process.stdout.write(
    `Converact media control agent listening on ${host}:${port} ` +
...
async function shutdown(signal: string): Promise<void> {
  ...
  process.stdout.write(`Converact media control agent stopping on ${signal}\n`);
  ...
  await transportRuntime.drain().catch((error) => {
    process.stderr.write(
      `Converact media control drain failed: ${safeError(error)}\n`
    );
  });
  try {
...
function tlsOptions(): MediaControlServerTlsOptions {
  return {
    key: readRequiredFile('CONVERACT_FABRIC_MEDIA_CONTROL_TLS_KEY_FILE'),
    cert: readRequiredFile('CONVERACT_FABRIC_MEDIA_CONTROL_TLS_CERT_FILE'),
    ca: readRequiredFile('CONVERACT_FABRIC_MEDIA_CONTROL_TLS_CA_FILE')
  };
}
...
function admissionTlsOptions(): ComponentNodeAdmissionClientTlsOptions {
  return {
    key: readRequiredFile(
      'CONVERACT_FABRIC_MEDIA_CONTROL_ADMISSION_TLS_KEY_FILE'
    ),
    cert: readRequiredFile(
      'CONVERACT_FABRIC_MEDIA_CONTROL_ADMISSION_TLS_CERT_FILE'
    ),
    ca: readRequiredFile(
      'CONVERACT_FABRIC_MEDIA_CONTROL_ADMISSION_TLS_CA_FILE'
    ),
    servername:
      resolveConveractEnv(
        process.env,
        'CONVERACT_FABRIC_MEDIA_CONTROL_ADMISSION_TLS_SERVERNAME'
      )?.trim() ||
      undefined
  };
}
...
async function openTransportRuntime(
  ...
  if (mode === 'hybrid') {
    ...
    try {
      const processing = new ProcessingMediaTransport({
        endpoint: requiredEnv('CONVERACT_FABRIC_PROCESSING_MEDIA_ENDPOINT'),
        bearer_token: secret(
          'CONVERACT_FABRIC_PROCESSING_MEDIA_TOKEN',
          'CONVERACT_FABRIC_PROCESSING_MEDIA_TOKEN_FILE'
        ),
        client_identity: requiredEnv(
          'CONVERACT_FABRIC_PROCESSING_MEDIA_CLIENT_IDENTITY'
        ),
        request_timeout_ms: integerEnv(
          'CONVERACT_FABRIC_PROCESSING_MEDIA_TIMEOUT_MS',
          2_000,
          50,
          300_000
        ),
        max_response_bytes: integerEnv(
          'CONVERACT_FABRIC_PROCESSING_MEDIA_MAX_RESPONSE_BYTES',
          262_144,
          256,
          4_194_304
        ),
        max_release_contexts: integerEnv(
          'CONVERACT_FABRIC_PROCESSING_MEDIA_MAX_RELEASE_CONTEXTS',
          100_000,
          1,
          10_000_000
        )
      ...
      return {
        transport: new MediaTransportRouter({
          ...
          max_bindings: integerEnv(
            'CONVERACT_FABRIC_MEDIA_CONTROL_TRANSPORT_MAX_BINDINGS',
            100_000,
            1,
            10_000_000
          )
  ...
  const client = new RtpengineNgClient({
    ...
    maxConnections: integerEnv(
      'CONVERACT_FABRIC_RTPENGINE_MAX_CONNECTIONS',
      4,
      1,
      64
    ),
    maxInFlight: integerEnv(
      'CONVERACT_FABRIC_RTPENGINE_MAX_IN_FLIGHT',
      1_024,
      1,
      100_000
    ),
    maxRequestBytes: integerEnv(
      'CONVERACT_FABRIC_RTPENGINE_MAX_REQUEST_BYTES',
      1_048_576,
      64,
      67_108_864
    ),
    maxResponseBytes: integerEnv(
      'CONVERACT_FABRIC_RTPENGINE_MAX_RESPONSE_BYTES',
      1_048_576,
      64,
      67_108_864
    ),
    maxQueuedBytes: integerEnv(
      'CONVERACT_FABRIC_RTPENGINE_MAX_QUEUED_BYTES',
      4_194_304,
      64,
      67_108_864
    ),
  ...
  const journal = await MediaCommandJournal.open({
    ...
    maxRecords: integerEnv(
      'CONVERACT_FABRIC_MEDIA_CONTROL_WAL_MAX_RECORDS',
      1_000_000,
      1,
      10_000_000
    ),
    maxBytes: integerEnv(
      'CONVERACT_FABRIC_MEDIA_CONTROL_WAL_MAX_BYTES',
      268_435_456,
      512,
      17_179_869_184
    ),
    maxRecordBytes: integerEnv(
      'CONVERACT_FABRIC_MEDIA_CONTROL_WAL_MAX_RECORD_BYTES',
      2_097_152,
      256,
      67_108_864
    ),
  ...
  try {
    ...
    const transport = await RtpengineMediaTransport.open({
      ...
      recoveryConcurrency: integerEnv(
        'CONVERACT_FABRIC_RTPENGINE_RECOVERY_CONCURRENCY',
        32,
        1,
        256
      ),
      maxSessions: integerEnv(
        'CONVERACT_FABRIC_RTPENGINE_MAX_SESSIONS',
        100_000,
        1,
        10_000_000
      ),
      maxCommands: integerEnv(
        'CONVERACT_FABRIC_RTPENGINE_MAX_COMMANDS',
        1_600_000,
        1,
        10_000_000
      ),
