...
service_name=${CONVERACT_RUSTDESK_SERVICE_NAME-rustdesk.service}
case "$service_name" in ''|*[!A-Za-z0-9_.@-]*) printf '%s\n' 'invalid RustDesk service name' >&2; exit 64 ;; esac
available=false
if command -v systemctl >/dev/null 2>&1 && systemctl show "$service_name" --property=LoadState --value 2>/dev/null | grep -qv '^not-found$'; then available=true; fi

if [ "$mode" = 'validate' ]; then
  printf '{"adapter":"linux-service-restart","mode":"validate","available":%s,"targeted":false,"collateral_sessions_may_disconnect":true}\n' "$available"
  exit 0
fi
[ "$available" = true ] || { printf '%s\n' 'RustDesk systemd service is unavailable' >&2; exit 21; }
systemctl restart "$service_name"
systemctl is-active --quiet "$service_name"
printf '{"adapter":"linux-service-restart","mode":"execute","status":"succeeded","targeted":false,"collateral_sessions_may_disconnect":true}\n'
