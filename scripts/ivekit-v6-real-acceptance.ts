...
export const IVEKIT_V6_REAL_ACCEPTANCE_GROUPS = [
  'providers',
  'tinode',
  'livekit_turn_egress',
  'rustdesk_windows',
...
export type IveKitV6RealAcceptanceGroupId =
  typeof IVEKIT_V6_REAL_ACCEPTANCE_GROUPS[number];
export type IveKitV6RealAcceptanceStatus = 'passed' | 'failed' | 'not_run';

export interface IveKitV6RealAcceptanceCheck {
  id: string;
...
export interface IveKitV6RealAcceptanceGroup {
  id: IveKitV6RealAcceptanceGroupId;
  status: IveKitV6RealAcceptanceStatus;
  reason_code: string;
  reason: string;
  command: string;
  run: null | {
  ...
  checks: IveKitV6RealAcceptanceCheck[];
}

export interface IveKitV6RealAcceptanceManifest {
  schema_version: 1;
  product: 'iveKit';
  ...
  groups: IveKitV6RealAcceptanceGroup[];
}
...
const GROUP_TEMPLATE: Record<IveKitV6RealAcceptanceGroupId, {
  reason_code: string;
  reason: string;
  command: string;
}> = {
  providers: {
    ...
    command: 'npm run ivekit:intelligence-preflight && npm run ivekit:provider-governance-acceptance'
  },
  tinode: {
  ...
  rustdesk_windows: {
    ...
    reason: 'Two physical Windows devices with the pinned iveKit RustDesk build are unavailable.',
  ...
  voice_pstn: {
    ...
    command: 'npm run ivekit:voice-preflight && npm run ivekit:voice-acceptance && npm run ivekit:rustpbx-sipp-acceptance'
  },
  notifications: {
...
export function createIveKitV6RealAcceptanceTemplate(input: {
  source_commit: string;
  generated_at?: string;
}): IveKitV6RealAcceptanceManifest {
  ...
  return {
    ...
    real_environment_only: true,
    groups: IVEKIT_V6_REAL_ACCEPTANCE_GROUPS.map((id) => ({
      id,
      status: 'not_run',
...
export function validateIveKitV6RealAcceptanceManifest(
  manifest: IveKitV6RealAcceptanceManifest,
  input: { base_dir: string; expected_source_commit?: string }
): void {
  if (
    manifest.schema_version !== 1 || manifest.product !== 'iveKit' ||
    manifest.foundation_version !== 'V6' || manifest.real_environment_only !== true ||
    !SOURCE_COMMIT.test(manifest.source_commit)
  ) throw new Error('invalid iveKit V6 real acceptance identity');
  ...
  if (!Array.isArray(manifest.groups) ||
      JSON.stringify(manifest.groups.map((group) => group.id)) !==
        JSON.stringify(IVEKIT_V6_REAL_ACCEPTANCE_GROUPS)) {
...
export function readAndValidateIveKitV6RealAcceptance(
  manifestFile: string,
  ...
  const manifest = JSON.parse(readFileSync(absolute, 'utf8')) as IveKitV6RealAcceptanceManifest;
...
function validateGroup(
  manifest: IveKitV6RealAcceptanceManifest,
  group: IveKitV6RealAcceptanceGroup,
  baseDir: string,
  evidencePaths: Set<string>
): void {
  if (!IVEKIT_V6_REAL_ACCEPTANCE_GROUPS.includes(group.id)) {
...
function validateObservation(
  manifest: IveKitV6RealAcceptanceManifest,
  group: IveKitV6RealAcceptanceGroup,
  check: IveKitV6RealAcceptanceCheck,
  baseDir: string,
  evidencePaths: Set<string>
): void {
