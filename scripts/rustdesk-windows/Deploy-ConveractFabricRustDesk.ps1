...
param(
  ...
  [string]$InstallRoot = (Join-Path $env:ProgramData 'Converact\RustDesk'),
  [string]$BaseUrl = '',
  [string]$TenantId = '',
...
function Read-Manifest {
  ...
  if ($manifest.schema_version -ne 1 -or $manifest.package_type -ne 'converact-rustdesk-windows-x86_64') {
...
function Resolve-NodeExecutable {
  ...
  if ([int]$Matches.major -lt 23) {
    throw 'The Converact Fabric RustDesk companion requires Node.js 23 or newer.'
  }
  return $resolved
}
...
function Save-RollbackState($Policy) {
  ...
  if ($companion) {
    throw 'A companion service already exists without Converact Fabric rollback metadata.'
  }
  $state = [ordered]@{
    schema_version = 1
...
function Restore-ConveractFabricRollback($State) {
...
function Install-Companion([string]$NodePath, [string]$RustDeskId, [string]$RustDeskExe) {
  ...
  $preciseDisconnectScript = Join-Path $InstallRoot 'edge\windows\Invoke-ConveractFabricRustDeskSessionDisconnect.ps1'
  $sessionResolverScript = Join-Path $InstallRoot 'edge\windows\Resolve-ConveractFabricRustDeskSession.ps1'
  ...
  if ($LASTEXITCODE -ne 0) { throw 'Unable to install the Converact Fabric RustDesk companion service.' }
  & $serviceExe start | Out-Null
  if ($LASTEXITCODE -ne 0) { throw 'Unable to start the Converact Fabric RustDesk companion service.' }
}
...
function Invoke-InstallOrRepair($Policy) {
  ...
  try {
    ...
    Restore-ConveractFabricRollback $state
    throw
  }
}
