...
if ([string]::IsNullOrWhiteSpace($RegistryFile)) {
  $RegistryFile = Join-Path $env:ProgramData 'Converact\RustDesk\state\rustdesk-native-sessions.json'
}
...
$mutex = [Threading.Mutex]::new($false, 'Global\ConveractFabricRustDeskNativeSessionRegistryV1')
$locked = $false
