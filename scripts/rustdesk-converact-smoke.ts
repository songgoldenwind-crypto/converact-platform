...
export interface RustDeskConveractFabricSmokeConfig {
  baseUrl: string;
  apiKey: string;
  tenantId: string;
...
export interface RustDeskConveractFabricSmokeStep {
  name: string;
  status: number;
}

export interface RustDeskConveractFabricSmokeResult {
  collaborationSessionId: string;
  remoteSessionId: string;
  deviceId: string;
  externalId: string;
  launchUrl: string;
  steps: RustDeskConveractFabricSmokeStep[];
  clientConfig: {
    idServer: string;
    manualKey: string;
    relayServer: string;
...
export function createRustDeskConveractFabricSmokeConfigFromEnv(env: NodeJS.ProcessEnv): RustDeskConveractFabricSmokeConfig {
  const rawBaseUrl =
    ...
    '';
  if (!rawBaseUrl) {
    throw new Error('CONVERACT_RUSTDESK_FABRIC_BASE_URL, CONVERACT_BASE_URL, CONVERACT_COLLABORATION_BASE_URL, CONVERACT_RUSTDESK_EDGE_BASE_URL, CONVERACT_RUSTDESK_CONTROL_PLANE_BASE_URL, or CONVERACT_REMOTE_GATEWAY_BASE_URL is required');
  }
  const apiKey = resolveBrandEnv(env, 'RUSTDESK_FABRIC_API_KEY') || resolveBrandEnv(env, 'COLLABORATION_API_KEY') || resolveBrandEnv(env, 'API_KEY') || '';
  if (!apiKey) throw new Error('CONVERACT_RUSTDESK_FABRIC_API_KEY, CONVERACT_COLLABORATION_API_KEY, or CONVERACT_API_KEY is required');
  const tenantId = resolveBrandEnv(env, 'RUSTDESK_FABRIC_TENANT_ID') || resolveBrandEnv(env, 'REMOTE_GATEWAY_TENANT_ID') || resolveBrandEnv(env, 'RUSTDESK_EDGE_TENANT_ID') || resolveBrandEnv(env, 'TENANT_ID') || '';
  if (!tenantId) throw new Error('CONVERACT_RUSTDESK_FABRIC_TENANT_ID, CONVERACT_REMOTE_GATEWAY_TENANT_ID, CONVERACT_RUSTDESK_EDGE_TENANT_ID, or CONVERACT_TENANT_ID is required');
  const rustdeskId = resolveBrandEnv(env, 'RUSTDESK_FABRIC_RUSTDESK_ID') || resolveBrandEnv(env, 'RUSTDESK_EDGE_RUSTDESK_ID') || resolveBrandEnv(env, 'REMOTE_GATEWAY_TARGET_ID') || '';
  if (!rustdeskId) throw new Error('CONVERACT_RUSTDESK_FABRIC_RUSTDESK_ID, CONVERACT_RUSTDESK_EDGE_RUSTDESK_ID, or CONVERACT_REMOTE_GATEWAY_TARGET_ID is required');
  const businessRefId = resolveBrandEnv(env, 'RUSTDESK_FABRIC_BUSINESS_REF_ID') || resolveBrandEnv(env, 'RUSTDESK_EDGE_BUSINESS_REF_ID') || `${tenantId}-rustdesk-converact-smoke-${Date.now()}`;
  return {
    baseUrl: normalizeBaseUrl(rawBaseUrl),
    apiKey,
    tenantId,
    actorIdentity: resolveBrandEnv(env, 'RUSTDESK_FABRIC_ACTOR_IDENTITY') || resolveBrandEnv(env, 'REMOTE_GATEWAY_ACTOR_IDENTITY') || 'agent_converact_rustdesk_smoke',
    customerIdentity: resolveBrandEnv(env, 'RUSTDESK_FABRIC_CUSTOMER_IDENTITY') || 'customer_converact_rustdesk_smoke',
    businessRefType: resolveBrandEnv(env, 'RUSTDESK_FABRIC_BUSINESS_REF_TYPE') || resolveBrandEnv(env, 'RUSTDESK_EDGE_BUSINESS_REF_TYPE') || 'service_order',
    businessRefId,
    businessRefDisplayName: resolveBrandEnv(env, 'RUSTDESK_FABRIC_BUSINESS_REF_DISPLAY_NAME') || 'RustDesk Converact Fabric smoke',
    rustdeskId,
    deviceDisplayName: resolveBrandEnv(env, 'RUSTDESK_FABRIC_DEVICE_DISPLAY_NAME') || resolveBrandEnv(env, 'RUSTDESK_EDGE_DEVICE_DISPLAY_NAME') || resolveBrandEnv(env, 'REMOTE_GATEWAY_TARGET_DISPLAY_NAME') || 'RustDesk Converact Fabric smoke device',
    permissions: splitScopes(resolveBrandEnv(env, 'RUSTDESK_FABRIC_CONSENT_SCOPES')),
    consentExpiresAt: resolveBrandEnv(env, 'RUSTDESK_FABRIC_CONSENT_EXPIRES_AT') || undefined
  };
}

export async function runRustDeskConveractFabricSmoke(
  config: RustDeskConveractFabricSmokeConfig,
  fetchImpl: FetchLike = fetch
): Promise<RustDeskConveractFabricSmokeResult> {
  const steps: RustDeskConveractFabricSmokeStep[] = [];
  const headers = authHeaders(config);
  const businessRef = {
    type: config.businessRefType,
    id: config.businessRefId,
    display_name: config.businessRefDisplayName,
    metadata: { source: 'rustdesk-converact-smoke' }
  };
  let externalId = '';
  ...
  try {
    ...
    const clientConfig = unwrapRecord(await jsonRequest(fetchImpl, steps, 'get_converact_client_config', `${config.baseUrl}/api/ivekit/rustdesk/client-config`, {
      headers
    }));
    const clientConfigSummary = assertClientConfig(clientConfig);

    const device = unwrapRecord(await jsonRequest(fetchImpl, steps, 'register_converact_device', `${config.baseUrl}/api/ivekit/rustdesk/devices`, {
      method: 'POST',
      headers,
      body: JSON.stringify({
        business_ref: businessRef,
    ...
    await jsonRequest(fetchImpl, steps, 'heartbeat_converact_device', `${config.baseUrl}/api/ivekit/rustdesk/devices/${encodeURIComponent(deviceId)}/heartbeat`, {
      method: 'POST',
      headers,
      body: JSON.stringify({
    ...
    const devicesByRef = unwrapArray(await jsonRequest(fetchImpl, steps, 'list_converact_devices_by_ref', urlWithQuery(config.baseUrl, '/api/ivekit/rustdesk/devices/by-ref', {
    ...
    if (!devicesByRef.some((row) => asRecord(row)?.id === deviceId)) {
      throw new Error('Converact Fabric RustDesk device was not returned by business ref lookup');
    }

    const tool = unwrapRecord(await jsonRequest(fetchImpl, steps, 'start_converact_gateway_session', `${config.baseUrl}/api/ivekit/rustdesk/gateway-sessions`, {
      method: 'POST',
      headers,
      body: JSON.stringify({
    ...
    if (String(tool?.provider || '') !== 'rustdesk') {
      throw new Error(`Converact Fabric gateway provider mismatch: ${String(tool?.provider || '')}`);
    }
    assertConveractFabricLaunchUrl(externalId, launchUrl);

    const launchPlan = unwrapRecord(await jsonRequest(fetchImpl, steps, 'get_converact_launch_plan', gatewayUrl(config, externalId, 'launch'), {
      headers
    }));
    ...
    await checkConveractFabricLaunchPage(fetchImpl, steps, launchUrl, externalId);

    await expectStatus(fetchImpl, steps, 'converact_invalid_clipboard_event_rejected', gatewayUrl(config, externalId, 'events'), {
      method: 'POST',
      headers,
      body: JSON.stringify({
        ...
        idempotency_key: `rustdesk-converact-smoke:${externalId}:invalid-clipboard`,
        metadata: {
          source: 'rustdesk-converact-smoke',
          clipboard_id: `rustdesk-converact-smoke-invalid-clipboard-${externalId}`,
          direction: 'sideways'
        }
      })
    }, 400);
    ...
    const audit = unwrapRecord(await jsonRequest(fetchImpl, steps, 'list_converact_gateway_audit', gatewayUrl(config, externalId, 'audit'), {
      headers
    }));
    ...
    await jsonRequest(fetchImpl, steps, 'end_converact_gateway_session', `${config.baseUrl}/api/ivekit/rustdesk/gateway-sessions/${encodeURIComponent(externalId)}`, {
      method: 'DELETE',
      headers,
      body: JSON.stringify({ actor_identity: config.actorIdentity })
    }, [204]);

    const endedLaunchPlan = unwrapRecord(await jsonRequest(fetchImpl, steps, 'get_converact_ended_launch_plan', gatewayUrl(config, externalId, 'launch'), {
      headers
    }));
    ...
    await checkConveractFabricEndedLaunchUrlRejected(fetchImpl, steps, launchUrl);

    await expectStatus(fetchImpl, steps, 'converact_after_end_event_rejected', gatewayUrl(config, externalId, 'events'), {
      method: 'POST',
      headers,
      body: JSON.stringify({
        ...
        actor_identity: config.actorIdentity,
        target: config.rustdeskId,
        idempotency_key: `rustdesk-converact-smoke:${externalId}:after-end-control`,
        metadata: {
          source: 'rustdesk-converact-smoke',
          operation_id: `rustdesk-converact-smoke-after-end-${externalId}`,
          action: 'mouse_click',
    ...
    const timeline = unwrapRecord(await jsonRequest(fetchImpl, steps, 'get_converact_timeline_after_end', remoteActionUrl(config, remoteSessionId, 'timeline'), {
      headers
    }));
    ...
    if (externalId && !cleanupAttempted) {
      try {
        await jsonRequest(fetchImpl, steps, 'cleanup_converact_gateway_session', `${config.baseUrl}/api/ivekit/rustdesk/gateway-sessions/${encodeURIComponent(externalId)}`, {
          method: 'DELETE',
          headers,
          body: JSON.stringify({ actor_identity: config.actorIdentity })
        }, [204]);
...
function rustDeskOperationEvents(
  ...
  return [
    {
      stepName: 'post_converact_control_event',
      payload: {
        event_type: 'remote.rustdesk.control_action.performed',
        actor_identity: config.actorIdentity,
        target,
        idempotency_key: `rustdesk-converact-smoke:${externalId}:control`,
        metadata: {
          source: 'rustdesk-converact-smoke',
          operation_id: `rustdesk-converact-smoke-control-${externalId}`,
          action: 'mouse_click',
          permission: 'control_mouse_keyboard'
        }
      }
    ...
    {
      stepName: 'post_converact_file_transfer_started_event',
      payload: {
        event_type: 'remote.rustdesk.file_transfer.started',
        actor_identity: config.actorIdentity,
        target,
        idempotency_key: `rustdesk-converact-smoke:${externalId}:file-transfer`,
        metadata: {
          source: 'rustdesk-converact-smoke',
          transfer_id: `rustdesk-converact-smoke-transfer-${externalId}`,
          direction: 'upload',
          file_name: 'rustdesk-converact-smoke.txt'
        }
      }
    },
    {
      stepName: 'post_converact_file_transfer_completed_event',
      payload: {
        event_type: 'remote.rustdesk.file_transfer.completed',
        actor_identity: config.actorIdentity,
        target,
        idempotency_key: `rustdesk-converact-smoke:${externalId}:file-transfer-completed`,
        metadata: {
          source: 'rustdesk-converact-smoke',
          transfer_id: `rustdesk-converact-smoke-transfer-${externalId}`,
          direction: 'upload',
    ...
    {
      stepName: 'post_converact_recording_started_event',
      payload: {
        event_type: 'remote.rustdesk.recording.started',
        actor_identity: config.actorIdentity,
        target,
        idempotency_key: `rustdesk-converact-smoke:${externalId}:recording`,
        metadata: {
          source: 'rustdesk-converact-smoke',
          recording_id: `rustdesk-converact-smoke-recording-${externalId}`,
          evidence_type: 'screen_recording'
        }
      }
    },
    {
      stepName: 'post_converact_recording_stopped_event',
      payload: {
        event_type: 'remote.rustdesk.recording.stopped',
        actor_identity: config.actorIdentity,
        target,
        idempotency_key: `rustdesk-converact-smoke:${externalId}:recording-stopped`,
        metadata: {
          source: 'rustdesk-converact-smoke',
          recording_id: `rustdesk-converact-smoke-recording-${externalId}`,
          evidence_type: 'screen_recording',
    ...
    {
      stepName: 'post_converact_clipboard_event',
      payload: {
        event_type: 'remote.rustdesk.clipboard.synced',
        actor_identity: config.actorIdentity,
        target,
        idempotency_key: `rustdesk-converact-smoke:${externalId}:clipboard`,
        metadata: {
          source: 'rustdesk-converact-smoke',
          clipboard_id: `rustdesk-converact-smoke-clipboard-${externalId}`,
...
function normalizeBaseUrl(rawBaseUrl: string): string {
  ...
  try {
    ...
    throw new Error('Converact RustDesk Converact Fabric base URL must be a valid URL');
  }
  if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
    throw new Error('Converact RustDesk Converact Fabric base URL must use http(s)');
  }
  return baseUrl;
}
...
function splitScopes(value: string | undefined): RemoteConsentScope[] {
  const scopes = (value || 'view_screen,control_mouse_keyboard,record_screen,transfer_file,clipboard')
    ...
    .filter(Boolean);
  if (!scopes.length) throw new Error('CONVERACT_RUSTDESK_FABRIC_CONSENT_SCOPES must include at least one scope');
  const unsupported = scopes.find((scope) => !allowedScopes.has(scope as RemoteConsentScope));
  if (unsupported) throw new Error(`unsupported CONVERACT_RUSTDESK_FABRIC_CONSENT_SCOPES value: ${unsupported}`);
  return scopes as RemoteConsentScope[];
}

function authHeaders(config: RustDeskConveractFabricSmokeConfig): Record<string, string> {
  return {
...
function consentExpiresAt(config: RustDeskConveractFabricSmokeConfig): string {
  return config.consentExpiresAt || new Date(Date.now() + 60 * 60 * 1000).toISOString();
}

function remoteActionUrl(config: RustDeskConveractFabricSmokeConfig, remoteSessionId: string, action: string): string {
  return `${config.baseUrl}/api/collaboration/remote-assistance/${encodeURIComponent(remoteSessionId)}/${action}`;
}

function gatewayUrl(config: RustDeskConveractFabricSmokeConfig, externalId: string, action: string): string {
...
async function checkConveractFabricLaunchPage(
  fetchImpl: FetchLike,
  steps: RustDeskConveractFabricSmokeStep[],
  launchUrl: string,
  externalId: string
  ...
  steps.push({ name: 'check_converact_launch_url', status: response.status });
  if (!response.ok) {
    throw new Error(`Converact Fabric RustDesk launch URL check failed with status ${response.status}`);
  }
  const text = await response.text();
  if (!text.includes('RustDesk Remote Launch') || !text.includes(externalId)) {
    throw new Error('Converact Fabric RustDesk launch page did not contain the expected launch content');
  }
}

async function checkConveractFabricEndedLaunchUrlRejected(
  fetchImpl: FetchLike,
  steps: RustDeskConveractFabricSmokeStep[],
  launchUrl: string
): Promise<void> {
  const response = await fetchImpl(launchUrl, { method: 'GET' });
  steps.push({ name: 'converact_ended_launch_url_rejected', status: response.status });
  if (response.status !== 409) {
    throw new Error(`Converact Fabric RustDesk ended launch URL must be rejected with 409, got ${response.status}`);
  }
}
...
function assertConveractFabricLaunchUrl(externalId: string, launchUrl: string): void {
  let parsed: URL;
  try {
    parsed = new URL(launchUrl);
  } catch {
    throw new Error('Converact Fabric RustDesk launch_url must be a valid URL');
  }
  if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
    throw new Error('Converact Fabric RustDesk launch_url must be http(s)');
  }
  if (parsed.pathname !== '/remote/rustdesk/launch') {
    throw new Error('Converact Fabric RustDesk launch_url path must be /remote/rustdesk/launch');
  }
  const sessionId = String(parsed.searchParams.get('session_id') || '').trim();
  if (sessionId !== externalId) {
    throw new Error('Converact Fabric RustDesk launch_url session_id must match external_id');
  }
  const token = String(parsed.searchParams.get('token') || '').trim();
  if (!token) {
    throw new Error('Converact Fabric RustDesk launch_url token is required');
  }
  if (!/^[a-f0-9]{64}$/i.test(token)) {
    throw new Error('Converact Fabric RustDesk launch_url token must be a 64 character hex HMAC');
  }
  const expiresAt = String(parsed.searchParams.get('expires_at') || '').trim();
  if (!expiresAt) {
    throw new Error('Converact Fabric RustDesk launch_url expires_at is required');
  }
  const expiresAtMs = Date.parse(expiresAt);
  if (!Number.isFinite(expiresAtMs) || expiresAtMs <= Date.now()) {
    throw new Error('Converact Fabric RustDesk launch_url expires_at must be a future ISO timestamp');
  }
}

function assertClientConfig(value: JsonRecord | null): RustDeskConveractFabricSmokeResult['clientConfig'] {
...
function assertLaunchPlan(
  ...
  expectedClientConfig?: RustDeskConveractFabricSmokeResult['clientConfig'],
  expectedPermissions: RemoteConsentScope[] = []
): RustDeskConveractFabricSmokeResult['launchPlan'] {
...
function assertAuditHas(auditEvents: unknown[], externalId: string, eventType: string): void {
  ...
  if (!found) throw new Error(`Converact Fabric RustDesk audit missing ${eventType}`);
}
...
function validateTimeline(
  ...
  externalId: string,
  operationEventTypes: string[]
): RustDeskConveractFabricSmokeResult['timeline'] {
...
async function main(): Promise<void> {
  const result = await runRustDeskConveractFabricSmoke(createRustDeskConveractFabricSmokeConfigFromEnv(process.env));
