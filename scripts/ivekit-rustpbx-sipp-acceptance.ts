...
export interface RustPbxSippReport {
  schema_version: 1;
  suite: 'iveKit RustPBX SIPp acceptance';
  status: 'passed' | 'failed';
  generated_at: string;
  duration_ms: number;
  tools: {
...
>): string {
  ...
  const cases = input.scenarios.map((scenario) => {
    const failure = scenario.status === 'failed'
      ...
      : '';
    return `  <testcase classname="ivekit.rustpbx.sipp" name="${xml(scenario.id)}" time="${(scenario.duration_ms / 1000).toFixed(3)}">${failure}\n  </testcase>`;
  }).join('\n');
  return [
    '<?xml version="1.0" encoding="UTF-8"?>',
    `<testsuite name="iveKit RustPBX SIPp acceptance" tests="${input.scenarios.length}" failures="${failures}" time="${(input.duration_ms / 1000).toFixed(3)}" timestamp="${xml(input.generated_at)}">`,
    cases,
    '</testsuite>',
    ''
  ].join('\n');
...
async function runScenario(
  ...
  const uasName = `ivekit-sipp-uas-${suffix}`;
  const uacName = `ivekit-sipp-uac-${suffix}`;
...
function optionsFromEnv(env: NodeJS.ProcessEnv): RustPbxSippOptions {
  const binary = required(env, 'IVEKIT_SIPP_BINARY');
  const network = boundedName(required(env, 'IVEKIT_RUSTPBX_ACCEPTANCE_NETWORK'), 'Docker network');
  const extensionPassword = required(env, 'IVEKIT_RUSTPBX_EXTENSION_PASSWORD');
  if (extensionPassword.length < 8 || extensionPassword.length > 256) {
    throw new Error('IVEKIT_RUSTPBX_EXTENSION_PASSWORD must contain 8 to 256 characters');
  }
  return {
    docker: env.IVEKIT_DOCKER_COMMAND || 'docker',
    network,
    ...
    scenario_filter: String(env.IVEKIT_RUSTPBX_ACCEPTANCE_SCENARIOS || '').trim()
  };
}
...
function boundedDigits(value: string): string {
  if (!/^\d{2,16}$/.test(value)) throw new Error('IVEKIT_RUSTPBX_EXTENSION is invalid');
  return value;
}
