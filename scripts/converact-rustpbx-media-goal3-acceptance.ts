...
function validateSourceIdentity(
  ...
  if (!COMMIT.test(identity.opc_commit)) {
    throw new Error('Converact source commit is invalid');
  }
  for (const field of [
    'rustpbx_commit',
    'rsipstack_commit',
...
function validateEnvironment(
  ...
  if (!/^converact-goal3-[a-z0-9][a-z0-9-]{0,62}$/.test(
...
function runCli(argv: string[]): void {
  ...
  if (!inputPath || !outputPath) {
    throw new Error(
      'usage: converact-rustpbx-media-goal3-acceptance <input.json> <output.json> [contract.json]'
    );
  }
