...
export interface ConveractFabricReleaseOperationsInput {
  sourceCommit: string;
  generatedAt: string;
...
export interface ConveractFabricReleaseContract {
  schema_version: 2;
  ...
  compose: {
    ...
    image_variable: 'CONVERACT_FABRIC_SERVICE_IMAGE';
  };
  helm: {
    chart: 'deploy/kubernetes/converact';
...
export interface ConveractFabricReleaseOperations {
  contract: ConveractFabricReleaseContract;
  runbook: string;
}
...
export function createConveractFabricReleaseOperations(
  input: ConveractFabricReleaseOperationsInput
): ConveractFabricReleaseOperations {
  assertInput(input);
  const immutableReference = input.imageDigest
    ? `${input.imageReference}@${input.imageDigest}`
    : '';
  const contract: ConveractFabricReleaseContract = {
    schema_version: 2,
  ...
  validateConveractFabricReleaseOperations(operations);
  return operations;
}

export function validateConveractFabricReleaseOperations(operations: ConveractFabricReleaseOperations): void {
  const { contract, runbook } = operations;
  if (contract.schema_version !== 2 || !isAcceptedFabricStandaloneProduct(contract.product)) {
    throw new Error('invalid Converact Fabric release contract');
  }
  ...
  if (contract.image.metadata_path !== 'service/image-metadata.json' ||
      ...
      contract.compose.service !== 'converact' ||
      contract.compose.migration_service !== 'migrate' ||
      contract.compose.image_variable !== 'CONVERACT_FABRIC_SERVICE_IMAGE' ||
      contract.helm.chart !== 'deploy/kubernetes/converact' ||
...
function assertInput(input: ConveractFabricReleaseOperationsInput): void {
...
function renderRunbook(contract: ConveractFabricReleaseContract): string {
  ...
  return [
    '# Converact Fabric Standalone Upgrade And Rollback Runbook',
    '',
    ...
    '',
    '```bash',
    `export CONVERACT_FABRIC_SERVICE_IMAGE='${immutableImage}'`,
    "export CONVERACT_FABRIC_ENV_FILE='/secure/converact/application.env'",
    'docker compose --project-name converact --env-file "$CONVERACT_FABRIC_ENV_FILE" -f deploy/application/docker-compose.yml config --quiet',
    'docker compose --project-name converact --env-file "$CONVERACT_FABRIC_ENV_FILE" -f deploy/application/docker-compose.yml run --rm runtime-role-init',
    'docker compose --project-name converact --env-file "$CONVERACT_FABRIC_ENV_FILE" -f deploy/application/docker-compose.yml run --rm migrate',
    'docker compose --project-name converact --env-file "$CONVERACT_FABRIC_ENV_FILE" -f deploy/application/docker-compose.yml up -d --no-deps converact',
    'docker compose --project-name converact --env-file "$CONVERACT_FABRIC_ENV_FILE" -f deploy/application/docker-compose.yml ps',
    ...
    '',
    '```bash',
    `export CONVERACT_FABRIC_IMAGE_REPOSITORY='${repository}'`,
    `export CONVERACT_FABRIC_IMAGE_DIGEST='${contract.image.digest || '<digest-required>'}'`,
    "export CONVERACT_FABRIC_NAMESPACE='converact'",
    "export CONVERACT_FABRIC_RELEASE='converact'",
    "export CONVERACT_FABRIC_VALUES_FILE='/secure/converact/values.yaml'",
    'helm upgrade --install "$CONVERACT_FABRIC_RELEASE" deploy/kubernetes/converact --namespace "$CONVERACT_FABRIC_NAMESPACE" --create-namespace --wait --atomic --values "$CONVERACT_FABRIC_VALUES_FILE" --set-string image.repository="$CONVERACT_FABRIC_IMAGE_REPOSITORY" --set-string image.digest="$CONVERACT_FABRIC_IMAGE_DIGEST"',
    'kubectl --namespace "$CONVERACT_FABRIC_NAMESPACE" rollout status deployment/"$CONVERACT_FABRIC_RELEASE"-converact --timeout=5m',
    'kubectl --namespace "$CONVERACT_FABRIC_NAMESPACE" get pods,jobs',
    '```',
    '',
    ...
    '',
    '```bash',
    "export CONVERACT_FABRIC_PREVIOUS_IMAGE='registry.example/converact/service@sha256:<previous-digest>'",
    'export CONVERACT_FABRIC_SERVICE_IMAGE="$CONVERACT_FABRIC_PREVIOUS_IMAGE"',
    'docker compose --project-name converact --env-file "$CONVERACT_FABRIC_ENV_FILE" -f deploy/application/docker-compose.yml up -d --no-deps converact',
    'curl --fail --show-error http://127.0.0.1:8300/health',
    '```',
    '',
    ...
    '',
    '```bash',
    'helm history "$CONVERACT_FABRIC_RELEASE" --namespace "$CONVERACT_FABRIC_NAMESPACE"',
    'helm rollback "$CONVERACT_FABRIC_RELEASE" <previous-revision> --namespace "$CONVERACT_FABRIC_NAMESPACE" --wait',
    'kubectl --namespace "$CONVERACT_FABRIC_NAMESPACE" rollout status deployment/"$CONVERACT_FABRIC_RELEASE"-converact --timeout=5m',
    '```',
    '',
