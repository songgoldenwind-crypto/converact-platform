...
export const CONVERACT_STAGE2_REQUIRED_MIGRATIONS = [
  '061_ivekit_file_security.sql',
  '062_tinode_file_delivery_operations.sql',
  '063_livekit_media_quality.sql'
] as const;

export const CONVERACT_STAGE2_CONFIGURATION_PROFILES = [
  'livekit_turn',
  'livekit_egress',
  'file_security'
] as const;

export type ConveractFabricStage2ConfigurationProfile =
  typeof CONVERACT_STAGE2_CONFIGURATION_PROFILES[number];

export interface ConveractFabricStage2ReleaseEvidenceInput {
  sourceCommit: string;
  generatedAt: string;
  imageDigest: string;
  migrations: Array<{ file: string; sha256: string }>;
  configurationArtifacts: Array<{
    profile: ConveractFabricStage2ConfigurationProfile;
    path: string;
    sha256: string;
  }>;
}

export interface ConveractFabricStage2ReleaseEvidence {
  schema_version: 1;
  ...
  configuration_template_fingerprints: Record<ConveractFabricStage2ConfigurationProfile, {
...
export function createConveractFabricStage2ReleaseEvidence(
  input: ConveractFabricStage2ReleaseEvidenceInput
): ConveractFabricStage2ReleaseEvidence {
  assertInput(input);
  const requiredMigrations = CONVERACT_STAGE2_REQUIRED_MIGRATIONS.map((file) => {
  ...
  const profiles = Object.fromEntries(CONVERACT_STAGE2_CONFIGURATION_PROFILES.map((profile) => {
  ...
  })) as ConveractFabricStage2ReleaseEvidence['configuration_template_fingerprints'];
  ...
  const evidence: ConveractFabricStage2ReleaseEvidence = {
    schema_version: 1,
    source_commit: input.sourceCommit,
  ...
  validateConveractFabricStage2ReleaseEvidence(evidence);
  return evidence;
}

export function validateConveractFabricStage2ReleaseEvidence(
  evidence: ConveractFabricStage2ReleaseEvidence
): void {
  ...
  if (JSON.stringify(evidence.required_migrations.map((entry) => entry.file)) !==
      JSON.stringify(CONVERACT_STAGE2_REQUIRED_MIGRATIONS)) {
  ...
  if (JSON.stringify(Object.keys(evidence.configuration_template_fingerprints)) !==
      JSON.stringify(CONVERACT_STAGE2_CONFIGURATION_PROFILES)) {
    throw new Error('stage 2 evidence has invalid configuration profiles');
  }
  for (const profile of CONVERACT_STAGE2_CONFIGURATION_PROFILES) {
...
function assertInput(input: ConveractFabricStage2ReleaseEvidenceInput): void {
  ...
  for (const entry of input.configurationArtifacts) {
    if (!CONVERACT_STAGE2_CONFIGURATION_PROFILES.includes(entry.profile) ||
