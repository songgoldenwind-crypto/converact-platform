...
export function capacityScalingFinalizerConfig(
  ...
  return {
    ...
    finalizer_id: safeId(
      required(env, 'OPC_IVEKIT_CAPACITY_SCALING_FINALIZER_ID'),
      'scaling finalizer ID'
    ),
    contract_path: absolutePath(
      required(env, 'OPC_IVEKIT_CAPACITY_SCALING_CONTRACT_PATH')
    ),
    submission_path: absolutePath(
      required(env, 'OPC_IVEKIT_CAPACITY_SCALING_SUBMISSION_PATH')
    ),
    lease_ttl_ms: integer(
      env.OPC_IVEKIT_CAPACITY_SCALING_FINALIZER_LEASE_MS || '15000',
      1_000,
      300_000
    ),
    evidence_prefix: required(
      env,
      'OPC_IVEKIT_CAPACITY_SCALING_EVIDENCE_PREFIX'
    ),
    evidence_s3: {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch((error) => {
    console.error('[ivekit-capacity-scaling-finalizer]', safeError(error));
    process.exitCode = 1;
  });
}
