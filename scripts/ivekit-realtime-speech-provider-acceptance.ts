...
export interface IveKitRealtimeSpeechProviderAcceptanceCheck {
  name: string;
  status: 'passed' | 'failed';
  code: string;
}

export interface IveKitRealtimeSpeechProviderAcceptanceReport {
  status: 'passed' | 'failed';
  verification_scope: 'controlled_loopback_realtime_provider';
  real_vendor_evidence: false;
  checks: IveKitRealtimeSpeechProviderAcceptanceCheck[];
}

export async function runIveKitRealtimeSpeechProviderAcceptance():
Promise<IveKitRealtimeSpeechProviderAcceptanceReport> {
  const checks: IveKitRealtimeSpeechProviderAcceptanceCheck[] = [];
