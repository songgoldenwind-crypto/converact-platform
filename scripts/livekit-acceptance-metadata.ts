...
export function createLiveKitAcceptanceMetadata(
  ...
  const runId = value(resolveBrandEnv(env, 'LIVEKIT_ACCEPTANCE_RUN_ID')) ||
    (options.generateRunId ? `lk-${randomUUID()}` : '');
  const environmentId = value(resolveBrandEnv(env, 'LIVEKIT_ACCEPTANCE_ENVIRONMENT_ID'));
  const deployedCommit = value(resolveBrandEnv(env, 'LIVEKIT_ACCEPTANCE_DEPLOYED_COMMIT'));
  const configuredFingerprint = value(resolveBrandEnv(env, 'LIVEKIT_ACCEPTANCE_DEPLOYMENT_FINGERPRINT'));
  const startedAt = value(resolveBrandEnv(env, 'LIVEKIT_ACCEPTANCE_STARTED_AT')) ||
    (options.generateRunId ? new Date().toISOString() : '');
  const coreDeploymentMode = value(resolveBrandEnv(env, 'LIVEKIT_DEPLOYMENT_MODE'));
  const acceptanceDeploymentMode = value(resolveBrandEnv(env, 'LIVEKIT_ACCEPTANCE_DEPLOYMENT_MODE'));
  ...
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{7,127}$/.test(runId)) {
    throw new Error('CONVERACT_LIVEKIT_ACCEPTANCE_RUN_ID must be 8-128 safe identifier characters');
  }
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{2,127}$/.test(environmentId) || placeholder(environmentId)) {
    throw new Error('CONVERACT_LIVEKIT_ACCEPTANCE_ENVIRONMENT_ID must be a non-placeholder environment identifier');
  }
  if (!/^[a-f0-9]{40}$/i.test(deployedCommit)) {
    throw new Error('CONVERACT_LIVEKIT_ACCEPTANCE_DEPLOYED_COMMIT must be a full 40-character Git SHA');
  }
  const deploymentFingerprint = configuredFingerprint || createHash('sha256').update([
    environmentId,
    deployedCommit.toLowerCase(),
    value(resolveBrandEnv(env, 'LIVEKIT_DEPLOYMENT_MODE')),
    cleanUrl(value(env.LIVEKIT_PUBLIC_URL)),
    cleanUrl(value(env.LIVEKIT_URL || resolveBrandEnv(env, 'LIVEKIT_URL')))
  ].join('\n')).digest('hex');
  if (!/^[a-f0-9]{64}$/i.test(deploymentFingerprint)) {
    throw new Error('CONVERACT_LIVEKIT_ACCEPTANCE_DEPLOYMENT_FINGERPRINT must be a 64-character SHA-256');
  }
  if (!isIsoTimestamp(startedAt)) {
    throw new Error('CONVERACT_LIVEKIT_ACCEPTANCE_STARTED_AT must be an ISO timestamp');
  }
...
export function optionalLiveKitAcceptanceMetadata(
  ...
  const keys = [
    resolveBrandEnv(env, 'LIVEKIT_ACCEPTANCE_RUN_ID'),
    resolveBrandEnv(env, 'LIVEKIT_ACCEPTANCE_ENVIRONMENT_ID'),
    resolveBrandEnv(env, 'LIVEKIT_ACCEPTANCE_DEPLOYED_COMMIT'),
    resolveBrandEnv(env, 'LIVEKIT_ACCEPTANCE_DEPLOYMENT_FINGERPRINT'),
    resolveBrandEnv(env, 'LIVEKIT_ACCEPTANCE_STARTED_AT')
  ];
...
export function liveKitAcceptanceMetadataEnv(
  ...
  return {
    CONVERACT_LIVEKIT_ACCEPTANCE_RUN_ID: metadata.run_id,
    CONVERACT_LIVEKIT_ACCEPTANCE_ENVIRONMENT_ID: metadata.environment_id,
    CONVERACT_LIVEKIT_ACCEPTANCE_DEPLOYED_COMMIT: metadata.deployed_commit,
    CONVERACT_LIVEKIT_ACCEPTANCE_DEPLOYMENT_FINGERPRINT: metadata.deployment_fingerprint,
    CONVERACT_LIVEKIT_ACCEPTANCE_STARTED_AT: metadata.started_at,
    CONVERACT_LIVEKIT_ACCEPTANCE_DEPLOYMENT_MODE: metadata.deployment_mode,
    CONVERACT_LIVEKIT_DEPLOYMENT_MODE: metadata.deployment_mode
  };
}
