...
export function loadRtpengineAcceptanceCliConfig(
  ...
  const baseUrl = required(env, 'CONVERACT_FABRIC_MEDIA_CONTROL_ENDPOINT');
  checkedHttpUrl(baseUrl);
  const mediaControlFetch = loadRtpengineAcceptanceFetch(baseUrl, env);
  const token = required(env, 'CONVERACT_FABRIC_MEDIA_CONTROL_TOKEN');
  if (token.length < 24 || token.length > 512 || /[\0\r\n]/.test(token)) {
    throw new Error('CONVERACT_FABRIC_MEDIA_CONTROL_TOKEN is invalid');
  }
  const bindAddress = required(
    env,
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_BIND_ADDRESS'
  );
  if (isIP(bindAddress) === 0) {
    throw new Error('CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_BIND_ADDRESS is invalid');
  }
  const expiresAt = required(
    env,
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_EXPIRES_AT'
  );
  if (!canonicalDate(expiresAt) || Date.parse(expiresAt) <= Date.now()) {
    throw new Error('CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_EXPIRES_AT is invalid');
  }
  const sourceCommit = required(
    env,
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_SOURCE_COMMIT'
  );
  const imageDigest = required(
    env,
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_IMAGE_DIGEST'
  );
  const configHash = required(
    env,
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_CONFIG_HASH'
  );
  if (!SOURCE_COMMIT.test(sourceCommit)) {
    throw new Error('CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_SOURCE_COMMIT is invalid');
  }
  if (!SHA256_DIGEST.test(imageDigest)) {
    throw new Error('CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_IMAGE_DIGEST is invalid');
  }
  if (!SHA256_DIGEST.test(configHash)) {
    throw new Error('CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_CONFIG_HASH is invalid');
  }
  const runtimeMode = required(
    env,
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_RUNTIME_MODE'
  );
  if (runtimeMode !== 'userspace' && runtimeMode !== 'kernel') {
    throw new Error('CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_RUNTIME_MODE is invalid');
  }
  const outputFile = absolutePath(
    required(env, 'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_OUTPUT'),
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_OUTPUT'
  );
  const sourceDir = absolutePath(
    required(env, 'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_SOURCE_DIR'),
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_SOURCE_DIR'
  );
  const dockerBinary = absolutePath(
    required(env, 'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_DOCKER_BINARY'),
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_DOCKER_BINARY'
  );
  const containerPrefix = required(
    env,
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_CONTAINER_PREFIX'
  );
  ...
  const ngHost = required(env, 'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_NG_HOST');
  if (isIP(ngHost) === 0) {
    throw new Error('CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_NG_HOST is invalid');
  }
  const mediaPortMinimum = integerEnv(
    env,
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_MEDIA_PORT_MIN',
    undefined,
    1_024,
    65_534
  );
  const mediaPortMaximum = integerEnv(
    env,
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_MEDIA_PORT_MAX',
    undefined,
    mediaPortMinimum + 1,
    65_535
  );
  ...
  return {
    ...
    ng: {
      ...
      port: integerEnv(
        env,
        'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_NG_PORT',
        undefined,
        1,
        65_535
      )
    ...
    maximum_active_calls: integerEnv(
      env,
      'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_MAX_ACTIVE_CALLS',
      undefined,
      1,
      32
    ),
    plaintext_packet_count: integerEnv(
      env,
      'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_PLAINTEXT_PACKETS',
      500,
      20,
      100_000
    ),
    srtp_packet_count: integerEnv(
      env,
      'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_SRTP_PACKETS',
      100,
      20,
      100_000
    ),
    packet_interval_ms: integerEnv(
      env,
      'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_PACKET_INTERVAL_MS',
      20,
      1,
      1_000
    ),
    receive_timeout_ms: integerEnv(
      env,
      'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_RECEIVE_TIMEOUT_MS',
      30_000,
      1_000,
      300_000
    ),
    outage_hold_ms: integerEnv(
      env,
      'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_OUTAGE_HOLD_MS',
      1_000,
      100,
      30_000
    ),
    maximum_jitter_ms: numberEnv(
      env,
      'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_MAXIMUM_JITTER_MS',
      30,
      0.1,
      1_000
    ),
...
async function runRegressionChecks(
  ...
  await executable(
    ...
    [
      ...
      join(config.source_dir, 'test/converact-rtpengine-ng.test.ts'),
      join(config.source_dir, 'test/converact-rtpengine-media-transport.test.ts')
    ],
    60_000
  );
}
...
async function mediaWalInode(
  ...
  const output = await docker(config, [
    ...
    'node',
    '-e',
    'process.stdout.write(String(require("fs").statSync("/var/lib/converact-media-control/media-command.wal").ino))'
  ]);
...
function checkedHttpUrl(
  value: string,
  field = 'CONVERACT_FABRIC_MEDIA_CONTROL_ENDPOINT'
): void {
  let url: URL;
  try {
    url = new URL(value);
...
function loadRtpengineAcceptanceAdmission(
  ...
  const fields = [
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_ENDPOINT',
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_TOKEN',
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_TENANT_ID',
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_REGION_ID',
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_ZONE_ID',
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_CELL_ID',
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_PROFILE_ID',
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_OWNER_NODE_ID',
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_REQUIRED_CAPACITY_JSON'
  ] as const;
  ...
  const tenantId = acceptanceIdentifier(
    required(env, 'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_TENANT_ID'),
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_TENANT_ID'
  );
  const regionId = acceptanceIdentifier(
    required(env, 'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_REGION_ID'),
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_REGION_ID'
  );
  const zoneId = acceptanceIdentifier(
    required(env, 'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_ZONE_ID'),
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_ZONE_ID'
  );
  const cellId = acceptanceIdentifier(
    required(env, 'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_CELL_ID'),
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_CELL_ID'
  );
  const profileId = acceptanceIdentifier(
    required(env, 'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_PROFILE_ID'),
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_PROFILE_ID'
  );
  const ownerNodeId = acceptanceIdentifier(
    required(env, 'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_OWNER_NODE_ID'),
    'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_OWNER_NODE_ID'
  );
...
function acceptanceCapacity(value: string): CapacityRequirement {
  if (Buffer.byteLength(value, 'utf8') > 4_096) {
    throw new Error(
      'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_ADMISSION_REQUIRED_CAPACITY_JSON is invalid'
    );
  }
  let parsed: unknown;
  try {
...
function loadRtpengineAcceptanceFetch(
  ...
  const identityFile = String(
    resolveConveractEnv(
      env,
      'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_TLS_IDENTITY_FILE'
    ) || ''
  ).trim();
  const caFile = String(
    resolveConveractEnv(
  ...
  const configuredServername = String(
    resolveConveractEnv(
      env,
      'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_TLS_SERVERNAME'
    ) || ''
  ).trim();
...
function checkedTlsServername(value: string): string {
  if (!value ||
    ...
    throw new Error(
      'CONVERACT_FABRIC_RTPENGINE_ACCEPTANCE_TLS_SERVERNAME is invalid'
    );
  }
  return value;
}
...
if (process.argv[1] &&
  ...
  try {
    ...
    process.stderr.write(
      `Converact Fabric RTPengine acceptance failed: ${
        message.replace(/[\0\r\n]+/g, ' ').slice(0, 512)
      }\n`
    );
    process.exitCode = 1;
