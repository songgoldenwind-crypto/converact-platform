  ...
  const markPhase = async (phase: 'blackout_started' | 'restored') => {
    await input.page.evaluate((value) => {
      const state = globalThis as typeof globalThis & {
        __CONVERACT_CAPACITY_RECONNECT_CONTROL__?: {
          blackout_started_at: number;
          restored_at: number;
        };
      };
      const control = state.__CONVERACT_CAPACITY_RECONNECT_CONTROL__;
    ...
    await input.page.waitForFunction(
      () => Boolean(
        (globalThis as typeof globalThis & {
          __CONVERACT_CAPACITY_RECONNECT_CONTROL__?: { media_ready?: boolean };
        }).__CONVERACT_CAPACITY_RECONNECT_CONTROL__?.media_ready
      ),
      undefined,
      { timeout: 30_000 }
    );
...
export const LIVEKIT_BROWSER_PAGE_BOOTSTRAP = [
  ...
  `globalThis.__CONVERACT_INSPECT_INBOUND_VIDEO_STATS__ = (${
    inspectLiveKitInboundVideoStats.toString()
  });`,
  `globalThis.__CONVERACT_INSPECT_SELECTED_ICE_CANDIDATE_PAIRS__ = (${
...
export async function measureLiveKitBaselineRoomInPage(
  ...
  const state = globalThis as typeof globalThis & {
    ...
    __CONVERACT_INSPECT_SELECTED_ICE_CANDIDATE_PAIRS__?: (
      reports: readonly LiveKitStatsReportLike[]
    ) => LiveKitSelectedIceCandidatePairEvidence;
    __CONVERACT_INSPECT_INBOUND_VIDEO_STATS__?: (
      report: LiveKitStatsReportLike
    ) => LiveKitInboundVideoStatsEvidence;
  };
  ...
  const reconnectControl = input.expected_reconnect_participants > 0
    ...
    : undefined;
  if (reconnectControl) {
    state.__CONVERACT_CAPACITY_RECONNECT_CONTROL__ = reconnectControl;
  }
  ...
  async function inboundVideoStats(
    ...
    const inspect = state.__CONVERACT_INSPECT_INBOUND_VIDEO_STATS__;
  ...
  try {
    ...
    await Promise.all([
      rooms[0].localParticipant.publishTrack(localAudio, {
        name: 'converact-capacity-marker-audio',
        source: sdk.Track.Source.Microphone,
        stream: 'converact-capacity-av',
        dtx: false,
        red: true
      }),
      rooms[0].localParticipant.publishTrack(localVideo, {
        name: 'converact-capacity-marker-video',
    ...
    if (localScreen) {
      await rooms[0].localParticipant.publishTrack(localScreen, {
        name: 'converact-capacity-marker-screen',
        source: sdk.Track.Source.ScreenShare,
        stream: 'converact-capacity-screen',
        simulcast: false,
        videoEncoding: {
    ...
    if (input.expected_forced_turn_participants > 0) {
      const inspectCandidatePairs =
        state.__CONVERACT_INSPECT_SELECTED_ICE_CANDIDATE_PAIRS__;
    ...
    if (state.__CONVERACT_CAPACITY_RECONNECT_CONTROL__ === reconnectControl) {
      delete state.__CONVERACT_CAPACITY_RECONNECT_CONTROL__;
    }
  }
}
