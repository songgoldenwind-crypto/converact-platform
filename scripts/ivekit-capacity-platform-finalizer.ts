...
export function capacityPlatformFinalizerConfig(
  ...
  return {
    ...
    finalizer_id: safeId(required(env, 'OPC_IVEKIT_CAPACITY_PLATFORM_FINALIZER_ID')),
    contract_path: absolutePath(required(env, 'OPC_IVEKIT_CAPACITY_PLATFORM_CONTRACT_PATH')),
    submission_path: absolutePath(required(env, 'OPC_IVEKIT_CAPACITY_PLATFORM_SUBMISSION_PATH')),
    lease_ttl_ms: integer(
      env.OPC_IVEKIT_CAPACITY_PLATFORM_FINALIZER_LEASE_MS || '15000',
      1_000,
      300_000
    ),
    evidence_prefix: required(env, 'OPC_IVEKIT_CAPACITY_PLATFORM_EVIDENCE_PREFIX'),
    evidence_s3: {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  main().catch((error) => {
    console.error('[ivekit-capacity-platform-finalizer]', safeError(error));
    process.exitCode = 1;
  });
}
