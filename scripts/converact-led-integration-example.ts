...
import {
  ...
  type ConveractFabricSdkFetch
} from '../src/agent-runtime/converact/index.js';

export interface ConveractFabricLedExampleConfig {
  baseUrl: string;
  apiKey: string;
  tenantId: string;
  userId: string;
...
export function converactFabricLedExampleConfigFromEnv(env: NodeJS.ProcessEnv): ConveractFabricLedExampleConfig {
...
export async function runConveractFabricLedExample(config: ConveractFabricLedExampleConfig): Promise<Record<string, unknown>> {
  ...
  const room = await sdk.media.createRoom({
    ...
    business_ref: businessRef,
    metadata: { collaboration_session_id: sessionId, source: 'converact-led-example' }
  });
  ...
  const message = await sdk.chat.postMessage(sessionId, {
    ...
    message_type: 'text',
    body: 'LED Converact Fabric integration probe'
  }, {
  ...
  if (config.remoteSessionId && (config.rustdeskDeviceId || config.rustdeskId)) {
    const rustdeskSdk = createConveractFabricRustDeskLedSdk({
      baseUrl: config.baseUrl,
      apiKey: config.apiKey,
      tenantId: config.tenantId,
      userId: config.userId,
      fetch: config.fetch,
      source: 'converact-led-example'
    });
...
async function main(): Promise<void> {
  const result = await runConveractFabricLedExample(converactFabricLedExampleConfigFromEnv(process.env));
