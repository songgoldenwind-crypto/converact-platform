...
import type {
  ...
  ConveractFabricWebhookDeliveryEnvelope
...
import { CONVERACT_FABRIC_PRODUCT } from './lib/converact-product-contract.js';
...
export async function runConveractFabricV5ControlledAcceptance(input: {
  ...
  const events = eventTypes.map((eventType, index): ConveractFabricStoredIntegrationEvent => ({
    id: String(index + 1),
  ...
  let completedCursor = '';
  const projected: ReturnType<typeof projectConveractFabricIntegrationEvent>[] = [];
  const summary = await runConveractFabricEventWebhookBatch({
    ...
    project: async (_claim, event) => { projected.push(projectConveractFabricIntegrationEvent(event)); }
  });

  const inner = projected[0];
  const delivery: ConveractFabricWebhookDeliveryEnvelope = {
  ...
  const claims = new Map<string, ConveractFabricWebhookReplayClaim>();
  const replayStore = {
  ...
  const checks = {
    catalog_families: CONVERACT_FABRIC_INTEGRATION_EVENT_CATALOG.families.length === 8,
...
function controlledSubscription(now: string): ConveractFabricEventWebhookSubscription {
  const patterns = normalizeConveractFabricEventPatterns(
    CONVERACT_FABRIC_INTEGRATION_EVENT_CATALOG.families.flatMap((family) => family.patterns)
  );
  return {
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  const outputDir = resolveFabricEnv(process.env, 'V5_CONTROLLED_ACCEPTANCE_DIR') ||
    resolve('.tmp/converact-v5-controlled-acceptance');
  await runConveractFabricV5ControlledAcceptance({
    source_commit: sourceCommit,
