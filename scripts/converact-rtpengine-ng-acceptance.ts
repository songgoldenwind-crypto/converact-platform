...
const host = String(resolveFabricEnv(process.env, 'RTPENGINE_NG_HOST') || '').trim();
const port = boundedInteger(
  resolveFabricEnv(process.env, 'RTPENGINE_NG_PORT'),
  1,
  65_535,
  'CONVERACT_FABRIC_RTPENGINE_NG_PORT'
);
const requestCount = boundedInteger(
  resolveFabricEnv(process.env, 'RTPENGINE_NG_REQUESTS') || '32',
  1,
  10_000,
  'CONVERACT_FABRIC_RTPENGINE_NG_REQUESTS'
);
if (!host) throw new Error('CONVERACT_FABRIC_RTPENGINE_NG_HOST is required');
