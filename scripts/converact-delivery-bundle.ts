...
} from './converact-standalone-build-context.js';
import {
  createConveractFabricReleaseOperations,
  validateConveractFabricReleaseOperations,
  type ConveractFabricReleaseContract
} from './converact-release-operations.js';
import {
  createConveractFabricStage2ReleaseEvidence,
  validateConveractFabricStage2ReleaseEvidence,
  type ConveractFabricStage2ReleaseEvidence
} from './converact-stage2-release-evidence.js';
import {
  VOICE_REQUIRED_ACCEPTANCE_CHECKS,
  createConveractFabricVoiceAcceptanceTemplate,
  renderConveractFabricVoiceAcceptanceRunbook
} from './converact-voice-acceptance.js';
import {
...
} from './converact-v6-real-acceptance.js';
import {
...
export interface ConveractFabricDeliveryManifestFile {
  path: string;
  bytes: number;
  sha256: string;
}

export interface ConveractFabricDeliveryManifest {
  schema_version: 1;
  ...
  controlled_environment_acceptance: {
    postgres: ConveractFabricControlledAcceptanceStatus;
    provider_protocol: ConveractFabricControlledAcceptanceStatus;
    browser: ConveractFabricControlledAcceptanceStatus;
    restart_recovery: ConveractFabricControlledAcceptanceStatus;
    full_chain: ConveractFabricControlledAcceptanceStatus;
  };
  capability_matrix: ConveractFabricDeliveryCapability[];
  acceptance_matrix: {
    automated: {
      status: 'required_before_release';
      command: 'npm run verify:converact:foundation';
    };
    controlled: ConveractFabricDeliveryManifest['controlled_environment_acceptance'];
    real_environment: ConveractFabricDeliveryManifest['real_environment_acceptance'];
  };
  known_not_run: ConveractFabricKnownNotRun[];
  files: ConveractFabricDeliveryManifestFile[];
}

export interface ConveractFabricDeliveryCapability {
  id: string;
  stage: 1 | 2 | 3 | 4 | 5;
...
export interface ConveractFabricKnownNotRun {
  id: string;
  status: 'not_run';
  reason: string;
}

export type ConveractFabricControlledAcceptanceStatus = 'not_run' | 'passed';

export interface ConveractFabricControlledAcceptanceEvidence {
  path: string;
  bytes: number;
  sha256: string;
}
...
export interface LoadedControlledAcceptancePackage {
  root: string;
  statuses: ConveractFabricDeliveryManifest['controlled_environment_acceptance'];
  checks: Record<keyof ConveractFabricDeliveryManifest['controlled_environment_acceptance'], {
    status: ConveractFabricControlledAcceptanceStatus;
    evidence: string[];
  }>;
  evidence: ConveractFabricControlledAcceptanceEvidence[];
}

export interface BuildConveractFabricDeliveryBundleOptions {
  repoRoot: string;
  outputDir: string;
  sdkTarball: string;
  clientDist: string;
...
const CAPACITY_RUNTIME_SOURCE_PATHS = [
  '.github/workflows/converact-capacity-ci.yml',
  ...
  'scripts/converact-capacity-controller.ts',
  'scripts/converact-capacity-dispatcher.ts',
  'scripts/converact-capacity-finalizer.ts',
  'scripts/converact-capacity-scaling-finalizer.ts',
  'scripts/converact-capacity-platform-finalizer.ts',
  'scripts/converact-capacity-worker.ts',
  'scripts/converact-capacity-network-impairment.ts',
  'scripts/converact-cell-admission.ts',
  'scripts/converact-cell-capacity-projector.ts',
  'scripts/converact-component-node-admission.ts',
  'scripts/converact-rustdesk-owner-binding.ts',
  ...
  'scripts/capacity/generators/converact-event-ws.ts',
...
export const DELIVERY_SOURCE_FILES: readonly DeliverySourceFile[] = [
  ...
  ...[
    ...
    destination: `deploy/kubernetes/converact/${name}`
  })),
  ...[
    'Chart.yaml',
    'values.yaml',
  ...
  {
    source: 'scripts/converact-rustpbx-recording-isolation-acceptance.ts',
  ...
  {
    source: 'scripts/converact-kamailio-acceptance.ts',
  ...
  ...[
    'converact-fabric-video-im-capability-design.md',
    'openapi.yaml',
    'converact-openapi.md',
    'converact-led-integration-guide.md',
    'converact-m5-unified-collaboration-plan.md',
    'converact-client-delivery-v1-roadmap.md',
    'converact-fabric-voice-foundation-v1-design.md',
    'converact-fabric-v3-intelligence-operations.md',
    'converact-fabric-v3-completion-audit.md',
    'converact-fabric-v5-shared-foundation-design.md',
    'converact-fabric-v5-stage1-content-intelligence-plan.md',
    'converact-fabric-v5-stage1-provider-resilience-plan.md',
    'converact-fabric-v5-stage2-im-livekit-file-plan.md',
    'converact-fabric-v5-stage3-rustdesk-windows-plan.md',
    'converact-fabric-v5-stage4-voice-notification-operations-plan.md',
    'converact-fabric-v5-stage5-integration-delivery-plan.md',
    'converact-backup-restore-runbook.md',
    'converact-notification-operations-runbook.md',
    'converact-monitoring-runbook.md',
    'converact-fabric-voice-cdr-durability-runbook.md',
    'converact-integration-event-webhook-runbook.md',
    'converact-rustdesk-windows-deployment.md',
    'converact-component-governance.md',
    'converact-fabric-v6-production-closure-design.md',
    'converact-fabric-v6-production-closure-plan.md',
    'converact-fabric-v6-real-environment-acceptance.md',
  ...
  ...[
    'converact-led-integration-example.ts',
    'converact-rustdesk-led-example.ts'
  ].map((name) => ({ source: `scripts/${name}`, destination: `examples/${name}` })),
  {
    source: 'sdk/converact/examples/webhook-receiver.ts',
    destination: 'examples/converact-webhook-receiver.ts'
  },
  ...[
    'README.md',
    'Dockerfile.runtime',
  ...
  {
    source: 'scripts/converact-rustdesk-owner-binding.ts',
    destination: 'deploy/rustdesk-server-fork/converact-rustdesk-owner-binding.ts'
  },
  {
  ...
  {
    source: 'scripts/converact-controlled-provider.ts',
    destination: 'acceptance/tools/converact-controlled-provider.ts'
  },
  {
    source: 'scripts/converact-controlled-voice-provider.ts',
    destination: 'acceptance/tools/converact-controlled-voice-provider.ts'
  },
  {
    source: 'scripts/converact-voice-acceptance.ts',
    destination: 'acceptance/tools/converact-voice-acceptance.ts'
  },
  {
    source: 'scripts/converact-v5-controlled-acceptance.ts',
    destination: 'acceptance/tools/converact-v5-controlled-acceptance.ts'
  },
  {
    source: 'scripts/converact-v6-real-acceptance.ts',
    destination: 'acceptance/tools/converact-v6-real-acceptance.ts'
  },
  ...[
  ...
  ...[
    'Deploy-ConveractFabricRustDesk.ps1',
    'Invoke-ConveractFabricRustDeskSessionDisconnect.ps1',
    'ConveractFabricRustDeskEdge.xml.template',
    'Publish-ConveractFabricRustDeskEvidence.ps1',
    'Resolve-ConveractFabricRustDeskSession.ps1'
...
const DELIVERY_ROOT_MARKER = '.converact-delivery-root';
const RUSTPBX_ACCEPTANCE_GENERATED_FILES = [
  'acceptance/rustpbx/package.json',
  'acceptance/rustpbx/package-lock.json',
  'acceptance/rustpbx/scripts/converact-rustpbx-management-acceptance.js',
  'acceptance/rustpbx/scripts/converact-rustpbx-rwi-acceptance.js',
  'acceptance/rustpbx/scripts/converact-rustpbx-sipp-acceptance.js',
...
const KNOWN_NOT_RUN: readonly ConveractFabricKnownNotRun[] = [
...
const CAPABILITY_MATRIX: readonly ConveractFabricDeliveryCapability[] = [
...
export function loadControlledAcceptancePackage(
  ...
  assertConveractFabricDeliverySourceState(sourceCommit, '');
  ...
  if (report.schema_version !== 1 || !isAcceptedFabricProduct(report.product)) {
  ...
  let totalBytes = 0;
  const verifiedEvidence: ConveractFabricControlledAcceptanceEvidence[] = [];
  ...
  const statuses = {} as ConveractFabricDeliveryManifest['controlled_environment_acceptance'];
...
export function buildConveractFabricDeliveryBundle(
  options: BuildConveractFabricDeliveryBundleOptions
): { outputDir: string; manifest: ConveractFabricDeliveryManifest } {
  ...
  writeFileSync(join(outputDir, DELIVERY_ROOT_MARKER), 'converact-delivery-bundle-v1\n', 'utf8');
  ...
  const edgeStaging = mkdtempSync(join(tmpdir(), 'converact-delivery-edge-'));
  try {
    run('npx', [
      'tsc',
      '--outDir', edgeStaging,
  ...
  const rustPbxAcceptanceStaging = mkdtempSync(
    join(tmpdir(), 'converact-delivery-rustpbx-acceptance-')
  );
  try {
    run('npx', [
      'tsc',
      ...
      '--types', 'node',
      '--skipLibCheck',
      'scripts/converact-rustpbx-management-acceptance.ts',
      'scripts/converact-rustpbx-rwi-acceptance.ts',
      'scripts/converact-rustpbx-sipp-acceptance.ts'
    ], repoRoot);
    ...
    const acceptancePackage = {
      name: 'converact-rustpbx-acceptance',
      version: '1.0.0',
      private: true,
      type: 'module',
      dependencies: { ws: '8.21.1' },
      scripts: {
        management: 'node scripts/converact-rustpbx-management-acceptance.js',
        rwi: 'node scripts/converact-rustpbx-rwi-acceptance.js',
        sipp: 'node scripts/converact-rustpbx-sipp-acceptance.js'
      }
    };
    writeFileSync(
  ...
  const serviceStaging = mkdtempSync(join(tmpdir(), 'converact-delivery-service-'));
  try {
    const contextDir = join(serviceStaging, 'build-context');
    const context = buildConveractFabricStandaloneContext({
      repoRoot,
  ...
  writeFileSync(join(outputDir, 'service', 'image-metadata.json'), `${JSON.stringify({
    schema_version: 1,
    source_commit: sourceCommit,
    reference: String(options.imageReference || `converact-fabric:${sourceCommit.slice(0, 12)}`).trim(),
    digest: imageDigest,
  ...
  const stage2Evidence = createConveractFabricStage2ReleaseEvidence({
    sourceCommit,
    generatedAt,
    imageDigest,
    migrations: [
    ...
    configurationArtifacts: [
      ...
      ...[
        ...
        'components/livekit-egress/infra/converact/livekit-egress/build-converact.sh',
      ...
      ...[
        ...
        'deploy/kubernetes/converact/values.yaml',
        'deploy/kubernetes/converact/templates/deployment.yaml',
        'deploy/kubernetes/converact/templates/clamav.yaml'
      ].map((path) => ({
  ...
  const releaseOperations = createConveractFabricReleaseOperations({
    sourceCommit,
    generatedAt,
    imageReference: String(options.imageReference || `converact-fabric:${sourceCommit.slice(0, 12)}`).trim(),
    imageDigest,
  ...
  writeFileSync(
    ...
    `${JSON.stringify(createConveractFabricVoiceAcceptanceTemplate({
  ...
  writeFileSync(
    ...
    `${JSON.stringify(createConveractFabricV6RealAcceptanceTemplate({
      source_commit: sourceCommit,
  ...
  const manifest: ConveractFabricDeliveryManifest = {
    schema_version: 1,
    product: CONVERACT_FABRIC_PRODUCT,
    foundation_version: 'V5',
    status: 'ready_for_handoff',
    ...
    contents: {
      ...
      operations: 'docs/converact-fabric-v3-intelligence-operations.md',
      release_operations: 'operations/upgrade-runbook.md',
      completion_audit: 'docs/converact-fabric-v3-completion-audit.md',
      ...
      voice_helm: 'deploy/kubernetes/converact/',
  ...
  validateConveractFabricDeliveryBundle(outputDir);
  return { outputDir, manifest };
}

export function validateConveractFabricDeliveryBundle(outputDirInput: string): ConveractFabricDeliveryManifest {
  ...
  const manifest = JSON.parse(readFileSync(join(outputDir, 'manifest.json'), 'utf8')) as ConveractFabricDeliveryManifest;
  if (manifest.schema_version !== 1 || !isAcceptedFabricProduct(manifest.product) ||
      manifest.foundation_version !== 'V5') {
    throw new Error('invalid Converact Fabric delivery manifest');
  }
  if (JSON.stringify(manifest.capability_matrix) !== JSON.stringify(CAPABILITY_MATRIX)) {
    throw new Error('Converact Fabric V5 capability matrix is incomplete');
  }
  ...
  if (manifest.acceptance_matrix.automated.status !== 'required_before_release' ||
      manifest.acceptance_matrix.automated.command !== 'npm run verify:converact:foundation' ||
    ...
    throw new Error('Converact Fabric V5 acceptance matrix is incomplete');
  }
  validateVoiceAcceptanceAssets(outputDir, manifest);
  validateV6RealAcceptanceAsset(outputDir, manifest);
  const contextManifest = validateConveractFabricStandaloneContext(join(outputDir, 'service', 'build-context'));
...
function prepareBundleFromCli(): { outputDir: string; manifest: ConveractFabricDeliveryManifest } {
  const repoRoot = resolve(fileURLToPath(new URL('..', import.meta.url)));
  const outputDir = resolve(
    resolveFabricEnv(process.env, 'DELIVERY_DIR') ||
      join(repoRoot, '.tmp', 'converact-led-delivery')
  );
  const sourceCommit = resolveSourceCommit(repoRoot);
  assertConveractFabricDeliverySourceState(
    sourceCommit,
    run('git', ['status', '--porcelain=v1', '--untracked-files=all'], repoRoot)
  );
  const stagingDir = mkdtempSync(join(tmpdir(), 'converact-delivery-build-'));
  try {
    ...
    return buildConveractFabricDeliveryBundle({
      repoRoot,
      outputDir,
      sdkTarball: join(stagingDir, filename),
      clientDist: join(repoRoot, 'clients', 'converact-reference', 'dist'),
...
export function assertConveractFabricDeliverySourceState(sourceCommit: string, porcelainStatus: string): void {
  if (!/^[a-f0-9]{40}$/.test(String(sourceCommit || '').trim())) {
    throw new Error('Converact Fabric delivery source must be a full 40-character Git commit');
  }
  const changedEntries = String(porcelainStatus || '').split(/\r?\n/).filter(Boolean).length;
  if (changedEntries) {
    throw new Error(`Converact Fabric delivery worktree is dirty (${changedEntries} entries)`);
  }
}

function renderBundleReadme(): string {
  return [
    '# Converact Fabric LED Delivery Bundle',
    '',
    'This directory is a curated integration handoff for the reusable Converact Fabric communication foundation.',
    'It contains the Converact Fabric V5 shared communication foundation, not Converact or LED business logic, and it contains no credentials.',
    '',
    '## Contents',
    '',
    '- `sdk/`: installable `@converact/sdk` npm package.',
    '- `client/`: production reference client static assets.',
    '- `deploy/application/`: standalone Converact Fabric service Compose with PostgreSQL and optional RustPBX overlay.',
    '- `deploy/kubernetes/converact/`: standalone digest-pinned Helm Chart with a migration gate.',
    ...
    '- `service/build-context/`: independently buildable Converact Fabric service source context with its own package lock.',
    ...
    '## Deployment boundary',
    '',
    'Build the Converact Fabric image directly from `service/build-context/`; no Converact root checkout is required.',
...
function assertSafeOutputDirectory(repoRoot: string, outputDir: string): void {
  if (outputDir === repoRoot || outputDir === resolve(repoRoot, '..') || outputDir === resolve('/')) {
    throw new Error('refusing unsafe Converact Fabric delivery output directory');
  }
...
function assertReplaceableOutputDirectory(outputDir: string): void {
  ...
  if (readFileSync(marker, 'utf8') !== 'converact-delivery-bundle-v1\n') {
    throw new Error('refusing to replace a directory with an invalid Converact Fabric ownership marker');
  }
}
...
function fileEntry(root: string, path: string): ConveractFabricDeliveryManifestFile {
...
function validateArtifactBindings(outputDir: string, manifest: ConveractFabricDeliveryManifest): void {
  ...
  )) as ConveractFabricReleaseContract;
  const stage2Evidence = JSON.parse(readFileSync(
    join(outputDir, artifacts.stage2_deployment_evidence.path), 'utf8'
  )) as ConveractFabricStage2ReleaseEvidence;
  validateConveractFabricStage2ReleaseEvidence(stage2Evidence);
  validateConveractFabricReleaseOperations({
...
function validateAcceptanceMetadata(outputDir: string, manifest: ConveractFabricDeliveryManifest): void {
  ...
  if (status.schema_version !== 2 || !isAcceptedFabricProduct(status.product) ||
...
function validateVoiceAcceptanceAssets(outputDir: string, manifest: ConveractFabricDeliveryManifest): void {
  ...
  if (runbook !== renderConveractFabricVoiceAcceptanceRunbook()) {
    throw new Error('Voice acceptance runbook does not match the validator contract');
  }
}

function validateV6RealAcceptanceAsset(outputDir: string, manifest: ConveractFabricDeliveryManifest): void {
  if (
  ...
  const template = JSON.parse(readFileSync(
    ...
    'utf8'
  )) as ConveractFabricV6RealAcceptanceManifest;
  validateConveractFabricV6RealAcceptanceManifest(template, {
    base_dir: outputDir,
...
function copyDeliverySource(outputDir: string, source: string, destination: string): void {
  ...
  const portableCompose = readFileSync(source, 'utf8')
    .replace(/\n  build:\n    context: \./, '')
    .replaceAll(
      '${CONVERACT_FABRIC_IMAGE:-converact-fabric:local}',
      '${CONVERACT_FABRIC_IMAGE:?CONVERACT_FABRIC_IMAGE is required}'
    );
  if (/^\s+build:/m.test(portableCompose) || portableCompose.includes('converact-fabric:local')) {
