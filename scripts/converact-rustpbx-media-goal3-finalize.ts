...
} from './converact-rustpbx-media-goal3-acceptance.js';
...
function validateIdentity(
  ...
  if (!identity || !COMMIT.test(identity.opc_commit)) {
    throw new Error('Goal 3 Converact source identity is invalid');
  }
  for (const id of [
...
function runCli(argv: string[]): void {
  ...
  if (!inputPath || !outputPath) {
    throw new Error(
      'usage: converact-rustpbx-media-goal3-finalize <input.json> <output.json>'
    );
  }
