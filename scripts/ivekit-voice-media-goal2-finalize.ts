...
export async function collectVoiceMediaGoal2FinalEvidence(
  ...
  const [
    ...
    jsonFile<Goal2Contract>(requiredPath(env, 'IVEKIT_RTPENGINE_GOAL2_CONTRACT')),
    jsonFile<Goal2SupplyChainEvidence>(
      requiredPath(env, 'IVEKIT_RTPENGINE_GOAL2_SUPPLY_CHAIN')
    ),
    jsonFile<Goal2StageEvidence[]>(
      requiredPath(env, 'IVEKIT_RTPENGINE_GOAL2_STAGES')
    ),
    jsonFile<Goal2AttemptManifestEntry[]>(
      requiredPath(env, 'IVEKIT_RTPENGINE_GOAL2_ATTEMPTS')
    ),
    jsonFile<Goal2FailureEvidence[]>(
      requiredPath(env, 'IVEKIT_RTPENGINE_GOAL2_FAILURE_EVIDENCE')
    )
  ]);
  ...
  const result = buildVoiceMediaGoal2FinalEvidence({
    generated_at: required(env, 'IVEKIT_RTPENGINE_GOAL2_GENERATED_AT'),
    contract,
    ...
    requested_capacity_claim: requestedCapacityClaim(
      required(env, 'IVEKIT_RTPENGINE_GOAL2_CAPACITY_CLAIM')
    )
  });
  await writeNewJson(
    requiredPath(env, 'IVEKIT_RTPENGINE_GOAL2_OUTPUT'),
    result
  );
  return result;
}
...
function requestedCapacityClaim(value: string): CapacityClaim {
  if (value !== 'none' &&
    ...
    throw new Error('IVEKIT_RTPENGINE_GOAL2_CAPACITY_CLAIM is invalid');
  }
  return value;
}
